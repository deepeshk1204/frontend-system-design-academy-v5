...
> REST, GraphQL, gRPC-web and the Backend-for-Frontend as an ownership boundary.

- Track: **Frontend Systems** · Level: **core** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/api-and-bff)

An API contract is a promise about what a client may send and what it will get back. A
...
When there was one web client and one backend, they could share a data model and it
...
*Each client team owns its own BFF and can change it at the speed of its UI. The domain services stay domain-shaped and are never reshaped for one screen.*

## REST, and what "maturity" actually means

The Richardson Maturity Model is the standard framing and it is worth knowing because
...
GraphQL lets the client specify exactly which fields it wants across a graph of related
...
**The three GraphQL controls, concretely**

```js
// 1. DataLoader: 51 queries become 2.
...
| Approach | Wins when | Real cost | HTTP caching | Browser support |
| --- | --- | --- | --- | --- |
...
> **tRPC is not a protocol, it is a type bridge**  
> tRPC's value is that a server function's TypeScript signature *is* the client contract --
...
## The BFF as an ownership boundary

The most valuable thing about a BFF is not technical, it is that it moves a decision
...
**What belongs in a BFF, and what must not**

- **Belongs in it** — - **Aggregation** -- fan out to 6 services in parallel over 0.5 ms links instead of 200 ms ones.
...
- **Must not live in it** — - **Business rules** -- pricing, entitlement, tax, eligibility. If two clients could disagree on the answer, it is domain logic.
...
**One BFF per client or one shared?** The honest answer is that it depends on whether the
...
## Pagination, error contracts, and evolution

**Offset pagination** (`?offset=40&limit=20`) is simple and gives you random access to
...
**An error contract worth copying (RFC 9457 problem details)**
...
On **evolution**: the only reliable technique is additive change plus the
...
| --- | --- | --- |
| GraphQL resolver without DataLoader | A 50-item list issues 51 queries; the database saturates at a fraction of expected traffic and the cause is invisible in API metrics. | Batched loaders per request as a default, plus a CI check that flags resolvers doing direct data access, plus per-request query-count telemetry. |
| Public GraphQL endpoint with no depth or complexity limit | One nested query consumes a whole database; trivial and unauthenticated denial of service. | Persisted queries as an allowlist in production, plus depth and complexity caps as defence in depth, plus per-client rate limits weighted by cost. |
| BFF fans out to 6 services and fails if any fails | Availability becomes the product of the upstreams -- six services at 99.9% gives 99.4%, roughly 4 hours of downtime a month. | Per-upstream timeouts and circuit breakers; return partial responses with a per-section error so the page degrades instead of failing. |
| Business logic accumulates in the BFF | Web and mobile compute different prices or entitlements; the bug is real money and nobody owns the rule. | Written boundary enforced in review: if two clients could disagree about the answer, it is domain logic and moves down a layer. |
| Offset pagination on a live feed | Users see duplicated and skipped items while scrolling, and page 500 times out. | Opaque keyset cursors over `(sort_key, id)`; keep offset only where a numbered pager is a product requirement over small data. |
| Clients branching on error message text | A copy edit to an error string breaks retry logic in a shipped mobile app you cannot update. | Stable machine-readable `code` and an explicit `retryable` flag in the contract; treat human-readable text as unversioned. |
| GraphQL returning `200` with an `errors` array | Status-code dashboards, alerts and load-balancer health checks report a healthy service during an outage. | Alert on the GraphQL error rate and on per-field resolver errors, not on HTTP status; assert this explicitly when onboarding the service to monitoring. |
| Field removed because "nobody uses it" | An 18-month-old mobile build crashes on a missing field; you cannot roll the clients forward. | Expand-migrate-contract with per-field usage telemetry keyed by client version, and consumer-driven contract tests in the provider CI. |
...
- **What problem does a BFF actually solve?** — An ownership problem. Screens need data joined across domain services, and a BFF lets the client team do that joining in a service it owns -- on 0.5 ms internal links rather than 200 ms client ones -- without negotiating a backend roadmap slot or reshaping a domain API for one view.
- **What must never live in a BFF?** — Business rules, its own primary database, cross-client shared logic, long-running work, and any server-side consumers. The test: if two clients could disagree about the answer, it is domain logic and belongs one layer down.
- **What is the GraphQL N+1 problem and its fix?** — A nested field resolver runs once per parent item, so 50 posts requesting an author means 51 queries. DataLoader batches all loads made within one event-loop tick into a single `WHERE id IN (...)` and caches per request. Without it a GraphQL API will saturate its database well below expected traffic.
- **Why does GraphQL break HTTP caching?** — Every query is a `POST` to a single `/graphql` URL, and every HTTP cache keys on URL plus method. Browser cache, service worker, CDN and reverse proxies all stop working at once. Persisted queries sent as `GET /graphql?id=<hash>` restore a cacheable URL.
- **What do persisted queries give you beyond caching?** — A closed allowlist. Clients register queries at build time and send only a hash, so an attacker cannot author a query at all -- which addresses the complexity-based denial of service more completely than depth or cost limits alone.
- **Offset vs cursor pagination -- the two real problems with offset?** — Correctness and cost. Concurrent inserts or deletes shift items across page boundaries, so users see duplicates or miss rows. And `OFFSET 100000` makes the database scan and discard 100,000 rows, so deep pages degrade linearly. Keyset cursors on `(sort_key, id)` are stable and O(limit) at any depth.
- **What three fields make an error contract operationally useful?** — A stable machine-readable `code` (clients must never branch on message text), an explicit `retryable` flag so the client does not infer it from a status code, and a `traceId` that appears in your logs so a support ticket becomes a query rather than an investigation.
- **What does consumer-driven contract testing add over an OpenAPI schema?** — It tests the fields consumers actually *use*, not the fields you *declare*. Each consumer publishes its expectations and the provider CI verifies all of them before deploy, so "does this change break anyone" becomes a build result instead of a changelog nobody read.
- **When is tRPC the wrong choice?** — As soon as you have a consumer you cannot deploy in lockstep with, or one that is not TypeScript. There is no language-agnostic schema artefact and no versioning story by design, so a Swift client, a Kotlin client or a partner integration needs OpenAPI, GraphQL SDL or protobuf instead.
...
You must deprecate the `legacy_price` field from a public API. There are three internal clients, roughly 40 partner integrations, and mobile app builds in the field going back 18 months that you cannot force-update. Plan the removal.
