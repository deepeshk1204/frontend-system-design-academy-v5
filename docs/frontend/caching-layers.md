...
> Seven layers between a pixel and a disk read, and who owns correctness at each.

- Track: **Frontend Systems** · Level: **core** · ~18 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/caching-layers)

A cache is a copy of an answer, kept somewhere cheaper to reach than the place that
...
Every layer of a stack is roughly an order of magnitude slower than the one above it,
...
**The latency gradient that justifies each layer**

- **~1 ns** — CPU L1 reference (Component memory territory)
- **~100 ns** — Main memory reference (In-process JS object)
- **~0.5 ms** — Same-region network RTT (App to Redis)
- **~5-15 ms** — CDN edge hit (vs 200 ms to origin)
- **~1-10 ms** — Indexed Postgres query, warm buffer pool (~50-200 ms cold on disk)
- **~200 ms** — Intercontinental RTT (The cost a CDN removes)
...
*A read can be answered at any layer. Each hop skipped is roughly an order of magnitude of latency and cost removed.*

**Who owns correctness at each layer**

| Layer | Keyed by | Typical lifetime | Who can invalidate it | The specific risk |
| --- | --- | --- | --- | --- |
...
> **Layer 3 is the one you cannot take back**  
> Every other layer has some operator with a lever. The HTTP browser cache has none: once
...
There are only a handful of ways to wire a cache to its backing store, and the choice
...
| Pattern | Write latency | Staleness window | Data loss on cache failure | Use when |
| --- | --- | --- | --- | --- |
...
## Stampede, hot keys, and the failure modes that matter

A **cache stampede** (or thundering herd) is what happens the instant a popular key
...
*Request collapsing: one recompute, and the rest either wait briefly or serve the previous value. Without it, all 5,000 hit the database.*

**Single-flight plus jitter plus negative caching**

```js
const inflight = new Map(); // collapses concurrent misses per process
...
A cache key is a claim that two requests deserve the same answer. Get it too broad and
...
**Invalidation strategies, in the order you should try them**

| Strategy | Propagation | Cost | Where it applies |
| --- | --- | --- | --- |
...
> **Delete, do not update, on write**  
> The classic cache-aside race: request A reads from the DB (getting value v1), then
...
| --- | --- | --- |
| Stampede at TTL expiry on a hot key | Thousands of simultaneous identical DB queries; database saturates and the miss becomes self-sustaining. | Request collapsing (single-flight lock), TTL jitter, early probabilistic recompute, and serve-stale-while-revalidating. |
| Hot key on one Redis shard | One node at 100% CPU while the cluster idles; adding shards changes nothing. | Replicate the key under N suffixes and read a random one, plus a 1-2 s in-process cache in front of Redis. |
| Personalised response cached under a shared key | User A's name, email or balance served to user B -- a reportable data incident, not a bug. | Default `private, no-store`; caching opt-in per route; CI check that no response with `Set-Cookie`/`Authorization` context is `public`. |
| No negative caching | Random-ID traffic passes every layer straight to the database; trivial to weaponise. | Cache not-found sentinels with a shorter TTL (~30 s). Consider a Bloom filter for very large key spaces. |
| Schema change with live old-shape entries | Mixed response shapes crash clients for the length of the longest TTL. | Version prefix in the key so a deploy abandons every old entry atomically. Never mutate a shape in place. |
| Cache-aside update race | Cache holds v1 while the DB holds v2, with a fresh TTL -- permanently inconsistent and invisible. | Delete on write rather than update. For a real guarantee, compare-and-set on a monotonic version. |
| Redis outage with cache-aside | 100% miss rate instantly; the database receives traffic it was never sized for. | Circuit-break to a degraded read path, load-shed non-essential queries, and keep an in-process L1 so a Redis blip is not a 20x DB multiplier. |
| Uncoordinated TTLs across layers | A value expires in Redis but the CDN holds it for 5 more minutes; freshness is the *longest* TTL, not the shortest. | Document the effective staleness budget as the sum of the layers and make TTLs decrease as you move outward. |
...
A product page shows an old price. Query cache staleTime is 30 s, CDN `s-maxage` is 60 s, the BFF caches for 10 s, and Redis holds the price for 300 s. What is the worst-case staleness a user can observe?
- A. 30 seconds -- the shortest TTL wins.
- B. 300 seconds -- the longest single TTL.
- C. About 400 seconds -- the layers compose, since each can be populated from an already-stale layer below. **(answer)**
- D. Unbounded, because the browser cache has no TTL.

  Staleness composes downward. Redis can hand a 299-second-old price to the BFF, which caches that for 10 s, which the CDN then caches for 60 s, which the query cache then holds for 30 s. The user's worst case is roughly the sum, not the max and definitely not the min. This is why a freshness requirement has to be allocated as a budget across layers -- and why the fix for "the price is stale" is almost always at the *innermost* long-TTL layer, not the one closest to the user.

You switch from `DELETE key` to `SET key <new value>` on every write, reasoning that it saves the next reader a cache miss. What did you introduce?
- A. Nothing -- writing through is strictly better than deleting.
- B. A race where a concurrent slow read writes its older value after your update, leaving the cache permanently inconsistent with the database. **(answer)**
- C. A stampede, because every write now expires the key.
- D. Higher memory use only.

  The interleaving is: reader R reads v1 from the database and stalls; writer W commits v2 and sets the cache to v2; R resumes and sets the cache to v1 with a fresh TTL. The cache now serves v1 while the database holds v2, and nothing corrects it until the TTL elapses -- there is no error, no log line, and no alert. Deleting on write does not eliminate the race but collapses it to the gap between delete and the next read, after which the cache repopulates from current state. A real guarantee needs compare-and-set against a monotonic version.

One Redis node is at 100% CPU while the other eleven are near idle. Traffic is dominated by reads of a single trending item. What actually helps?
- A. Add more shards to the cluster.
- B. Increase the TTL on that key.
- C. Replicate the value under N suffixed keys and read a random one, and add a 1-2 s in-process cache in front of Redis. **(answer)**
- D. Switch the cluster to write-back.

  Redis Cluster assigns a key to a slot by hashing the key, so a single key is always on exactly one node no matter how many shards exist -- adding capacity cannot spread a single key. You either spread the key artificially (`item:42:0` through `item:42:9`, read one at random, invalidate all on write) or you stop the requests reaching Redis at all, which is what a very short in-process L1 does: at 5,000 rps across 20 hosts, a 1-second local TTL turns 5,000 Redis reads per second into 20. A longer TTL does not reduce read rate, and write-back is about write durability, which is not the problem here.

<details><summary>Related topics and how they connect</summary>

Layer 5 gets full treatment in **CDN & Edge Delivery**, including the personalisation
...
- **Which cache layer can you never invalidate?** — The HTTP browser cache. Once a response is stored with a `max-age`, it lives on the user's disk for that duration and no purge, deploy or API reaches it. The only recovery is serving from a different URL -- which is why content-hashed filenames exist.
- **What is a cache stampede and what are the three defences?** — When a hot key expires, every concurrent request misses at once and all hit the origin for the same value. Defences: request collapsing (single-flight lock so one recomputes), TTL jitter so keys do not expire together, and early probabilistic recompute or serve-stale-while-revalidating so nobody meets a cold key.
- **Why does adding Redis shards not fix a hot key?** — Cluster mode maps a key to a slot by hashing the key, so one key always lives on one node regardless of cluster size. You must either spread the value across N suffixed keys or prevent the reads reaching Redis at all with a very short in-process cache.
- **Cache-aside vs write-back -- what is the real difference?** — Cache-aside populates on miss and writes go to the database, so a cache failure means slow, not lost. Write-back acknowledges the write from the cache and flushes asynchronously, so writes are fast but unflushed data is permanently lost if the cache dies. Acceptable for counters and metrics, never for orders.
- **Why delete a cache key on write rather than update it?** — Updating races with an in-flight read that already holds the old row: the slow reader writes its stale value after your update, with a fresh TTL, leaving a silent inconsistency that never resolves. Deleting shrinks the window to the gap before the next read, which repopulates from current state.
- **What does a version prefix in a cache key buy you?** — `product:v3:42` means a schema change is deployed by bumping the prefix. Every old-shape entry is abandoned atomically with no purge and no window where clients receive two different shapes; the orphans simply expire.
- **What is negative caching and why is the TTL shorter?** — Storing a "not found" sentinel so repeated lookups of a non-existent ID do not pass through to the database -- the defence against enumeration and broken retry loops. The TTL is shorter (say 30 s vs 300 s) because not-found becoming found is a far likelier transition than the reverse.
- **How do you calculate the staleness a user can actually observe?** — Roughly the sum of the TTLs along the path, not the minimum -- each layer can be populated from an already-stale layer beneath it. Allocate freshness as a budget across layers, and make TTLs shorten as you move outward toward the user.
- **Why is a very high hit rate a risk as well as a win?** — It hides how the origin behaves under real load. A 96% hit rate means the database has never seen 25x its current read volume, so a cache outage or cold start is an untested capacity event. Find out deliberately with a cold-start game day rather than during an incident.

## Drills

### Drill

A merchant updates a product price in your admin tool. Support tickets say customers still see the old price up to eight minutes later, inconsistently -- sometimes the listing is new and the detail page is old. The stack is React Query, a CDN, a Node BFF, Redis, and Postgres. Find it and fix it.
...
Your Redis cluster fails over during peak traffic. The 30-second gap produces a 100% miss rate, Postgres hits connection-pool exhaustion, and the site is down for eleven minutes -- eight of them after Redis was already healthy. Design the resilience story.
