...
> Connection lifecycle, fanout, backpressure, presence and reconnection you must design for.

- Track: **Frontend Systems** · Level: **core** · ~18 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/realtime-frontend)

Realtime means the server can tell the browser that something changed without the browser
...
Start with the naive alternative and cost it out. A dashboard polls `GET /notifications`
...
## The five options, and how to choose

**Transport decision table**

| Transport | Direction | Real cost | Reconnection | Choose it when |
| --- | --- | --- | --- | --- |
...
> **SSE and the HTTP/1.1 connection limit**  
> Browsers cap concurrent connections per origin at 6 on HTTP/1.1. An SSE stream holds one
...
**SSE, including the part people forget**

```js
// Server: the headers are the whole protocol.
res.writeHead(200, {
...
## Connection lifecycle: the part that is actually the work

A WebSocket that works on your laptop tells you almost nothing. The connection will
...
*Resuming and Resync are different states. Collapsing them means every reconnect costs a full snapshot, which is what turns a gateway restart into an outage.*

**Full jitter, heartbeat, and resume in about 40 lines**
...
## Ordering and idempotent application

A single WebSocket connection preserves order, because it is one TCP stream. Almost
...
## Presence, fanout and backpressure

**Presence** looks trivial and is not, because a browser tab does not reliably tell
...
- **Server to slow client** — A backgrounded tab or a client on a weak link stops draining. The OS send buffer fills and the gateway buffers in user space -- 10,000 slow clients at 1 MB of queued messages each is 10 GB and an OOM. Cap the per-connection queue, and on overflow drop the connection with a close code that means "reconnect and resync" rather than growing forever.
- **Client to server** — Check `ws.bufferedAmount` before sending. If it is climbing, your messages are queueing locally, not in flight. Coalesce -- one cursor position per animation frame instead of one per `mousemove`, which is a 60x reduction -- and drop stale intermediate states rather than delivering a backlog of positions nobody cares about.
- **Subscription multiplexing** — Open one socket and multiplex logical subscriptions over it with a framing envelope carrying a channel id. Six sockets for six widgets costs six handshakes, six auth checks and six reconnect storms. Route on the client by channel.
- **Render backpressure** — A 200-message burst must not be 200 React renders. Buffer incoming messages and flush on a `requestAnimationFrame` or a 50 ms timer, applying them as one batched state update. Otherwise the main thread is the bottleneck and your UI freezes while it is "working".

## Auth on a connection that outlives its token

HTTP auth is per request, so an expired token produces a 401 on the next call and you
...
**Infrastructure timeouts that will close your connection**

| Layer | Typical default | What you see | What to do |
| --- | --- | --- | --- |
...
| --- | --- | --- |
| Reconnect with no backoff or jitter | A gateway restart synchronises 40,000 clients into a retry storm that keeps the system down. | Full jitter -- uniform in [0, min(30 s, 500 ms × 2^n)) -- plus a server-side connection-rate limit as a second line of defence. |
| No resume token | Every reconnect requests a full snapshot; a mass reconnect becomes a self-inflicted load spike on the data path. | Monotonic `seq` per message, `since=lastSeq` at handshake, server replays only the gap from a bounded buffer. |
| Load balancer idle timeout below heartbeat interval | Connections drop on a precise cadence and look like a mysterious network problem. | Heartbeat at 20-30 s, raise the idle timeout to 300 s or more, and alert on close-code distribution -- 1006 clustering is the tell. |
| Delta messages applied twice after replay | Corrupted local state -- badge counts and balances that drift and never self-correct. | Send facts with versions rather than increments, and drop any message at or below `lastSeq`. |
| Unbounded per-connection send queue | Slow clients grow gateway memory until the node OOMs, dropping every connection it held. | Cap the queue, then disconnect with a resync close code. Alert on queue depth per connection. |
| Presence tracked by explicit leave events | Ghost users accumulate forever; "12 people viewing" when there is one. | Lease-based presence with a heartbeat-renewed TTL; expire after 2-3 missed beats. |
| Authorisation never revalidated | A user who lost document access keeps streaming updates for hours -- a data-exposure incident. | Publish revocation events on the bus; gateway drops affected subscriptions and forces re-auth. |
| One socket per widget | Six handshakes, six auth checks and six reconnect loops per user; connection limits hit on mobile. | One multiplexed connection with a channel-id envelope, shared across tabs via `SharedWorker`. |
...
You are building a live deploy-log viewer: server streams text, client sends nothing. What transport?
- A. WebSocket, since it is the general realtime answer.
- B. SSE, because it is one-way and gives reconnection with `Last-Event-ID` replay for free. **(answer)**
- C. Long polling, for maximum proxy compatibility.
- D. WebTransport, for lowest latency.

  The traffic is one-way and text, which is precisely SSE's shape, and the browser's built-in reconnect-and-replay is the code you would otherwise write, test and get subtly wrong. Choosing WebSocket here means hand-rolling backoff, heartbeat and resume for no benefit. The condition to check is HTTP/2 end to end, since an HTTP/1.1 hop reimposes the six-connections-per-origin limit.

Your gateway is redeployed and stays down for eight minutes after coming back. Logs show auth service saturation. What is the most likely bug?
- A. Heartbeat interval is too short.
- B. Clients reconnect immediately, or with a fixed delay plus small jitter, so the fleet retries in lockstep. **(answer)**
- C. The load balancer idle timeout is too low.
- D. Messages are not idempotent.

  This is a synchronised retry storm. Every client was disconnected at the same instant, so any near-uniform retry delay keeps them clumped and each wave re-saturates auth, which causes more failures and more retries. Full jitter -- a uniform random value between zero and the growing ceiling -- spreads the fleet across the window. Pair it with a server-side connection-rate limit, because you cannot trust every client version in the wild to behave.

After a reconnect, users report their unread badge is wrong -- sometimes double the real count.
- A. The WebSocket delivered messages out of order.
- B. Replayed messages were applied again because the updates are increments rather than versioned facts. **(answer)**
- C. The heartbeat is racing the badge update.
- D. Two sockets are open.

  At-least-once replay after reconnect is normal and expected. An `increment badge` message applied twice permanently corrupts the count with no path back to correctness. A message saying `unread = 3, version 118` is naturally idempotent -- applying it twice is a no-op -- and combined with dropping anything at or below `lastSeq` it makes duplicate delivery a non-event. Two sockets would double *live* messages too, not only after reconnect.

A backgrounded tab keeps getting reaped by the server as dead even though the user has not left. Why?
- A. Browsers close WebSockets when a tab is hidden.
- B. Background timers are throttled to roughly one per minute, so the heartbeat stops firing inside the server's expiry window. **(answer)**
- C. The access token expired.
- D. `navigator.onLine` flipped to false.

  Hidden tabs have their timers heavily throttled -- typically around one execution per minute -- so a 20-second heartbeat simply does not run, and the server correctly concludes the client is gone. Fighting the throttle is not an option. The right design is to accept it: close the socket after a grace period on `visibilitychange` to hidden, then reconnect and resume by sequence number when the tab becomes visible again.

<details><summary>Related topics and how they connect</summary>

The sequence-number and resync machinery here is the same machinery that drives
...
- **When is SSE the better choice than WebSocket?** — When traffic is one-way server-to-client and text: notifications, live logs, LLM token streams. You get automatic reconnection with `Last-Event-ID` replay for free, provided you have HTTP/2 end to end.
- **Why does SSE need HTTP/2?** — On HTTP/1.1 browsers allow six connections per origin and each SSE stream holds one for its lifetime, so a few tabs starve your normal API calls. HTTP/2 multiplexes streams on one connection, raising the practical limit to around 100.
- **What is full jitter and why not fixed-delay-plus-random?** — Full jitter picks a uniform random delay in [0, min(cap, base × 2^n)). Adding a small random offset to a fixed delay leaves the fleet clumped, so each retry wave still arrives together and re-saturates the service.
- **What is a resume token for?** — So a reconnecting client can say `since=lastSeq` and receive only the gap. Without it every reconnect costs a full snapshot, and a mass reconnect after a gateway restart becomes a bigger outage than the restart.
- **Why prefer facts over deltas in realtime messages?** — Delivery is at-least-once, so replays happen. Applying `unread = 3, version 118` twice is a no-op; applying `increment unread` twice permanently corrupts state with no path back to correct.
- **Why must presence be lease-based?** — Browsers do not reliably announce departure -- no `beforeunload` on a crash, force-quit or OS tab discard. So online means "renewed a TTL recently"; expire after 2-3 missed heartbeats or you accumulate ghosts forever.
- **Both directions of WebSocket backpressure?** — Server side: cap the per-connection send queue and disconnect with a resync code, or slow clients OOM the gateway. Client side: check `ws.bufferedAmount`, coalesce to one message per frame, and drop stale intermediate states.
- **How do you authenticate a WebSocket without headers?** — Mint a single-use ticket valid for about 30 seconds over authenticated HTTP and exchange it at handshake, so the credential in the URL and access logs is nearly worthless. Then renew over the connection with an `auth.renew` message.
- **Why do connections drop exactly every 60 seconds?** — A default idle timeout -- AWS ALB and nginx `proxy_read_timeout` are both 60 s. Heartbeat inside the window *and* raise the timeout; a cluster of close code 1006 is the diagnostic signal.

## Drills

### Drill

Design the realtime layer for a customer-support console. 8,000 agents are logged in at once, each watching a queue of about 30 conversations plus presence for their team. A new message must appear within a second. Agents work from home on variable connections and from offices behind corporate proxies.
