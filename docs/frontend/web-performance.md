...
> Core Web Vitals, RUM vs lab, long tasks, budgets in CI, and tying latency to revenue.

- Track: **Frontend Systems** · Level: **core** · ~21 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/web-performance)

Performance engineering is the practice of deciding what "fast enough" means, measuring
...
For a long time the industry measured the wrong things. `window.onload` was the headline
...
## The metrics, with current thresholds

**Core Web Vitals, measured at p75 across all page loads**

| Metric | Good | Needs improvement | Poor | What it actually measures | Dominant cause |
| --- | --- | --- | --- | --- | --- |
...
Three details about these metrics that are routinely misunderstood and that an
...
## RUM vs lab, and why they disagree

Lab measurement -- Lighthouse, WebPageTest, a CI run -- is a synthetic load on fixed
...
*Lab and field answer different questions. Gating on lab and deciding on field is the only combination that works; swapping them is the classic failure.*

**Field data segmented -- the table that changes priorities**

| Segment | Share of sessions | p75 LCP | p75 INP | What it tells you |
| --- | --- | --- | --- | --- |
...
> **The aggregate is the least useful number you have**  
> Read the table above: the aggregate p75 LCP of 2.6 s is a rounding error away from
...
A performance budget is only a budget if exceeding it blocks something. A dashboard
...
**A budget configuration that actually blocks a merge**

```json
// size-limit: runs in seconds on every PR, deterministic, no flake.
// package.json
{
  "size-limit": [
...
**RUM collection with the attribution you will actually need**

```js
import { onLCP, onCLS, onINP, onTTFB } from 'web-vitals/attribution';
...
The attribution field is what makes RUM actionable rather than merely alarming. A
...
## Images and fonts: the two biggest levers

The LCP element is an image on the large majority of pages, so image strategy *is* LCP
...
**Fonts** produce two distinct failure modes and the CSS property `font-display` picks
...
**The LCP image and the font, done properly**

```html
<!-- Discovered in the initial HTML, fetched at high priority,
...
Third-party scripts are frequently the largest single cost on a page and the one the
...
## Tying latency to money

Performance work needs a business case, and there are published figures you can cite --
...
**Published figures -- cite with their source, not as universal law**

- **+8%** — Vodafone sales, from a 31% LCP improvement (web.dev case study)
- **+53.4%** — Rakuten 24 revenue per visitor (After reaching good CWV)
- **+8.4%** — Retail conversion per 0.1 s mobile speed gain (Deloitte, Milliseconds Make Millions)
- **+10.1%** — Travel conversion per 0.1 s (Same Deloitte study)
- **-1%** — Amazon sales per 100 ms (2006 Greg Linden talk; widely misattributed)
...
| --- | --- | --- |
| Optimising against aggregate p75 | Mid-tier Android in emerging markets fails badly at 4.6 s LCP while the aggregate reads 2.6 s; a quarter of users are invisible in the number that drives decisions. | Segment RUM by device class and country, set per-segment targets, and review the worst segment rather than the aggregate. |
| Lazy-loading the hero image | LCP regresses several hundred milliseconds because the browser deliberately defers the most important resource on the page. | `loading="eager"` plus `fetchpriority="high"` plus a `preload` for anything that could be the LCP element. Lazy-load below the fold only. |
| Green Lighthouse score, users complaining | Engineering believes the problem is solved while real INP is 600 ms; trust in the performance programme erodes. | Treat lab as a regression gate only. Never accept a local Lighthouse run as evidence about real users; INP cannot be measured in the lab at all. |
| RUM collected without device, network, route or release dimensions | You can see that something regressed and not what, where, or which deploy caused it. | Attach release SHA, route pattern, device memory, core count and effective connection type at collection time; use `web-vitals/attribution` for element and phase breakdown. |
| Third-party scripts added without a budget or owner | 600 KB of vendor JavaScript on the main thread; INP is dominated by code you did not write and cannot profile. | Inventory with long-task attribution by script URL, cap count and size in CI, require one-out-one-in, and name an owner who can refuse. |
| `font-display: block` on a webfont | Up to 3 s of invisible text; if the LCP element is text, LCP is delayed by the entire font load. | `font-display: swap`, preload the above-the-fold faces, self-host, and tune fallback metrics with `size-adjust` and ascent/descent overrides so the swap does not shift layout. |
| Flaky Lighthouse CI gate on a single run | False failures, developers learn to re-run until green, and within a month the check is disabled entirely. | Median of 5 runs, assert on stable metrics, keep bundle-size checks as the deterministic first line, and provide a named override path. |
| Quoting "100 ms costs 1% of revenue" as a general law | A stakeholder checks the source, finds a 2006 talk about a different business, and the whole performance case loses credibility. | Cite figures with their source and audience, then measure your own funnel by metric bucket, and run a holdback for a causal number. |
...
Your p75 INP is 480 ms. Attribution shows 340 ms input delay, 40 ms processing time, and 100 ms presentation delay. What do you fix?
- A. Optimise the click handler -- it is clearly doing too much work.
- B. Find and break up the long task that was already occupying the main thread when the click arrived. **(answer)**
- C. Reduce the size of the rendered output to cut presentation delay.
- D. Debounce the interaction so fewer handlers run.

  INP decomposes into input delay (waiting for the main thread to become free), processing time (your handler), and presentation delay (rendering the resulting frame). Here the handler is 40 ms -- entirely healthy -- and 71% of the total is queueing before it ever ran. That means something unrelated was blocking the thread: hydration, a large re-render, an analytics flush, or a third-party script. Optimising the handler would remove at most 40 ms from 480 ms. This is exactly why attribution matters: without it, the intuitive move is to optimise the handler, and it would have been close to wasted work.

A team reports that Lighthouse improved from 68 to 94 after their optimisation sprint, but field CLS and INP both got worse. What is the most likely explanation?
- A. The field data is stale and will catch up within a few days.
- B. Lab and field measure different populations and different things -- Lighthouse never interacts, so it cannot see INP, and its single cold desktop-emulated run does not reflect the device mix or the interaction patterns of real sessions. **(answer)**
- C. Lighthouse scores and Core Web Vitals are unrelated metrics.
- D. The team must have made a measurement error in the RUM collection.

  Lighthouse is one synthetic cold load on emulated hardware from one location, with no user interaction at all -- so INP is not measurable there and TBT is only a proxy. It is a good regression gate and a poor description of your users. Real sessions span a five-fold range of CPU speed, warm and cold caches, lossy networks, and interactions nobody scripted. A common concrete version of this divergence: the team added deferred-loading behaviour that improved the lab score while injecting content later in real sessions, which raised field CLS. The correct posture is lab in CI for regressions, field data for deciding what to work on and whether it worked.

You add a base64-inlined low-quality placeholder for the hero image. LCP improves from 3.2 s to 1.6 s. Should you report this as a win?
- A. Yes -- LCP is the metric and it halved.
- B. Not without checking: if the placeholder is now the LCP element, the metric improved while the user waits the same time for the real image, and the inlined bytes have grown the HTML. **(answer)**
- C. Yes, and CLS will also improve automatically.
- D. No -- LQIPs are always counted as a layout shift.

  LCP measures the largest contentful paint, and a full-width placeholder can satisfy that definition perfectly well. So the number moves without the experience changing, which is the textbook example of optimising a metric rather than an outcome. Verify with attribution which element LCP actually resolved to. There is also a second cost: inlined base64 is roughly 33% larger than the binary and lands in your HTML, competing with the first TCP congestion window that your critical CSS needs. A placeholder is still often worth having for perceived performance, but report it honestly -- and if the goal is genuinely faster hero content, the fix is preload plus `fetchpriority="high"` plus a modern format.

A stakeholder asks for the revenue impact of a proposed 800 ms LCP improvement. What is the most defensible answer?
- A. Cite Amazon's 100 ms / 1% figure and extrapolate to 8%.
- B. Segment your own funnel conversion by LCP bucket as directional evidence, state plainly that it is correlational, and propose a 10% holdback to get a causal number. **(answer)**
- C. Decline to estimate, since performance impact cannot be measured.
- D. Use the Deloitte 8.4% figure as a forecast for your site.

  Published figures establish that the mechanism is real and worth testing; they are not forecasts for your product, and a stakeholder who checks the source of the Amazon number will find a 2006 talk about a different business, which damages your whole case. Your own funnel segmented by LCP bucket is better evidence but still confounded -- slow sessions correlate with older devices, worse networks and different demographics, all of which independently affect conversion. A holdback is the only design that separates the effect of speed from the characteristics of the users who experience it: ship to 90%, keep 10% on the old path, compare. It costs a little revenue on the holdback and it produces a number that survives scrutiny.

<details><summary>Related topics and how they connect</summary>

Long tasks, the frame budget and what makes INP bad are **Browser & Rendering
...
- **What are the Core Web Vitals thresholds, and at what percentile?** — LCP ≤ 2.5 s, CLS ≤ 0.1, INP ≤ 200 ms, all evaluated at the 75th percentile of page loads. p75 means one in four users can be worse and you still pass, which is why segmenting by device and geography matters more than the aggregate.
- **Why is INP harder than the FID it replaced in March 2024?** — FID measured only the input delay of the first interaction -- essentially queueing time -- so a page could pass while every interaction took a second to show a result. INP measures input to next paint for approximately the worst interaction, so it includes the handler and the rendering that follows.
- **What are the three parts of INP, and why does the split matter?** — Input delay (waiting for a free main thread), processing time (your handler), presentation delay (rendering the frame). If most of it is input delay, the handler is irrelevant and you need to find the long task blocking the thread -- usually hydration or a third-party script.
- **What does LCP decompose into, and which slice is usually largest?** — TTFB, resource load delay, resource load duration, and element render delay. Load *delay* is often the biggest -- the image was discovered late because it was referenced from CSS, injected by JavaScript, or lazy-loaded. That is a preload and `fetchpriority` fix, not a compression fix.
- **Name four reasons lab and field data disagree.** — Device distribution (real traffic spans a 5x CPU range), cache state (lab is always cold), network reality (packet loss and the 100-500 ms mobile radio promotion), and interaction (lab tools never click, so INP is unmeasurable and TBT is only a proxy). Plus geography and unfiltered bot traffic.
- **What is the most common LCP bug in the wild?** — A lazy-loaded hero image. `loading="lazy"` tells the browser to deliberately defer the most important resource on the page, adding hundreds of milliseconds. Use `loading="eager"`, `fetchpriority="high"` and a `preload` for anything that could be the LCP element.
- **FOIT vs FOUT -- which do you choose and how do you mitigate it?** — `font-display: block` gives FOIT, up to 3 s of invisible text which can delay LCP entirely. `font-display: swap` gives FOUT, immediate fallback text with a shift on swap. Choose swap, then remove the shift with `size-adjust`, `ascent-override` and `descent-override` on a fallback `@font-face`.
- **Why is a single Lighthouse run a bad CI gate?** — Variance is high enough to produce false failures, and a flaky gate gets disabled within a month. Take the median of about five runs, keep deterministic bundle-size checks as the first line on every PR, and provide a named override path so the gate is not deleted during an incident.
- **How do you make a credible business case for performance work?** — Cite published figures with their source and audience (Vodafone +8% sales from a 31% LCP improvement; Deloitte +8.4% retail conversion per 0.1 s) as evidence the mechanism is real, then segment your own funnel by metric bucket while naming the confounders, then run a holdback -- 90% treated, 10% on the old path -- for a causal number.
...
A product team wants to add a session-replay tool. It is 180 KB gzipped, executes on the main thread, and the analytics team says it is essential for understanding a checkout drop-off. Your INP budget is already at 190 ms against a 200 ms target. What do you do?
