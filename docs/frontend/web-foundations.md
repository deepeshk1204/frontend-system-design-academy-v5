...
> DNS, TCP, TLS and HTTP as one traceable path you can reason about end to end.

- Track: **Frontend Systems** · Level: **foundation** · ~16 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/web-foundations)

You type a URL and press Enter. Somewhere between 300 milliseconds and four seconds
...
The web was designed for a world where a document lived on one machine and a reader
...
*Three separate network phases before a single byte of HTML: name resolution, transport setup, and the request itself.*

**What the browser actually does**

1. **Parse and normalise the URL.** Scheme, host, port, path, query, fragment. The fragment never leaves the browser. The browser also checks its **HSTS** list here -- if the host is on it, an `http://` URL is upgraded to `https://` internally with no network request and no redirect.
2. **Resolve the name.** Consult, in order: the in-process DNS cache, the OS resolver cache, then the configured recursive resolver. Only a full miss walks root, TLD and authoritative servers.
3. **Open a transport connection.** TCP three-way handshake, one RTT. Over QUIC this merges with the crypto handshake.
4. **Negotiate TLS.** One RTT on TLS 1.3, two on 1.2. ALPN in the ClientHello is where `h2` or `h3` is chosen, so protocol selection is free -- it rides the handshake you were already paying for.
5. **Send the request.** One RTT to first byte at best, plus however long the server takes to produce it. This is the server-think part of TTFB.
6. **Stream and parse the HTML.** The parser discovers subresources as bytes arrive and starts fetching them before the document is complete. This is why where you put a `<script>` still matters.
7. **Build the render tree, lay out, paint, composite.** Covered in *Browser & Rendering Pipeline*.

## DNS and its four caching layers

DNS resolution is almost never a full recursive walk, and that is the whole point. There
...
**Where a DNS answer comes from**

| Layer | Typical lookup cost | Respects record TTL? | What it means for you |
| --- | --- | --- | --- |
...
> **Anycast is why this is not worse**  
> Recursive resolvers and CDN edges both advertise the *same* IP address from hundreds of
...
## TCP: the handshake and the slow start you forgot about

The three-way handshake -- SYN, SYN-ACK, ACK -- costs one RTT before you may send
...
## TLS 1.3, and what 0-RTT really buys

TLS 1.2 needed two round trips: ClientHello/ServerHello, then key exchange and Finished.
...
- **3 RTT** — Cold TCP + TLS 1.2 + request (The legacy path)
- **2 RTT** — Cold TCP + TLS 1.3 + request (Today's default)
- **1 RTT** — QUIC 1-RTT, or resumed TLS 1.3 (Handshake merged)
- **0 RTT** — QUIC 0-RTT early data (Idempotent requests only)

## The latency budget

Here is the same page load at three network profiles. The point of the table is not the
...
**Time to first byte, by network profile**

| Phase | Same-region fibre (RTT 10 ms) | Cross-continent (RTT 150 ms) | 4G mobile, cold (RTT 70 ms + radio) |
| --- | --- | --- | --- |
...
> **The mobile radio tax nobody budgets for**  
> On cellular, an idle radio must be promoted to a connected state before any packet
...
**Resource hints, in decreasing order of aggression**

```html
<!-- Full DNS + TCP + TLS to an origin you will definitely use.
...
| --- | --- | --- |
| Third-party origin sprawl | Each of 12 origins costs a cold DNS+TCP+TLS on first paint; mobile LCP degrades by seconds. | Inventory origins in CI and fail the build past a threshold. Self-host fonts. `preconnect` only the 2-3 that are truly critical. |
| DNS TTL set to 86400 for agility, then a region fails | Failover DNS change takes up to a day to propagate; browsers pin longer still. | TTL of 30-60s on user-facing records, plus health-checked Anycast or a load balancer so failover does not depend on DNS at all. |
| 0-RTT enabled for all methods | A captured `POST` is replayed; a payment or write executes twice. | Restrict early data to safe methods at the edge. Enforce server-side idempotency keys for anything that mutates. |
| Critical CSS over 14 KB | First paint waits an extra round trip or two for the congestion window to grow. | Inline only above-the-fold CSS, keep the first flight under ~14 KB compressed, defer the rest. |
| Redirect chains on the entry URL | `http://x` to `https://x` to `https://www.x` is two extra full connection setups before any HTML. | Preload HSTS, canonicalise to one host at the DNS/edge layer, and never redirect across origins on the landing path. |
| Optimising server time when RTT dominates | Weeks of backend work moves p75 LCP by 40 ms because the user pays 800 ms in setup. | Break TTFB into DNS / connect / TLS / wait using `PerformanceNavigationTiming` and fix the largest slice. |
...
A user in Sydney loads your US-hosted page. RTT is 200 ms, the server takes 50 ms to render. TTFB is measured at roughly 650 ms. What is the biggest single win available?
- A. Optimise the server render from 50 ms to 20 ms.
- B. Terminate TLS at an edge PoP near Sydney so the handshake round trips are local. **(answer)**
- C. Enable brotli instead of gzip on the HTML.
- D. Lower the DNS TTL so resolvers refresh faster.

  Three of those 650 ms are round trips: TCP, TLS 1.3, and the request itself -- 600 ms of the 650. Server time is 50 ms, so even making it instant wins under 8%. Terminating TLS at a nearby PoP turns two of the three round trips into ~5 ms local ones; only the final origin fetch still crosses the Pacific, and a cache hit removes even that. Compression affects transfer, not setup, and DNS TTL affects freshness, not latency.

Why is TLS 1.3 0-RTT early data restricted to safe methods in practice?
- A. Early data is sent unencrypted, so it would leak request bodies.
- B. Early data can be captured and replayed, and the server cannot distinguish a replay from the original. **(answer)**
- C. The congestion window is too small to carry a POST body.
- D. Browsers do not implement 0-RTT for POST.

  Early data is encrypted, but it is sent before the handshake gives the server forward-secrecy and freshness guarantees for that flight, so an on-path attacker can replay the bytes verbatim and the server has nothing to reject them with. That is harmless for a cacheable `GET` and dangerous for a transfer or an order. The defence pairing is: allow 0-RTT only on safe methods at the edge, and enforce idempotency keys server-side so a replay of any mutation is a no-op.

Your critical CSS is 42 KB after compression and inlined in the HTML head. Why might splitting it hurt less than you expect and inlining hurt more?
- A. Inlined CSS is never cached, so it is always slower.
- B. A new TCP connection can only send ~14 KB in its first flight, so 42 KB takes about three round trips of transfer before first paint. **(answer)**
- C. HTTP/2 refuses to stream responses above 32 KB.
- D. Brotli cannot compress inlined CSS.

  TCP slow start begins at an initial congestion window of roughly 10 segments -- about 14 KB -- and doubles per RTT. A 42 KB first flight therefore needs 14 KB, then 28 KB: two extra round trips before the browser has the styles it needs to paint. At 150 ms RTT that is 300 ms of first paint you bought by inlining. The fix is to inline only genuinely above-the-fold rules and keep the initial document under the first window.

<details><summary>Related topics and how they connect</summary>

The round trips described here are exactly what **CDN & Edge Delivery** removes by
...
- **How many round trips before the first request byte on a cold TLS 1.3 connection?** — Two -- one for the TCP handshake, one for TLS 1.3. TLS 1.2 needs three total. QUIC merges transport and crypto into one, and resumption with 0-RTT can reach zero for safe methods.
- **Why does the initial TCP congestion window matter to first paint?** — A fresh connection may only send about 10 segments (~14 KB) before waiting for acknowledgements, doubling each RTT. Anything critical above ~14 KB costs additional round trips of pure transfer.
- **Difference between `dns-prefetch`, `preconnect` and `preload`?** — `dns-prefetch` resolves the name only. `preconnect` does DNS + TCP + TLS and holds a socket. `preload` actually fetches a specific resource and requires an `as` attribute to set priority and the `Accept` header.
- **Why is 0-RTT early data unsafe for `POST`?** — The early-data flight is replayable -- an on-path attacker can resend it and the server has no handshake freshness context to reject it. Safe for idempotent `GET`; mutations need either 1-RTT or server-side idempotency keys.
- **What is Anycast and what does it break?** — One IP address advertised from many physical sites, with BGP routing each client to the nearest. It gives global low-latency from a single address, but it also means "the IP is up" says nothing about whether a specific site is healthy.
- **Which DNS cache ignores your record TTL?** — The browser's. Chrome pins entries to roughly 60 seconds regardless of TTL, so DNS changes alone cannot be relied on for fast failover -- you need health-checked Anycast or a load balancer behind a stable address.
- **What is the mobile radio promotion tax?** — On cellular an idle radio must be promoted to connected before any packet is sent -- typically 100-500 ms on LTE. It precedes your first RTT, never appears in server metrics, and is the main reason lab TTFB understates real mobile TTFB.
- **Where is the HTTP protocol version actually chosen?** — In ALPN, carried inside the TLS ClientHello. The client offers `h2`, `http/1.1` (and advertises `h3` via `Alt-Svc` or DNS HTTPS records) and the server picks, so negotiation costs no extra round trip.

## Drills

### Drill

Your p75 TTFB in India is 1.4 s while your US p75 is 180 ms. The origin is a single region in us-east-1 and server-side render time is a steady 60 ms at p95. You have two weeks. Walk me through how you find the real cost and what you change.
...
A payments team wants 0-RTT enabled at the edge because it removes a round trip from their checkout API and their p95 is over budget. Their endpoint is `POST /v1/charges`. What do you tell them, and what would make it acceptable?
