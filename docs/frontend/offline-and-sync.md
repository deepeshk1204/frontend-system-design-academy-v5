...
> Local source of truth, outbox pattern, conflict resolution, and idempotent mutations.

- Track: **Frontend Systems** · Level: **staff** · ~19 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/offline-and-sync)

An offline-first app treats the copy of the data on the device as the thing the user
...
Two separate forces push apps here, and they want different things.
...
**Browser storage, honestly compared**

| Store | Shape | Sync or async | Realistic limit | Use it for |
| --- | --- | --- | --- | --- |
...
IndexedDB has a deserved reputation for an unpleasant API -- versioned upgrade
...
**Storage figures worth knowing**

- **~60%** — Of free disk, Chrome origin quota (Shared across IndexedDB, Cache Storage and OPFS)
- **~1 GB** — Typical Safari cap per origin (Prompts the user beyond it; historically much lower)
- **7 days** — Safari eviction of unused origin storage (Without a persistence grant or app install)
- **~5-10 MB** — localStorage, synchronous (Every read blocks the main thread)
- **~100 ms** — Threshold for an interaction to feel instant (Why local-first wins even when online)

## The service worker, and the update trap

A service worker is a script that sits between your page and the network and can serve
...
Here is the core mechanism. When a user performs a write, you do three things in one
...
**An outbox record schema that survives contact with production**
...
*Four distinct outcomes. Collapsing 409 into the retry path is how an app retries a doomed mutation for three days.*

## Idempotent mutations

Offline sync guarantees duplicate delivery, so idempotency is not optional. The specific
...
**The wire format for a retryable mutation**

```http
PATCH /api/tasks/0f3a7c21-9bd4-4e11-8a52-2c9f0e1d7b33
...
Detection and resolution are separate problems and conflating them is the usual mistake.
...
| Mechanism | How it works | Cost | Fits |
| --- | --- | --- | --- |
...
For resolution, the decision hinges on whether silently discarding one side's work is
...
> **The queue that retries forever**  
> A mutation that can never succeed -- the entity was deleted server-side, the user lost
...
## Partial sync: pulling changes efficiently

Pushing is the outbox. Pulling is a change feed, and the shape that works is a cursor,
not a timestamp.
...
Offline bugs are timing bugs, and the DevTools offline checkbox tests almost none of
...
| --- | --- | --- |
| Local write and outbox append in separate transactions | Silent data loss -- the UI shows a saved change that will never sync. | One IndexedDB transaction spanning both stores. Test with a crash injected between the two writes. |
| Retrying without an idempotency key | Duplicate entities and double charges when a success response is lost. | Client-generated UUID as `Idempotency-Key`, stable across retries; server stores key-to-result for 24 h or more. |
| Conflicts detected by client timestamp | The user with the most skewed clock wins every race; edits vanish unpredictably. | Server-assigned versions with `If-Match`; client clocks are for display only. |
| Permanent failure retried forever | A strictly ordered queue blocks all later mutations; users report saving has stopped. | Classify 4xx as permanent, cap at ~10 attempts, park the record and surface retry or discard in the UI. |
| Service worker never activates | Users stay on an old build for weeks and hit chunk-load errors. | Detect the waiting worker, prompt, then `skipWaiting` plus reload. Serve the worker script with `no-cache`. |
| Storage evicted without a persistence grant | The local database disappears -- on Safari after seven days of non-use -- taking unsynced writes with it. | Call `navigator.storage.persist()` and check the result; sync opportunistically rather than hoarding; warn when unsynced work is at risk. |
| Sync feed has no tombstones | Deleted records live forever on devices and reappear when a user edits one. | Explicit deletion events in the feed with a 30-day retention, and force a full resync for cursors older than the window. |
| Two tabs draining the same outbox | Duplicate in-flight requests and lost updates as they race on the same records. | Elect one leader via the Web Locks API or a `SharedWorker`; a single drain loop per device. |
...
A field technician edits a job offline, then reconnects. Your client PUTs the whole job object. A dispatcher had reassigned it while the technician was offline. What happens?
- A. The server rejects the write because the version is stale.
- B. The reassignment is silently overwritten, because the PUT carries every field including a stale `assignee`. **(answer)**
- C. The fields merge automatically.
- D. A conflict dialog appears.

  A full-object PUT asserts values for fields the user never touched, so the technician's stale `assignee` overwrites the dispatcher's change with no error anywhere. Two fixes and you want both: send partial patches containing only changed fields, and send `If-Match` with the base version so the server can reject a stale write instead of applying it. Without the precondition the server has no way to know the write was based on old state.

Your outbox drains strictly in order. One record gets a 403 because the user lost project access. What is the user-visible symptom?
- A. Only that one change fails.
- B. Every later change stops syncing while the queue retries a doomed request. **(answer)**
- C. The outbox is cleared.
- D. The app goes offline.

  Strict ordering plus indiscriminate retry means one permanently-failing record becomes a head-of-line block, and the user sees saving stop entirely with no explanation. Classify errors -- 4xx other than 408, 409 and 429 are permanent -- park the record after a capped number of attempts, and surface it with retry or discard. Ordering only genuinely matters between mutations on the same entity, so consider per-entity queues rather than one global one.

You ship a new service worker on Tuesday. On Thursday, 30% of sessions still run Monday's build. Most likely cause?
- A. The CDN cached the app shell too long.
- B. The new worker is stuck in `waiting` because those users never closed every controlled tab, and a reload does not activate it. **(answer)**
- C. IndexedDB migration failed.
- D. Users declined the update prompt.

  A new worker installs and then waits until every page controlled by the old one is closed. Reloading does not help, because the old worker survives the navigation, so a user with a pinned tab can stay on an old build indefinitely. The fix is to detect the waiting worker, tell the user, and call `skipWaiting` plus a reload when they accept -- doing it unconditionally instead risks the loaded page requesting chunks the new build no longer has.

<details><summary>Related topics and how they connect</summary>

The sequence-cursor and resume machinery here is the same idea as reconnection in
...
- **What is the outbox pattern?** — Apply the mutation to the local store and append an intent record to an outbox queue in one atomic transaction, then have a background loop drain the queue to the server with retries, backoff and error classification.
- **Why must the local write and outbox append share a transaction?** — Otherwise a crash between them leaves the UI showing a change that will never be sent -- silent data loss that is effectively unreproducible in development.
- **Why generate entity ids on the client?** — Creates become idempotent by construction, the local record never needs its id rewritten, and records created offline can already reference their parents without temporary ids and a rewrite pass.
- **What exact failure does `Idempotency-Key` protect against?** — The server committing the mutation and the response being lost. The client cannot distinguish that from a request that never arrived, so it retries; the key lets the server replay the stored result instead of re-executing.
- **Why never detect conflicts with client timestamps?** — Client clocks are routinely minutes off and can be set deliberately, so timestamp-based last-write-wins hands the race to whoever has the most skewed clock. Use server-assigned versions with `If-Match`.
- **When should a conflict be shown to the user?** — Only when both sides changed the same field of content the user authored. Different fields should merge, and prompting on every version mismatch just trains people to dismiss the dialog.
- **Why does a new service worker not take effect on reload?** — It installs and waits until every page controlled by the old worker is closed; a reload keeps the old worker alive. You must detect the waiting worker and call `skipWaiting` plus a reload, ideally with user consent.
- **What does `navigator.storage.persist()` do?** — Requests the persistent storage bucket so your origin is exempt from ordinary eviction. It can be denied based on engagement signals, so check the returned value -- especially on Safari, which clears script-writable storage after seven days of non-use.
- **Why does a sync change feed need tombstones?** — A client cannot infer deletion from absence, so deletions must be explicit events. Tombstones need a retention window, and a client with a cursor older than that window must be forced into a full resync.

## Drills

### Drill

Design offline support for a field-service app. Technicians work in basements with no signal for up to six hours, complete checklists and capture photos, and dispatchers reassign jobs from the office in the meantime. Each technician has about 40 jobs assigned, and photos average 2 MB.
...
You need to change the local IndexedDB schema: a `tasks` store gains a required `projectId`, and you are splitting one store into two. Some users have been offline for three weeks with 200 pending outbox records written by the old client. Design the migration.
