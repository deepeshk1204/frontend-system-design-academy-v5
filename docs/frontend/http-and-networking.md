...
> Idempotency, status semantics, multiplexing, QUIC, and why retries are dangerous.

- Track: **Frontend Systems** · Level: **foundation** · ~16 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/frontend/http-and-networking)

HTTP is an agreement about the shape of a request and a response, plus a set of promises
...
The original problem was interoperability: any client should be able to talk to any
...
## Safe, idempotent, cacheable -- three different things

These are routinely conflated in interviews and they are genuinely distinct properties.
...
**Method properties, and what each implies for retries**

| Method | Safe | Idempotent | Cacheable | Can a proxy retry it on timeout? |
| --- | --- | --- | --- | --- |
...
## Idempotency keys: the concrete pattern

A network timeout is ambiguous. The client does not know whether the request never
...
*The key is claimed before the work starts, so an in-flight duplicate is detectable rather than a race.*

**Retry-safe mutation, wire level**
...
> **Retries without jitter are a self-inflicted DDoS**  
> When an API starts timing out, every client retries. If they all retry after exactly
...
The classes are worth stating precisely because the boundary between 4xx and 5xx is a
...
| Code | Means | When you actually reach for it |
| --- | --- | --- |
...
A conditional request attaches a validator so the server can answer "nothing changed"
...
## The wire format: 1.1, 2, and 3

**HTTP/1.1** sends one request per connection at a time. Pipelining was specified but is
...
*HTTP/2 solved head-of-line blocking at the application layer and inherited it at the transport layer. QUIC is the fix.*

**Protocol comparison, practical view**
...
> **Connection coalescing -- why "split your assets across subdomains" reversed**  
> If `cdn.example.com` and `img.example.com` resolve to the same IP and are covered by
...
## Prioritisation, Early Hints, and compression

HTTP/2's original priority scheme was a dependency tree with weights, and it was
...
## CORS preflight, mechanically

CORS is a browser-enforced rule, not a server-side security control. The server always
...
**Preflight for a credentialed JSON POST**

```http
OPTIONS /v1/orders HTTP/2
...
| --- | --- | --- |
| Client retries `POST` on timeout with no idempotency key | Duplicate orders and double charges; reconciliation work and refunds. | Server-side `Idempotency-Key` claimed before work, replaying the stored response; `409` while in flight; `422` if the body differs. |
| Synchronised exponential backoff across all clients | Retry storm keeps the service down after the trigger clears. | Full jitter, client circuit breakers, `Retry-After` on `429`/`503`, and never retry a 4xx. |
| Validation errors returned as `500` | On-call paged for user typos; real incidents lost in the noise; clients retry unretryable requests. | `400` for malformed, `422` for semantically invalid; alert on 5xx rate only and keep 4xx on a separate dashboard. |
| Missing `Vary: Origin` on a CORS response behind a CDN | Cached `Access-Control-Allow-Origin` for tenant A served to tenant B; the API appears randomly broken. | `Vary: Origin` on every dynamic CORS response, and an allowlist rather than reflecting whatever `Origin` arrives. |
| HTTP/2 on a lossy mobile network | p95 worse than HTTP/1.1 because one lost packet stalls all streams. | Enable HTTP/3 with `Alt-Svc` and keep TCP fallback; measure p95 by protocol and network type in RUM, not just the mean. |
| Domain sharding retained after the HTTP/2 migration | Four extra DNS+TCP+TLS setups and four cold congestion windows on first paint. | Consolidate onto one origin, or ensure shards share an IP and certificate SAN so the browser coalesces connections. |
| Unconditional `PUT` from two concurrent editors | Silent lost update -- the second save erases the first with no error anywhere. | `If-Match` with the current ETag, `412` on mismatch, and `428` to reject unconditional writes outright. |
...
Your gateway retries any request that times out after 2 s. Which endpoint is most likely to cause a customer-visible correctness bug?
- A. `GET /v1/invoices/42`
- B. `PUT /v1/users/42/profile` with the full profile body
- C. `PATCH /v1/accounts/42` with `{"op":"increment","path":"/credits","value":50}` **(answer)**
- D. `DELETE /v1/sessions/abc`

  `GET` is safe, and both `PUT` (full replacement) and `DELETE` are idempotent -- running them twice leaves the same end state, so a blind retry is harmless. The relative `PATCH` is the outlier: applying an increment twice adds 100 credits instead of 50, and nothing in the protocol lets the gateway detect that. This is exactly why `PATCH` must not be in a gateway retry policy unless the patch semantics are absolute or an idempotency key is enforced.

You migrate from HTTP/1.1 to HTTP/2 and p50 latency improves, but p95 on mobile in a market with ~2% packet loss gets worse. What explains it?
- A. HPACK compression fails on large cookies, forcing renegotiation.
- B. One TCP connection means a single lost packet blocks delivery of all multiplexed streams, whereas six connections isolated the damage. **(answer)**
- C. HTTP/2 disables TLS session resumption.
- D. Stream priorities starve the lowest-priority requests indefinitely.

  HTTP/2 removed application-layer head-of-line blocking but runs over TCP, which guarantees in-order byte delivery. A lost segment makes the kernel buffer everything received after it, including frames belonging to completely unrelated streams, until retransmission arrives. Six HTTP/1.1 connections spread that risk so a loss only stalls one sixth of your traffic. The real fix is HTTP/3, where QUIC implements streams in user space with independent loss recovery -- which is why you should measure p95 by protocol and network type rather than trusting an aggregate improvement.

Two editors open the same document and save 10 seconds apart. The second save silently erases the first editor's changes. What is the minimal protocol-level fix?
- A. Switch the save endpoint from `PUT` to `PATCH`.
- B. Have clients send `If-Match` with the ETag they loaded, and have the server return `412` when it does not match. **(answer)**
- C. Add a `Cache-Control: no-store` header to the document response.
- D. Add an `Idempotency-Key` to the save request.

  This is a lost update, and conditional writes are the standard answer: the client proves which version it edited by sending `If-Match: "<etag>"`, and the server refuses with `412 Precondition Failed` if the resource has moved on, forcing a re-read and a merge decision. `PATCH` narrows the blast radius for non-overlapping fields but still loses concurrent edits to the same field. An idempotency key solves duplicate submission of *the same* operation, which is a different problem. If you want to make unconditional writes impossible, answer them with `428 Precondition Required`.

Why did HTTP/2 server push get removed from Chrome in favour of `103 Early Hints`?
- A. Push was incompatible with TLS 1.3.
- B. The server cannot know what the client already has cached, so pushed bytes were frequently discarded; Early Hints lets the client choose. **(answer)**
- C. Push required a second TCP connection.
- D. Early Hints compresses better under HPACK.

  Push put the decision on the wrong side. A server pushing `app.css` has no reliable view of the browser cache, so on repeat visits it spends bandwidth -- and competes with the critical path -- on bytes that get thrown away. Cache digests were proposed and never shipped. `103 Early Hints` sends `Link: rel=preload` headers during server think time and leaves the fetch decision with the browser, which does know its own cache. You get the same latency win on cold loads with no waste on warm ones.

<details><summary>Related topics and how they connect</summary>

Handshake round trips and 0-RTT are in **Web Foundations: URL to Pixels**. The
...
- **Safe vs idempotent -- what is the difference?** — Safe means read-only, no intended state change (`GET`, `HEAD`, `OPTIONS`). Idempotent means N identical requests leave the same state as one. `DELETE` and `PUT` are idempotent but not safe. `POST` is neither; `PATCH` is not guaranteed idempotent.
- **Why is `PATCH` not idempotent?** — A patch body can express a relative change -- increment a counter, append to a list -- and applying it twice compounds. An absolute-value patch happens to be idempotent, but the method makes no such promise, so no proxy or gateway may retry it.
- **How does an idempotency key work, including the edge cases?** — Client sends a unique key per logical operation. Server claims the key *before* doing work; a repeat of a completed key replays the stored response, a repeat while in flight returns `409`, and the same key with a different body returns `422` because that is a client bug, not a retry.
- **Does HTTP/2 solve head-of-line blocking?** — Only at the application layer. TCP still delivers one ordered byte stream, so one lost packet stalls every multiplexed stream on that connection. Above roughly 2% loss a single HTTP/2 connection can be worse than six HTTP/1.1 ones. HTTP/3 over QUIC fixes it with per-stream loss recovery.
- **What is connection coalescing?** — If two hostnames resolve to the same IP and are covered by the same certificate, an HTTP/2 browser reuses one connection for both. It is why domain sharding became an anti-pattern -- extra hostnames now cost handshakes and cold congestion windows instead of buying parallelism.
- **What makes a CORS request "simple", and why does it rarely matter?** — `GET`/`HEAD`/`POST`, no custom headers, and `Content-Type` limited to form-urlencoded, multipart, or text/plain. `application/json` is excluded, so nearly every modern API call triggers an `OPTIONS` preflight -- a full extra round trip unless `Access-Control-Max-Age` is set.
- **Why does `Vary: Origin` matter on CORS responses?** — Without it a shared cache stores one origin's `Access-Control-Allow-Origin` value and serves it to requests from other origins, so the API appears to fail intermittently for some tenants. Combined with credentialed requests -- which forbid `*` -- it is a correctness requirement, not a tuning knob.
- **`412` vs `428` -- when do you use each?** — `412 Precondition Failed` answers a conditional write whose `If-Match` ETag no longer matches, preventing a lost update. `428 Precondition Required` rejects a write that arrived with *no* precondition at all, which is how you make blind overwrites impossible by policy.
- **Why did server push lose to `103 Early Hints`?** — The server cannot see the client cache, so push wasted bandwidth on assets the browser already had and competed with the critical path. Early Hints sends `Link: rel=preload` during server think time and lets the browser, which knows its own cache, decide whether to fetch.

## Drills

### Drill

A checkout service processes roughly 800 requests per second. During a 90-second database failover, clients time out and retry; afterwards support reports about 1,200 duplicate charges and the service stayed degraded for 15 minutes after the database recovered. Design the fix, client and server side.
...
You own a widely embedded JSON API. p75 latency from browsers is 340 ms but your server-side p75 is 40 ms, and the gap is larger in Brazil and India. The API is on `api.example.com` while the embedding apps are on many customer domains. Where is the 300 ms and what do you change?
