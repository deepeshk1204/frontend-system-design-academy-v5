...
> KV cache, continuous batching, TTFT vs throughput, quantisation, and GPU capacity planning.

- Track: **AI Engineering** · Level: **staff** · ~21 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/ai/inference-serving)

Serving a language model means running a fixed set of weights against a stream of
...
The naive way to serve a model is one request at a time: load the weights, run the
...
Read the last two rows together, because that is the whole tension. Decode wants the
...
*One pass for the whole prompt, then one pass per output token. The KV cache is what makes the second part possible.*

## The KV cache, with the arithmetic

Attention at position *n* needs the key and value vectors for every position before it.
...
**KV cache size, and a worked example**
...
> **Why grouped-query attention mattered so much**  
> Compare the two blocks above: GQA cut the per-token KV footprint by roughly 8x in that
...
**GPU memory budget on an 80 GB accelerator, 70B-class model**

- **~140 GB** — Weights at BF16 (Does not fit; needs 2+ GPUs or quantisation)
- **~70 GB** — Weights at FP8 (Fits on one 80 GB card with little room left)
- **~35 GB** — Weights at INT4/W4 (Leaves ~40 GB for KV and activations)
- **2-5 GB** — Activations and workspace (Scales with batch and chunk size)
- **~0.31 MiB** — KV per token, GQA example above (Multiply by every token of every live request)
- **~128 concurrent** — 8k-token requests in ~40 GB of KV (Your real concurrency ceiling)

Note what that last figure means in practice. Your concurrency limit is a *memory*
...
## PagedAttention, and why fragmentation mattered

Early serving stacks allocated KV cache as one contiguous block per request, sized for
...
Static batching is the intuitive design and it is badly wrong for this workload:
...
*The scheduler re-decides every iteration. Decode is prioritised; prefill fills the leftover token budget.*

## Prefix caching, chunked prefill, speculative decoding, disaggregation

Four optimisations that stack, each attacking a different part of the problem.
...
**What each knob does to TTFT, ITL and throughput**

| Knob | TTFT | ITL | Throughput | Notes |
| --- | --- | --- | --- | --- |
...
**Two vLLM configurations for the same model, tuned for opposite goals**

```bash
# Interactive chat: protect TTFT and smooth streaming.
...
Quantisation stores numbers in fewer bits. It buys three things at once -- less memory
...
## Parallelism and capacity planning

When a model does not fit on one GPU you split it, and the two ways to split are not
interchangeable.
...
The economics are not close at low volume, and they invert at high volume. An API
...
**Running your own inference tier**
...
**How inference tiers fall over**

| Failure mode | What the user sees | Mitigation |
| --- | --- | --- |
| KV cache exhaustion under load | Requests preempted and their KV recomputed; throughput collapses non-linearly while GPU utilisation still reads high. | Admission control on free KV blocks, cap `max-model-len` to real traffic, alert on preemption rate and KV utilisation, autoscale on queue depth. |
| Long prefill stalling the decode batch | Every streaming user sees a multi-hundred-millisecond pause when one user pastes a long document. | Chunked prefill with a tuned `max_num_batched_tokens`; disaggregated prefill/decode if tail ITL is contractual. |
| Prompt prefix reordered by a "harmless" change | Prefix cache hit rate falls to near zero; TTFT and cost both jump with no code change visible in review. | Treat prompt prefix order as an interface: stable content first, volatile last; alert on cache hit rate as a first-class metric. |
| Quantised model regresses on a narrow slice | JSON conformance or long-context accuracy drops; aggregate benchmarks look fine; downstream parsers break. | Run your own eval suite per slice on the exact quantised artefact, and gate the rollout on schema conformance specifically. |
| Autoscaling on GPU utilisation | Scale-up never triggers because batched decode always looks busy; queue grows and TTFT violates SLO. | Scale on queue depth and time-in-queue; use KV utilisation and preemption rate as secondary signals. |
| Cold start on scale-out | New replica takes minutes to load weights; the spike is over before capacity arrives. | Warm replica floor, weights on local NVMe or baked into the image, pre-warm before routing, plus an API overflow path. |
| Speculative decoding enabled globally | Latency improves in staging, total throughput drops under production saturation. | Enable adaptively by load; monitor draft acceptance rate; disable above a concurrency threshold. |
| Advertising max context you do not need | Per-request KV reservation caps batch size; concurrency and throughput silently far below hardware capability. | Set `max-model-len` from measured p99 context length; offer long context on a separate, differently-tuned pool. |
...
Your chat service streams smoothly most of the time, but users report occasional multi-hundred-millisecond pauses mid-response. GPU utilisation is high and steady. What is the most likely cause?
- A. Network jitter between the client and the load balancer.
- B. A long prefill from another request is occupying the batch, stalling every decoding sequence. **(answer)**
- C. The KV cache is full and requests are being preempted.
- D. The model is too large for the GPU.

  Prefill is compute-bound and, unless chunked, occupies the forward pass for its whole duration -- so one user pasting a 30k-token document pauses everyone else’s stream. That is the classic ITL spike. The fix is chunked prefill with a smaller `max_num_batched_tokens`, which slices the prefill and interleaves it with decode steps, or disaggregated prefill/decode if tail ITL is contractual. KV preemption is a real failure mode but presents as throughput collapse and rising preemption counters rather than isolated mid-stream pauses.

You are serving a 70B-class model with GQA (80 layers, 8 KV heads, head dim 128) in BF16 on 4x80 GB GPUs. Average context is 8k tokens. What most directly determines your maximum concurrency?
- A. The number of tensor cores available for matrix multiplication.
- B. GPU memory left after the weights, divided by roughly 0.31 MiB per token times context length. **(answer)**
- C. The provider rate limit.
- D. Network bandwidth to the clients.

  KV per token is 2 x 80 x 8 x 128 x 2 bytes, about 0.31 MiB, so an 8k-token request needs roughly 2.5 GiB of KV. Concurrency is whatever memory remains after weights and activations divided by that figure -- compute is almost never the binding constraint in decode, because decode is memory-bandwidth-bound. This is why FP8 KV cache, GQA and a realistic `max-model-len` all raise throughput: they are concurrency levers disguised as memory settings.

Why is autoscaling an inference tier on GPU utilisation a mistake?
- A. GPU utilisation is expensive to measure.
- B. Batched decode keeps utilisation high whether or not requests are queueing, so it does not signal user-visible pain. **(answer)**
- C. GPU utilisation is always 100% on modern accelerators.
- D. Utilisation only reflects prefill, never decode.

  The scheduler keeps the GPU busy by design -- that is what continuous batching does -- so utilisation stays high while queue wait time grows and TTFT breaches the SLO. Scale on queue depth and time-in-queue, which lead the user-visible symptom, with KV cache utilisation and preemption rate as secondary signals. Rising preemption in particular means you are recomputing KV for evicted requests and throughput is about to fall non-linearly.

A colleague proposes enabling speculative decoding globally to reduce latency. What is the correct caveat?
- A. It changes the output distribution, so quality may drop.
- B. It helps latency at low-to-moderate load but can reduce total throughput under saturation, since verification compute competes with other requests’ tokens. **(answer)**
- C. It requires FP8 hardware support.
- D. It only works with a fine-tuned draft model.

  Verification is mathematically equivalent to normal sampling, so there is no quality cost -- that is the appeal. The cost is compute: on an idle-ish server the spare capacity is free and you get several tokens for roughly one token’s memory traffic, but on a saturated server that compute would otherwise have produced real tokens for other requests. Gate it on concurrency and monitor draft acceptance rate, because a low acceptance rate means you are paying for verification and discarding the drafts.

<details><summary>Related topics and how they connect</summary>

Prefix caching here is the same mechanism exploited commercially in **Model Routing,
...
- **Why are prefill and decode fundamentally different workloads?** — Prefill processes all input tokens in one parallel pass and is compute-bound. Decode produces one token per pass, sequentially, and is memory-bandwidth-bound because each pass reads the whole weight matrix. Prefill governs TTFT, decode governs inter-token latency.
- **Write the KV cache size formula.** — 2 (K and V) x layers x kv_heads x head_dim x bytes_per_element x sequence_length. For 80 layers, 8 KV heads, head dim 128, BF16: about 0.31 MiB per token, so an 8k-token request is roughly 2.5 GiB.
- **What did PagedAttention actually fix, and what was the bigger win?** — It replaced contiguous per-request KV allocation sized for max output -- which wasted most of the cache on reservations -- with fixed 16-token blocks and a block table. The bigger win is that blocks became *shareable*, which is the mechanism underneath prefix caching and cheap parallel sampling.
- **Why does continuous batching beat static batching?** — Static batching runs until the longest sequence finishes, leaving completed slots idle. Continuous batching evicts finished sequences and admits queued ones every iteration, so the GPU never waits for a straggler. The win grows with output-length variance.
- **What does `max_num_batched_tokens` trade off?** — It is the prefill token budget per scheduler step. Smaller (around 2,048) gives better ITL because prefills interrupt decode less; larger (8,192+) gives better TTFT and throughput. It is the clearest single expression of the latency-throughput frontier.
- **When does speculative decoding hurt?** — Under saturation. Verification compute competes with other requests’ real tokens, so total throughput can fall even as single-request latency improves. It is a latency optimisation for low-to-moderate load, and its value scales with draft acceptance rate.
- **Weight-only versus weight-and-activation quantisation?** — Weight-only (W4A16, W8A16) shrinks stored weights and dequantises to compute in higher precision -- directly attacks the decode bottleneck with modest quality risk. Weight-and-activation (FP8 W8A8, W4A8) also narrows activations so the matmuls run in low precision: faster, riskier, needs calibration.
- **Tensor versus pipeline parallelism?** — Tensor parallelism shards each layer across GPUs, cutting memory *and* latency but paying an all-reduce per layer -- needs NVLink-class interconnect, use within a node. Pipeline parallelism assigns whole layers to GPUs with small communication but adds latency and bubbles -- use it to make a model fit across nodes, not to go faster.
- **What should you autoscale an inference tier on?** — Queue depth and time-in-queue, with KV cache utilisation and preemption rate as secondary signals. Never GPU utilisation -- batched decode reads as busy whether or not anyone is waiting, so it never triggers scale-up while TTFT breaches.
...
Your self-hosted inference tier has been stable for months. After a release, p95 TTFT went from 900 ms to 4.2 s and throughput fell about 35%. GPU utilisation is unchanged and high. No infrastructure change was deployed -- only application code. Diagnose it.
