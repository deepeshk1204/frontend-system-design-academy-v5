...
> Cascades, semantic and prefix caching, per-tenant budgets, and provider failover.

- Track: **AI Engineering** · Level: **staff** · ~19 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/ai/model-routing-and-cost)

Model routing is deciding, per request, which model should handle it -- and cost control
...
In a conventional service, cost is roughly proportional to traffic and dominated by
...
- **10-100x** — Spread in cost per request, one endpoint (Why request counts tell you nothing about spend)
- **usually >70%** — Share of spend that is input tokens (Prompt, context, history -- resent every call)
- **often 5-15%** — Spend on retries and abandoned runs (Measure it; it is rarely zero)
- **order of 10x** — Price gap between frontier and small models (The prize routing is chasing)
- **up to ~90%** — Typical discount on cached input tokens (Varies by provider; check current terms)
- **~50%** — Typical discount for async batch APIs (In exchange for a long completion window)
- **quadratic** — Growth of agent input tokens in step count (History resent each step)

> **Build the unit-economics model before you optimise**  
> Write down, for one representative request: system prompt tokens, retrieved context
...
## Cascades: small model first, escalate on a signal

The central observation is that request difficulty is very unevenly distributed. Most
...
*Two routing decisions: a cheap up-front classification, and a verification-driven escalation. Both get logged as route reason.*

Routing by **task class** is separate from routing by difficulty, easier, and often
...
## Prompt-prefix caching: the highest-leverage lever

If you do one thing, do this. Providers and self-hosted engines both cache the KV state
...
**Prompt assembly ordered for cache hits**

```typescript
// Most stable first, most volatile last. This ordering is an interface:
...
## Response caching, and where it becomes dangerous

Caching whole responses is different from caching prefixes, and considerably riskier.
...
**Caching tiers, what they save, and what they can break**

| Tier | Key | Typical saving | Risk |
| --- | --- | --- | --- |
...
## The other cost knobs

**Batch and async APIs.** Most providers offer an asynchronous batch endpoint at a
...
## Per-tenant budgets and graceful degradation

Without per-tenant accounting you cannot price the product, cannot stop one customer
...
**Budget check with staged degradation**

```python
# Enforced in the gateway, against a shared counter. Per-process budgets do not work.
...
You want a single internal interface in front of several providers, for three reasons
...
Cost regressions arrive the same way quality regressions do -- in an innocent-looking
...
**Routing, cascades and layered caching**
...
**Cost and routing failures**

| Failure mode | What the user sees | Mitigation |
| --- | --- | --- |
| Prompt reordered, prefix cache hit rate collapses | Cost and TTFT both jump with no visible cause; reviewers saw nothing wrong in the diff. | Stable-first prompt assembly, a CI assertion on the prefix, and an alert on cache hit rate as a first-class metric. |
| Semantic cache without tenant scoping | One customer receives another customer’s answer. Security incident, not a cost bug. | Tenant and permission scope in the cache key, always. Whitelist safe intents, short TTL, log original and matched query for audit. |
| Semantic cache threshold too loose | Confidently wrong answers to near-miss questions -- especially negations and qualifiers -- that no error rate reveals. | Tune the threshold against labelled pairs from your own traffic; restrict to stable non-personalised intents; measure cache-hit answers in your eval set. |
| Escalation rate creeps up unnoticed | Cascade savings evaporate and you now pay for two calls per request instead of one. | Escalation rate as a monitored metric with an alert; investigate as a quality regression, since it usually means retrieval got worse. |
| Per-process budget enforcement | Ten replicas each allow the full limit, so the effective cap is 10x the intended one. | Shared atomic counter with a windowed TTL, enforced in the gateway rather than in each application. |
| Hard 429 as the only budget behaviour | A cost threshold becomes a customer outage and an escalation someone has to resolve manually. | Staged degradation -- notify, then cheap tier and lower k, then cache-only and deferral, then a clear message with a reset time and an upgrade path. |
| Retries on non-transient failures | A malformed-output failure is retried three times at full cost and fails identically each time. | Retry only transport-level errors with jittered backoff; on validation failure, change something -- repair prompt or escalate tier -- or give up. |
| Interactive pricing for background work | Nightly enrichment costs roughly double what it needs to, indefinitely. | Route anything without a waiting human to the async batch endpoint; make that the default for background jobs in the platform. |
| Silent failover to an untuned provider | Quality drops noticeably during a provider incident and nobody can explain why; possibly a compliance breach if the region was constrained. | Per-model tuned prompt variants gated by evals; treat failover as a declared capability tier; encode residency constraints in the router. |
...
Your AI feature costs 3x forecast. Where do you look first?
- A. Reduce `max_tokens` to shorten responses.
- B. Input token composition -- system prompt, tool schemas, retrieved context, history -- plus retry and escalation rates. **(answer)**
- C. Negotiate a volume discount with the provider.
- D. Switch to a cheaper provider across the board.

  Input tokens usually dominate because everything static is re-sent on every call and history is re-sent every agent step, so a 12k-token system prompt or k=20 retrieval is a far bigger lever than output length. Retries and cascade escalations are the other common surprise -- work you paid for and discarded. Shortening outputs optimises the smaller side of the ledger, and switching provider before understanding the composition just moves an unexamined bill.

A teammate adds `Current date: {{today}}` to the top of the system prompt for date-awareness. What happens?
- A. Nothing measurable -- it is a few tokens.
- B. Prefix cache hit rate collapses, so input-token cost and TTFT both rise sharply. **(answer)**
- C. The model refuses requests with unstable prompts.
- D. Only requests spanning midnight are affected.

  Prefix caching is prefix-exact: the cache matches leading tokens up to the first difference, so a value that changes near the start invalidates everything after it. Every request now pays full prefill and full input-token price on content that used to be heavily discounted. The fix is placement, not removal -- put volatile values after the stable system prompt, tool schemas and few-shot examples. This is why cache hit rate needs an alert: the diff looks harmless and the regression is invisible until the invoice.

Which is the strongest reason to be cautious about semantic caching in a multi-tenant B2B product?
- A. Embedding the query adds latency.
- B. Without tenant and permission scope in the key it can serve one customer’s answer to another, and near-miss queries with different answers look similar in embedding space. **(answer)**
- C. Vector databases are expensive to operate.
- D. It only works for English.

  Two failures stack. The correctness one: similar is not the same -- "cancel my subscription" versus "cancel before renewal", "encrypted at rest" versus "in transit", and negations, which embeddings handle poorly. The security one is worse: a globally-keyed semantic cache is a cross-tenant data leak with a fuzzy match function, the same class of failure as caching personalised HTML in a shared CDN. If you use it, scope by tenant and permissions, whitelist stable intents, keep the TTL short, and log original and matched query for audit.

A tenant hits their monthly AI budget on day 22. What is the best default behaviour?
- A. Return 429 for the rest of the month.
- B. Degrade in stages: cheap tier and reduced context, then cache-only and deferral of background work, then a clear message naming the reset time and an upgrade path. **(answer)**
- C. Keep serving and invoice the overage silently.
- D. Fail over to a different provider to spread the cost.

  A hard 429 converts a cost threshold into a customer outage and a manual escalation under pressure. Staged degradation keeps the feature useful while cutting spend substantially -- a smaller model with k=4 can be a large multiple cheaper and only slightly worse. Silent overage destroys the point of a budget and creates a billing dispute. Failing over to another provider does not reduce cost at all; it just moves which invoice it lands on.

<details><summary>Related topics and how they connect</summary>

Prefix caching is the commercial face of the KV-cache mechanics in **Inference
...
- **Where does AI spend actually go?** — Mostly input tokens -- system prompt, tool schemas, retrieved context and history, all re-sent every call, and re-sent every step in an agent loop. Retries and abandoned runs are usually the second largest and least tracked line.
- **Why is prefix caching the highest-leverage cost lever?** — It cuts input-token price substantially (discounts around 90% are common) *and* reduces TTFT, with zero quality cost. Nothing else improves cost and latency at the same time.
- **What breaks prefix caching?** — Any token change near the start, because matching is prefix-exact. Timestamps, user names or request IDs at the top of the system prompt, and non-deterministically ordered retrieved chunks. Order prompts stable-first, volatile-last, and alert on cache hit rate.
- **What makes a good cascade escalation signal?** — A deterministic verification failure -- output did not parse, schema invalid, citation IDs do not resolve, SQL did not execute, tests failed. Then explicit `INSUFFICIENT_CONTEXT` style self-assessment, then log-prob signals, then a classifier trained on your own escalation labels.
- **When does a cascade stop paying?** — When escalation is common. At a 10x tier price gap, escalating 20% of traffic still saves roughly 70%; escalating 60% saves almost nothing once you count the wasted first call. Monitor escalation rate and treat a rise as a quality regression.
- **Why is semantic caching dangerous?** — Similar questions can have different answers -- qualifiers and negations especially, since embeddings handle negation poorly. And without tenant plus permission scope in the key it leaks data across tenants, the same failure class as caching personalised HTML in a shared CDN.
- **How should a tenant budget be enforced and what happens at the limit?** — A shared atomic counter in the gateway, never per process, or the effective cap multiplies by replica count. Degrade in stages: notify at ~70%, cheap tier and lower k at ~85%, cache-only plus deferral at 100%, then a clear message naming the reset time and an upgrade path.
- **Why is retrieval-k both a cost and a quality dial?** — k drives the largest volatile part of the prompt, so k=20 to k=5 cuts that component about 4x -- and quality often improves, because long contexts lose the middle and irrelevant chunks distract. Plot eval score against k and take the knee.
- **What is the honest caveat about multi-provider failover?** — The API surface is portable; the prompts are not. Models differ in instruction following, tool-call syntax, refusal boundaries, long-context behaviour and tokenisation. Treat failover as a declared capability tier with a tuned, eval-gated prompt variant per model.
