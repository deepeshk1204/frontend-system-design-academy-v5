...
> Ingest, chunk, index, retrieve, rerank, ground, cite -- and the eight places it silently fails.

- Track: **AI Engineering** · Level: **core** · ~22 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/ai/rag-architecture)

Retrieval-augmented generation means: before asking the model a question, go and find
...
A model's weights are a frozen, lossy, unattributable compression of its training
...
*Two loops: an offline ingestion loop and an online query loop. Most teams under-invest in the top one.*

## Ingestion: the underrated hard part

Every RAG tutorial starts from a folder of clean `.txt` files. No real corpus looks
...
> **Look at the extracted text before you index it**  
> Sample fifty documents across every source type and *read* the extracted text. Not the
...
## Chunking: a retrieval decision, not a preprocessing step

A chunk is the atom of retrieval: it is both what gets embedded into a single vector
...
**Chunking strategies and when each is right**

| Strategy | How it works | Use when | Fails when |
| --- | --- | --- | --- |
...
> **The heading-path trick**  
> Prepend the document title and heading breadcrumb to the text of every chunk *before*
...
**Starting points — then measure, do not defend them**

- **300-600 tok** — Child chunk for embedding (Small enough that one vector means one thing)
- **10-15%** — Overlap for fixed/recursive chunking (Guards against cutting a sentence in half)
- **1-2K tok** — Parent section returned to the LLM (Enough context to be self-contained)
- **4-8** — Chunks in the final prompt after reranking (Past this, precision falls and cost rises)

## Metadata is what makes retrieval controllable

Chunks are not just text. Attach, at minimum: `doc_id` and `chunk_index` so you can
...
**The chunk record, and what each field is for**
...
## Retrieval and the k trade-off

More context is not better context. Raising k raises the chance the answer is
...
The grounding prompt has three jobs: give the model the passages, tell it the answer
...
**Grounding prompt: the parts that matter**
...
**Post-generation verification — cheap, deterministic, high value**
...
**Keeping an index current without re-embedding the world**

1. Prefer **change events** over polling -- webhooks, CDC, or a source-side change feed. Polling a large corpus is expensive and always lags.
2. On a document change, re-parse and re-chunk it, then compare each chunk's `content_hash` against the index. Only changed chunks are re-embedded; on a typical edit that is a small fraction of the document.
3. Deletes must be **propagated, not filtered**. A deleted document that is merely hidden at render time still leaks through summarisation and through "what do you know about X".
4. Permission changes are deletes-for-someone. Treat ACL revocation as a high-priority index update with an explicit lag SLO, and still re-check authorisation before rendering.
5. Track **index lag** — the age of the oldest unpropagated change — as a first-class SLO. "The assistant told me something we corrected last week" is an index-lag incident.
6. Keep a monotonic `indexed_at` and a reconciliation job that re-walks the source to catch dropped events. Event streams lose messages; the reconciliation job is what makes the system eventually correct.

## Evaluate retrieval and generation separately

This is the highest-leverage idea in the topic, and skipping it is why RAG projects
...
**Two eval suites, two sets of signals**
...
## The eight places RAG fails silently

**Each row: the failure, what the user experiences, and the signal that detects it**

| Failure mode | What the user sees | Mitigation |
| --- | --- | --- |
| **1. Extraction mangled the source** — two-column PDF interleaved, table flattened, scan not OCRed. | A document is effectively absent from the index; the assistant insists the policy does not exist. | Read raw extracted text for 50 sampled docs per source type. Alert on documents producing near-zero text or abnormal character distributions. Keep extraction reproducible so you can redo it. |
| **2. Chunk boundary split the answer** — the term is in chunk 7, the condition in chunk 8. | Confidently incomplete answers that omit the exception. Worse than no answer. | Structural chunking plus parent-child retrieval. Detect it by checking whether labelled answer spans cross chunk boundaries in your eval set. |
| **3. Retrieval missed the right chunk** — dense-only search on an exact term, ID or rare word. | Plausible answer built from adjacent-but-wrong documents. | Hybrid BM25 + vector with RRF; measure recall@50 against labelled chunk IDs in CI. This is the single most common root cause. |
| **4. The right chunk was retrieved but ranked 30th** | It never reaches the prompt, so it looks identical to failure 3 from the outside. | Track recall@50 and recall@5 separately. A gap between them is a *reranking* problem, not a retrieval one — add a cross-encoder rather than a different embedding model. |
| **5. Context is right but the model ignored it** — lost-in-the-middle, or a conflicting prior. | The model answers from training data while citing your document. | Keep k at 4-8 and order best-first. Measure faithfulness with retrieval held fixed. Verify that cited passages actually support the claim. |
| **6. Stale or deleted content still indexed** | The assistant quotes a policy revoked last quarter, with a citation, which makes it more believable. | Event-driven updates plus a reconciliation sweep; index-lag SLO; hard-delete propagation, never render-time filtering. |
| **7. Permission leak via retrieval** — ACL filtered after search or not at all. | A user is shown or summarised content they cannot access. A security incident, not a quality issue. | ACL denormalised into the index and filtered during traversal; a final authorisation check before render; a test suite of cross-tenant queries in CI. |
| **8. Injection through retrieved content** — a document containing instructions to the model. | Exfiltration or unintended tool calls, triggered by content an attacker put in your corpus. | Delimit and label sources as data; never let retrieved text authorise an action; restrict tools available during grounded generation (see *AI Security & Guardrails*). |
| **9. No refusal path** — weak retrieval still produces an answer. | The worst answers are generated exactly when the system knows least, and there is no signal distinguishing them. | Threshold on the reranker score before generating; explicit refusal instruction; evaluate refusal accuracy on an unanswerable set; surface "low confidence" in the UI. |
| **10. Query and corpus vocabulary diverge** — users ask in support language, docs are written in engineering language. | Uniformly mediocre retrieval that no index tuning improves. | Query rewriting and HyDE; contextual retrieval at ingest; mine real user queries and check them against the corpus rather than inventing test questions. |
...
Retrieval recall@50 is 0.94 on your labelled set, but end-to-end answer quality is poor. Best next move?
- A. Swap to a better embedding model.
- B. Check recall@5 after reranking, and if the right chunk is being retrieved but ranked low, add or improve a cross-encoder reranker. **(answer)**
- C. Increase k from 8 to 30 so more context reaches the model.
- D. Reduce chunk size to improve precision.

  High recall@50 proves the retriever is finding the right chunks — the embedding model is not the problem, and replacing it is expensive and disruptive. The question is whether those chunks survive into the final prompt. If recall@5 is much lower than recall@50, the defect is ordering, which a cross-encoder fixes directly. Raising k to 30 usually makes things worse: more distractors, position effects in a longer context, and higher cost.

Users report the assistant citing a policy that was revoked last quarter. What is the systemic fix?
- A. Add "prefer recent documents" to the system prompt.
- B. Propagate deletes and updates into the index via change events with an index-lag SLO and a reconciliation sweep, rather than filtering stale content at render time. **(answer)**
- C. Lower the retrieval score threshold.
- D. Re-embed the corpus weekly.

  Render-time filtering is not enough, because retrieved content also feeds summarisation and follow-up reasoning — the revoked text still shapes the answer even when it is not shown. The index must be the source of truth about what exists. Event-driven updates handle the common path, a reconciliation sweep catches dropped events, and index lag becomes a monitored SLO. A prompt instruction cannot fix data that should not be retrievable.

Which chunking approach best balances embedding precision against generation context for a technical documentation corpus?
- A. Fixed 1,000-token chunks with 200-token overlap.
- B. Heading-aware structural chunking into ~400-token children with the heading path prepended, indexed for retrieval, returning the ~1,500-token parent section to the model. **(answer)**
- C. One chunk per document.
- D. Semantic chunking on sentence-similarity drops.

  The two jobs of a chunk conflict: a single vector represents a small focused passage well and a long one poorly, while the model needs surrounding context to answer correctly. Parent-child resolves the conflict by separating what you match on from what you send. The prepended heading path is what makes a chunk beginning "This does not apply to..." retrievable at all. Semantic chunking is a reasonable choice for unstructured narrative, but on documents that already carry structure you are paying to infer boundaries the author already marked.

<details><summary>Related topics and how they connect</summary>

Hybrid search, RRF, rerankers, query rewriting and permission-aware retrieval are
...
- **Why must retrieval and generation be evaluated separately?** — End-to-end quality mixes two independent subsystems with opposite fixes. Retrieval eval (recall@k against labelled chunk IDs) needs no LLM, runs in CI in seconds. Generation eval holds retrieval fixed and measures faithfulness. Without the split you cannot tell whether the retriever missed the document or the model ignored it.
- **What does a large gap between recall@50 and recall@5 tell you?** — The right chunks are being found but ranked poorly. That is a reranking problem — add or improve a cross-encoder — not an embedding problem. Swapping embedding models would be expensive and would not address it.
- **What is parent-child (small-to-big) chunking?** — Index small, focused child chunks so each vector represents one idea precisely, but return the larger parent section to the LLM so the passage is self-contained. It resolves the conflict between what makes a good embedding and what makes good generation context.
- **Why prepend the heading path to a chunk before embedding?** — Chunks frequently begin with "This does not apply to..." or "The exception is..." and are unretrievable because nothing in them names the subject. A breadcrumb like `Refund Policy > EU > Digital goods` costs a few tokens, fixes the most common retrieval miss, and improves citation accuracy.
- **Why ask for `[S3]` citations instead of URLs?** — A URL in the output is a string the model generates, so it can be paraphrased or invented. A short ID is a lookup your code resolves, and it makes verification mechanical: check that every cited ID exists and flag any that do not.
...
- **Why is deleting content from the source not enough?** — If the chunk remains in the index, it still feeds retrieval, summarisation and follow-up reasoning even when it is not displayed. Deletes must propagate into the index, with an index-lag SLO and a reconciliation sweep to catch dropped change events.
- **What is the most underrated hard part of RAG?** — Ingestion. PDFs have no paragraphs, two-column layouts interleave under naive extraction, tables lose their row/column meaning when flattened, and HTML boilerplate makes every chunk look alike. Read the raw extracted text for a sample of every source type before tuning anything downstream.
- **Why does raising k often make answers worse?** — Each extra chunk adds a chance of a plausible-but-wrong distractor competing for attention, positions relevant material in the middle of the context where it is attended to less reliably, and raises cost and TTFT. Retrieve wide for recall, then rerank narrow to 4-8, ordered best-first.

## Drills

### Drill

You inherit an internal RAG assistant over 400K documents — Confluence pages, PDF policies, Jira tickets and Slack exports. Users say it is "confidently wrong about half the time". There is no eval set, retrieval is dense-only with k=20, and chunking is fixed 1,000 tokens. Give your first two weeks.
...
Legal requires that when a document is deleted or a user loses access to a workspace, the assistant must stop using that content within 5 minutes, and you must be able to demonstrate compliance. Design for this.
