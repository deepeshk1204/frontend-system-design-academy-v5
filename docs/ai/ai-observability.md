...
> Spans for prompts and retrievals, token accounting, quality telemetry, and feedback you can trust.

- Track: **AI Engineering** · Level: **core** · ~18 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/ai/ai-observability)

Observability for an AI feature means being able to answer, after the fact and without
...
Your existing monitoring stack answers "is it up and is it fast". For an LLM feature
...
| Question during an incident | Standard APM | What you need |
| --- | --- | --- |
...
## The span model for LLM applications

The structure is straightforward once you see it. A **trace** is one user-visible
...
*One trace per user request. The feedback event arrives later and must join back on trace ID.*

Use the OpenTelemetry GenAI semantic conventions rather than inventing attribute
...
**Instrumenting a model call with GenAI conventions**
...
> **Record requested and resolved model separately**  
> `gen_ai.request.model` may be a floating alias; `gen_ai.response.model` is the dated
...
| Attribute | Why you will want it |
| --- | --- |
...
**Model-call and tool-call spans**
...
> **What you must not capture, or must redact first**  
> Prompts contain whatever the user pasted, and users paste anything -- API keys, medical
...
## Cost attribution as a first-class requirement

On a normal service you learn cost per tenant by modelling it, because CPU time is
...
**Cost-telemetry figures worth watching**

- **10-100x** — Spread in cost per request on one endpoint (Why request counts are not a cost proxy)
- **usually >70%** — Share of spend that is input tokens (History, system prompt, retrieved context)
- **often 5-15%** — Spend on retries and abandoned runs (Track it explicitly; it is rarely zero)
- **2 numbers** — Latency SLOs you need (TTFT for perception, total for completion)
- **1-5%** — Sensible payload sampling rate at scale (Plus 100% of errors and flagged sessions)
- **7-30 days** — Typical payload retention (Metrics keep far longer than payloads)

## Quality telemetry in production

The hardest thing to measure is the thing that matters most, and the trap is to reach
...
**Joining feedback back to the trace**

```typescript
// The client keeps the trace ID from the response header and attaches it
...
## Sampling, retention and replay

You cannot store full payloads for every request at scale, and you should not want to.
...
The mistake is to alert on error rate and average latency, which for an AI feature are
nearly flat while it degrades. Alert on the signals that move when quality moves.

**Alerts worth a page, and what each one is really telling you**

| Signal | Why it moves | Suggested shape |
| --- | --- | --- |
...
## The dashboard someone opens during an incident

A dashboard is a diagnostic instrument, not a gallery. The test is whether an on-call
...
**Observability failures, which are the worst kind**

| Failure mode | What the user sees | Mitigation |
| --- | --- | --- |
| No tenant dimension on spans | A cost spike is visible in total and unattributable; you cannot answer "who" and cannot back-fill it. | Tenant, feature and prompt version as required attributes enforced in the client wrapper, from the first commit. |
| Raw prompts exported to a vendor | Customer PII and pasted secrets leave your boundary; compliance incident independent of any breach. | Redact before export, keep payloads in your own store with short retention, gate raw access behind a separate audited permission. |
| Feedback with no trace ID | Thousands of thumbs-down that cannot be linked to a prompt version, model or retrieval -- unusable. | Return the trace ID to the client and require it on every feedback event; denormalise key dimensions onto the event. |
| Head sampling at 1% | The failure you were paged about was not recorded; the investigation ends at "cannot reproduce". | Tail sampling with a keep-rule for errors, outlier cost, latency-budget breaches and negative feedback. |
| TTFT and total duration averaged together | Users report the feature feels slow while the latency dashboard is green. | Separate metrics and separate SLOs; stream-based products live or die on TTFT. |
| Truncation counted as success | Answers silently cut off mid-sentence; HTTP 200 and no alert; users quietly stop trusting the feature. | Record `finish_reason` on every span and alert when `length` exceeds a low threshold. |
| Cost computed at query time from a price table | Historical cost analysis silently changes when prices are updated; numbers stop reconciling. | Compute and store cost on the span at write time, with the price-table version recorded. |
| Prompt version not recorded | Quality drops after a rollout and there is no way to attribute it; the team argues instead of reverting. | Prompts are versioned artefacts from a registry; the version is a required span attribute and a dashboard filter. |
...
Your AI assistant’s error rate is 0.1% and p95 latency is unchanged, but support tickets about wrong answers tripled this week. Which telemetry finds it fastest?
- A. CPU and memory utilisation on the inference service.
- B. Retrieval-empty rate and retrieved document IDs per query, filtered by index version. **(answer)**
- C. Total request count by endpoint.
- D. HTTP status code distribution.

  Wrong answers with healthy infrastructure metrics almost always means the retrieval stage stopped supplying the right context -- a re-index, an embedding-model mismatch, or an ACL filter that became too tight. Retrieval-empty rate shows a step change immediately, and the per-query document IDs and scores let you confirm whether the correct chunk was even a candidate. Status codes cannot see this, because a grounded answer and a hallucinated one are both HTTP 200.

Why compute and store cost on the span at write time instead of deriving it later from token counts?
- A. Token counts are unreliable.
- B. Prices change, so a query-time join silently rewrites historical cost and your numbers stop reconciling. **(answer)**
- C. It is required by the OpenTelemetry conventions.
- D. It reduces span size.

  A price table is mutable; spans are immutable facts. Joining at query time means last quarter’s cost analysis changes when someone edits a price, and reconciliation against the provider invoice becomes impossible. Computing at write time -- and recording which price-table version was used -- makes the span a durable record. It also makes cost a directly groupable dimension by tenant, feature and prompt version without every consumer reimplementing pricing.

You can afford to store full prompt payloads for 2% of requests. How should you choose the 2%?
- A. Uniform random head sampling for statistical validity.
- B. Tail sampling: keep all errors, outlier-cost runs, latency-budget breaches and negative-feedback sessions, plus a small random baseline. **(answer)**
- C. Only the highest-paying tenants.
- D. The first 2% of requests each hour.
...
Which is the strongest reason to follow the OpenTelemetry GenAI semantic conventions rather than your own attribute names?
- A. They are stable and will not change.
- B. AI spans land in the same trace as your HTTP and database spans, and vendors consume them, so you can switch tooling without re-instrumenting. **(answer)**
- C. They automatically redact PII.
- D. They are required for prompt caching.

  The value is a single correlated trace and vendor portability -- one query can span the model call, the retrieval and the Postgres call behind it, and Langfuse, Phoenix, Braintrust and Datadog all ingest the same shape. They are explicitly *not* stable: `gen_ai.system` became `gen_ai.provider.name` and `prompt_tokens`/`completion_tokens` became `input_tokens`/`output_tokens`, so during a transition coalesce old and new names with a precedence rule and never sum them, since compatibility layers emit both.

<details><summary>Related topics and how they connect</summary>

Traces are the raw material for the golden datasets in **Evaluation & Quality
...
- **Why is standard APM insufficient for LLM features?** — A wrong answer is an HTTP 200 with a fluent paragraph -- there is no error to alert on. Cost per request varies 10-100x on one endpoint, latency is two numbers (TTFT and total), quality lives in what the user did next, and inputs are non-deterministic and unlogged by default.
- **What are the five span kinds in an LLM trace?** — Retrieval, model call, tool call, guardrail, and an agent-step span that parents the others inside a loop. Nesting matters because the useful questions are about relationships between spans -- did the retrieval that preceded this generation contain the answer?
- **Why record requested and resolved model separately?** — `gen_ai.request.model` may be a floating alias; `gen_ai.response.model` is the snapshot that actually served you. When behaviour shifts with no deploy, grouping by resolved model finds a silent provider swap immediately.
- **What must never go into exported telemetry?** — Credentials and secrets at any sampling rate, and un-redacted PII from prompts or retrieved chunks. Redact before export, keep payloads in your own store with short retention, and gate raw payload access behind a separate audited permission.
- **Why is tail sampling right for payloads?** — Payloads exist for debugging, and debugging is about outliers. Tail sampling decides after the trace completes, so you can keep 100% of errors, outlier-cost runs, latency breaches and negative feedback. Head sampling discards exactly the traces you get paged about.
- **Why are thumbs-up/down weak, and what replaces them?** — Single-digit response rates with selection bias toward the delighted and furious, so the ratio describes your sampling. Use implicit signals joined on trace ID: edit survival, regenerate rate, rephrase-and-ask-again, abandonment point, escalation.
- **What do you need to replay a failed run?** — Exact serialised messages, prompt template version, resolved model snapshot and sampling parameters, tool schema version, and raw tool responses. Cached replay isolates your own logic; live replay tells you whether the world drifted.
- **Which AI-specific signals deserve alerts?** — Retrieval-empty rate, schema/parse failure rate, refusal-rate deviation in either direction, p95 TTFT separately from total, cache hit rate, cost-per-request rate of change, truncation on `finish_reason=length`, and escalation/regenerate rate.
- **Why is a drop in refusal rate also worth alerting on?** — Refusals are a feature. A sudden drop can mean a guardrail stopped firing, a safety filter was misconfigured, or a prompt change removed a constraint -- so alert on deviation in either direction, not just increases.
...
At 02:10 you are paged: an internal agent that files tickets has created 4,000 duplicate tickets in ninety minutes. It normally creates about 60 an hour. You have full GenAI span instrumentation. Walk through the investigation and say which panels and queries you use.
