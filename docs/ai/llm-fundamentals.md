...
> Tokens, context windows, sampling, why latency is two different numbers, and what the model cannot know.

- Track: **AI Engineering** · Level: **foundation** · ~19 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/ai/llm-fundamentals)
...
Before 2018, making a machine do a language task meant building a model per task:
...
## Tokens: the unit you are billed and limited in

Models do not see characters or words. A tokeniser -- almost always a
...
**Tokenisation rules of thumb (English, modern BPE tokenisers)**

- **~4 chars** — One token, average English prose (Roughly 0.75 words)
- **~1.3x** — Tokens per word, English (Use this for quick budgeting)
- **2-4x** — Token inflation for many non-Latin scripts (Same meaning, more tokens, more cost)
- **~1 token** — Per 2-3 chars of minified JS or JSON (Punctuation-dense text tokenises badly)

Three failure classes follow directly:
...
> **Count tokens, do not estimate them, at the boundary**  
> Estimating at 4 chars per token is fine for a capacity plan and dangerous for a hard
...
## Context window and why long context costs more than it looks

The context window is the maximum number of tokens -- input plus output -- the model
...
*One request, two completely different performance regimes. Measure them separately.*

## Prefill vs decode: the single most useful mental model

Inference has two phases with opposite performance characteristics, and conflating
...
**Two regimes, two metrics, two levers**
...
> **Why one "latency" number is a broken SLO**  
> A 9,000-token RAG prompt returning 80 tokens and a 200-token chat turn returning 900
...
## Sampling: temperature, top-p, and the determinism myth

The model emits logits -- an unnormalised score per vocabulary token. Sampling
...
| Setting | Typical use | What it actually does to failure modes |
| --- | --- | --- |
...
> Greedy decoding is deterministic in theory. In practice, repeated identical requests
...
The model is trained to produce a *plausible continuation*, not a *true* one. There
...
## Knowledge cutoff and what the model cannot know

Weights are frozen at training time. Anything after the cutoff -- your latest deploy,
...
Providers price input and output tokens separately, and output is typically several
...
**Per-interaction cost model — fill in your provider's rates**

```js
// Rates are per 1M tokens. Use YOUR provider's current numbers.
...
**Orders of magnitude worth carrying in your head (2026, hosted frontier models)**

- **10-100x** — Cost spread: small model vs frontier model (The entire case for routing)
- **3-10x** — Output token price vs input token price (Short outputs are a real cost lever)
- **~10x** — Discount on cached input prefixes (Order the prompt: stable content first)
- **50-150 tok/s** — Typical single-stream decode rate (Faster than reading speed (~5-8 tok/s))

## Trade-offs of building on a hosted LLM
...
| --- | --- | --- |
| Prompt grows past the context limit mid-conversation | Hard 400s for long-running or high-value sessions only. | Count tokens with the real tokeniser before sending; reserve an explicit output budget; compact history at a threshold, not at the wall. |
| Latency SLO set on total response time | Long-prompt routes look healthy while TTFT quietly triples; users report "it hangs". | Separate TTFT and inter-token-latency SLOs per route; alert on TTFT p95. |
| Test asserts exact model output | Flaky CI, team stops trusting the suite, real regressions get ignored. | Assert on schema, invariants and eval scores against a golden set; never on the exact string. |
| Provider silently rolls a model behind a stable alias | Quality or format shifts overnight with no deploy of yours. | Pin dated/specific version strings; run the eval suite on a schedule, not just on PRs; alert on output-shape drift. |
| Launch in a non-English market | Token spend per user rises 2-4x and some prompts stop fitting the window. | Measure tokens-per-request per locale before launch; budget context in tokens, not characters. |
| Model answers a post-cutoff question confidently | Users act on stale facts; trust damage is disproportionate to frequency. | Inject current date; route time-sensitive intents to retrieval or tools; evaluate refusal on an unanswerable set. |
...
Your RAG endpoint has p95 total latency of 4.1 s. You halve the number of retrieved chunks and total latency drops to 2.6 s, but users still say it "feels slow to start". What is most likely happening?
- A. Decode is the bottleneck and needs a larger batch size.
- B. TTFT is still dominated by prefill over a large system prompt and tool schema. **(answer)**
- C. The vector index needs a higher `efSearch`.
- D. The network round trip to the provider is the bottleneck.

  Prefill time scales with total input tokens, and retrieved chunks are only part of that. A long system prompt, a big tool schema and conversation history all sit in front of every request. Measure TTFT separately, break input tokens down by section, then put the stable parts first so prefix caching can skip them.

You set `temperature: 0` and a fixed `seed`, then write a CI test asserting the model returns exactly `{"intent":"refund"}`. Why is this a bad test?
- A. Because `seed` is ignored when temperature is 0.
- B. Because hosted inference is not bit-reproducible — non-associative GPU reductions and varying batch composition mean identical inputs can yield different text. **(answer)**
- C. Because JSON key order is randomised by the API.
- D. Because temperature 0 disables structured output.

  Greedy decoding is deterministic on paper, but hosted serving is not: floating-point reductions depend on how work is partitioned, and that depends on who else is in your batch. Providers also update builds behind stable aliases. Assert on parsed structure and invariants, and gate regressions on eval scores over a golden set.

A support agent conversation costs $0.004 at turn 1 and $0.031 at turn 15, with no change in message size. Why?
- A. The provider applies progressive rate-limit surcharges.
- B. Every turn re-sends the whole conversation, so input tokens grow with turn count and cost grows roughly quadratically over the session. **(answer)**
- C. Decode gets slower as the KV cache grows, and you are billed by time.
- D. Tool schemas are re-tokenised at a higher rate after repeated use.
...
The prefill/decode split is the foundation for **Inference Serving & Performance**
...
- **Why is TTFT a different metric from inter-token latency?** — They come from different phases with opposite bottlenecks. TTFT is prefill: compute-bound, scales with input length, fixed by prompt size and prefix caching. Inter-token latency is decode: memory-bandwidth-bound, scales with model size, barely affected by prompt length.
- **Why does batching help decode much more than prefill?** — Decode reads the entire model weights from HBM to produce one token, so it is bandwidth-starved and nearly idle on compute — batching amortises that weight read across many sequences. Prefill already saturates the GPU's arithmetic units, so there is little spare capacity to fill.
- **Is `temperature: 0` deterministic?** — No, not in hosted serving. GPU floating-point reductions are non-associative, batch composition changes how work is partitioned, MoE routing can be batch-sensitive, and providers update builds behind stable aliases. Reproducible intent, not reproducible bytes.
- **Why do LLMs fail at counting letters and multi-digit arithmetic?** — BPE tokens do not align with characters or digits. `1234567` may be three tokens that cut across place value, and the model never sees individual letters as units. Route these to a tool rather than prompting harder.
- **What does quadratic attention actually cost you in production?** — Attention compute grows as O(n²) in sequence length while the KV cache grows linearly. FlashAttention lowers the constant but not the asymptotics. In practice the KV cache is what caps concurrency per GPU, and prefill time is what caps your TTFT.
- **Why is hallucination not fixable by prompting?** — The training objective rewards plausible continuations, not true ones, and there is no separate truth signal to consult. You manage it structurally: ground in retrieved text, constrain with schemas, verify claims against systems of record, and reward an explicit refusal path.
- **Why is the same feature more expensive in Hindi or Japanese?** — BPE merges are learned mostly from English-heavy corpora, so non-Latin scripts fragment into more tokens — often 2-4x for the same meaning. Both cost and effective context length are affected, with no code change involved.
- **What belongs in the prompt prefix and why?** — Everything byte-stable: system prompt, tool definitions, long-lived policy text — ordered first. Providers cache processed prefixes at a large discount and skip re-prefilling them, which cuts both TTFT and input cost. A single interpolated timestamp at the top destroys the hit.

## Drills

### Drill

You own a customer-support assistant. p95 total latency is 6 s and finance says cost per resolved ticket is 3x the forecast. The prompt is a 2,400-token system prompt with tool schemas, 10 retrieved chunks of ~600 tokens each, and up to 20 turns of history. Diagnose and propose fixes, in priority order.
...
A product manager asks you to guarantee the assistant "never makes things up" before launch, and suggests adding "Do not hallucinate. Only state facts." to the system prompt. Respond as the engineering owner.
