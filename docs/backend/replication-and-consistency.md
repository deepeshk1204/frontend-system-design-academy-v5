...
> Leader-follower, quorums, CAP and PACELC, read-your-writes, and monotonic reads.

- Track: **Backend & Distributed Systems** · Level: **staff** · ~22 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/replication-and-consistency)

Replication is keeping more than one copy of your data on more than one machine. You do it for three different
...
Disks fail, kernels panic, availability zones lose power, and someone eventually runs `DROP TABLE` on the wrong
...
*Single-leader avoids write conflicts by construction. The other two accept conflicts in exchange for write availability or locality.*

| Topology | How writes work | Conflict handling | Real systems | Choose when |
| --- | --- | --- | --- | --- |
...
> **Multi-leader is the trap answer**  
> "Two regions, both writable" sounds like the obvious way to get low write latency everywhere. What you have actually
...
## Synchronous, asynchronous, and the semi-sync compromise

The knob that matters most is when the leader tells the client "committed".
...
**Postgres: the settings that decide your durability story**

```ini
# Asynchronous -- fastest writes, loses up to <lag> on leader loss
...
**What each choice costs on a write**

- **~0.5-2 ms** — Async commit (Local NVMe fsync only)
- **~2-4 ms** — Sync to same-AZ standby (Sub-millisecond network hop)
- **~3-6 ms** — Sync to cross-AZ standby (The standard production default)
- **~70-140 ms** — Sync to cross-region standby (Usually unacceptable for OLTP)

## Replication lag, and what users actually see

Lag is usually milliseconds and occasionally minutes. It blows out during bulk imports, long-running vacuum or index
...
- **Read-your-writes violated** — The user edits their profile, the app redirects to the profile page, the read hits a lagging replica, and the old name comes back. The user concludes the save failed and saves again.
- **Monotonic reads violated** — Two successive reads hit different replicas with different lag, so the user sees a comment appear and then vanish. Time appears to run backwards.
- **Consistent prefix violated** — A reply is replicated before the message it replies to, because they travelled through different partitions. The conversation reads as an answer to nothing.

These three are **client-centric guarantees**: they are promises about what *one session* observes, not about global
...
In a leaderless system you choose how many replicas must respond. With N replicas, a write that reaches W nodes and
...
**Quorum configurations with N = 3**

| W | R | R+W>N? | Write availability | Read latency | Use for |
| --- | --- | --- | --- | --- | --- |
...
> **Quorum is not linearisability**  
> `R + W > N` guarantees *overlap*, not *ordering*. Concurrent writes to the same key can still produce divergent
...
The popular version of CAP -- "pick two of consistency, availability, partition tolerance" -- is wrong in a way that
...
**PACELC** is the more useful framing. It reads: *if there is a Partition, choose between Availability and
...
**PACELC classification of systems you will actually use**

| System | On partition | Otherwise | Reading |
| --- | --- | --- | --- |
...
Leader failure is where the theory becomes an incident. Trace it properly.
...
*Two independent hazards: acknowledged writes lost on async promotion, and two nodes both accepting writes.*

The lost-write window equals the replication lag at the instant of failure. If your follower was 400 ms behind at
...
| --- | --- | --- |
| Async promotion discards acknowledged writes | Orders confirmed to customers vanish; support has receipts your database does not. | Semi-sync with `ANY 1 (a,b,c)`; alert on lag in bytes not seconds; refuse automatic promotion beyond a configured lag threshold. |
| Split-brain -- two leaders accept writes | Divergent histories requiring manual reconciliation; unique constraints violated on merge. | Majority-quorum promotion plus epoch fencing enforced at the storage layer, not at the coordinator. |
| Read-after-write on a lagging replica | Users see saves fail and re-submit, creating duplicates; "the app lost my edit" tickets. | LSN/GTID token carried by the client with replica wait-or-forward, or leader-sticky reads for N seconds after any write. |
| Replica lag spike from a long transaction or index build | All replica-backed reads go minutes stale simultaneously; stale-read bugs appear across the product at once. | Cap `max_standby_streaming_delay`, run migrations with `CONCURRENTLY`, and take replicas out of rotation automatically above a lag threshold. |
| Failover flapping on transient network blips | Repeated promotions, each with its own lost-write window and connection storm. | Require 3 consecutive failed checks from multiple observers, add a cooldown, and never promote without quorum agreement. |
| Multi-leader last-write-wins on skewed clocks | Silent overwrite -- the newer edit loses because that node's clock was 200 ms behind. | Version vectors or CRDTs, or partition write ownership so each row has exactly one home region. |
| Connection pools still pointed at the old primary DNS | Write errors continue for minutes after a successful failover. | Route through a proxy (PgBouncer, ProxySQL, RDS Proxy) or a virtual IP, with a short TTL and aggressive pool invalidation. |
...
A user updates their display name, is redirected, and sees the old name. Replication lag is 300 ms. Cheapest correct fix?
- A. Switch all replication to synchronous.
- B. Route this session's reads to the leader for a few seconds, or carry the write's LSN and have the replica wait for it. **(answer)**
- C. Add a 500 ms delay after the write before redirecting.
- D. Increase the replica count.

  This is a read-your-writes violation, which is a *client-centric* guarantee -- you only need this one session to see its own write, not the whole system to be linearisable. Sticky-to-leader is the crude version; carrying the log position (LSN, GTID, `afterClusterTime`) and having the replica wait or forward is the precise one. Making all replication synchronous solves a session-scoped problem with a global latency tax, and more replicas makes staleness more likely, not less.

Cassandra with N=3, W=2, R=2. Can you safely implement "decrement inventory if greater than zero"?
- A. Yes -- R+W>N guarantees you read the latest value.
- B. No -- quorum overlap gives recency, not atomicity, so two concurrent decrements can both read 1 and both succeed. **(answer)**
- C. Yes, if you use `LOCAL_QUORUM`.
- D. Only with a read repair configured.
...
Your leader dies. The chosen follower was 800 ms behind at 4,000 writes/sec. What happened?
- A. Nothing -- replication catches up after promotion.
- B. Roughly 3,200 acknowledged writes were permanently lost. **(answer)**
- C. The writes are recoverable from the old leader once it returns.
- D. Clients get errors for those writes and will retry.

  Async replication acknowledges before shipping the log, so everything in the lag window was confirmed to clients but never left the leader. Promotion discards it. The old leader's WAL may physically contain those records, but merging a diverged timeline back into a promoted leader is a manual forensic exercise, not a recovery procedure -- and clients never got an error, so they will not retry. Semi-sync converts this data-loss risk into a write-availability risk.

Which statement about CAP is accurate?
- A. You choose two of the three properties at design time.
- B. During a network partition, you must choose between availability and linearisability; partition tolerance is not optional. **(answer)**
- C. CAP proves eventual consistency is the only scalable option.
- D. A single-node database is CP.

  CAP is a statement about behaviour *during* a partition, with linearisability and total availability as its strict definitions. Partitions happen regardless of your design, so "choosing CA" is not a choice, it is a system that misbehaves when the network does. This is why PACELC is more useful: it adds the else-branch, where you trade latency against consistency on a perfectly healthy network -- which is the decision you actually make every day.

<details><summary>Related topics and how they connect</summary>

Every shard from **Sharding & Partitioning** has its own replication topology, so these two compound. Quorum-based
...
- **What exactly is the data-loss window on asynchronous failover?** — The current replication lag at the instant the leader dies, measured in log bytes. Every write acknowledged but not yet shipped is permanently gone, and the client was told it committed. Alert on lag in bytes, not seconds.
- **Why prefer semi-synchronous over fully synchronous replication?** — Fully sync couples write availability to one specific replica -- if it is slow, writes block. Semi-sync (`ANY 1 (a,b,c)`) requires any one of several, so a single sick replica cannot stall the write path while still guaranteeing no single-node data loss.
- **State CAP precisely.** — During a network partition, a system must choose between remaining available (every non-failing node answers every request) and remaining linearisable. Partition tolerance is not a choice. CAP says nothing about the healthy case, which is why PACELC is more useful.
- **What does PACELC add?** — The else-branch: when there is no Partition, you still trade Latency against Consistency. That is the decision behind every "can this read go to a replica" call, and it applies on a perfectly healthy network.
- **What does `R + W > N` guarantee, and what does it not?** — It guarantees the read set overlaps the write set, so a read can observe the latest committed write. It does not give ordering or atomicity -- concurrent read-modify-writes can still lose updates, so compare-and-set needs a lightweight transaction.
- **How do you implement read-your-writes properly?** — Return the leader's log position (LSN/GTID/`operationTime`) from the write, have the client carry it, and make the replica either wait until it has replayed past that position or forward the read to the leader. Sticky-to-leader routing works but concentrates load.
- **What is monotonic reads and how do you get it?** — A guarantee that a session never sees time run backwards. Achieve it by pinning the session to one replica (hash the session ID) rather than round-robining, and only ever re-pin forward on failover.
- **Why is a fencing token required for safe failover?** — A paused old leader cannot distinguish itself from a dead one. Each leadership term gets a monotonically increasing epoch; the storage layer rejects anything with a lower epoch. Without that, a node waking from a GC pause overwrites newer data.
- **Why is multi-leader replication usually the wrong answer?** — It makes write conflicts permanent and structural. Last-write-wins on wall clocks silently loses data within the clock-skew window. Single leader plus regional read replicas, or partitioning write ownership by region, solves most of the motivating cases.
...
A social product moved 80% of reads to replicas to relieve the primary. Support tickets now report: comments disappearing after posting, a counter that goes 42 then 41 then 43, and replies appearing above the message they reply to. Diagnose each and propose targeted fixes.
