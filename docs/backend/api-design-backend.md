...
> REST vs gRPC vs GraphQL, idempotency keys, pagination, versioning and error taxonomy.

- Track: **Backend & Distributed Systems** · Level: **foundation** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/api-design-backend)

An API is a promise about behaviour that you cannot take back. Once a client depends on
...
Inside a process, a function call either happens or does not, returns or throws, and
...
**Figures that shape the contract**

- **24 h** — Typical idempotency key TTL (Stripe's window; long enough for any client retry)
- **100,020** — Rows read by `LIMIT 20 OFFSET 100000` (Keyset reads 20)
- **30-60%** — Protobuf wire saving versus JSON (Plus much cheaper parsing)
- **months** — Calendar time for a mobile client to upgrade (The real cost of a breaking change)

## Resource modelling in REST

REST's useful core is small: model *nouns* as URLs, use the HTTP method to say what you
...
**Method semantics you are promising whether you meant to or not**

| Method | Safe (no side effects) | Idempotent | Cacheable | Practical note |
| --- | --- | --- | --- | --- |
...
The honest summary: use REST at the edge because the ecosystem is free, gRPC between
...
A client sends `POST /payments` for $400. The response is lost to a timeout. The client
...
**The handler, including the in-flight race**
...
> If the side effect is a call to an *external* payment processor rather than a database
...
Offset pagination is the default everywhere and it degrades in a way that is invisible
...
**Offset versus keyset, with the index that makes keyset work**
...
Two details make keyset pagination production-grade. The sort key must be **unique**, so
...
| Property | Offset / limit | Keyset / cursor |
| --- | --- | --- |
...
The question a client is really asking when it gets an error is: *should I retry, fix my
...
**A problem response worth building clients against**
...
**Error taxonomy: what each class tells a client to do**

| Status | Meaning | Client action | Common misuse |
| --- | --- | --- | --- |
...
A timeout is a per-hop number; a **deadline** is an absolute instant shared by the whole
...
*Each hop subtracts network overhead and reserves time to serialise a response. The database statement timeout is the last link in the chain -- if it is longer than the remaining budget, you have a query nobody is waiting for.*

**Deadline propagation over HTTP by convention**

```javascript
// Inbound: trust an internal header, clamp it to a maximum.
...
Versioning strategy is a question about *who is forced to act*. A URL version
...
**Protobuf changes: safe, unsafe, and why**
...
> **The expand-migrate-contract loop**  
> Any field change becomes two safe deploys plus a wait. **Expand**: add the new field,
...
**Strict contracts, idempotency and deadline propagation**
...
| --- | --- | --- |
| Idempotency key generated per attempt rather than per operation | Every retry is a new key, so a network timeout becomes a duplicate charge. | Generate the key where the user intent originates, persist it with the attempt, reuse across retries. |
| Key recorded outside the transaction that has the side effect | Crash between the two leaves a charge with no key; the next retry charges again. | Single transaction for local writes; for external calls, write `in_flight` first and reconcile stale rows. |
| Per-hop timeouts instead of a shared deadline | 4 hops of 2 s means 8 s of work for a user who left after 2 s; overload feeds on itself. | Propagate an absolute deadline, subtract at each hop, push down to `statement_timeout`. |
| Downstream timeout returned as `500` | Clients cannot tell retryable from permanent, so they either never retry or retry everything. | `503` for shed or unhealthy, `504` for upstream deadline, `500` only for genuine bugs; add an explicit `retryable` field. |
| Offset pagination on a public list endpoint | Crawlers walking to page 5,000 execute 900 ms queries and saturate the read replica. | Keyset cursors with a unique tiebreaker; cap or forbid deep offsets. |
| Enum value added without an `UNSPECIFIED = 0` | Old clients silently map the new value to the first enum member and mis-handle it. | Always reserve 0 as unspecified; treat unknown values as unspecified and log them. |
| Tag number reused after a field was deleted | Old serialised bytes are reinterpreted as a different type -- data corruption, not an error. | `reserved 7, 9;` plus `reserved "old_name";` and a CI breaking-change check. |
| Error bodies without a stable code | Clients regex the `detail` string; a log-message tidy-up breaks production integrations. | Stable `type` URI and `code` enum treated as public API and covered by tests. |
...
A mobile client retries `POST /orders` after a timeout, generating a fresh `Idempotency-Key` for each attempt. What happens and why?
- A. Nothing -- the server deduplicates on request body.
- B. Duplicate orders, because the key identifies the attempt rather than the operation. **(answer)**
- C. The server returns `409` for the second attempt.
- D. The first key expires and the retry replays the original response.

  The key must be minted once where the user intent originates and reused across every retry of that intent, including retries after an app restart -- which means persisting it locally with the pending request. A per-attempt key makes the mechanism a no-op while looking correct in code review. Body hashing does not save you either: two identical orders are legitimately different operations, so the server cannot deduplicate on content alone.

Your API gateway allows 3 s, and each of three internal hops independently allows 3 s to its dependency. A user abandons after 3 s. What is the worst case, and why does it matter beyond latency?
- A. 3 s total; the gateway cancels everything downstream.
- B. Up to 9 s of work continues for a request no one is waiting for, consuming threads and DB connections. **(answer)**
- C. 9 s, but harmless because the response is discarded.
- D. The innermost service times out first, so the total is 3 s.

  Per-hop timeouts add up down the chain, and without cancellation the abandoned work keeps holding a worker, a pooled connection and a running query. Under load that turns a latency incident into a capacity incident, because the system spends an increasing share of its resources on results nobody will read. An absolute deadline propagated per hop -- and pushed into `statement_timeout` -- lets each layer stop as soon as the budget is gone.

A protobuf field `int32 quantity = 7;` is deleted in a release. Six months later someone adds `string sku = 7;`. What is the consequence?
- A. A compile error in every client.
- B. Old encoded messages are misparsed, because the tag number is the field identity. **(answer)**
- C. Nothing -- field names are what matter.
- D. The new field is ignored by all readers.

  Protobuf encodes the tag number, not the name, so tag 7 in an old message body is now decoded as a `string` when it holds varint bytes -- a parse error at best and silent corruption at worst. `reserved 7;` plus `reserved "quantity";` makes reuse a compile-time error, and a CI breaking-change detector catches it before merge. This is why removing a field is only "safe" when paired with reservation.

Which pagination design is correct for a public feed API where clients also run nightly full exports?
- A. `LIMIT/OFFSET` with a hard cap of 1,000 pages.
- B. Keyset pagination on `(created_at, id)` with an opaque base64 cursor. **(answer)**
- C. Keyset pagination on `created_at` alone.
- D. `LIMIT/OFFSET` plus a covering index to keep it fast.

  Keyset is constant cost at any depth and stable while rows are being inserted, which matters most for exactly the export use case. The tiebreaker is not optional: ordering on `created_at` alone with duplicate timestamps will repeat or drop rows across page boundaries. An index cannot rescue offset, because the database still has to traverse and discard every skipped entry before it reaches the requested window.

<details><summary>Related topics and how they connect</summary>

The retry safety that idempotency keys enable is only half the story -- the backoff,
...
- **Why must an idempotency key be per-operation rather than per-attempt?** — The whole point is that the server recognises a retry of the same intent. A new key per attempt makes every retry a distinct operation, so a lost response becomes a duplicate write while the code looks correct.
- **How do you handle two concurrent retries with the same idempotency key?** — Claim the key with a unique-constrained insert. The loser sees state `in_flight` and returns `409` with `Retry-After` rather than executing; once the winner commits `completed`, subsequent calls replay the stored response.
- **Why hash the request body alongside the idempotency key?** — Same key with a different body is a client bug. Replaying the old response would hide it, so return `422 idempotency-key-reused` instead.
- **Why does `LIMIT 20 OFFSET 100000` get slow?** — The database reads and discards 100,000 rows to locate the window, so cost grows linearly with page depth. It is also unstable: concurrent inserts shift the offset and rows get skipped.
- **What two details make keyset pagination correct?** — A unique sort key -- compare a tuple like `(created_at, id)` so duplicate timestamps cannot repeat or drop rows -- and an opaque, ideally signed cursor so clients cannot construct one and you can change the sort key later.
- **Difference between a timeout and a deadline?** — A timeout is a per-hop duration and therefore multiplies down a call chain. A deadline is an absolute instant shared by the whole tree; each hop spends what remains minus its own reserve, so total time stays bounded.
- **What should `500` versus `503` versus `504` tell a client?** — `500` is an unexpected bug -- retry only if idempotent. `503` is temporary unavailability or shedding -- retry with backoff, honouring `Retry-After`. `504` is an upstream deadline -- the work may still be running, so retry cautiously.
- **Which protobuf changes are safe?** — Adding a field with a fresh tag, widening `int32` to `int64`, and removing a field *if* you `reserved` its tag and name. Changing a tag number, changing wire type, altering cardinality, or moving a field into a `oneof` are breaking.
- **Why does every protobuf enum need a zero value named `UNSPECIFIED`?** — Unknown enum values decode to the zero value in old readers. If zero is a real business meaning, a newly added value silently becomes that meaning; if zero is `UNSPECIFIED`, the client can detect and log it.
...
An internal gRPC service has 14 client teams. You need to change `int32 amount_cents` to a `Money` message with currency, because a new market needs non-USD. You cannot coordinate a simultaneous deploy, and two clients are on a quarterly release train. Plan the migration.
