...
> Scheduling, probes, resource limits, blue/green vs canary, and zero-downtime schema migration.

- Track: **Backend & Distributed Systems** · Level: **core** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/infra-and-deployment)

A container is a normal Linux process that has been lied to about what it can see. A Kubernetes cluster is a control
...
## Containers, briefly and correctly

There is no such thing as a container in the Linux kernel. There is a process, plus three mechanisms.
...
## Kubernetes: the objects that matter and the loop

Kubernetes is a set of controllers, each running the same loop: read desired state, observe actual state, take one
...
*The Service sends traffic only to pods in EndpointSlices, and only ready pods are listed. That single fact is why readiness probes matter more than liveness probes.*

The **scheduler** is a bin-packing algorithm, and it packs by *requests* -- not by actual usage and not by limits.
...
## Requests, limits, and the two failure modes

`requests` is what the scheduler reserves. `limits` is the ceiling the kernel enforces. CPU and memory behave
...
**Throttling arithmetic for `limits.cpu: 1`, 8 threads**

- **100 ms** — CFS period (The default quota window)
- **12.5 ms** — Wall time to burn the quota (8 threads x 12.5 ms = 100 ms of CPU)
- **87.5 ms** — Frozen until the next period (A hard floor added to your tail latency)
- **~40%** — Average CPU shown on dashboards (Which is why nobody suspects throttling)

> **The CPU-limit position, stated honestly**  
> Removing CPU limits is widely recommended and it is not free. CPU *requests* already provide proportional-share
...
## Probes, and the cascading-restart mistake

| Probe | On failure | Should check | Must never check |
| --- | --- | --- | --- |
...
The mistake is writing one `/health` handler that checks the database and wiring it to both probes. It looks
...
**Probes that do not amplify an outage**

```yaml
startupProbe:                  # buys a slow JVM 150s without tripping liveness
...
## Graceful shutdown: the deploy-time 502s

When a pod is terminated, two things happen **concurrently and independently**: the kubelet sends `SIGTERM` to
...
1. A `preStop` hook sleeps 5-15 seconds. This does nothing except hold the pod alive and still-serving while endpoint removal propagates through every proxy in the cluster.
2. Your `SIGTERM` handler immediately starts returning 503 from the readiness endpoint, so any lagging load balancer marks the pod down.
3. Stop accepting new connections and stop consuming from queues, but keep serving in-flight requests to completion.
4. Flush what must not be lost: commit consumer offsets, drain the outbox relay, complete open transactions.
5. Exit cleanly. `terminationGracePeriodSeconds` must exceed preStop plus your longest request, or the kubelet sends `SIGKILL` and you lose in-flight work.

> **Warning**  
> If your p99.9 request takes 30 seconds, a 30-second grace period silently truncates it. Genuinely
...
The HPA default is CPU utilisation as a percentage of *requests*, and for many services it is the wrong signal. If
...
**HPA on RPS with asymmetric scaling behaviour**
...
> **Warning**  
> A PDB with `minAvailable` equal to your replica count blocks node drains entirely, so cluster upgrades hang and
...
| Strategy | Mechanism | Rollback | Extra capacity | Catches | Costs |
| --- | --- | --- | --- | --- | --- |
...
Canary is the only one of these that catches a *performance* regression before most users see it, and that is the
...
**Argo Rollouts canary with automated abort**
...
## Zero-downtime schema migration: expand, migrate, contract

Every deployment strategy above runs two versions of your application at the same time. That means **your database
...
*Renaming one column is four deploys and two migrations. Every arrow is independently reversible.*

**Renaming `full_name` to `first_name` + `last_name`, fully worked**

```sql
-- ============ EXPAND (migration 1, no deploy dependency) ============
...
**Postgres DDL: which operations are actually safe online**

| Operation | Lock and cost | Safe pattern |
| --- | --- | --- |
...
> **The lock queue nobody expects**  
> A DDL statement waiting for `ACCESS EXCLUSIVE` blocks every subsequent query on that table, even plain
...
## Configuration, secrets and GitOps

Configuration belongs outside the image so the same artifact can be promoted from staging to production unchanged
...
| --- | --- | --- |
| Liveness probe checks the database | A 30-second database blip kills every pod at once; cold starts stampede the database and the restart loop becomes self-sustaining. A brief degradation becomes a 20-minute outage. | Liveness is process-local only. Dependency checks go in readiness, where failure sheds traffic without killing the pod. |
| CPU limit throttling a multi-threaded service | p99 latency has an ~90 ms floor while average CPU reads 40%, so every dashboard says there is headroom and the cause is invisible. | Alert on `container_cpu_cfs_throttled_seconds_total`; remove CPU limits for latency-sensitive services and size requests from observed usage. |
| Memory limit below real working set | OOMKill with exit 137, no stack trace, in-flight requests dropped; `CrashLoopBackOff` under exactly the load you needed to handle. | Size from observed p99 RSS plus headroom, and tell the runtime its ceiling (`MaxRAMPercentage`, `--max-old-space-size`). |
| No preStop hook | 502s on every single deploy because endpoint removal has not propagated before the process exits. | `preStop` sleep of 5-15 s, return 503 from readiness on SIGTERM, drain in-flight, and a grace period exceeding preStop plus your longest request. |
| Column renamed in one migration | Old pods query a column that no longer exists; 100% errors on that path until the rollout completes, and rollback is impossible. | Expand/migrate/contract: add, dual-write, backfill, switch reads, stop writing, drop -- four deploys, each independently reversible. |
| Backfill as one big `UPDATE`, or DDL queued behind a long transaction | Millions of row locks push replication lag into minutes; or a pending `ACCESS EXCLUSIVE` lock queues every subsequent `SELECT` and the table goes offline while the migration appears to be merely waiting. | Bounded PK-ordered batches with a resumable high-water mark and a lag-gated throttle; `SET lock_timeout = '3s'` before DDL so it fails fast and retries. |
| HPA on CPU for an IO-bound service | Latency triples while CPU sits at 20%, so no scaling happens and the autoscaler looks perfectly healthy. | Scale on RPS per pod or in-flight concurrency; for consumers, scale on queue lag via KEDA. |
| PDB `minAvailable` equal to replica count | Node drains never complete, cluster upgrades hang indefinitely, and the platform team escalates. | Set `minAvailable` below the replica count to what you genuinely need to serve, and keep HPA `minReplicas` above it. |
...
Your `/health` endpoint checks database connectivity and is used for both readiness and liveness. The database has a 30-second blip. What happens?
- A. Pods are removed from load balancing and return when the database recovers.
- B. Every pod fails liveness and is killed simultaneously, and the cold-start stampede prolongs the incident well past 30 seconds. **(answer)**
- C. Kubernetes detects the shared cause and suppresses the restarts.
- D. Only pods with active database connections restart.

  Liveness failure means kill and restart, and since the cause is shared, it happens to the entire fleet at once. You then get empty caches, a connection stampede against the already-struggling database, and JIT warm-up on every request -- so the probe re-fails and the loop sustains itself. A 30-second degradation becomes a 20-minute outage caused by your own health check. Liveness must only detect process-local wedges that a restart genuinely fixes; dependency checks belong in readiness, where failure sheds traffic without killing anything.

A Go service with `limits.cpu: "1"` and 8 goroutine workers shows p99 latency of 95 ms and average CPU utilisation of 40%. Most likely cause?
- A. The service needs more memory.
- B. CFS throttling: 8 threads burn the 100 ms quota in ~12 ms, then all freeze until the next period. **(answer)**
- C. Network latency to the database.
- D. The HPA has scaled down too far.

  A CPU limit is a quota per 100 ms period and parallel threads consume it in parallel -- eight threads each running 12.5 ms exhausts a 1-core quota, after which every thread is descheduled for the remaining 87.5 ms. That puts a ~90 ms floor under your tail latency while *average* utilisation reads 40% and suggests headroom. The revealing metric is `container_cpu_cfs_throttled_seconds_total`; the fix is to remove the CPU limit (requests still provide fair-share) or raise it above peak parallel demand.

You need to rename `full_name` to `first_name` and `last_name` in a 40M-row table with rolling deploys and no downtime. What is the minimum safe sequence?
- A. `ALTER TABLE ... RENAME COLUMN` in a migration that runs before the new code.
- B. Add nullable columns, deploy dual-write, backfill in batches, deploy read-new, deploy stop-writing-old, then drop the old column. **(answer)**
- C. Blue-green deploy with the rename applied at cutover.
- D. A short maintenance window with the rename and a single `UPDATE`.

  Rolling deploys guarantee both code versions serve simultaneously, so any schema the old version cannot read is an outage for as long as the rollout takes -- and rollback becomes impossible. Expand/migrate/contract keeps every intermediate state readable by both versions, and critically each step is a separate deploy that is independently reversible. Blue-green does not help because the database is shared between blue and green. The batched, throttled backfill matters as much as the DDL: a single `UPDATE` over 40M rows bloats WAL and pushes replication lag into minutes.

Every deploy produces a burst of 502s for about two seconds. Pods start correctly and probes pass. Most likely cause?
- A. The readiness probe interval is too long.
- B. No preStop hook -- the process exits on SIGTERM before endpoint removal has propagated to all proxies. **(answer)**
- C. The image pull is slow.
- D. `maxSurge` is set too high.

  SIGTERM delivery and endpoint removal are concurrent and independent, and removal must propagate to every kube-proxy and ingress controller, which takes hundreds of milliseconds to seconds. A process that exits promptly on SIGTERM leaves a window where load balancers still route to a closed socket. The fix is a preStop sleep of 5-15 seconds so the pod keeps serving during propagation, a SIGTERM handler that immediately returns 503 from readiness, in-flight draining, and a grace period exceeding preStop plus your longest request.

<details><summary>Related topics and how they connect</summary>

Canary analysis is only as good as the SLIs behind it -- see **Observability, SLOs & Error Budgets** for burn-rate
...
- **What does the Kubernetes scheduler bin-pack by?** — By `requests` only -- not by actual usage and not by limits. So requests are the entire input to capacity planning: set too high and you pay for idle nodes, too low and the scheduler overcommits the node into starvation.
- **Why is a CPU limit dangerous for a latency-sensitive service?** — It is a CFS quota per 100 ms period, consumed in parallel by all threads. Eight threads with a 1-core limit burn the quota in 12.5 ms and then freeze for 87.5 ms, putting a ~90 ms floor under p99 while average CPU reads 40%. Watch `container_cpu_cfs_throttled_seconds_total`.
- **Why must a memory limit be set even though a CPU limit often should not?** — Memory is incompressible -- there is no way to give a process less than it is using, so the kernel OOM-kills instead of throttling. Exit 137 with no stack trace and dropped in-flight requests. CPU can be throttled; memory cannot, so the limit is the only bound.
- **What must a liveness probe never check, and why?** — Any external dependency. Liveness failure kills the container, and restarting your app does not fix someone else's database -- so a shared dependency blip kills the entire fleet at once and the cold-start stampede sustains the loop. Dependency checks belong in readiness.
- **Why do you need a preStop hook?** — SIGTERM delivery and endpoint removal happen concurrently, and removal takes time to propagate to every proxy. A preStop sleep of 5-15 seconds keeps the pod serving during propagation, which is what eliminates the deploy-time 502s.
- **What should an HPA scale on for an IO-bound service?** — Requests per second per pod or in-flight concurrency -- CPU stays flat at 20% while latency triples, so a CPU-based HPA never fires. Queue consumers should scale on consumer lag via KEDA. Scale up fast, scale down slowly (~300 s stabilisation).
- **What does expand/migrate/contract mean and why is it required?** — Add the new schema (expand), run code that writes both and backfill, switch reads, stop writing the old, then drop it (contract). Required because rolling deploys run both code versions simultaneously, so every intermediate state must be readable by both and independently reversible.
- **How do you backfill 40M rows safely, and why set `lock_timeout` before DDL?** — Bounded PK-ordered batches with a resumable high-water mark, short transactions so replicas keep up, and a throttle gated on replication lag. Set `lock_timeout` because DDL waiting for `ACCESS EXCLUSIVE` queues every subsequent query including plain `SELECT`s, so one long transaction turns a metadata change into a table outage.
- **What does canary catch that rolling and blue-green do not?** — Performance regressions -- a deploy that crashes nothing, passes every probe, and is 40% slower at p99. Compare against the stable version running concurrently so time-of-day effects cancel, require a minimum sample size, and route by a sticky session key.

## Drills

### Drill

Your checkout service runs 20 pods on Kubernetes. Deploys produce a 2-second burst of 502s that the team treats as normal. Separately, p99 is 340 ms against a 200 ms SLO while average CPU sits at 35%, and last week a 40-second RDS failover caused a 25-minute full outage. Diagnose all three and fix them.
