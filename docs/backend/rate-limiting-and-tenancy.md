...
> Token bucket vs sliding window, distributed counters, quotas, and noisy-neighbour isolation.

- Track: **Backend & Distributed Systems** · Level: **staff** · ~18 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/rate-limiting-and-tenancy)

A rate limiter answers one question: given that this request has arrived, and given
...
Three distinct problems, and conflating them is why so many limiter designs are wrong for
their purpose.
...
**Figures behind the design choices**

- **2x** — Overshoot of a fixed window at the boundary (100/min becomes 200 in one second)
- **2 values** — Token bucket state per key (Tokens plus last-refill timestamp)
- **0.2-0.5 ms** — Cost of a shared-counter round trip (Paid on allowed requests too)
- **/64** — IPv6 prefix a single household gets (Per-address limiting is meaningless)
- **~429** — Client exceeded its limit (`503` means *you* are shedding -- different instruction)

## The five algorithms

| Algorithm | Mechanism | Memory per key | Burst behaviour | Accuracy | Use for |
| --- | --- | --- | --- | --- | --- |
...
The fixed-window flaw is worth seeing concretely, because it is the reason the naive
...
*Token bucket rejects the excess; leaky bucket delays it. Capacity B is the burst you are willing to absorb and R is the rate you can sustain -- two separate business decisions.*

## A distributed token bucket in Redis

The hard part of a distributed limiter is atomicity. Read the token count, decide, write
...
**Atomic token bucket. Lazy refill, no background timer, self-expiring keys.**
...
**Calling it, with the failure mode decided in advance**

```javascript
const script = await redis.script('LOAD', luaSource);   // EVALSHA afterwards
...
> **Pass the clock in, and mind the slot**  
> Never call `redis.call('TIME')` inside a Lua script -- it is non-deterministic and was
...
## Accuracy versus coordination cost

A globally exact limit requires every instance to agree on a single counter, which means
...
| Location | Sees | Cost of a rejection | Good for | Blind to |
| --- | --- | --- | --- | --- |
...
You want more than one, and the ordering follows a single principle: **reject as early as
...
The limiter key determines who bears the cost of a rejection, so getting it wrong is not
...
**Key choice and its consequence**

| Key | Blast radius of a false positive | Evadable by | Use when |
| --- | --- | --- | --- |
...
## The response: `429`, `Retry-After`, and `RateLimit-*`

A limiter that rejects without telling the client when to come back has created a polling
...
```http
# Allowed -- the client can self-pace and never hit a 429.
HTTP/1.1 200 OK
...
## Quota, rate, and concurrency are three different limits

These are routinely collapsed into one setting, and they constrain genuinely different
things.
...
## Noisy neighbours and fair queueing

Rate limits cap what a tenant can ask for. They do not allocate what is left when demand
...
*A backlog of 10,000 items from tenant B cannot delay tenant C, because the scheduler serves per-queue rather than per-item. Idle weight is redistributed, so isolation does not cost utilisation.*

## Per-tenant data isolation models

**Four isolation models and what each costs**

| Model | Mechanism | Blast radius of a bug | Noisy-neighbour isolation | Per-tenant restore | Tenants per cluster | Cost |
| --- | --- | --- | --- | --- | --- | --- |
...
Row-level is right for the overwhelming majority of SaaS products, and the honest reason
...
**Making the database enforce tenancy**
...
The pragmatic pattern that most successful SaaS companies converge on is **hybrid**:
...
**Per-tenant limits plus weighted fair queueing**
...
| --- | --- | --- |
| Fixed window at the boundary | 2x the intended rate served in one second, at exactly the moment the limit mattered. | Sliding window counter with two weighted buckets, or a token bucket where burst is explicit. |
| Non-atomic read-modify-write across instances | Limit exceeded by a wide margin under concurrency; the limiter appears to work in testing and not in production. | Single-key Lua script (or `redis-cell`) so read-decide-write is indivisible. |
| Rate limit with no cost weighting | A tenant stays within 1,000 rpm while each request runs a 30-second report -- capacity consumed legally. | Per-endpoint weights debited from the bucket, plus a per-tenant concurrency limit. |
| Limiting on IP behind CGNAT | One script blocks an entire campus or mobile carrier region; support cannot explain it. | Limit on identity once authenticated; IP only for unauthenticated endpoints, and on the /64 prefix for IPv6. |
| Trusting the leftmost `X-Forwarded-For` value | Any caller forges a different client IP per request and bypasses the limiter entirely. | Take the entry appended by your trusted proxy, counting from the right; configure trusted-proxy depth explicitly. |
| `429` without `Retry-After` | Clients poll blindly, so rejected traffic generates more load than accepted traffic did. | `Retry-After` on every rejection and `RateLimit-*` on every success, so clients can self-pace. |
| Limiter fails open for a billing quota | A Redis outage gives away metered capacity, or allows unlimited password attempts. | Choose per limiter: fail open for abuse protection, closed for billing and auth, with a local fallback limiter in between. |
| FIFO queue shared across tenants | A 10,000-item backlog from one tenant delays every other tenant's three items behind it. | Per-tenant queues with deficit round robin, weighted by plan. |
| Missing `WHERE tenant_id` in a shared-schema query | Cross-tenant data exposure -- the highest-severity bug this architecture permits. | Postgres RLS with `SET LOCAL`, so the database enforces scoping and a forgotten predicate returns nothing. |
| Session-level `SET` for tenant context behind pgBouncer | Transaction-mode pooling reuses the connection, so one tenant's context applies to another's query. | `SET LOCAL` inside an explicit transaction, always. |
...
With a fixed-window limit of 100 requests per minute, what is the maximum a client can send in any 1-second period?
- A. 100
- B. 150
- C. 200 **(answer)**
- D. Unbounded

  Sending 100 at 10:00:59 and 100 at 10:01:00 satisfies both windows while delivering 200 requests in about one second -- twice the intended rate, concentrated at the boundary. A sliding window counter weights the previous window by its remaining overlap and removes the artefact for two integers of storage, and a token bucket makes the permitted burst an explicit parameter rather than an accident of the window boundary.

Your limiter reads the token count from Redis, decides, then writes it back from 40 app instances. Load tests at low concurrency pass; production overshoots badly. Why?
- A. Redis replication lag.
- B. The read-modify-write is not atomic, so concurrent instances interleave and all see the same stale count. **(answer)**
- C. Clock skew between instances.
- D. The TTL is too short.

  Between one instance's read and its write, dozens of others have read the same value and each concluded there was room, so the effective limit becomes roughly the true limit times the concurrency. A single-key Lua script executes on Redis's command thread with nothing interleaved, making the whole decision indivisible. Keep it to one key so the script keeps working after a cluster reshard -- and pass the clock in from the caller rather than reading it inside the script.

Every tenant is within their published rate limit, yet p99 latency has doubled and one tenant accounts for most of the database load. What is missing?
- A. A lower global rate limit.
- B. Cost weighting and per-tenant concurrency limits -- rate bounds arrival frequency, not the work each request implies. **(answer)**
- C. More Redis capacity.
- D. IP-based limiting.

  A request count is a poor proxy for resource consumption: 1,000 cheap lookups and 1,000 thirty-second reports are identical to a rate limiter and nothing alike to your database. Assigning each endpoint a weight and debiting that from the bucket makes expensive calls cost proportionally more, and a per-tenant semaphore on the connection pool prevents one tenant holding every connection regardless of their arrival rate. Fair queueing then allocates the remaining contention.

Which per-tenant isolation model fits a product with 60,000 small tenants and 5 large regulated ones?
- A. Database-per-tenant for all 60,005.
- B. Row-level with Postgres RLS for the long tail, dedicated databases or cells for the 5 large tenants. **(answer)**
- C. Schema-per-tenant for all.
- D. Row-level for all, with application-layer filtering only.

  60,005 databases means 60,005 backup schedules, migrations and connection pools -- an operational tax that never ends -- while schema-per-tenant at that count causes serious catalog bloat. Row-level with RLS enforced by the database keeps the long tail cheap and makes a forgotten `WHERE tenant_id` return zero rows rather than other tenants' data, which application-layer filtering alone cannot guarantee. Dedicating infrastructure to the few tenants who need and will pay for it also turns isolation into a sellable SKU.

<details><summary>Related topics and how they connect</summary>

Load shedding, admission control and why `503` differs from `429` are developed in
...
- **Why is a fixed window limiter flawed?** — It allows up to twice the limit across a boundary -- 100 requests at 10:00:59 plus 100 at 10:01:00 satisfies both windows while delivering 200 in a second. A sliding window counter or token bucket removes the artefact.
- **How does a sliding window counter work?** — Keep counters for the current and previous fixed windows and weight the previous one by its remaining overlap: `current + 0.75 * previous` at 15 seconds into a 60-second window. Two integers per key, approximate because it assumes the previous window was evenly spread.
- **Why is token bucket the usual default?** — Capacity `B` and refill rate `R` separate permitted burst from sustained rate, which matches how real clients behave. It stores two values per key, refills lazily on read, and needs no background timer.
- **When is a leaky bucket better than a token bucket?** — When you want to *delay* rather than reject -- smoothing bursty traffic to a fragile downstream that must never exceed a fixed rate. The cost is added latency, and the queue must still be bounded.
- **Why must a distributed limiter use a Lua script?** — Read-modify-write across many instances interleaves, so the effective limit becomes the true limit times the concurrency. Lua runs on Redis's single command thread, making the decision indivisible. Keep it to one key so it survives cluster resharding, and pass the timestamp in from the caller.
- **Why is limiting on IP address dangerous?** — Carrier-grade NAT can put hundreds of thousands of mobile subscribers behind one IPv4 address, so one script blocks a region. Conversely, an IPv6 household has a /64 -- 18 quintillion addresses -- so per-address limiting is trivially evaded. Limit identity when authenticated, and prefixes for IPv6.
- **Distinguish rate, concurrency and quota limits.** — Rate bounds arrival frequency. Concurrency bounds resources held simultaneously, which is what actually causes outages and is right for long-running work. Quota bounds total consumption over a billing period and must be accurate and auditable.
- **Why does rate limiting not solve noisy neighbours?** — It caps demand but does not allocate supply. Every tenant can be within their limit while the sum exceeds capacity, and requests differ in cost by orders of magnitude. You need cost weights, per-tenant concurrency limits and weighted fair queueing.
- **What is deficit round robin and why use it?** — Each per-tenant queue receives a quantum of credit per round, dequeues while credit remains, and carries the remainder forward. It is O(1) per dequeue, handles variable item costs correctly, and redistributes idle tenants' shares so isolation does not waste capacity.
