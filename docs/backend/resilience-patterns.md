...
> Timeout budgets, retry storms, circuit breakers, bulkheads, load shedding and graceful degradation.

- Track: **Backend & Distributed Systems** · Level: **core** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/resilience-patterns)

Resilience engineering is the discipline of deciding, in advance and in code, what your
...
A distributed system has a property that a single process does not: it can enter a state
...
*The arrow that does not exist is Metastable back to Healthy on its own. That absence is why load shedding is a design requirement rather than an incident tactic.*

**Defaults worth arguing about**

- **10%** — Retry budget as a share of successful traffic (Envoy `retry_budget`; stops storms automatically)
- **1.6x** — Offered load at a 20% failure rate with 3 retries (At the moment capacity is lowest)
- **243** — Attempts from 3 retries at 5 layers deep (Why you retry at exactly one layer)
- **p99.9** — The percentile a leaf timeout comes from (A p50 timeout means a 50% error rate)
- **~2 s** — Sensible bound on queue depth, in work (Deeper means everything queued has expired)

## Timeout budgets that shrink down the chain

A timeout is a statement about how long you are willing to wait. If every service in a
...
*Every number is smaller than the one above it, and the leaf timeouts plus the reserve fit inside the parent. If any leaf timeout exceeds its parent budget, that hop can never be observed succeeding.*

**Deriving a timeout instead of picking one**

| Layer | Rule | Worked value |
| --- | --- | --- |
...
> **Warning**  
> Set the leaf timeout from the dependency's **p99.9, not its p50**, and then check it fits
...
## Retries: only when safe, only with jitter, only within budget

Retries are the highest-leverage and most dangerous tool here. They convert transient
...
**Full jitter plus a retry budget**

```javascript
// The budget: retries may be at most 10% of successful traffic.
class RetryBudget {
...
> **Retry amplification through layers**  
> Retries multiply through a call chain. If the gateway retries 3 times, and each of its
...
A circuit breaker is a state machine that stops you calling something that is clearly
...
*The minimum-request threshold is what stops 2 failures out of 3 from opening the circuit on a low-traffic endpoint.*

Three parameters decide whether a breaker helps or hurts. Use a **failure ratio over a
...
A bulkhead is a partition that stops one leak from sinking the ship. In software it means
...
**Semaphore bulkheads, sized from Little's Law**

```javascript
// Limit = target throughput x expected latency, then bounded by
...
When offered load exceeds capacity, you have exactly two options: serve some requests and
...
**Priority classes and shed order**

| Class | Examples | Shed at | Rationale |
| --- | --- | --- | --- |
...
**Admission control: deadline check plus adaptive concurrency limit**

```javascript
// 1. Cheapest possible shed: drop what is already worthless.
function admit(req) {
...
Backpressure is the propagation of "slow down" from the bottleneck all the way back to
...
Decide the degradation ladder before the incident, because during the incident you will
...
## Health checks: liveness, readiness, and deep checks

Three different questions, routinely conflated, with genuinely different failure modes
...
| Check | Question | Action on failure | Must not |
| --- | --- | --- | --- |
...
**A full resilience stack: budgets, breakers, bulkheads, shedding**
...
| --- | --- | --- |
| Retries without jitter | Synchronised herds at each backoff boundary; load arrives in spikes that keep re-breaking recovery. | Full jitter: `random(0, min(cap, base * 2^n))`. Honour `Retry-After` when present. |
| Retries without a budget | A 20% failure rate becomes 1.6x offered load at the worst moment -- retry storm into metastable failure. | Token-bucket retry budget capped at ~10% of successful traffic; retries stop automatically during systemic failure. |
| Retries at every layer | One user request becomes 27 database queries; each layer looks individually reasonable. | Retry at exactly one layer -- the one that knows idempotency -- and pass failures through elsewhere. |
| Retrying a timed-out non-idempotent write | Duplicate charges and duplicate orders, because a timeout is an unknown outcome rather than a failure. | Idempotency keys on every mutating endpoint; never replay non-idempotent operations on unknown. |
| Shared thread pool across dependencies | The least important dependency exhausts all workers and takes down checkout and login. | Per-dependency semaphores or pools sized from Little's Law; non-critical pools deliberately small. |
| Timeout longer than the caller's budget | Work continues for a caller that already gave up, so capacity is consumed producing unread responses. | Propagate an absolute deadline; each hop subtracts a reserve and pushes the remainder into `statement_timeout`. |
| Unbounded queue anywhere in the pipeline | Memory grows until OOM, and every queued request has already timed out before it is served. | Bound every queue at ~2 seconds of work; drop or reject when full; prefer LIFO under overload. |
| Liveness probe that checks the database | A database blip restarts every pod simultaneously -- a recoverable issue becomes an outage plus a cold-start storm. | Liveness checks only local process health; dependency checks belong in readiness, with a fleet-level minimum-healthy override. |
| Circuit breaker on a hard dependency with no fallback | 30% failures become 100% failures once the breaker opens. | Breakers only where a meaningful fallback exists; use concurrency limits for hard dependencies. |
| Half-open probing with full traffic | A recovering dependency is immediately re-saturated, so the breaker oscillates and never closes. | Allow one or a few concurrent probes; require several consecutive successes before closing. |
...
A service degrades to a 20% failure rate. Clients retry up to 3 times with exponential backoff and full jitter. What happens to offered load, and what is missing?
- A. Load is unchanged because jitter spreads retries out.
- B. Load rises up to ~1.6x at the worst moment; jitter fixes clustering but not volume -- a retry budget is missing. **(answer)**
- C. Load falls because backoff delays requests.
- D. Nothing -- 3 retries is a safe default.

  Jitter and backoff change *when* retries arrive, not how many. Failing 20% and retrying up to three times adds substantial extra work precisely when capacity is lowest, which is the standard path into a retry storm. A token-bucket budget capping retries at roughly 10% of successful traffic self-disables during systemic failure while still retrying an isolated blip aggressively -- which is exactly the behaviour you want from both regimes.

Your Kubernetes liveness probe calls `/health`, which runs `SELECT 1`. The database has a 30-second blip. What happens?
- A. Pods are removed from the load balancer and re-added after recovery.
- B. Every pod fails liveness and is restarted simultaneously, so a recoverable blip becomes an outage plus a cold-start storm. **(answer)**
- C. Nothing -- the probe has a grace period.
- D. Only pods with active database connections restart.

  Liveness failure means "restart this container", and since every pod shares the dependency, every pod restarts at once -- and then all of them contend to re-establish connection pools and warm caches against a database that is only just recovering. Dependency checks belong in readiness, which removes an instance from rotation without killing it, and even that needs a fleet-level minimum-healthy override so a shared outage does not drain every target from the load balancer.

Under overload, why is a LIFO request queue often better than FIFO?
- A. LIFO is faster to implement.
- B. FIFO serves the oldest requests first, which are the most likely to have already exceeded their deadline, so you do full work for zero value. **(answer)**
- C. LIFO guarantees fairness.
- D. FIFO cannot be bounded.

  When the queue is deeper than the deadline allows, the head of a FIFO queue is exactly the set of requests whose clients have already given up -- so you burn capacity producing responses nobody reads, and every request in turn times out. LIFO at least serves requests that are still fresh enough to be useful, which keeps some fraction of traffic succeeding. Pair it with a dequeue-time deadline check so expired requests are dropped before any work is done; under normal load the queue is shallow and the ordering is irrelevant.

A dependency with no viable fallback fails 30% of requests. Is a circuit breaker the right tool?
- A. Yes -- it prevents cascading failure.
- B. No -- opening it turns 30% failures into 100%. A concurrency limit bounds resource consumption without refusing requests that would have succeeded. **(answer)**
- C. Yes, but only with a longer cool-down.
- D. No -- use more retries instead.

  A breaker trades availability for resource protection, which only pays off when you have something better to do than call the dependency. With no fallback, the 70% of requests that would have succeeded now fail too. A concurrency limit or bulkhead achieves the real goal -- bounding how many of your threads can be held waiting -- while still letting successful calls through, and it degrades continuously instead of stepping to zero.

<details><summary>Related topics and how they connect</summary>

The queueing curve that makes shedding necessary is derived in **Scalability & Capacity
...
- **What is a metastable failure mode?** — A state the system stays in after the original trigger is gone, sustained by its own retry traffic. There are two stable states and no self-recovery path from the broken one -- exiting requires external intervention such as shedding load or restarting clients.
- **Why is full jitter better than plain exponential backoff?** — Backoff alone leaves clients synchronised, so they retry together at each boundary and arrive as a herd. Full jitter -- `random(0, min(cap, base * 2^n))` -- spreads them across the whole interval, reducing both total work and completion time.
- **What is a retry budget and why is it the piece usually missing?** — A token bucket capping retries at a small fraction (~10%) of successful traffic. Per-request retry limits still allow load to multiply during a broad failure; a budget empties and stops retries automatically, so you retry hard for a blip and not at all for a systemic outage.
- **Why can you not retry a timed-out `POST`?** — A timeout is an unknown outcome, not a failure -- the write may have succeeded with the response lost. Retrying duplicates it unless the endpoint honours an idempotency key.
- **What does a circuit breaker actually buy you?** — It converts a slow failure into an instant one, so threads are not held waiting, and it gives the struggling dependency room to recover. It is only appropriate where a meaningful fallback exists -- on a hard dependency it turns partial failure into total failure.
- **What is a bulkhead, and what does it cost?** — Separate resource pools per dependency, so exhaustion is contained -- a slow recommendations service fills its own 10 permits instead of all 200 workers. The cost is lost utilisation, because a class cannot burst into idle capacity reserved for others.
- **What is the right signal for load shedding?** — Queue wait time measured at dequeue, compared against the request deadline. A request queued longer than its deadline is worthless, so drop it before parsing the body. Latency alone conflates slow work with queued work.
- **Why must every queue be bounded?** — An unbounded queue converts a throughput problem into an OOM crash, and everything in a deep queue has already timed out. Bound at roughly two seconds of work and make the full behaviour an explicit choice: block, drop newest, drop oldest, or reject.
- **Difference between liveness, readiness and a deep health check?** — Liveness asks whether the process is wedged -- failure means restart, so it must never check a dependency. Readiness asks whether this instance can serve now -- failure removes it from the load balancer. Deep checks verify the whole graph and should page a human, never drive routing, and you need a minimum-healthy override so a shared outage does not zero your capacity.
