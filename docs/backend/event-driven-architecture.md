...
> Outbox pattern, distributed transactions without 2PC, projections and eventual consistency UX.

- Track: **Backend & Distributed Systems** · Level: **staff** · ~21 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/event-driven-architecture)

Event-driven architecture means services communicate by publishing facts about things that already happened, rather
...
Here is the code every team writes first, and it is broken.

**The broken version -- two writes, no shared transaction**

```js
await db.orders.insert(order);              // write 1: durable
...
Two independent systems, two independent writes, no shared transaction. Four outcomes exist and two of them are
...
> **Note**  
> Do not be reassured by how rarely you see it. At 1,000 orders per second, a crash window of 5 ms means roughly 5
...
The fix is to make the event part of the same transaction as the state change, by writing it to a table in the same
...
*The atomic boundary is the database commit. Everything after it is at-least-once, which is why each consumer owns a dedupe store.*

**Outbox schema and the one transaction that makes it work**
...
You have two ways to drain the outbox. **Polling** is simple, needs no extra infrastructure, and its latency is your
...
> **The replication slot is the CDC failure mode to know**  
> A Postgres logical replication slot pins WAL until the consumer confirms it. If Debezium is down for six hours on a
...
## Three patterns people call "events", with different costs

| Pattern | Payload | Consumer must | Coupling | Cost |
| --- | --- | --- | --- | --- |
...
These get conflated constantly, and the distinction is worth being precise about because the middle one is what most
...
A booking spans three services: payment, inventory and notification. You need all three or none. Two-phase commit
...
*Compensation is semantic, not transactional. A voided authorisation still leaves a trace on the customer statement.*

Compensating actions are where the design work actually is, and they are rarely symmetric with the forward action.
...
The honest recommendation: orchestrate anything involving money or a compensation you would have to explain to a
...
## CQRS and the projection lag the UI has to survive

CQRS separates the write model from the read model. Writes go to a normalised store optimised for invariants; a
...
- **Optimistic UI** — Render the expected result immediately from what the client just submitted, and reconcile when the projection catches up. Best UX, and you must design the failure path -- what the screen does when the write is rejected after you already showed success.
- **Read-your-writes token** — The write returns a projection position; the client passes it on the next read; the read waits until the projection has caught up to it or falls back to the write model. Precise, and it needs support on both sides.
- **Return the projected entity** — The command handler returns the full post-write representation, so the UI does not need to re-read at all. Simplest fix, and it only covers the screen you just came from.
- **Explicit pending state** — Show the item as `Processing` with a real affordance. Honest, sometimes the only truthful option for genuinely long flows, and users accept it when the state is named.

**Projection lag budget**

- **<100 ms** — Healthy projector (Unnoticeable with optimistic UI)
- **1-5 s** — Under moderate load (Users notice on a redirect-after-save)
- **minutes** — During a rebuild or backlog (Needs an explicit UI state, not a spinner)
- **~2 hrs** — Full rebuild of 500M events (At 70k events/sec with batched writes)

## Schema evolution: events are a forever contract

An HTTP API version can be deprecated because you can see who calls it. An event in a retained log will be read by
...
**Envelope worth standardising on day one**
...
Four fields there do disproportionate work. `eventId` is what the consumer dedupes on. `sequence` lets a
...
## Ordering across aggregates, and replay

Within one aggregate, partitioning by `aggregateId` gives you total order, and that is almost always the right
...
| --- | --- | --- |
| Dual write -- DB commit then publish, with a crash between | Orders exist that no downstream service knows about; discovered weeks later as a reconciliation gap with no audit trail. | Transactional outbox in the same commit, drained by a polling relay or CDC. This is the whole reason the pattern exists. |
| Publish-then-commit with a rolled-back transaction | Consumers act on an order that does not exist; downstream state is built on a fiction and cannot be repaired automatically. | Same fix. There is no safe ordering of two non-transactional writes. |
| Non-idempotent consumer with at-least-once delivery | Duplicate charges, double inventory decrements, two emails for one order. | Dedupe on `eventId` in the same transaction as the effect; commit the offset only afterwards. |
| Debezium down, replication slot pinning WAL | Primary database disk fills and the whole application goes down -- caused by a pipeline component, not the database. | Alert on slot lag, set `max_slot_wal_keep_size` so the slot is invalidated instead of the disk filling, and document the re-snapshot procedure. |
| Breaking schema change to a live event type | Every consumer starts failing deserialisation at once; messages pile into DLQs across several teams. | Schema registry with backwards-compatibility enforced in CI; additive-only changes; a new event type when the meaning changes. |
| Saga step fails and compensation is itself unreliable | Money captured, seat never reserved, no compensation recorded -- permanently inconsistent state requiring manual repair. | Persist saga state per step, make compensations idempotent and retriable with their own outbox, and alert on instances stuck beyond a timeout. |
| Replay into a consumer with external side effects | Three years of receipt emails sent in ten minutes; a customer-trust incident and possibly a blocked sending domain. | Explicitly tag consumers replay-safe or not, enforce it in the replay tool, and route replays to a dedicated consumer group with side effects disabled. |
| Projection rebuilt in place | Search or dashboard returns empty results for the hours the rebuild takes. | Build into a new table or index and switch an alias atomically when it catches up. |
...
A service inserts an order then publishes `OrderPlaced` to Kafka. It crashes between the two. What is the correct fix?
- A. Publish to Kafka first, then insert the order.
- B. Wrap both in a database transaction.
- C. Insert the event into an outbox table in the same transaction, and publish from there via polling or CDC. **(answer)**
- D. Retry the publish with exponential backoff.

  This is the dual-write problem and no ordering of two non-transactional writes is safe -- publishing first just swaps a missing event for a phantom one. A database transaction cannot enrol the Kafka publish. Retries only help if the process survives, and a crash is exactly the case that defeats them. The outbox makes the event durable in the same atomic commit as the state change; the relay can then crash freely and only ever cause duplicates, which idempotent consumers absorb.

Your booking saga captures payment, then reserves inventory, then emails confirmation. Inventory fails. What is the design problem?
- A. Sagas cannot handle failures; you need 2PC.
- B. Payment was captured rather than authorised, so compensation is a refund with fees rather than a clean void -- the hardest-to-reverse step ran first. **(answer)**
- C. The email should have been sent before inventory.
- D. Nothing -- compensating transactions handle this transparently.

  Compensation is semantic, not transactional, and its cost varies enormously by step. Authorise-then-capture-at-the-end means a failure is a void, which is invisible to the customer; capturing up front turns the same failure into a refund with fees, a statement entry and a support conversation. The heuristic is to order steps so the most expensive-to-compensate action happens last -- which is also why sending email is always the final step, since it cannot be compensated at all.

After adopting CQRS, users report their newly created item is missing from the list they are redirected to. Best first fix?
- A. Make the projection synchronous with the write.
- B. Have the command return the projected representation, or render optimistically from what the client submitted. **(answer)**
- C. Add a 2-second delay before redirecting.
- D. Read the list from the write model instead.
...
Which change to a live, retained event schema is safe?
- A. Renaming `total` to `totalCents` for clarity.
- B. Adding an optional `couponCode` field with a default. **(answer)**
- C. Changing `quantity` from string to integer.
- D. Reusing the unused `notes` field to carry a status code.

  Only additive, optional changes are backwards compatible: old consumers ignore the new field and new consumers handle its absence. A rename is a delete plus an add, so every existing consumer breaks. A type change breaks deserialisation. Repurposing a field is the most dangerous of the four because it passes every schema check and silently corrupts the meaning of the data -- including for replays of historical events. When semantics change, emit a new event type and run both in parallel.

<details><summary>Related topics and how they connect</summary>

The broker mechanics underneath all of this -- partitions, delivery semantics, DLQs, consumer lag -- are in **Queues
...
- **What is the dual-write problem?** — Writing to your database and publishing to a broker are two independent non-transactional writes. A crash between them either loses the event silently or announces a state change that was rolled back. No ordering of the two is safe, so it must be designed away rather than handled carefully.
- **How does the transactional outbox fix it?** — The event is inserted into a table in the same database transaction as the state change, so one atomic commit covers both. A separate relay reads the table and publishes, retrying freely -- it can only ever cause duplicates, which idempotent consumers absorb.
- **Why capture the outbox table with CDC rather than the business tables?** — Because the event payload then stays a deliberately designed published contract. Capturing business tables makes your internal column names part of a forever contract, so you can never rename them.
- **What is the main operational risk of Debezium on Postgres?** — The logical replication slot pins WAL until the consumer confirms it. A stalled connector on a busy database fills the primary's disk and causes a full outage. Cap `max_slot_wal_keep_size` so the slot is invalidated instead, and monitor slot lag.
- **Event notification vs event-carried state transfer?** — Notification sends only an id, so consumers must call back -- re-creating the synchronous coupling the broker removed, plus a read-replica race. State transfer sends enough state to act independently, at the cost of larger messages and stricter schema discipline. State transfer is usually the right default.
- **Orchestration vs choreography -- when to use each?** — Orchestrate anything with money or a compensation you would have to explain to a customer, because you get one queryable place for saga state and centralised, testable compensation. Choreograph loosely-coupled reactions like search indexing and notifications.
- **What is the key heuristic for ordering saga steps?** — Put the hardest-to-compensate action last. Voiding an authorisation is clean, refunding a capture costs fees, and an email cannot be un-sent -- so notification is always the final step.
- **Four ways to handle projection lag in the UI?** — Render optimistically from the submitted data and reconcile; carry a read-your-writes token so the read waits for the projection; return the projected entity directly from the command; or show an explicit pending state. Different screens want different ones.
- **Which event schema changes are safe on a retained log?** — Only adding optional fields. Never rename, never change a type, and never repurpose a field -- the last passes every compatibility check while silently corrupting meaning, including for replays. When semantics change, emit a new event type and run both.
- **How do you rebuild a projection safely?** — Build into a new table or index while the old one keeps serving, then switch a pointer or alias atomically once it has caught up. Never rebuild in place, and never replay into a consumer with external side effects like email.
...
A team proposes full event sourcing for a new B2B project-management product: tasks, comments, assignments, boards. They cite audit history and the ability to build new read models later. You have three backend engineers, none of whom has run an event-sourced system. Give your recommendation.
