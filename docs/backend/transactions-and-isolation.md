...
> ACID in practice, MVCC, the anomalies each level permits, and locking under contention.

- Track: **Backend & Distributed Systems** · Level: **core** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/transactions-and-isolation)

A transaction is a promise that a group of reads and writes behaves as though nothing
...
Two things go wrong the moment more than one thing happens at once. Concurrent
...
- **1 s** — Postgres `deadlock_timeout` default (How long before the cycle is detected)
- **40001** — SQLSTATE for serialization failure (`40P01` is deadlock -- both retryable)
- **~200 M** — Transaction ids left before wraparound protection (At which point writes are refused)
- **~2.5x** — Scan cost on a 60%-dead-tuple table (You read past every dead version)
- **milliseconds** — Target transaction duration (Not seconds, and never a network call)

## ACID, precisely

| Property | What it actually guarantees | What it does not | Mechanism |
| --- | --- | --- | --- |
...
The two words people over-read are Consistency and Durability. **Consistency** in ACID is
...
Postgres never overwrites a row in place. Every tuple carries `xmin` -- the transaction
...
*Readers and writers never block each other because they read different versions. The dead tuple left behind is the cost, and vacuum can only remove it once no snapshot could still need it.*

**Watching MVCC happen**
...
Dead tuples are the bill for this design. They occupy pages, they are still referenced by
...
> **The most damaging habit in application code**  
> Opening a transaction, then making an HTTP call, then writing. A 3-second downstream call
...
## The anomalies, and which level permits them

**What each isolation level actually allows**
...
**Write skew** is the anomaly that matters at Staff level, because it is the one snapshot
...
## Why "Repeatable Read" means different things

The SQL standard defines isolation levels by which anomalies they forbid, which means two
...
| Behaviour | Postgres Repeatable Read | MySQL InnoDB Repeatable Read |
| --- | --- | --- |
...
## Serializable Snapshot Isolation and the retry loop

Postgres implements `SERIALIZABLE` as **Serializable Snapshot Isolation**: optimistic
...
> **Warning**  
> The retry must re-run the *whole* transaction including the reads, because the aborted
...
When two requests genuinely contend for the same row, you either take a lock and make one
...
**Three patterns, three contention profiles**

```sql
-- Pessimistic: bounded wait, explicit lock.
BEGIN;
...
A deadlock needs nothing exotic: two transactions each holding a lock the other wants.
...
**A deadlock in eight statements**
...
Ordering by a deterministic key -- sort the account ids, always lock the lower first --
...
Most transaction bugs are boundary bugs, not isolation bugs. Four rules cover the
majority of them.
...
**Running Serializable instead of Read Committed**
...
| --- | --- | --- |
| Read-modify-write at Read Committed | Lost updates. Two concurrent increments produce one; no error is raised and totals silently drift. | In-place arithmetic (`SET x = x - 1`), `SELECT FOR UPDATE`, a version column, or Serializable with retry. |
| Write skew on a cross-row invariant | Zero doctors on call, double-booked room, inventory oversold. Snapshot isolation permits it by design. | Serializable, or materialise the conflict by locking a row representing the invariant, or express it as a unique/check constraint. |
| Serializable without a retry loop | Random `40001` errors surfacing as 500s, worsening exactly as traffic rises. | Wrap every transaction in a retry with full jitter; alert on retry rate, not just on error rate. |
| Long-running or idle-in-transaction session | Autovacuum blocked fleet-wide, bloat on unrelated tables, plan regressions, ultimately xid wraparound read-only mode. | `idle_in_transaction_session_timeout`, `statement_timeout`, and an alert on the oldest transaction age. |
| Network call inside a transaction | Locks and connections held for seconds; at 200 rps hundreds of open transactions and a pool exhaustion cascade. | Fetch first, then `BEGIN`. Drive external effects from a committed outbox row. |
| Inconsistent lock ordering across code paths | Deadlock rate that grows superlinearly with traffic; failures concentrated on the hottest rows. | `ORDER BY id FOR UPDATE` to impose a total order; log both statements and treat the rate as a design defect. |
| Relying on MySQL RR snapshot semantics for a locking read | Silently mixes two points in time -- the `SELECT` shows 5000 while the `UPDATE` computes from 4900. | Understand that locking reads bypass the snapshot; use in-place arithmetic or explicit `FOR UPDATE` before deciding. |
| Lock held across user think time | A single open form blocks other users for minutes; timeouts cascade into retries. | Optimistic concurrency with a version column and a conflict-resolution UI. |
...
Two concurrent requests each read `balance = 500`, subtract 100 in application code, and write 400. Both commit at Read Committed. What is the balance, and what is this called?
- A. 300 -- both writes apply.
- B. 400 -- a lost update; one write silently overwrote the other. **(answer)**
- C. One transaction receives a serialization error.
- D. 500 -- the second write is rejected.
...
A rota system requires at least one engineer on call. Two engineers simultaneously check `count(*) = 2` and each remove themselves, under Postgres Repeatable Read. What happens?
- A. One gets a serialization failure -- snapshot isolation detects the conflict.
- B. Both succeed and the shift has zero engineers; this is write skew. **(answer)**
- C. A deadlock occurs and one is aborted.
- D. The second read sees 1 and the removal is blocked.

  Each transaction deleted a row the other did not read or write, so there is no version conflict for snapshot isolation to find -- the conflict is between one transaction's write and the other's *predicate*, which only Serializable tracks. Fixes are to run Serializable and retry, to lock a row representing the shift so the transactions genuinely conflict, or to encode the invariant as a constraint the database can enforce.

A Postgres table shows steadily rising `n_dead_tup` and slowing sequential scans, and so do several unrelated tables. Autovacuum is running and configured normally. Most likely cause?
- A. Autovacuum needs more workers.
- B. A long-running or idle-in-transaction session is holding an old snapshot, so no newer tuple version can be removed anywhere. **(answer)**
- C. The tables need `REINDEX`.
- D. `fillfactor` is too high.

  Vacuum may only reclaim a version once no existing snapshot could still need it, so one forgotten `BEGIN` pins the horizon for the entire cluster and bloat accumulates on tables that session never touched -- which is exactly the fleet-wide signature. Query `pg_stat_activity` for `state = 'idle in transaction'` ordered by `xact_start`, and set `idle_in_transaction_session_timeout` so it cannot happen unattended. Left long enough this escalates to transaction-id wraparound, which forces the database read-only.

Why must a Serializable retry re-execute the whole transaction rather than just the failing statement?
- A. Because Postgres caches the statement plan.
- B. Because the aborted transaction's reads are precisely what became stale, so the decisions built on them must be recomputed. **(answer)**
- C. Because the connection is closed on abort.
- D. Because locks are only released at commit.

  SSI aborts a transaction when its reads can no longer be reconciled with concurrent commits into any serial order. Re-running only the write reuses the stale values that caused the abort, reproducing the very anomaly the isolation level exists to prevent -- and mechanically the transaction is already aborted, so no statement can succeed within it. This is also why side effects such as charging a card must live outside the retried block.

<details><summary>Related topics and how they connect</summary>

Dead tuples, the visibility map and index-only scans continue in **Databases, Indexes
...
- **What does the C in ACID actually guarantee?** — Only that constraints you declared -- keys, checks, foreign keys -- hold at commit. A business invariant such as "balance never goes negative" is protected only if it is an actual `CHECK` constraint.
- **Explain MVCC visibility in Postgres via xmin and xmax.** — Each tuple records the creating transaction (`xmin`) and the deleting/superseding one (`xmax`). A snapshot sees a tuple if `xmin` committed before the snapshot and `xmax` is unset or uncommitted. Readers and writers therefore never block each other.
- **Why can one idle transaction bloat your whole database?** — Vacuum can only remove a tuple version once no live snapshot could need it. An old snapshot pins that horizon cluster-wide, so dead tuples accumulate on tables the session never touched, and index-only scans degrade as the visibility map goes stale.
- **What is write skew, and why does snapshot isolation permit it?** — Two transactions read the same set, then write *different* rows, jointly breaking an invariant. No transaction writes a row the other read, so there is no version conflict to detect -- only Serializable tracks predicate conflicts.
- **Three ways to defend against write skew?** — Run Serializable and retry `40001`; materialise the conflict by locking a row that represents the invariant; or encode the invariant as a unique or check constraint so it holds regardless of isolation level.
- **How does Postgres Repeatable Read differ from MySQL InnoDB Repeatable Read?** — Postgres raises `40001` if you update a row changed since your snapshot. MySQL lets locking reads (`FOR UPDATE`, and the internal reads of `UPDATE`/`DELETE`) see the latest committed row instead of your snapshot, mixing two points in time silently. MySQL also uses gap locks to block phantoms.
- **What is SSI and what does it demand from the application?** — Serializable Snapshot Isolation detects dangerous read/write dependency structures using non-blocking predicate locks and aborts one transaction with `40001`. The application must retry the entire transaction with jitter; aborts are the mechanism, not a bug.
- **When is optimistic concurrency better than `SELECT FOR UPDATE`?** — When conflicts are rare and the decision takes a long time -- a user editing a form. No lock is held while thinking. It degrades under high contention, where a bounded pessimistic lock is better.
- **What removes a whole class of deadlocks?** — Consistent lock ordering -- acquire rows in a deterministic total order, e.g. `WHERE id IN (...) ORDER BY id FOR UPDATE`, so two transactions can never hold locks the other needs in the opposite order.
