...
> Cache-aside, write-through, stampede protection, hot keys, and eviction under memory pressure.

- Track: **Backend & Distributed Systems** · Level: **core** · ~19 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/server-side-caching)

A server-side cache is a second, faster, less trustworthy copy of your data that you put
...
Two independent pressures. The first is latency: some values are expensive to produce --
...
**The figures that drive cache design**

- **~0.2 ms** — Redis GET, same AZ (Dominated by the network round trip)
- **~100k ops/s** — Single Redis core, simple commands (One thread for command execution)
- **95% to 90%** — Doubles origin load (2,500 rps becomes 5,000 rps at 50k total)
- **1 ms** — O(N) command on 100k elements (A `KEYS` or big `HGETALL` blocks everything)
- **16,384** — Redis Cluster hash slots (Fixed; keys map to slots by CRC16)

## The five patterns

| Pattern | Read path | Write path | Right when | Cost |
| --- | --- | --- | --- | --- |
...
Cache-aside is the default because it is the only pattern that is cheap when your key
...
## Redis data structures mapped to real problems

Treating Redis as a string-to-string map wastes most of what makes it useful. The data
...
**Structure, command, and the problem it removes**

```text
# Sorted set: leaderboards. Rank and range are O(log N), not a sort.
...
The HyperLogLog case is the most striking economically. Counting distinct daily visitors
...
A key with 8,000 rps of traffic and a 60-second TTL has a specific, predictable
...
*One request rebuilds, the rest serve the stale copy. The stale copy is the important half -- without it, 7,999 requests either block or fail.*

**Locking plus stale-while-revalidate, the production shape**

```javascript
const HARD_TTL = 600;   // Redis key lifetime -- the stale copy lives this long
...
The third technique is **probabilistic early expiry**, from the XFetch paper, and it is
...
Redis Cluster shards by hashing the key to one of 16,384 slots, so a single key lives on
...
When Redis reaches `maxmemory` it consults `maxmemory-policy`, and the default in
...
| `maxmemory-policy` | Behaviour | Use for |
| --- | --- | --- |
...
> **The `volatile-*` trap**  
> With `volatile-lru`, keys without a TTL are never evicted. One code path that writes a
...
## Persistence, failover, and what you lose

Redis offers RDB snapshots, an append-only file, or both. **RDB** forks the process and
...
## Cluster: slots and multi-key limits

In Redis Cluster every key hashes via CRC16 to one of 16,384 slots, and each slot is owned
...
```text
# Fails: keys hash to different slots.
...
## Cache consistency, and the delete-versus-update debate

On a write, you can update the cache with the new value or delete the key and let the next
...
**Negative caching** is the other half of consistency people forget. If `GET
...
**Introducing a Redis cache in front of your database**
...
| --- | --- | --- |
| Stampede at TTL expiry on a hot key | 8,000 concurrent identical queries hit the database; latency spike, connection exhaustion, retry amplification. | Soft/hard TTL with stale-while-revalidate, `SET NX` rebuild lock, in-process coalescing, and jittered TTLs. |
| Synchronised TTLs from a bulk warm-up | Thousands of keys written in one batch all expire in the same second; periodic origin load spikes. | Randomise each TTL by ±20% at write time. |
| Hot key exceeding one node's throughput | One cluster node at 100% CPU, cluster-wide latency rise, adding nodes has no effect. | Short-TTL local cache in front of Redis, key replication across suffixes, or client-side caching with tracking. |
| Cache and locks in the same instance under `allkeys-lru` | A distributed lock is silently evicted; two workers process the same job. | Separate instances. `noeviction` for correctness data, `allkeys-lfu` for the cache, and fencing tokens regardless. |
| Cache-fill race writing a stale value | A reader's pre-write value lands after the invalidation and persists until the TTL. | Short TTL so it self-heals, delayed double-delete, or CDC-driven invalidation ordered by commit. |
| No negative caching | Requests for non-existent keys bypass the cache entirely; a scanner drives hit rate to zero. | Cache absence with a 30-60 s TTL and a sentinel; Bloom filter for very sparse key spaces. |
| Cold cache after a restart or flush | Origin receives 100% of traffic it was never sized for and collapses; the cache cannot refill because everything times out. | Load shedding and concurrency limits on the origin path, staged warm-up, and never flush the whole cache as a remedy. |
| O(N) command on a large collection | `KEYS *` or `HGETALL` on 100k fields blocks the single command thread; every client sees a multi-second stall. | `SCAN` and `HSCAN`, `UNLINK` instead of `DEL`, and a lint rule banning `KEYS` and `FLUSHALL` in application code. |
| Cached value used for authorisation | Revoked permissions remain effective for the TTL -- a security incident, not a staleness bug. | Short TTL plus explicit invalidation on revoke, or never cache the decision, only the inputs. |
...
A key serving 8,000 rps with a 60-second TTL causes a database latency spike every minute. Which single change removes the user-visible spike?
- A. Increase the TTL to 600 seconds.
- B. Keep the value in Redis beyond its logical freshness and serve it stale while one request rebuilds. **(answer)**
- C. Add more Redis nodes.
- D. Move the query to a read replica.

  Separating a soft TTL (when we want fresh data) from a hard TTL (when the key is removed) means there is always something to return, so no user waits for the rebuild -- and a `SET NX` lock ensures only one request performs it. A longer TTL only makes the spike less frequent and staler when it happens; extra Redis nodes do not help because the bottleneck is the database; a replica moves the spike rather than removing it.

On a write, is it better to update the cache with the new value or delete the key?
- A. Update -- it avoids a miss on the next read.
- B. Delete -- updating can be applied out of order relative to commits, leaving a permanently stale value. **(answer)**
- C. Both are equivalent given a TTL.
- D. Update, but only inside the database transaction.

  Two writers can commit in one order and apply their cache updates in the other, so the cache ends up holding the older value with no TTL-driven self-correction, because it was just written. Deleting makes the next read authoritative and also avoids caching keys nobody requests. Doing either inside the transaction is worse still: a concurrent reader can repopulate from the uncommitted state.

Your Redis instance holds cache entries, user sessions and distributed locks, with `maxmemory-policy allkeys-lru`. What is the specific risk?
- A. Sessions will be evicted, logging users out.
- B. A lock can be evicted under memory pressure, so two workers process the same job. **(answer)**
- C. LRU tracking consumes too much memory.
- D. Locks will never be evicted because they have TTLs.

  `allkeys-lru` treats every key as disposable, including a lock key that has not been read recently -- and a lock silently disappearing breaks mutual exclusion with no error. Losing sessions is bad but recoverable; losing a lock is a correctness failure. Split the instances so cache data can use `allkeys-lfu` while correctness data uses `noeviction`, and add a fencing token, because asynchronous replication means a failover can duplicate a lock even without eviction.

In Redis Cluster, `MGET user:7:profile user:99:profile` returns `CROSSSLOT`. Why, and what is the fix if these must be fetched together atomically?
- A. The client is misconfigured; enable cluster mode.
- B. The keys hash to different slots on different nodes; use a hash tag such as `user:{7}:profile` to co-locate related keys -- accepting that co-location concentrates load. **(answer)**
- C. Use `MULTI` instead of `MGET`.
- D. Increase the number of hash slots.

  Keys map by CRC16 to one of 16,384 slots owned by a single node, so any multi-key command requires all keys in the same slot. Hash tags hash only the `{...}` substring, which lets you deliberately co-locate a group -- and that is exactly how hot slots are created, since all of one tenant's keys then share a node. Pipelining is the usual answer for throughput without atomicity, and cluster-aware clients split it per node automatically.

<details><summary>Related topics and how they connect</summary>

Sliding-window and token-bucket limiters built on sorted sets and Lua are developed in
...
- **Why is the difference between a 95% and 90% hit rate so significant?** — It doubles origin load. At 50,000 rps total, 95% leaves 2,500 rps on the database and 90% leaves 5,000. The cache is sized to make the database survivable, so hit rate directly determines whether it does.
- **Describe cache stampede and the three composable defences.** — At TTL expiry every concurrent request for a hot key misses and runs the expensive load at once. Defend with in-process request coalescing, a `SET NX` rebuild lock across instances, and a soft/hard TTL so a stale copy is served while one caller rebuilds. Jitter TTLs so keys do not expire together.
- **What is probabilistic early expiry?** — Each reader refreshes with a probability rising near expiry, weighted by measured recompute cost -- `now - delta * beta * ln(random()) >= expiry`. It spreads refreshes across the window with no locks or coordination, and refreshes expensive keys earlier.
- **Why can you not scale a hot Redis key by adding nodes?** — A key hashes to exactly one of 16,384 slots, owned by one node, executing commands on one thread -- roughly 100,000 ops/s. Mitigate with a short-TTL local cache in front, key replication across suffixes, or client-side caching.
- **Why should cache data and locks live in different Redis instances?** — They need opposite `maxmemory-policy` values. A cache wants `allkeys-lfu` so it discards under pressure; locks and queues want `noeviction` so they error instead, because silently evicting a lock breaks mutual exclusion.
- **What do you lose on a Redis failover?** — Replication is asynchronous, so a promoted replica may be missing writes the primary already acknowledged. RDB loses everything since the last snapshot; AOF with `appendfsync everysec` loses up to a second. This is why Redis locks need fencing tokens.
- **Delete or update the cache on write, and why?** — Delete. An update is a second write that can fail independently, and concurrent writers can apply updates in the opposite order to their commits, leaving a permanently stale value. Delete also avoids caching keys nobody reads. Invalidate after the commit, never inside the transaction.
- **What is negative caching and when do you need it?** — Caching the absence of a value with a short TTL and a sentinel. Without it, every request for a missing key reaches the database, so a scanner probing random ids drives your hit rate to zero. A Bloom filter handles very sparse key spaces.
- **Why is `KEYS *` dangerous, and what else is on that list?** — Redis executes commands on one thread, so an O(N) command on a large collection blocks every client. Also avoid `HGETALL` on huge hashes, `SMEMBERS` on large sets, `ZRANGE 0 -1`, and `DEL` of big collections -- use `SCAN`, `HSCAN` and `UNLINK`.
