...
> Raft in intuition, leader election, fencing tokens, and why Redlock is contested.

- Track: **Backend & Distributed Systems** · Level: **staff** · ~20 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/consensus-and-coordination)

Consensus is a group of machines agreeing on a single value -- or, more usefully, on a single *ordered sequence* of
...
Suppose three nodes need to know which one is the leader. Node A cannot simply ask B and C, because in an
...
> **The most useful thing you can know about consensus**  
> It is expensive and it is often unnecessary. A majority-quorum round trip per decision means every write costs at
...
## Raft, with actual intuition

Raft splits the problem into three pieces that can be understood separately: leader election, log replication, and
...
**Election.** A follower that hears no heartbeat within its election timeout increments the term, votes for itself,
...
**Raft cluster arithmetic**

- **3 nodes** — Tolerates 1 failure (Majority is 2)
- **5 nodes** — Tolerates 2 failures (Majority is 3 -- the etcd recommendation)
- **4 nodes** — Still tolerates only 1 (Even counts buy nothing -- majority is 3)
- **~1-5 ms** — Commit latency, same-region (One round trip to a majority plus fsync)
- **~150-300 ms** — Typical election timeout (Randomised; leader loss costs roughly this)

> **Paxos, Multi-Paxos, Raft, ZAB**  
> Paxos agrees on one value; Multi-Paxos runs it repeatedly with a stable leader to agree on a log. Raft is
...
## When you need consensus, and when you are cargo-culting it

| Situation | Do you need consensus? | What to do |
| --- | --- | --- |
...
That last row is worth internalising. If the resource you are protecting is a single database, that database *is*
...
## Leases: leadership with an expiry

A lock held forever is a liability, because the holder can die without releasing it. A **lease** is a lock with a
...
*Without the token check at the storage layer, Client 1 overwrites Client 2 and neither ever sees an error.*

**Any lock that does not hand out a monotonically increasing token, and whose protected resource does not reject
...
**Fencing with nothing more exotic than a version column**

```sql
-- The lease holder learns its token when it acquires leadership.
...
## Redlock, fairly presented

Redlock is an algorithm for distributed locking across N independent Redis instances: acquire the lock on a majority
...
## Idempotency: the cheaper alternative

Most distributed-locking requirements dissolve under the question "why does running twice hurt?" If you can make the
...
**An idempotency table that replaces a distributed lock**
...
## Clocks, and why "just use timestamps" loses data

Every distributed-systems bug involving time comes from one of two mistakes: treating wall-clock time as ordered
...
**Clock mechanisms and what they buy**

| Mechanism | What it orders | Cost / requirement | Used by |
| --- | --- | --- | --- |
...
Spanner is instructive because it shows the price of real external consistency: it does not eliminate clock
...
## etcd, ZooKeeper and Consul in practice

| System | Protocol | Primitive you actually use | Typical role |
| --- | --- | --- | --- |
...
> **These clusters are your most dangerous dependency**  
> A Raft cluster stops accepting writes the moment it cannot reach a majority. Put a 3-node etcd cluster in one AZ and
...
| --- | --- | --- |
| Zombie leader after a long GC pause | Two workers process the same queue or write the same rows; duplicate side effects with no error anywhere. | Monotonically increasing fencing token rejected at the storage layer, plus a holder-side monotonic-clock check with a safety margin before acting. |
| Redis `SETNX` lock without a TTL | Holder crashes, lock is held forever, the protected job never runs again until someone deletes a key by hand. | Always a TTL, renewed by the holder; treat expiry as the normal path and make the work idempotent. |
| Redis lock TTL shorter than the work | Lock expires mid-job, a second worker starts, both run concurrently -- the exact thing the lock existed to prevent. | Renew from a watchdog while working, and fence at the resource so the slow first worker cannot commit. |
| etcd cluster loses quorum | All leader elections and config updates freeze; controllers stop reconciling; no new deploys. | Spread members across 3 AZs, use 5 members, alert on quorum health and Raft leader changes, and keep etcd off the user request path. |
| Even-numbered cluster size | A 4-node cluster still tolerates only one failure, and a 2-2 partition means no side can make progress. | Always odd -- 3 or 5. Use a learner/non-voting member if you need a fourth copy for reads. |
| NTP step causes LWW to discard a newer write | Silent data loss with no conflict recorded; the user's edit simply does not exist. | Hybrid logical clocks or version vectors so concurrency is detected rather than resolved by a skewed number. |
| Timeout measured with wall-clock time | An NTP correction makes a 30-second timeout fire instantly or never; retry logic behaves randomly. | Use the monotonic clock for every duration; reserve wall clock for display and audit timestamps. |
...
A worker acquires a 30-second Redis lock, pauses for 40 seconds in GC, then writes to S3. A second worker acquired the lock and already wrote. What prevents corruption?
- A. A longer lock TTL.
- B. Nothing in this design -- the write needs to carry a fencing token that the storage layer rejects if stale. **(answer)**
- C. Checking the lock is still held just before writing.
- D. Redlock across five Redis instances.

  A longer TTL moves the window without closing it, and checking the lock before writing does not help because the pause can happen between the check and the write. Redlock has the same gap -- it issues no token. The only place the stale write can be stopped is at the resource: hand out a monotonically increasing token with the lease and have the destination reject anything lower than the highest it has seen. If your destination cannot do conditional writes, design for idempotency rather than mutual exclusion.

Why does growing a Raft cluster from 3 nodes to 4 not improve fault tolerance?
- A. It does -- 4 nodes tolerate 2 failures.
- B. A majority of 4 is 3, so you still only tolerate one failure, and a 2-2 split stalls both halves. **(answer)**
- C. Raft only supports 3 or 5 nodes.
- D. The fourth node becomes a read-only learner automatically.

  Fault tolerance is `floor((N-1)/2)`: 1 for both N=3 and N=4, 2 for N=5. The fourth node adds replication cost and one more thing to fail while adding no tolerance, and it makes an even split possible where neither side has a majority. Always use odd counts; if you want an extra copy for reads or geographic coverage, add a non-voting learner so it does not affect quorum arithmetic.

You need exactly one of five API replicas to run a nightly reconciliation job against your Postgres database. Simplest safe design?
- A. A 3-node etcd cluster with a lease and a watch.
- B. `pg_try_advisory_lock` in the same Postgres database, with the job doing idempotent upserts. **(answer)**
- C. A Redis `SETNX` lock with a 24-hour TTL.
- D. Have all five run it and deduplicate afterwards.
...
In a last-write-wins store, node A's clock is 250 ms ahead of node B's. What is the practical consequence?
- A. Writes are delayed by 250 ms.
- B. A write made on A before a later write on B can win, silently discarding the newer data. **(answer)**
- C. The cluster refuses writes until NTP resynchronises.
- D. Only read latency is affected.

  LWW compares timestamps generated on different machines, so clock skew directly reorders causally later writes behind earlier ones. There is no conflict recorded and no error surfaced -- the losing write simply never existed. This is why version vectors (detect concurrency, surface siblings) or hybrid logical clocks (preserve causality) exist, and why Spanner needs atomic clocks plus a deliberate commit-wait to offer real external consistency.

<details><summary>Related topics and how they connect</summary>

Leader election and fencing are what make the failover story in **Replication & Consistency Models** safe. The shard
...
- **Why does a majority quorum make Raft safe?** — Any two majorities of the same cluster share at least one node, and a voter only supports a candidate whose log is at least as current as its own. So any future leader has spoken to a node that saw every committed entry -- a committed entry can never be lost.
- **How many failures does an N-node Raft cluster tolerate?** — `floor((N-1)/2)`. Three nodes tolerate one, five tolerate two, and four still tolerate only one -- even counts add cost and an even-split risk without adding tolerance.
- **What is a fencing token and why is it mandatory?** — A monotonically increasing number issued with each lease, carried on every write, and compared by the *resource*, which rejects anything lower than the highest it has seen. It is the only defence against a holder that was paused long enough for its lease to expire.
- **Why is Redlock contested?** — It depends on bounded clock drift and bounded process pauses for safety, and it issues no fencing token, so a paused client can write after losing the lock. It is reasonable for efficiency locks (avoid duplicate work) and insufficient for correctness locks (prevent corruption).
- **When should you use an idempotency key instead of a distributed lock?** — Whenever you can make the operation safe to repeat. A unique constraint on a client-supplied key in the database that already owns the data is cheaper, has no extra failure domain, and stays correct under arbitrary retries and pauses.
- **Monotonic clock vs wall clock -- when to use each?** — Monotonic (`CLOCK_MONOTONIC`) for all durations, timeouts and lease self-checks, because it never jumps. Wall clock (`CLOCK_REALTIME`) only for display and audit timestamps, because NTP can step it forwards or backwards.
- **What does a Hybrid Logical Clock give you over a Lamport clock?** — The same causal-ordering guarantee, but with values that stay close to physical time, so timestamps are meaningful to a human reading a log and comparable to wall-clock events. Used by CockroachDB and YugabyteDB.
- **What does Spanner do about clock uncertainty?** — It measures it rather than ignoring it: TrueTime returns an interval with bounded error (~1-7 ms), and a commit deliberately waits out that interval before acknowledging, which is what buys external consistency. The cost is GPS and atomic clocks in every datacentre.
- **Why keep etcd off the user request path?** — A Raft cluster stops accepting writes when it cannot reach a majority, so its availability is deliberately lower than a single node's. Kubernetes puts it behind the API server so the control plane can be down while the data plane keeps serving traffic.
...
A team proposes storing per-user session state in a 5-node etcd cluster because "it is strongly consistent and we already run it for Kubernetes". Traffic is 12,000 requests per second, each needing a session read and occasionally a write. Evaluate.
