...
> Kafka partitions, delivery semantics, ordering guarantees, consumer lag and DLQs.

- Track: **Backend & Distributed Systems** · Level: **core** · ~21 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/queues-and-streaming)

A queue or a stream sits between a producer and a consumer so that the producer does not have to wait for the
...
Consider a checkout endpoint that must charge a card, send a receipt, update inventory, notify the warehouse, and
...
## Queue vs log -- the decision that shapes everything else
...
*In a queue the broker tracks which messages are outstanding. In a log the consumer tracks its own offset, so replay is free and a second consumer costs nothing.*

That difference cascades into almost every operational property.
...
**Brokers, honestly compared**

|  | Kafka | RabbitMQ | SQS (standard) | SNS + SQS | Google Pub/Sub | NATS JetStream |
| --- | --- | --- | --- | --- | --- | --- |
...
> **SQS FIFO deserves its own line**  
> Standard SQS has *no* ordering and *at-least-once* delivery, which surprises people who assume "queue" implies
...
## Kafka mechanics you are expected to know

A **topic** is a named stream, split into **partitions**. Each partition is an append-only file with a monotonically
...
**The Kafka configuration that determines whether you lose data**

```properties
# --- Producer: durable, ordered, deduplicated at the broker ---
...
> **The two settings that cause real data loss**  
> `unclean.leader.election.enable=true` allows a replica that was *not* in the ISR to become leader when all ISR
...
## Ordering is per partition, and that is the entire story

Kafka guarantees that messages within a partition are delivered in offset order. It guarantees *nothing* about order
...
**Partition-count arithmetic**

- **= partitions** — Max consumers per group (Extra consumers idle; this is your parallelism ceiling)
- **~10 MB/s** — Reasonable per-partition throughput (Plan partitions from target throughput, not vibes)
- **2-3x** — Headroom multiplier on partition count (Increasing partitions later breaks key-to-partition mapping)
- **~4,000** — Practical partition ceiling per broker (Each partition is open file handles plus controller metadata)

> **Warning**  
> You can add partitions to a topic but you can never remove them, and adding them **changes the key-to-partition
...
## Delivery semantics, without the marketing

| Semantic | Mechanism | What you risk | When it is correct |
| --- | --- | --- | --- |
...
Kafka's transactions are real and they do what they claim, but the claim is narrower than the phrase "exactly once"
...
**Effectively-once consumer -- the dedupe and the effect in one transaction**
...
Note the ordering: the effect and the dedupe record commit together, and the offset commits *after*. Any crash point
...
## Consumer lag: the one metric that matters

Lag is the difference between the partition's log-end offset and the consumer group's committed offset -- how many
...
**Drain-time maths worth doing out loud**

- **4M msgs** — Backlog after a 20-min outage (At a 3,300/sec produce rate)
- **1.7x** — Consumer capacity needed to catch up (Drain in 30 min while still taking new traffic)
- **= partitions** — Hard ceiling on catch-up scaling (You cannot add consumers past this)
- **~15 min** — Typical time-lag alert threshold (Tune to what downstream staleness actually breaks)

## Poison messages, DLQs and head-of-line blocking

A **poison message** is one your consumer cannot process -- malformed payload, a deserialisation failure against a
...
*Tiered retry topics keep the main partition moving. The replay path back into the source topic is the part teams forget to build.*

A dead-letter queue is only useful if you can get messages *out* of it, and this is where most implementations stop.
...
## Rebalance storms and backpressure

When a consumer joins or leaves a group, partitions are reassigned. The classic "eager" protocol does this with a
...
Kafka offers two cleanup policies and they serve genuinely different purposes. `cleanup.policy=delete` with
...
| --- | --- | --- |
| Poison message retried in place forever | One partition halts; lag climbs on 1 of 20 partitions while averaged dashboards look healthy. | Bounded attempts, then tiered retry topics and a DLQ. Alert on per-partition lag, never on the group average. |
| Consumer commits offsets before processing | Silent message loss on every crash or deploy; nobody notices until a reconciliation months later. | `enable.auto.commit=false`; commit only after the effect and its dedupe row have committed together. |
| Rebalance storm | Group spends more time rebalancing than consuming; throughput collapses under a rolling restart. | `CooperativeStickyAssignor`, lower `max.poll.records`, raise `max.poll.interval.ms` to fit real batch time, and static `group.instance.id`. |
| Retention expires before a lagging consumer catches up | Permanent data loss plus an `OffsetOutOfRange` that silently skips to latest. | Alert on lag against the retention horizon -- not just on absolute lag -- and set `auto.offset.reset=none` so it fails loudly. |
| `acks=all` with ISR shrunk to one replica | Acknowledged writes lost when that single broker dies, with no error to the producer. | `min.insync.replicas=2` and `unclean.leader.election.enable=false`; alert when ISR size drops below the configured minimum. |
| Hot partition from a skewed key | One consumer saturated while the rest idle; adding consumers changes nothing. | Composite key with a bucket suffix where per-key order can be relaxed, or route the whale tenant to its own topic. |
| DLQ with no replay tooling or alert | 40,000 orders sitting in a topic nobody watches; a day of revenue silently dropped. | Alert on DLQ arrival rate, retain original key and headers, and build the replay tool before you need it at 3 a.m. |
...
Your consumer group has 4 members reading a topic with 3 partitions. Lag keeps growing. What happens if you scale to 8 consumers?
- A. Throughput roughly doubles.
- B. Nothing -- 3 consumers work and 5 sit idle, because a partition is assigned to at most one consumer per group. **(answer)**
- C. Kafka splits partitions automatically to match.
- D. The group rebalances into a round-robin across all 8.

  Partition count is a hard parallelism ceiling per consumer group: each partition goes to exactly one member, so members beyond the partition count consume nothing. Your options are to add partitions (which changes the key-to-partition mapping for new messages and can break per-key ordering across the boundary), make each message cheaper to process, or move to an internal work-distribution pattern where one consumer fans a partition out to a local worker pool -- at the cost of losing ordering.

A consumer does an HTTP call to a payment provider, then commits the offset. It crashes after the call but before the commit. What happens and what is the correct design?
- A. Kafka rolls back the HTTP call.
- B. The message replays and the payment is duplicated unless the consumer is idempotent, typically via a dedupe key committed with the effect. **(answer)**
- C. The message is lost.
- D. Enabling `enable.idempotence=true` on the producer prevents it.

  This is at-least-once delivery working exactly as designed, and no broker feature can undo an external side effect. Producer idempotence deduplicates *producer retries* at the broker; it says nothing about consumer replays. The fix is a stable message id recorded in your own database in the same transaction as the effect -- or an idempotency key passed to the payment provider so the duplicate call returns the original charge. Kafka transactions do not help because the side effect is outside Kafka.

One of 20 partitions has a message that throws on every attempt. Your consumer retries it indefinitely. What does the monitoring show?
- A. Nothing unusual -- the other 19 partitions compensate.
- B. Total group lag rises slowly while one partition stalls completely, so an averaged dashboard looks close to normal. **(answer)**
- C. The broker automatically moves the message to a DLQ.
- D. The consumer group rebalances to route around it.

  Retrying in place halts that partition entirely -- classic head-of-line blocking -- and 5% of your keys stop being processed. Group-level or averaged lag dilutes it by a factor of 20, which is why per-partition lag is the metric you alert on. The fix is bounded attempts, then a tiered retry topic or a DLQ, so the partition keeps moving and the problem message is preserved with enough context to replay.

What does `cleanup.policy=compact` give you that `delete` does not?
- A. Better compression of message bodies.
- B. The topic retains the latest value per key indefinitely, so it is both a change stream and a full current-state snapshot. **(answer)**
- C. Automatic deduplication of identical messages.
- D. Lower disk usage for all workloads.

  Compaction garbage-collects superseded values per key while keeping the newest one forever. That makes the topic self-bootstrapping: a brand-new consumer can read from the beginning and arrive at correct current state without a separate backfill, which is why Connect offsets, KRaft metadata and CDC topics use it. A null value is a tombstone marking deletion, retained for `delete.retention.ms` so consumers get a chance to observe it.

<details><summary>Related topics and how they connect</summary>

The dual-write problem that makes you reach for a broker in the first place, and the outbox pattern that fixes it,
...
- **Queue vs log, in one sentence each?** — A queue has the broker track per-message delivery state and the message disappears on ack, enabling priority, delay and unbounded consumer scaling. A log is an immutable append-only sequence where each consumer group stores only an offset, making replay free and extra consumer groups cost nothing.
- **Why is `acks=all` insufficient for durability?** — It waits for all *in-sync* replicas, and the ISR can shrink to just the leader. Then one acknowledgement equals one copy. You also need `min.insync.replicas=2` so the broker rejects the write, plus `unclean.leader.election.enable=false`.
- **What is the maximum useful parallelism of a Kafka consumer group?** — The partition count. Each partition is assigned to exactly one consumer in the group, so additional consumers idle. Partition count is therefore a capacity decision made at topic creation, and adding partitions later changes key-to-partition mapping.
- **Why is consumer-side idempotency required even with Kafka transactions?** — Kafka transactions atomically commit writes to Kafka topics plus the offsets. Any side effect outside Kafka -- an HTTP call, a Postgres row, an email -- is not covered, and there is no two-phase commit with an external system. So a dedupe store keyed on message identity is the actual mechanism.
- **Correct order of operations in an at-least-once consumer?** — Write the effect and the dedupe row in one database transaction, then commit the Kafka offset. Any crash leaves either a harmless replay or a no-op. Committing the offset first silently builds at-most-once.
- **Why alert on the slope of consumer lag rather than its absolute value?** — A lag spike that drains on its own is the queue absorbing a burst -- exactly its purpose -- and paging for it trains people to ignore the alert. Sustained growth means consumer throughput is below produce rate, so the backlog is unbounded and will eventually hit retention.
- **What is head-of-line blocking in a partition?** — Retrying a failing message in place halts that partition, so every message behind it waits. With 20 partitions, 5% of keys stop being processed while group-averaged lag looks nearly normal. Fix with bounded attempts, tiered retry topics and a DLQ, and alert per partition.
- **What does log compaction give you?** — The latest value per key retained indefinitely, with older values garbage-collected. The topic is simultaneously a change stream and a current-state snapshot, so new consumers bootstrap by reading from the beginning. Null values are tombstones marking deletion.
- **What causes a rebalance storm and how do you stop it?** — Slow processing makes a member miss `max.poll.interval.ms`, triggering a rebalance that pauses the group and makes everyone slower. Fix with `CooperativeStickyAssignor`, a lower `max.poll.records`, an interval that fits real batch time, and static `group.instance.id`.
...
A team wants to replace their SQS-based job queue with Kafka because "Kafka is the standard". The workload is 400 messages/sec, one consumer service, jobs take 1-90 seconds, some jobs must run 10 minutes after enqueue, and priority matters for paid customers. Evaluate the proposal.
