...
> Shard keys, consistent hashing, rebalancing, hot partitions and cross-shard queries.

- Track: **Backend & Distributed Systems** · Level: **staff** · ~21 min
- [Open in the academy](https://deepeshk1204.github.io/staff-engineer-academy/#/topic/backend/sharding-and-partitioning)

Sharding is splitting one logical dataset across several independent databases so that no single machine has to hold
...
A single Postgres instance on modern hardware is far more capable than folklore suggests. A 64-core box with 512 GB
...
**Rough thresholds where people actually shard**

- **~10 TB** — Single-instance data size (Backup/restore time becomes the pain, not queries)
- **~20-50k** — Sustained writes/sec on one leader (Hardware-dependent; measure, do not guess)
- **>1 hr** — Time to restore from backup (A recovery-time problem forces splitting)
- **2-4x** — Engineering cost multiplier (Every feature now considers shard locality)

> **Cheaper things to try first, in order**  
> Add the missing index. Move the analytics queries off the primary onto a replica or a warehouse. Move append-only
...
Two different axes get called "partitioning" and conflating them causes confusion in interviews.
...
## How rows get assigned to shards
...
*The routing layer can be a proxy (Vitess, Citus coordinator), a library in the app, or a shard map the app reads from etcd.*

**The three assignment strategies**

| Strategy | How it maps | Range scans | Hot-spot risk | Rebalancing | Real users |
| --- | --- | --- | --- | --- | --- |
...
For B2B SaaS the directory approach is usually right and underrated. It is a small table, it caches trivially, and
...
## Why plain modulo is a trap, and what consistent hashing fixes

Suppose you shard with `shard = hash(user_id) mod 4` and you add a fifth shard. A key now maps to `hash mod 5`.
...
The naive ring has a second problem: with only 4 random points on a ring, the arcs are wildly unequal. The standard
...
**Consistent hash ring with virtual nodes -- the whole idea in 20 lines**

```js
// Ring is a sorted array of { pos, shard }. Built once, rebuilt on topology change.
...
> Most mature systems do not hash directly onto shards. They hash onto a fixed, large number of **logical partitions**
...
## Shard key selection: the one irreversible decision

A shard key is good when it satisfies four properties at once, and they fight each other. It must have **high
...
**Shard key candidates, honestly assessed**

| Key | Works because | Breaks when |
| --- | --- | --- |
...
## Hot and celebrity partitions

Uniform key distribution does not give you uniform *traffic*. When a celebrity with 40 million followers posts, or
...
- **Key salting / write sharding** — Append a bucket suffix: `post:9931#0` through `post:9931#15`. Writes spread across 16 keys on up to 16 shards. Reads must now fan out to all 16 and merge, so this trades write hot-spotting for read amplification. Use it for counters and append-heavy data.
- **Read-path caching** — A celebrity row is read millions of times and written rarely, which is the ideal cache shape. Redis in front of the shard with request collapsing turns a hot partition into a hot cache key, which is a much easier problem to throw replicas at.
- **Dedicated shard placement** — Only possible with a directory scheme. Move the whale tenant to its own cluster and record it in the shard map. This is how multi-tenant SaaS survives its largest customer, and it is the strongest argument for lookup-based sharding.
- **Adaptive splitting** — DynamoDB and Bigtable split a hot partition automatically on access patterns, not just size. You get this for free from the managed store, but only when the heat is spread across a key *range*, not concentrated on one key.

## What you lose: cross-shard operations

The moment data spans shards you lose three things the database used to give you free.
...
## Scatter-gather and tail latency amplification

A query without the shard key must ask every shard. This is the single most underappreciated cost of sharding,
...
**Fan-out tail maths, per-shard p99 = 10 ms**

- **10 ms** — p99 with 1 shard (Baseline)
- **~60%** — Under 10 ms with 50 shards (0.99^50 -- so p99 is far worse)
- **~37%** — Under 10 ms with 100 shards (Fan-out width is the enemy)
- **2 of 3** — Hedged request threshold (Send a duplicate after p95 elapses)

Mitigations: keep the fan-out narrow by including the shard key in hot queries; push a **secondary index into a
...
|  | Local index (per shard) | Global index (separately sharded) |
| --- | --- | --- |
...
You will get the shard count wrong, so the migration procedure matters more than the initial layout. The safe
pattern is the same one used for schema changes: never flip, always overlap.

**Splitting shard 3 into 3a and 3b with zero downtime**

1. Provision the new shards and put the routing layer behind a **shard map** it reads at runtime, so topology changes do not require a deploy.
2. Turn on **double-writes**: every write to shard 3 is also applied to 3a/3b according to the new mapping. Make these writes idempotent -- use the row version or an upsert -- because you will replay them.
3. Run a **backfill** that copies historical rows in primary-key order, in bounded batches, throttled to a fixed fraction of shard capacity. Record a high-water mark so it is resumable.
4. Run a **continuous verifier** comparing checksums of row ranges between old and new. Do not proceed while the diff is non-zero for anything other than in-flight rows.
5. Shift **reads** first, gradually: 1% of traffic, then 10%, then 50%, comparing results against the old shard in shadow mode. Reads are reversible; writes are not.
6. Flip the shard map so writes go only to 3a/3b. Keep double-writing *back* to shard 3 for a cooling-off period so rollback stays available.
7. Stop the backwards write, then leave shard 3 read-only and untouched for at least one backup cycle before decommissioning.

> **Note**  
> Vitess automates most of this as `MoveTables`/`Reshard` with `VDiff` for verification, and it is worth knowing
...
| --- | --- | --- |
| Monotonic shard key (timestamp or auto-increment ID) | 100% of writes land on the newest shard; the rest of the fleet idles while one machine saturates. | Hash the key, or prefix with a high-cardinality bucket. Reserve range-on-time for read-mostly archival tables. |
| Celebrity key exceeds one partition ceiling | Throttling and timeouts on one key while the table shows 5% utilisation. | Cache with request collapsing, salt writes into N sub-keys, or pin the key to a dedicated shard. |
| Uneven tenant sizes on hash sharding | One shard at 90% disk and CPU, others at 15%; you cannot fix it without moving the tenant. | Switch to a directory/lookup map so placement is explicit and a whale can be relocated by changing one row. |
| Stale shard map cached in an app instance | Writes land on the old shard after a reshard; rows silently disappear from reads. | Version the map, fence writes with the version, and reject writes carrying a stale version rather than accepting them. |
| Cross-shard transaction abandoned mid-flight | Money debited on shard A, never credited on shard B; no error anywhere. | Saga with an explicit compensating action, an outbox for durability, and a reconciliation job that alerts on non-zero drift. |
| Fan-out query added to a hot path | p99 collapses as shard count grows -- the system gets *slower* when you scale out. | Budget fan-out width per endpoint, add a global index or search store, use hedged requests and deadline-bounded partial results. |
| Backfill unthrottled | Copy job saturates shard IO and takes production latency with it during the migration. | Bounded batches keyed on the PK, explicit rate limit tied to a replication-lag or latency signal, and a kill switch. |
...
You shard with `hash(user_id) mod 4` and need to add a fifth shard. Roughly how much data must move?
- A. About 20% -- one shard's worth
- B. About 80% **(answer)**
- C. None -- hashing is stable
- D. About 50%

  A key stays put only when `h mod 4 == h mod 5`, which is roughly 1 key in 5, so about 80% relocates. This is precisely the problem consistent hashing solves: adding one node to a ring of N moves only `1/(N+1)` of keys, about 20% here, and only from adjacent arcs. Better still, hash into a fixed large partition count and move whole partitions.

Each shard answers in under 10 ms 99% of the time. A dashboard query fans out to 50 shards. What is the practical effect?
- A. p99 stays 10 ms because the calls are parallel.
- B. Only ~60% of these queries finish within 10 ms, so the tail degrades sharply. **(answer)**
- C. Throughput drops but latency is unaffected.
- D. The coordinator caches results so it is faster after warm-up.

  The fan-out is only as fast as its slowest leg, so you need all 50 to be fast: 0.99^50 = 0.605. Parallelism bounds the *sum* of work, not the *maximum* latency. The effective p99 of the fan-out sits around the p99.98 of a single shard. Fix it with narrower fan-out, a global secondary index, hedged requests, or deadline-bounded partial results.

A B2B analytics product has 4,000 tenants; the largest is 35% of total load. Which sharding scheme fits best?
- A. Range sharding on `created_at`
- B. Hash sharding on `tenant_id`
- C. A directory/lookup map keyed by `tenant_id` **(answer)**
- D. No shard key -- replicate everything everywhere

  Hash on `tenant_id` gives good average distribution but no control: the 35% tenant lands wherever the hash puts it and will permanently overload that shard with no remedy short of resharding. A lookup table is a tiny, cacheable indirection that lets you pin that tenant to dedicated hardware by updating one row. Explicit placement is the whole reason multi-tenant SaaS prefers directory sharding.

Users log in by email, but the table is sharded on `user_id`. What is the right fix?
- A. Scatter-gather the login query across all shards.
- B. Add a local secondary index on `email` to every shard.
- C. Maintain a global `email -> user_id` lookup index, sharded on `email`. **(answer)**
- D. Reshard the whole table on `email`.

  Login is high-volume and never carries the shard key, so scatter-gather puts your most critical path on the widest fan-out you have. A local index does not help -- you still must ask every shard which one holds the email. A global lookup index (Vitess calls this a lookup vindex, DynamoDB a GSI) turns it into two single-shard reads. Resharding on `email` would just break every other query.

<details><summary>Related topics and how they connect</summary>

Sharding multiplies the number of independent replication topologies you operate, so read **Replication &
...
- **Why does adding one shard to a `mod N` scheme move most of the data?** — A key only stays put when `h mod N == h mod (N+1)`, true for roughly 1 in N+1 keys. Going 4 to 5 shards relocates ~80% of rows. Consistent hashing moves only `1/(N+1)`, about 20%, and only from neighbouring arcs.
- **What problem do virtual nodes solve?** — With one ring position per shard, random arcs are very uneven and one shard can own double its share. Giving each shard 128-256 positions averages the arcs out, and on failure the dead shard's load spreads across many neighbours instead of one.
- **What are the four properties of a good shard key?** — High cardinality, uniform access (no hot values), present in most queries, and aligned with your transaction boundary so atomic writes stay on one shard.
- **Why is a fan-out query worse at p99 than a single-shard query?** — It completes only when the slowest shard responds. With per-shard p99 of 10 ms and 50 shards, the chance all are fast is 0.99^50 = ~60%, so the fan-out p99 approaches a single shard's p99.98. Widening the fleet makes it worse.
- **Local vs global secondary index?** — A local index covers only the rows on its own shard, so a lookup without the shard key must scatter-gather. A global index is its own table sharded on the index key -- one lookup, but the write is cross-shard and therefore eventually consistent.
- **How do you handle a celebrity key that exceeds one partition's throughput?** — Cache reads with request collapsing; salt writes across N sub-keys and merge on read; or, with a directory scheme, pin the key to a dedicated shard. Hashing does not help -- one key is one key.
- **Safe order of operations for a live reshard?** — Runtime shard map, then idempotent double-writes, throttled resumable backfill, checksum verification, gradual read shift (1% -> 10% -> 50%), then flip writes while still writing back to the old shard so rollback remains possible.
- **What should you try before sharding?** — Missing indexes, moving analytics to a replica or warehouse, functionally partitioning the largest append-only table into its own store, archiving cold rows, caching with collapsing, and a bigger instance. Sharding is a 2-4x engineering cost multiplier.

## Drills

### Drill

A B2B workflow product on a single Postgres primary has 8 TB of data, 18,000 writes per second at peak, and 6,000 tenants. The largest tenant is 30% of write volume; the median tenant is under 1 GB. p99 on the hot `tasks` table has gone from 12 ms to 140 ms over six months. Design the path forward.
...
Your DynamoDB table is partitioned on `post_id`. A post from a large account is being read 80,000 times per second and receiving 4,000 likes per second. The table shows 6% provisioned-capacity utilisation, yet these requests are being throttled. Explain and fix.
