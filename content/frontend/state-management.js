export default {
  blocks: [
    {
      t: 'prose',
      md: `Most arguments about state management are arguments about the wrong thing. The question
...
at different times.`
    ...
    {
      t: 'prose',
      md: `Early client apps had one kind of state and it was simple: some variables in a closure.
...
client state to begin with.`
  ...
  C --> D`,
      caption: 'Two questions resolve most of it. "Can something else change this?" separates server state from client state; "should a shared link reproduce it?" separates URL state from everything else.'
    },
    {
      t: 'prose',
      md: `The first question is the load-bearing one, and it is worth stating as a test you can
...
belong in the URL.`
    ...
    {
      t: 'table',
      title: 'The taxonomy, with the actual decision',
      cols: ['Category', 'Examples', 'Lives in', 'Invalidated by', 'The failure if you misplace it'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'danger',
      title: 'Derived state is the bug factory',
      md: `Storing \`filteredItems\` next to \`items\` and \`filter\` means you now have two facts that
...
selected" boolean. If you can calculate it, do not store it.`
    ...
    { t: 'h', text: 'Server-state libraries are caches, so treat them like caches' },
    {
      t: 'prose',
      md: `Once you accept that React Query is a cache, its API stops being a list of options and
...
lets you invalidate at the right level rather than choosing between too much and too little.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'Optimistic update with a correct rollback',
...
//   ['orders', 'detail', id]        -> one entity`
    ...
    {
      t: 'prose',
      md: `Three things about optimistic updates that separate a real implementation from a demo.
...
the user would make a different decision if they knew it failed.`
    ...
    { t: 'h', text: 'URL as state' },
    {
      t: 'prose',
      md: `The URL is a state container with properties no store can match. It survives a refresh,
...
malformed URL is a bad look in a shared link.`
    ...
    { t: 'h', text: 'Why global stores rot, and normalisation' },
    {
      t: 'prose',
      md: `A global store degrades for a reason that is structural rather than cultural. It is the
...
read-only widgets; you will pay real complexity for a consistency problem you do not have.`
    ...
    {
      t: 'table',
      title: 'Form state: three approaches',
      cols: ['Approach', 'Rerenders on keystroke', 'Best for', 'Watch out for'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Server data in a form is a merge problem, not a fetch problem',
      md: `An edit form initialised from server data is a fork. The moment the user types, the
...
last-write-wins system on top of a form, and the user who loses their edit gets no error at all.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Server data mirrored into a global store', blast: 'Hand-written loading flags per endpoint, stale data after mutations, and two components disagreeing about the same entity.', fix: 'Move it to a query cache with explicit `staleTime` and invalidation on mutation. Apply the "can another tab change this?" test in code review.' },
        { mode: 'Filters and tabs in `useState`', blast: 'Shared links do not reproduce the view; back after clicking a row loses the filters; refresh discards the user\'s work.', fix: 'Move to `searchParams` with short keys, omitted defaults, `replaceState` for high-frequency changes, and validation with fallbacks.' },
        { mode: 'Optimistic update with no snapshot', blast: 'A failed mutation leaves the UI showing a change that never happened, permanently, with no error.', fix: 'Snapshot in `onMutate`, restore in `onError`, invalidate in `onSettled`, and tell the user the change was undone.' },
        { mode: 'Auth token in `localStorage`', blast: 'Any XSS -- including from a compromised transitive dependency -- exfiltrates a valid session token; logout does not revoke it.', fix: '`HttpOnly`, `Secure`, `SameSite` cookie set by the BFF; keep the refresh token server-side entirely.' },
        { mode: 'Refetch on window refocus while a form is dirty', blast: 'The user returns to the tab and their half-typed input silently reverts to server values.', fix: 'Disable refocus refetching while dirty, and use `If-Match` on submit so a concurrent change returns `412` instead of a silent overwrite.' },
        { mode: 'Same entity stored under three unnormalised keys', blast: 'Editing from the detail view leaves the list and search results showing the old value; users report the app "not saving".', fix: 'Normalise by entity ID, or design a query-key hierarchy and invalidate at the right prefix on every mutation.' },
        { mode: 'Derived value stored alongside its inputs', blast: 'Filtered list disagrees with the filter; a total disagrees with the line items. No error, just wrong numbers.', fix: 'Compute at render. Use `useMemo` only for measured cost, since it caches without creating a second source of truth.' },
        { mode: 'Form draft in `localStorage` without a user namespace', blast: 'On a shared device, user B opens the form and sees user A\'s draft content.', fix: 'Namespace the key by user ID, clear on submit and on logout, and set an expiry.' }
      ]
    },

    {
      t: 'staff',
      md: `The signal is answering a state question by categorising before naming a tool, and being
...
user-visible failure that the wrong placement produces.`
...

    {
      ...
      items: [
        {
          q: 'A table\'s filters, sort and page number are held in `useState`. A user filters to "overdue", opens a row, presses back, and the table is unfiltered. What is the correct fix?',
          options: [
            'Persist the filter state to `localStorage` on change and restore it on mount.',
            'Move filter, sort and page into URL search params so navigation, sharing and refresh all reproduce the view.',
            'Lift the state to a global store so it survives the route change.',
            'Render the detail view in a modal so the table never unmounts.'
          ],
          answer: 1,
          why: 'The browser already has a state container with exactly the semantics needed: history integration, shareability and refresh survival. Putting the filters in `searchParams` makes back restore them for free and makes the URL sendable to a colleague, which is almost always a real product requirement for a filtered report. `localStorage` restores state but breaks sharing and produces the surprising behaviour of a filter persisting across sessions with no visible cause. A global store survives the route change but still loses everything on refresh and is not shareable. The modal avoids the symptom while leaving the URL unable to describe the view.'
        },
        {
          q: 'A `useMutation` writes optimistically in `onMutate` but does not snapshot. The request fails with a `500`. What does the user see?',
          options: [
            'React Query automatically reverts the cache to its previous value.',
            'The optimistic change stays in the cache indefinitely, so the UI shows a change that never happened.',
            'The query refetches automatically and corrects itself.',
            'The component throws and the error boundary catches it.'
          ],
          answer: 1,
          why: 'The cache holds whatever you last wrote to it. Without a snapshot in `onMutate` there is nothing to restore in `onError`, so the optimistic value persists until something else invalidates that key -- which may be a refocus, a remount, or never within the session. The user believes their edit saved. The complete pattern is three-part: snapshot and return it from `onMutate`, restore it in `onError`, and invalidate in `onSettled` so the server reconciles the value regardless of outcome. And the rollback must be user-visible, because a change that silently vanishes is indistinguishable from one that applied.'
        },
        {
          q: 'Your app stores a JWT in `localStorage` and attaches it as a bearer token. A dependency three levels deep in the tree is compromised. What is the impact, and what changes it?',
          options: [
            'Nothing -- the token is scoped to your origin, so other code cannot read it.',
            'Any script on the page can read the token and exfiltrate it; moving to an `HttpOnly` cookie makes it unreadable to JavaScript.',
            'The token is safe as long as it expires within an hour.',
            'Content Security Policy prevents any script from reading `localStorage`.'
          ],
          answer: 1,
          why: '`localStorage` is origin-scoped, not script-scoped: every script running on your origin -- including every transitive dependency and every injected analytics tag -- has full read access. Exfiltration is one `fetch` call, and because a bearer token is valid until expiry there is often no revocation path. An `HttpOnly` cookie set by your BFF is attached by the browser automatically and is unreadable from JavaScript, so the same XSS can still act *as* the user in-page but cannot steal a credential to replay elsewhere. The cost is that you now need CSRF protection, which `SameSite=Lax` plus a token pattern covers, and that is a much better trade. A short expiry narrows the window but does not change the exposure.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The query cache is layers 1 and 2 of **The Caching Stack**, and every lesson about
...
storage is **Frontend Security**.`
  ...
  drills: [
    ...
    {
      prompt: 'Design the state model for a Kanban board: 8 columns, up to 400 cards, drag and drop between columns, multiple users editing concurrently with live updates over a websocket, filters by assignee and label, and a card detail panel. Be specific about where each piece lives.',
      probes: [
