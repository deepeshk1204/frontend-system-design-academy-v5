export default {
  blocks: [
    {
      t: 'prose',
      md: `A browser turns a stream of HTML bytes into coloured pixels through a fixed pipeline:
...
\`for\` loop and a scroll animation are competing for the same CPU.`
    ...
    {
      t: 'prose',
      md: `HTML and CSS are declarative: you describe what you want, not how to draw it. That is
...
changed by the compositor without consulting the main thread at all.** Everything else cannot.`
  ...
  X --> Px["Pixels"]`,
      caption: 'The full pipeline. Changing a geometric property re-runs from Layout; changing a colour re-runs from Paint; changing transform or opacity re-runs only Composite.'
    },
    {
      t: 'steps',
      ordered: true,
      title: 'The stages, and what each actually produces',
      items: [
        '**Parse to DOM.** Tokenise HTML into a tree of nodes. The parser is incremental -- it works on partial bytes -- but a synchronous \`<script>\` blocks it dead, because that script could call \`document.write\`.',
        '**Parse to CSSOM.** Stylesheets are **render-blocking**: the browser will not paint until it has them, because painting with the wrong styles then correcting would flash. A stylesheet in the head is also a blocker for any script after it, since scripts can read computed style.',
        '**Build the render tree.** DOM plus CSSOM, minus anything not rendered. Note the distinction: \`display: none\` removes a node from the render tree entirely, while \`visibility: hidden\` keeps it -- it still occupies layout space.',
        '**Layout (reflow).** Compute the exact box geometry for every node in the render tree. This is the expensive stage and its cost scales with node count.',
        '**Paint.** Turn boxes into ordered draw commands -- fill this rect, draw this text, apply this shadow -- grouped into paint records per layer.',
        '**Raster and composite.** Rasterise layers into tiles, upload as GPU textures, and have the compositor thread assemble the final frame. The compositor can re-assemble with new transforms without any main-thread work.'
      ]
    },

    { t: 'h', text: 'Reflow vs repaint vs compositor-only' },
    {
      t: 'prose',
      md: `This is the single most practically useful distinction in frontend performance, and it
...
which is why \`filter: blur()\` animates acceptably but animating \`box-shadow\` does not.`
    ...
    {
      t: 'table',
      title: 'What your CSS change actually costs',
      cols: ['Property you change', 'Layout', 'Paint', 'Composite', 'Safe to animate at 60 fps?'],
      rows: [
    ...
    { t: 'h', text: 'Layout thrashing, concretely' },
    {
      t: 'prose',
      md: `The browser batches style changes and defers layout until it is actually needed. That
...
\`scrollWidth\`. Reading them is not itself slow -- reading them *after a write* is.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'The same work, 100x apart',
      code: `// SLOW: read-write-read-write. Each offsetWidth read forces layout
...
new ResizeObserver(entries => {
  for (const e of entries) {
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'It hides inside libraries',
      md: `You rarely write the slow loop yourself. You write a React component whose \`useLayoutEffect\`
...
\`ResizeObserver\`.`
    ...
    { t: 'h', text: 'The frame budget' },
    {
      t: 'prose',
      md: `A 60 Hz display gives you 16.7 ms per frame, and the browser needs a few of those
...
from user input to the next rendered frame, is largely a long-task metric in disguise.`
    },
    {
      ...
      items: [
        { v: '16.7 ms', k: 'One frame at 60 Hz', note: '~10-12 ms usable by you' },
        { v: '8.3 ms', k: 'One frame at 120 Hz', note: 'Modern flagship phones' },
        { v: '50 ms', k: 'Long-task threshold', note: 'Anything above blocks input' },
        { v: '200 ms', k: 'INP "good" at p75', note: '500 ms is "poor"' },
        { v: '~5x', k: 'Mid-tier Android vs laptop CPU', note: 'Your 10 ms task is their 50 ms' }
      ]
    },
    {
      t: 'diagram',
  ...
  W -.->|postMessage| JS
  Cm --> Scr["Screen"]`,
      caption: 'Only the compositor thread can produce a frame while the main thread is blocked -- and only for transform and opacity changes.'
    },

    { t: 'h', text: 'Yielding, and the tools that help' },
    {
      t: 'prose',
      md: `Since tasks cannot be preempted, the only way to stay responsive during heavy work is to
...
actually animating.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'Yielding, and correct will-change usage',
      code: `// Break a 400 ms parse into input-responsive chunks.
...
list.addEventListener('touchstart', onTouch, { passive: true });`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Animating `top`/`left` or `width` in a `requestAnimationFrame` loop', blast: 'Full layout+paint every frame; scroll drops to 20 fps on mid-tier Android.', fix: 'Animate `transform` and `opacity` only. Verify in the Performance panel that frames show no purple Layout bars.' },
        { mode: 'Layout thrashing in a component effect', blast: 'One 200-400 ms long task on mount; INP breaches 500 ms on the first interaction.', fix: 'Separate read and write phases, hoist measurement to a parent, or replace with `ResizeObserver`/`IntersectionObserver`.' },
        { mode: '`will-change: transform` on a shared utility class', blast: 'Hundreds of layers, GPU memory exhaustion, and *worse* jank than not using it; can crash tabs on low-memory devices.', fix: 'Set `will-change` imperatively before the animation and clear it on completion. Audit stylesheets for blanket declarations.' },
        { mode: 'Non-passive `touchmove` on a scroll container', blast: 'Scroll waits on the handler every frame; users report the page feeling "stuck to the finger".', fix: '`{ passive: true }` on any listener that never calls `preventDefault()`; use CSS `touch-action` to express intent declaratively.' },
        { mode: 'One 900 ms hydration or parse task', blast: 'Clicks during that window are queued, not dropped -- the UI responds a second later to a tap the user already repeated.', fix: 'Split by route, defer below-the-fold hydration, and chunk with `scheduler.yield()` so input can be serviced.' },
        { mode: 'Third-party script doing synchronous layout reads on every scroll', blast: 'Jank you cannot fix in your own code, attributed to your app in RUM.', fix: 'Load in a sandboxed iframe or worker where possible, budget third parties explicitly, and alert on long tasks attributed by script URL.' }
      ]
    },

    {
      t: 'staff',
      md: `The signal here is mechanical precision -- naming the stage, not the symptom. Sentences
...
the proposal rather than a detail.`
...

    {
      ...
      items: [
        {
          q: 'A modal fades in using `opacity`, and simultaneously a background list animates its `height`. The fade stays smooth while the list stutters. Why?',
          options: [
            'The fade is shorter, so fewer frames can be dropped.',
            '`opacity` on a promoted layer is handled by the compositor thread, while `height` invalidates layout and must run on the blocked main thread each frame.',
            '`height` animations are not GPU-accelerated because they use integers.',
            'The modal is in a separate stacking context, which bypasses paint.'
          ],
          answer: 1,
          why: 'These animations run on different threads. Once an element has its own compositor layer, `opacity` and `transform` changes are applied during compositing with no main-thread involvement, so they survive a busy main thread. `height` changes box geometry, so every frame re-runs style, layout, paint and raster on the main thread -- and if anything else is competing for it, frames get dropped. The fix for the list is to animate `transform: scaleY()` or animate a fixed-height wrapper, accepting the visual difference.'
        },
        {
          q: 'Which sequence forces the fewest synchronous layouts for 300 elements?',
          options: [
            'For each element: read `offsetHeight`, then write `style.height`.',
            'For each element: write `style.height`, then read `offsetHeight`.',
            'Read all `offsetHeight` values into an array, then write all heights.',
            'Wrap each read/write pair in `requestAnimationFrame`.'
          ],
          answer: 2,
          why: 'Reads are only expensive when pending writes must be flushed to answer them. Batching all reads first means one layout serves all 300 measurements; the subsequent writes are queued and resolved in a single later layout. Both interleaved orders force roughly 300 layouts. Wrapping each pair in `requestAnimationFrame` spreads the thrash across 300 frames -- it stops one giant long task but takes five seconds to finish and still does the same total layout work.'
        },
        {
          q: 'Why does a 600 ms long task hurt INP even when the user clicks *before* the task starts?',
          options: [
            'Input events are discarded while a task runs.',
            'The click is queued until the task completes, so the time from input to the next painted frame includes the remainder of the task.',
            'INP measures the longest task on the page, not interactions.',
            'Long tasks reset the interaction timer.'
          ],
          answer: 1,
          why: 'The main thread has no preemption, so the browser cannot dispatch the click handler until the running task yields. The event is queued, not dropped, and INP measures input to next paint -- which now includes queuing delay, handler time, and the rendering that follows. This is why the highest-leverage INP work is usually removing or chunking unrelated long tasks rather than optimising the handler. It also explains the "ghost click" pattern: a user taps, nothing happens, they tap again, and both taps fire when the thread frees up.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Long tasks and INP are measured and budgeted in **Performance Engineering**. The
...
**Web Foundations: URL to Pixels** and **CDN & Edge Delivery**.`
  ...
  flashcards: [
    { q: 'Which two CSS properties can be animated without main-thread work?', a: '`transform` and `opacity`, on an element that has its own compositor layer. The compositor thread applies them while assembling the frame, so they stay smooth even when JavaScript is blocking. `filter` is also GPU-accelerated but is fill-rate heavy.' },
    { q: 'What is a forced synchronous layout?', a: 'Reading a geometric property (`offsetWidth`, `getBoundingClientRect()`, `getComputedStyle()`, `scrollTop`) after a style write. The browser must flush pending changes and run layout immediately to give a correct answer. In a loop it becomes layout thrashing -- one layout per iteration.' },
    { q: 'Difference between `display: none` and `visibility: hidden` in the pipeline?', a: '`display: none` removes the node from the render tree, so it costs no layout or paint and occupies no space. `visibility: hidden` keeps it in the render tree -- it is laid out and takes up space, it is just not painted.' },
    { q: 'Why is 50 ms the long-task threshold?', a: 'The RAIL model puts 100 ms as the ceiling for an interaction to feel instantaneous. A task must stay under 50 ms so that even if input arrives just as it starts, the browser has ~50 ms left to dispatch and respond within budget.' },
    { q: 'What does `{ passive: true }` actually promise?', a: 'That your handler will not call `preventDefault()`. Without it the browser must run your `touchstart`/`wheel` handler before it knows whether to scroll, putting your JavaScript on the scroll critical path. With it, scrolling proceeds on the compositor immediately.' },
    { q: 'What does `content-visibility: auto` do and what does it break?', a: 'Skips style, layout and paint for off-screen elements until they approach the viewport. It breaks intrinsic sizing -- the element reports zero height -- so scroll height jumps and anchor links misfire unless you supply `contain-intrinsic-size`.' },
    { q: 'Why is `will-change: transform` in a stylesheet usually a mistake?', a: 'It permanently promotes every matching element to its own compositor layer, and each layer costs GPU memory proportional to its pixel area. On a list of hundreds of items this causes worse jank than it prevents. Set it in JS immediately before the animation and clear it on completion.' },
    { q: 'Why does `setTimeout(fn, 0)` yield worse than `scheduler.yield()`?', a: '`setTimeout` queues a new macrotask behind everything already pending, so your continuation can be starved. `scheduler.yield()` returns a prioritised continuation that resumes ahead of other pending tasks, giving you responsiveness without losing throughput.' },
    { q: 'What can a Web Worker not do, and what is the cost of using one?', a: 'No DOM access and no layout APIs -- it is for pure computation. The cost is structured-clone serialisation on `postMessage`, which is significant for large objects; transferable `ArrayBuffer`s avoid the copy where the data format allows.' }
  ],

  drills: [
    {
      prompt: 'Users report that your infinite-scroll feed "sticks to the finger" on mid-range Android but is fine on iPhone 15 and on desktop. Scrolling is smooth when the feed is empty. Each card has a drop shadow, a rounded avatar, and a lazily-loaded image. Diagnose it and tell me what you would change.',
      probes: [
    ...
    {
      prompt: 'A data-grid component takes 850 ms to mount with 500 rows, showing as a single long task. Each row measures its own content width in a layout effect to decide whether to truncate text. Product wants the truncation behaviour kept. What do you do?',
      probes: [
