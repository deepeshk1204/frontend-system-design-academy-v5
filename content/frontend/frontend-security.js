export default {
  blocks: [
    {
      t: 'prose',
      md: `Frontend security starts from one assumption: the code you ship runs on a machine
...
invited in.`
    ...
    { t: 'h', text: 'Why it exists: the same-origin model and its holes' },
    {
      t: 'prose',
      md: `The browser's core protection is the same-origin policy: script from \`https://acme.com\`
...
different from each other.`
  ...
  SC --> D5["Lockfile, scoped registry,<br/>no install scripts"]`,
      caption: 'Every class is a different hole in one model, which is why the five defences look nothing like each other.'
    },

    { t: 'h', text: 'XSS: the one that gives away everything' },
    {
      t: 'prose',
      md: `Cross-site scripting means attacker-controlled script executing in your origin. Once that
...
bypass escaping.`
    },
    {
      t: 'code',
      lang: 'jsx',
      title: 'Dangerous sinks, and the one that looks safe',
      code: `// 1. The obvious one. Whatever is in bio executes.
...
location.assign(ALLOWED.has(next) ? next : '/dashboard');`
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Server-side rendering adds a sink you cannot see',
      md: `Serialising state into an inline script element is standard practice and is a live
...
because the browser never treats those bytes as executable code at all.`
    ...
    { t: 'h', text: 'CSP and Trusted Types' },
    {
      t: 'prose',
      md: `Content Security Policy is a header telling the browser which script it is allowed to
...
through JSONP endpoints and open redirects on allowlisted domains.`
    },
    {
      t: 'code',
      lang: 'http',
      title: 'A strict CSP that survives a real application',
...
<script nonce="r4nd0mPerResponse" src="/assets/app.a91f.js"></script>`
    ...
    {
      t: 'prose',
      md: `Rolling this out matters as much as writing it. Deploy with
...
permissive default.`
    ...
    { t: 'h', text: 'CSRF and cookie semantics' },
    {
      t: 'prose',
      md: `CSRF exploits the fact that browsers attach cookies based on the *destination*, not on
...
\`SameSite\` is the primary defence and its three values behave quite differently.`
    ...
    {
      t: 'table',
      title: 'SameSite, precisely',
      cols: ['Value', 'Cookie sent when', 'Consequence', 'Use for'],
      rows: [
    ...
    {
      t: 'prose',
      md: `\`SameSite=Lax\` is necessary and not sufficient, because it stops cross-*site* requests,
...
one-sided.`
    ...
    { t: 'h', text: 'Where to keep a token' },
    {
      t: 'table',
      title: 'Token storage, with the honest costs',
      cols: ['Location', 'XSS exposure', 'CSRF exposure', 'Practical cost', 'Verdict'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'info',
      title: 'The honest version of this argument',
      md: `The internet insists \`localStorage\` is insecure and \`HttpOnly\` cookies are secure. The
...
effort on Trusted Types and CSP first, then pick cookies.`
    ...
    { t: 'h', text: 'OAuth in a browser: PKCE' },
    {
      t: 'prose',
      md: `A browser app cannot hold a client secret, so the original OAuth implicit flow returned
...
token family.`
    ...
    { t: 'h', text: 'Framing, CORS and postMessage' },
    {
      t: 'prose',
      md: `**Clickjacking** loads your page in an invisible iframe over attacker content, so a click
...
so does a check for a substring.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'postMessage, both ends done properly',
      code: `// Sending: name the origin. '*' means "anyone who happens to be in this frame".
...
// includes('acme.com')          accepts https://evil.com/?x=acme.com`
    ...
    { t: 'h', text: 'Supply chain and third-party script' },
    {
      t: 'prose',
      md: `A typical React application resolves to somewhere between 1,000 and 1,500 transitive
...
None of those are caught by reading a diff. The controls are structural.`
    },
    {
      ...
      cols: 2,
      items: [
        { b: 'Lockfiles, and CI that honours them', md: '`npm ci` or `yarn --immutable`, never `npm install` in CI. An unpinned transitive range means your Tuesday build contains code that did not exist on Monday, from an author you never evaluated.' },
        { b: 'Scoped registry configuration', md: 'Map `@acme/*` to your internal registry explicitly in `.npmrc`. This is the direct fix for dependency confusion, and it is two lines.' },
        { b: 'Ignore install scripts by default', md: '`npm config set ignore-scripts true`, with an allowlist for the handful that genuinely need them. Most supply-chain payloads execute at install time, before any code review would see them run.' },
        { b: 'Subresource Integrity for anything external', md: 'Add `integrity="sha384-..."` and `crossorigin="anonymous"` to every externally-hosted script tag. The browser refuses to execute a body whose hash changed. Useless for versionless URLs, which is itself a reason to avoid them.' },
        { b: 'Provenance and signed attestations', md: 'npm provenance links a published tarball to the source commit and the CI run that built it. Prefer dependencies that publish it, and publish it yourself.' },
        { b: 'Budget the dependency count', md: 'Treat adding a package as a decision with a cost, and review transitive additions in pull requests. A four-line utility with 80 transitive dependencies is a bad trade.' }
      ]
    },
    {
      t: 'prose',
      md: `Third-party script you add deliberately is the same risk with a friendlier name. An
...
contains -- it is usually more than anyone remembers approving.`
    ...
    { t: 'h', text: 'Prompt injection as a frontend concern' },
    {
      t: 'prose',
      md: `If your app renders model output, you have a new untrusted input source, and it is one
...
it reaches the DOM.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Rendering user HTML through `dangerouslySetInnerHTML`', blast: 'Stored XSS -- full session theft for every viewer of that content.', fix: 'DOMPurify with an explicit tag and attribute allowlist, plus Trusted Types so the raw-string path is rejected by the browser.' },
        { mode: 'Redirect target taken from a query parameter', blast: 'Open redirect used for phishing, and `javascript:` targets escalate to XSS.', fix: 'Allowlist redirect paths; parse with `new URL` and check the protocol; never accept a full URL from a parameter.' },
        { mode: '`Access-Control-Allow-Origin` reflecting the request origin with credentials allowed', blast: 'Any website can read your users\' authenticated API responses -- a full data breach.', fix: 'Static allowlist of origins; if you must reflect, validate against that allowlist and never wildcard with credentials.' },
        { mode: '`postMessage` origin checked with `startsWith`', blast: '`https://acme.com.evil.com` passes, so an attacker frame receives tokens or drives privileged actions.', fix: 'Exact-match against a `Set` of allowed origins, then validate the payload shape and bounds.' },
        { mode: '`npm install` in CI with unpinned transitive ranges', blast: 'A compromised dependency reaches production without any code change of yours.', fix: '`npm ci` with a committed lockfile, scoped registry mapping, `ignore-scripts`, and provenance checks.' },
        { mode: 'Tag manager with no review path', blast: 'Arbitrary script injected into production by a non-engineering team; it can read your checkout form.', fix: 'Remove third-party script from credential and payment pages, restrict via CSP, sandbox on a separate origin, and audit container contents quarterly.' },
        { mode: 'CSP shipped in enforce mode without a report-only phase', blast: 'Critical third-party functionality silently blocked for a user segment you did not test.', fix: '`Content-Security-Policy-Report-Only` for one to two weeks, triage every violation, then enforce.' },
        { mode: 'Model output rendered as markdown with raw HTML allowed', blast: 'Prompt-injected payload becomes XSS; an image URL exfiltrates conversation content with no click.', fix: 'Sanitise model output like anonymous user input, forbid raw HTML, allowlist URL schemes, restrict `img-src` and `connect-src`.' }
      ]
    },

    {
      t: 'staff',
      md: `Most candidates list vulnerabilities. The Staff-level answer talks about controls,
...
exfiltration framing of token storage are all strong signals.`
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `A nonce in CSP means HTML varies per response, which collides directly with the cache
...
reports only help if someone sees them, which is **Frontend Observability**.`
  ...
  flashcards: [
    { q: 'Why is XSS the highest-priority frontend vulnerability?', a: 'Injected script runs as your origin, so it reads non-`HttpOnly` cookies and storage, reads the DOM including typed input, and issues fully authenticated requests. Every other frontend control is downstream of not having it.' },
    { q: 'What is DOM-based XSS?', a: 'Your own JavaScript reads an attacker-influenced value -- a query parameter, a fragment, a `postMessage` -- and writes it to a dangerous sink like `innerHTML` or a `javascript:` URL. The server is never involved, so server-side filtering cannot help.' },
    { q: 'Why nonce plus `strict-dynamic` instead of a host allowlist?', a: 'Allowlisting a host trusts every endpoint on it, and JSONP callbacks or open redirects on allowlisted domains let an attacker get code served from a trusted origin. Nonces trust specific script instances, and `strict-dynamic` makes browsers ignore host allowlists.' },
    { q: 'What does Trusted Types change?', a: 'With `require-trusted-types-for \'script\'`, dangerous sinks reject plain strings and accept only objects from a registered policy. The review surface collapses from every sink usage to a handful of policies.' },
    { q: 'Difference between `SameSite=Lax` and `Strict`?', a: '`Lax` sends the cookie on same-site requests and on top-level cross-site `GET` navigations, blocking cross-site `POST`. `Strict` withholds it even on a top-level navigation, so users arriving from an email link appear logged out.' },
    { q: 'Why is `SameSite=Lax` insufficient on its own?', a: 'It is same-*site*, not same-*origin*. A compromised subdomain such as `blog.acme.com` is the same site as `app.acme.com` and can issue requests carrying your cookie. Add a double-submit or synchroniser token and an `Origin` check.' },
    { q: 'The honest difference between `localStorage` and `HttpOnly` for tokens?', a: 'Exfiltration. With XSS an attacker can make requests either way, but a `localStorage` token can be sent to their server and replayed off-device for its full lifetime. An `HttpOnly` cookie cannot leave the browser, confining them to the live page.' },
    { q: 'What does PKCE protect against, and what does it not?', a: 'It stops an intercepted authorisation code from being exchanged, since the attacker lacks the `code_verifier` behind the SHA-256 challenge. It does not cover CSRF on the redirect itself -- that is still the `state` parameter\'s job.' },
    { q: 'What is dependency confusion?', a: 'If an internal package name is unregistered publicly, an attacker publishes it to the public registry and build systems configured with both may prefer it, executing their code in your pipeline. Fix it by mapping your scope to the internal registry in `.npmrc`.' }
  ],
  ...
  drills: [
    ...
    {
      prompt: 'You are adding an AI assistant to a customer-support tool. It reads ticket contents and knowledge-base articles, renders markdown responses, and can trigger actions such as issuing a refund or closing a ticket. Threat-model the frontend.',
      probes: [
