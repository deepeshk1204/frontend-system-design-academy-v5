export default {
  blocks: [
    {
      t: 'prose',
      md: `React at small scale is a library. React at large scale is an architecture problem, and
...
worth its price, and the specific failure patterns large apps converge on.`
    ...
    {
      t: 'prose',
      md: `React's core promise is that you describe the UI as a function of state and it works out
...
comes from the second.`
    ...
    { t: 'h', text: 'Render vs commit, and what reconciliation really does' },
    {
      t: 'prose',
      md: `A React update has two phases and conflating them causes most wrong conclusions about
performance.
...
goal; the goal is to prevent *expensive* renders.`
  ...
  P --> E["useEffect<br/>after paint"]`,
      caption: 'Work in useLayoutEffect delays the pixels. Work in useEffect does not. This is why measurement effects are the usual cause of a slow commit.'
    },

    { t: 'h', text: 'The economics of memoisation' },
    {
      t: 'prose',
      md: `\`useMemo\`, \`useCallback\` and \`React.memo\` are not free, and reasoning about them as
...
cannot restructure your tree or split your context.`
    ...
    {
      t: 'table',
      title: 'When memoisation pays, concretely',
      cols: ['Situation', 'Worth it?', 'Why'],
      rows: [
    ...
    { t: 'h', text: 'Context fan-out' },
    {
      t: 'prose',
      md: `Context is the most common self-inflicted performance problem in large React apps,
...
component subscribe to one field and re-render only when that field changes.`
    },
    {
      t: 'code',
      lang: 'jsx',
      title: 'Context split by change frequency',
      code: `// BAD: one context, four unrelated concerns, new object every render.
...
const count = useStore(s => s.cart.items.length);   // Zustand selector`
    ...
    { t: 'h', text: 'Boundaries: from domain and change frequency' },
    {
      t: 'prose',
      md: `Most codebases are organised by technical type -- \`components/\`, \`hooks/\`, \`utils/\`,
...
should accept composition (\`children\`, slots) instead of configuration.`
    ...
    { t: 'h', text: 'Rendering long lists, and concurrent features' },
    {
      t: 'prose',
      md: `**Virtualisation** renders only the rows in the viewport plus a small overscan buffer,
...
hydration hurts INP less there than in React 17.`
    ...
    {
      t: 'numbers',
      title: 'Figures worth carrying',
      items: [
        { v: '~1 ms/KB', k: 'JS parse + compile, mid-tier mobile', note: 'Before your code runs' },
        { v: '4-6x', k: 'Mid-tier Android vs dev laptop', note: 'Single-threaded JS' },
        { v: '~10-12 ms', k: 'Usable budget in a 60 Hz frame', note: '~5 ms at 120 Hz' },
        { v: '~500', k: 'Rows above which virtualisation clearly pays', note: 'Below ~100, do not bother' },
        { v: '~170 KB', k: 'Sensible gzipped initial JS budget', note: 'Route-split beyond that' }
      ]
    },

    { t: 'h', text: 'Bundle graph hygiene' },
    {
      t: 'prose',
      md: `Route-level code splitting is the highest-leverage bundle work and the easiest to get
...
minutes; the same regression found next quarter costs a week of archaeology.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'One root context holding theme, user, modals and socket status', blast: 'A websocket heartbeat every 2 s re-renders the entire consumer tree; profiles show constant background work with no user action.', fix: 'Split providers by change frequency, memoise each value, and use selector subscriptions for fine-grained reads. `React.memo` will not save you -- context bypasses it.' },
        { mode: 'Array index used as `key` in a reorderable list', blast: 'State attaches to the wrong row -- a checked box or a focused input follows position, not item -- and data corruption follows if the user acts on it.', fix: 'Stable domain IDs as keys. Lint rule banning index keys on anything that can reorder or filter.' },
        { mode: '`React.memo` with an inline object or array prop', blast: 'Comparison runs and always fails, so you pay the cost and re-render anyway; the team believes the component is memoised.', fix: 'Memoise the prop or pass primitives. Verify with the Profiler that the memo actually prevents renders instead of assuming it does.' },
        { mode: '5,000-row table rendered in full', blast: '~60,000 DOM nodes, hundreds of milliseconds of layout, unusable scrolling on mid-tier Android, and high memory.', fix: 'Virtualise with overscan, plus correct `aria-rowcount`/`aria-rowindex`, a measurement cache for variable heights, and a plan for in-page search.' },
        { mode: 'Wrapper component swapped conditionally around the same children', blast: 'React sees a different element type at that position, unmounts the subtree, and all form state and scroll position is lost.', fix: 'Keep the element type stable and vary styling or layout via props, or hoist the children so they are not re-parented.' },
        { mode: 'Barrel file importing a heavy dependency transitively', blast: 'A 230 KB date library ends up in every route chunk; route splitting delivers almost no benefit.', fix: 'Deep imports, accurate `sideEffects` declaration, per-chunk size budget failing CI, and a bundle report required in review.' },
        { mode: 'Measurement work in `useLayoutEffect` across many instances', blast: 'Layout thrashing plus a blocked commit; paint is delayed by hundreds of milliseconds on mount.', fix: 'Hoist measurement to the parent and measure once, or move to `ResizeObserver`. Reserve `useLayoutEffect` for cases where a visual flash is otherwise unavoidable.' },
        { mode: 'Shared component accumulating nineteen boolean props', blast: 'Effectively untestable; every team is blocked on changes to it, and each change risks a caller nobody knew about.', fix: 'Split into distinct components by job, or convert configuration into composition via `children` and slots. Treat prop-count growth as a boundary smell in review.' }
      ]
    },

    {
      t: 'staff',
      md: `The differentiator at this level is measuring before optimising, and preferring
...
memoising, and state the cost of your own proposal.`
...

    {
      ...
      items: [
        {
          q: 'A single `AppContext` provides `{ theme, user, socketStatus }`. `socketStatus` updates every 2 seconds. Components reading only `theme` are wrapped in `React.memo` but still re-render. Why?',
          options: [
            'The `React.memo` comparison is shallow, so it fails on the context object.',
            'Context propagation bypasses `React.memo` entirely -- any consumer re-renders when the context value changes by reference, regardless of which field it reads.',
            '`theme` is being recreated on every render.',
            'React batches context updates, which invalidates memo boundaries.'
          ],
          answer: 1,
          why: '`React.memo` compares the props a parent passes down. A context value does not arrive as a prop -- `useContext` subscribes the component directly to the provider, and React notifies every subscriber when the provider value changes by reference. The memo boundary is simply not on that path. React also has no field-level granularity: it compares the whole value by reference and cannot know that you only read `theme`. The fix is structural -- separate providers split by change frequency, so the two-second heartbeat only notifies the components that display connection status -- or a selector-based external store where a component subscribes to one field.'
        },
        {
          q: 'A list renders 40 rows with `key={index}`. The user checks row 3, then sorts the list so that row moves to position 12. What happens?',
          options: [
            'The checkbox correctly follows the item to position 12.',
            'The checked state stays at position 3, now applied to a different item, because React matches siblings by key and the keys did not move.',
            'React throws a duplicate-key warning and clears all checkboxes.',
            'Nothing visible changes until the next render.'
          ],
          answer: 1,
          why: 'Keys are React\'s identity for siblings. With index keys, the element at position 3 has key `3` before and after the sort, so React concludes it is the same element and keeps its component instance and state -- while the props now describe a completely different item. Local state such as a checkbox, an input value or focus stays with the *position* rather than the data. This is a correctness bug with no warning: the user checks one row and a different one is submitted. Any list that can be sorted, filtered, reordered or have items inserted needs a stable domain ID as its key, and a lint rule is the cheapest enforcement.'
        },
        {
          q: 'Typing in a search box that filters a 3,000-item list drops input characters. You wrap the filtered-list update in `useTransition`. Input is now smooth, but the results take noticeably longer to appear. Is this the right trade, and what would *not* be fixed by it?',
          options: [
            'Wrong trade -- transitions should make both faster.',
            'Right trade for responsiveness, but it would not help if a single component in the list did one large block of synchronous work, since React cannot interrupt inside a component render.',
            'Wrong trade -- `useDeferredValue` would have made both faster.',
            'Right trade, and it also reduces the total rendering work performed.'
          ],
          answer: 1,
          why: 'A transition reprioritises rather than accelerates: the keystroke renders at urgent priority so the input stays responsive, while the expensive list render is marked non-urgent and can be interrupted and restarted when new input arrives. Total work goes up slightly, and results lag deliberately -- that is the trade, and it is usually the right one because a laggy input feels broken while slightly late results do not. The limit worth stating is that interruption happens *between* units of work React controls. If one component performs a 200 ms synchronous sort inside its render, React cannot yield partway through it, and no concurrent feature helps; you must memoise that computation, move it off the render path, or move it to a worker.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Long tasks, the frame budget and forced synchronous layout are **Browser & Rendering
...
CI are **Performance Engineering**.`
  ...
  flashcards: [
    { q: 'Is a re-render the same as a DOM update?', a: 'No. The render phase calls your components and diffs the result; if the output is unchanged, the commit phase does nothing. Rendering 200 components that produce identical trees costs the function calls and the diff -- often under a millisecond -- and zero DOM work. Target expensive renders, not all renders.' },
    { q: 'What happens when the element type at a position changes?', a: 'React unmounts the entire subtree and remounts it, destroying all state within it. This is why conditionally swapping a wrapper component around the same children clears form inputs, focus and scroll position.' },
    { q: 'Why does `React.memo` fail to stop rerenders from context?', a: 'Because `useContext` subscribes the component directly to the provider rather than receiving a prop, so the memo comparison is not on that path. React also compares the whole context value by reference and has no field-level granularity.' },
    { q: 'When is `useCallback` load-bearing, and when is `React.memo` worse than nothing?', a: '`useCallback` earns its cost when the function goes to a `React.memo` child (otherwise the memo can never hit) or into another hook\'s dependency array (otherwise the effect re-runs every render, a correctness issue). `React.memo` is actively harmful with an inline object or array prop: the shallow comparison runs every parent render and always fails, so you pay it and re-render anyway while believing the component is memoised.' },
    { q: 'What does `useTransition` actually change?', a: 'It marks an update non-urgent so React keeps urgent input responsive and can interrupt and restart the expensive render. It makes rendering interruptible, not faster or parallel -- and it cannot interrupt inside a single component\'s render, so one large synchronous computation is unaffected.' },
    { q: 'What are the real costs of virtualisation?', a: 'In-page search cannot find unrendered rows; accessibility needs correct `aria-rowcount`/`aria-rowindex` since the DOM no longer reflects the list; variable-height rows need a measurement cache, which is where most bugs live; and `scrollIntoView` to an arbitrary row needs explicit support.' },
    { q: 'Why do index keys cause bugs rather than just slowness?', a: 'React matches siblings by key, so with index keys the instance and its local state stay with the *position* while the props describe a different item. A checked checkbox, an input value or focus follows position through a sort -- a silent correctness bug with no warning.' },
    { q: 'What is the test for whether a component belongs in the shared design system?', a: 'Used by three or more domains *and* its API has stopped changing. Promoting a still-evolving component makes every future change a coordinated migration across the whole application.' },
    { q: 'Why do barrel files undermine code splitting?', a: '`export * from ...` pulls the whole module graph into anything that imports one symbol, so a heavy dependency lands in every chunk and route splitting delivers little. Whether tree-shaking rescues you depends on side-effect analysis you are not checking -- so use deep imports and verify against the actual bundle report.' }
  ],
  ...
  drills: [
    ...
    {
      prompt: 'A dashboard shows 12 widgets. One widget subscribes to a websocket pushing a price tick roughly every 300 ms. The whole page re-renders on every tick and the browser sits at 40% CPU while idle. Walk me through the diagnosis and the fix, and tell me what you would reject.',
      probes: [
