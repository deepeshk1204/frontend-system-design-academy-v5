export default {
  blocks: [
    {
      t: 'prose',
      md: `Every page of HTML is produced somewhere. It can be built in the user's browser from
...
personalisation requirements, and there is no single strategy that is correct for all four.`
    ...
    {
      t: 'prose',
      md: `The web started server-rendered because there was nothing else -- HTML came from a
...
the fast first paint.`
  ...
  STR --> PH["Progressive hydration<br/>or RSC, no hydration for<br/>server-only components"]`,
      caption: 'The branch point is when HTML is produced. Everything after it is about how much JavaScript must run before the page responds to input.'
    },
    {
      t: 'prose',
      md: `**CSR** ships an empty HTML shell plus JavaScript. The browser boots the framework,
...
just rescheduling it.`
    ...
    { t: 'h', text: 'The hydration bill, with real numbers' },
    {
      t: 'prose',
      md: `Hydration cost is dominated by two things: how much JavaScript you execute, and how
...
Vitals on LCP and a wave of "the button does nothing" complaints.`
    ...
    {
      t: 'numbers',
      title: 'Hydration arithmetic on a mid-tier Android',
      items: [
        { v: '~1 ms/KB', k: 'JS parse + compile', note: 'Before any of your code runs' },
        { v: '4-6x', k: 'Mid-tier Android vs laptop', note: 'Single-threaded JS' },
        { v: '~700-1200 ms', k: 'Hydrating a 500 KB app, few thousand nodes', note: 'One unyieldable long task pre-React-18' },
        { v: '0 KB', k: 'JS shipped by a server-only RSC', note: 'The only strategy that removes rather than defers' },
        { v: '~14 KB', k: 'First TCP flight', note: 'What the streamed shell should fit in' }
      ]
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Partial hydration is not the same as lazy hydration',
      md: `**Lazy** or progressive hydration defers the work -- the component hydrates when it
...
interchangeably.`
    ...
    { t: 'h', text: 'Streaming: what it actually looks like on the wire' },
    {
      t: 'prose',
      md: `Non-streaming SSR calls \`renderToString\`, waits for every data dependency, and sends
...
JSON-LD) belongs in the shell, not behind a \`Suspense\` boundary.`
    },
    {
      t: 'code',
      lang: 'jsx',
      title: 'Streaming boundaries, and what each one commits you to',
      code: `// Anything that can change the status code or headers must resolve
...
// boundary becomes a layout shift and CLS goes red.`
    ...
    { t: 'h', text: 'Choosing per route' },
    {
      t: 'table',
      title: 'Decision table by route type',
      cols: ['Route', 'Freshness need', 'Needs SEO?', 'Personalised?', 'Strategy', 'Why'],
      rows: [
    ...
    {
      t: 'prose',
      md: `Read the personalisation column again, because it is where the real design work is. The
...
halves can have different cache policies.`
    ...
    { t: 'h', text: 'What SSR costs you as an operator' },
    {
      t: 'prose',
      md: `SSG is a file on a CDN, which means capacity planning is someone else's problem and a
...
and \`stale-if-error\` so an origin failure degrades to slightly-old HTML rather than a 500.`
    ...
    {
      t: 'table',
      title: 'Metric implications, honestly',
      cols: ['Strategy', 'TTFB', 'FCP / LCP', 'INP after load', 'Server cost', 'Cacheable at edge?'],
      rows: [
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Hydration mismatch from non-deterministic render', blast: 'React discards the server HTML and re-renders client-side; you paid for SSR and shipped a CSR experience, often with a visible flash.', fix: 'No `Date.now()`, `Math.random()`, locale or timezone formatting, or `window` checks during render. Render such values in an effect after mount, and fail CI on hydration warnings.' },
        { mode: 'SSR migration ships green LCP and a wave of unresponsive-button reports', blast: 'Page paints in 1.2 s and accepts input at 2.4 s; INP breaches 500 ms at p75 on Android.', fix: 'Track TBT and INP alongside LCP, budget hydration explicitly, and move non-interactive subtrees to server components or islands.' },
        { mode: 'One slow upstream inside a non-streaming SSR render', blast: 'TTFB tracks the slowest dependency; a 3 s recommendations API makes every page 3 s.', fix: 'Per-dependency timeouts with a rendered fallback, a circuit breaker per upstream, and move the slow section behind a `Suspense` boundary so it streams.' },
        { mode: 'Personalised SSR HTML cached at the edge', blast: 'One user\'s name, email or balance served to everyone hitting that PoP for the TTL -- a reportable data incident.', fix: 'Default `private, no-store` on authenticated routes; split shareable shell from personalised fragment; automated CI check that no `Set-Cookie` response is `public`.' },
        { mode: 'SEO content behind a `Suspense` boundary', blast: 'Title, meta description and primary copy missing from the initial HTML; social previews blank and non-Google crawlers index nothing.', fix: 'Keep metadata, canonical URL, Open Graph tags and primary copy in the shell. Verify with a raw `curl` of the first flush, not with a rendered browser view.' },
        { mode: 'Streamed boundaries with zero-height skeletons', blast: 'Every resolved chunk pushes content down; CLS goes red even though each individual shift is small.', fix: 'Skeletons must reserve the final dimensions. Use `aspect-ratio` and explicit `width`/`height` on media, and monitor CLS attributed to the specific boundary.' },
        { mode: 'Redirect or 404 decided after the first flush', blast: 'A `200 OK` was already sent, so the framework cannot redirect; users land on a broken page and crawlers index a soft 404.', fix: 'Resolve auth, existence and redirect logic before the shell renders. Treat "what can change the status code" as an explicit pre-flush checklist.' },
        { mode: 'SSG build time growing past the deploy window', blast: 'A 50,000-page build takes 33 minutes at 40 ms/page; hotfixes become hour-long events.', fix: 'Move to ISR with on-demand revalidation, pre-build only the high-traffic long tail, and generate the rest on first request.' }
      ]
    },

    {
      t: 'staff',
      md: `The differentiator is refusing to answer "which rendering strategy?" as a single
...
that would catch it going wrong.`
...

    {
      ...
      items: [
        {
          q: 'You migrate a CSR dashboard to non-streaming SSR. LCP improves from 3.1 s to 1.4 s, but support tickets about unresponsive buttons triple. What happened?',
          options: [
            'SSR broke event delegation, so handlers never attached.',
            'The page now paints complete-looking content before hydration attaches handlers, so there is a long window where it looks interactive and is not.',
            'LCP and INP measure the same thing, so one of the numbers is wrong.',
            'Server-rendered HTML disables React event handling until the CSS loads.'
          ],
          answer: 1,
          why: 'This is the hydration uncanny valley. CSR showed a skeleton -- ugly, but an honest signal that the page was not ready, so users waited. SSR paints the finished UI at 1.4 s while the 500 KB bundle still needs downloading, parsing and hydrating, which on a mid-tier Android is easily another second of unbreakable main-thread work. Taps in that window are queued, not dropped, so the user taps twice and both fire later. LCP improved and TBT/INP regressed; measuring only LCP hid the trade. The fixes are to reduce the JavaScript that must run (server components, islands) rather than to reschedule it.'
        },
        {
          q: 'A product page streams its reviews section behind `Suspense`. The reviews service starts failing. What does the user get, and what does your monitoring see?',
          options: [
            'A 500 error page, because the render threw.',
            'A 200 response with a working page and an error state inside the reviews section -- so error-rate dashboards based on status codes see nothing wrong.',
            'The request hangs until the stream times out.',
            'React automatically retries the boundary until it succeeds.'
          ],
          answer: 1,
          why: 'Once the shell has flushed with a `200`, the status code is committed and cannot be retracted, so a post-flush failure has to be rendered inside its boundary. That is the desired user experience -- the rest of the page is fine -- but it creates a monitoring gap: HTTP status is now a poor proxy for correctness, and "the page returned 200 with its main content replaced by an error" is a state you must instrument deliberately, via per-boundary error events in RUM. It also means every streamed hole needs an explicit error boundary; without one the failure propagates to the nearest ancestor and can blank a much larger region than intended.'
        },
        {
          q: 'A 50,000-SKU catalogue currently uses SSG. Prices change several times a day and the build takes 34 minutes. What is the most appropriate change?',
          options: [
            'Switch the whole site to SSR so prices are always current.',
            'Keep static generation but move to ISR with on-demand revalidation, and fetch the volatile price and stock client-side or at the edge.',
            'Reduce the build time by parallelising it across more CI runners.',
            'Cache the built pages at the edge with a 60-second TTL.'
              ],
          answer: 1,
          why: 'Two separate problems are bundled here. The build duration is solved by generating on demand and revalidating incrementally, so a price change triggers a single page regeneration rather than a full build. The freshness problem is solved by *splitting the response*: the description, images and specifications are stable and belong in the cacheable page, while price and stock are volatile and belong in a small client fetch or an edge-injected fragment. Moving everything to SSR throws away CDN delivery for 2% of the pixels and turns a bandwidth cost into a compute cost. Parallelising the build treats the symptom and still leaves you unable to publish a price change in under half an hour.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Hydration is the largest long task in most apps, which is **Browser & Rendering
...
Budgeting TBT and INP for a strategy change is **Performance Engineering**.`
  ...
  flashcards: [
    { q: 'Why can SSR make INP worse than CSR?', a: 'SSR paints a complete-looking page before the bundle has hydrated, so for hundreds of milliseconds it looks interactive and queues input instead of responding. CSR shows a skeleton that honestly signals "not ready". LCP improves while TBT and INP regress, which is invisible if you only track LCP.' },
    { q: 'Roughly what does JavaScript cost to parse and compile on a mid-tier phone?', a: 'About 1 ms per KB before any of your code executes, plus execution and reconciliation on top. A 500 KB bundle is therefore ~500 ms of parse/compile alone, and a mid-tier Android is 4-6x slower than a developer laptop on single-threaded JS.' },
    { q: 'Lazy hydration vs partial hydration -- what is the difference?', a: 'Lazy (progressive) hydration defers when the work happens; the same JavaScript is still shipped and eventually executed. Partial hydration (islands, RSC server components) means some components never ship JavaScript at all. Only the second one reduces the total bill.' },
    { q: 'What does streaming SSR decouple TTFB from?', a: 'Your slowest data dependency. The shell flushes in tens of milliseconds so the browser can start fetching CSS and fonts, and each `Suspense` boundary streams in as its query resolves, rather than the whole document waiting on the slowest query.' },
    { q: 'What is locked once the first chunk of a streamed response flushes?', a: 'The status code and all response headers. Auth checks, 404 decisions, redirects and `Set-Cookie` must be resolved before the shell goes out; anything failing after the flush must render an error inside its boundary instead of returning a 500.' },
    { q: 'Why is streamed content an SEO risk despite Googlebot rendering JavaScript?', a: 'Googlebot generally handles it, but social-preview scrapers (Slack, LinkedIn, Twitter) and many non-Google crawlers take only the initial HTML. Title, meta description, Open Graph tags, canonical URL, JSON-LD and primary copy must therefore live in the shell.' },
    { q: 'How do you estimate SSR capacity?', a: '`renderToString` blocks the Node event loop, so throughput is roughly cores divided by render time. At 60 ms per render a core sustains ~16 renders/second, meaning 5,000 rps needs on the order of 300 cores before headroom, GC pauses and p99 render time.' },
    { q: 'What causes a hydration mismatch and what does it cost?', a: 'Any non-deterministic render -- `Date.now()`, `Math.random()`, locale or timezone formatting, `typeof window` branches. React discards the server HTML and re-renders on the client, so you paid the SSR compute and delivered a CSR experience, often with a visible flash.' },
    { q: 'What is the right strategy for a page that is 98% shareable and 2% personalised?', a: 'Split the response. Cache the shareable majority aggressively (SSG/ISR with `s-maxage`), and fetch or edge-inject the personalised fragment separately, so one cache policy does not have to serve both. Making the whole page per-request sacrifices cacheability for 2% of the pixels.' }
  ],

  drills: [
    {
      prompt: 'You own an e-commerce site: marketing pages, 120,000 product pages, a search page, a logged-in account area, and checkout. Prices update several times daily, stock updates every few minutes, and organic search is the primary acquisition channel. Design the rendering strategy and justify each choice.',
      probes: [
    ...
    {
      prompt: 'A team proposes migrating a large CSR React app (620 KB of JS, p75 LCP 3.4 s, p75 INP 180 ms) to streaming SSR with React Server Components, estimating one quarter of work. You are the reviewing Staff engineer. What do you require before approving, and what would make you say no?',
      probes: [
