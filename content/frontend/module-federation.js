export default {
  blocks: [
    {
      t: 'prose',
      md: `Module Federation lets one JavaScript application import a module from another
...
Everything interesting about Module Federation is how that reconciliation works.`
    ...
    {
      t: 'prose',
      md: `Before federation, sharing code between independently deployed frontends meant one of two
bad options.
...
matters more than memorising config keys.`
    ...
    { t: 'h', text: 'The five concepts' },
    {
      t: 'grid',
      cols: 2,
      items: [
        { b: 'Host (a.k.a. consumer)', md: 'The build that imports federated modules. Usually your shell. A build can be both a host and a remote at once.' },
        { b: 'Remote (a.k.a. producer)', md: 'The build that exposes modules for others to import. Identified by a name and the URL of its entry file.' },
        { b: '`remoteEntry.js`', md: 'A tiny manifest-plus-loader the remote emits. It declares what the remote exposes and what it can contribute to the shared scope. Must never be cached immutably.' },
        { b: '`exposes`', md: 'The public API of a remote: a map from a public path such as `./Checkout` to an internal source file. This is a contract -- semver it mentally.' },
        { b: '`shared`', md: 'The packages this build is willing to provide to, or consume from, the shared scope, with version constraints and flags like `singleton`.' }
      ]
    },
    {
      t: 'code',
      lang: 'js',
      title: 'A real host config — webpack 5 with Module Federation 2',
...
module.exports = {
  ...
  plugins: [
    ...
    },
    {
      t: 'code',
      lang: 'js',
      title: 'The matching remote',
      code: `// billing/webpack.config.js
new ModuleFederationPlugin({
  name: 'billing',
...
//   /billing/v/2026-09-13-a91f/chunk.7c1e.js    -> max-age=31536000, immutable`
    ...
    { t: 'h', text: 'How shared-scope negotiation actually works' },
    {
      t: 'prose',
      md: `This is the part people cannot explain in interviews, and it is not complicated once you
...
navigation into the feature that triggers it.`
  ...
  M-->>H: component, sharing one React with the host`,
      caption: 'The host does not automatically win the negotiation. Highest satisfying semver wins, and the instance is created once.'
    },
    {
      t: 'table',
      title: 'The flags that decide your failure mode',
      cols: ['Option', 'What it does', 'Use it when', 'What goes wrong without it'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Eager vs lazy, concretely',
      md: `Lazy shared modules are loaded through an async boundary, which is why the standard host
...
bootstrap is roughly one extra round trip on an already-cached file -- pay it.`
    ...
    { t: 'h', text: 'The mixed-fleet problem: N-1, N, N+1' },
    {
      t: 'prose',
      md: `In a monolith there is one version of the app in production. With federation, the thing
...
The discipline that makes this tractable is straightforward, just unglamorous.`
    ...
    {
      t: 'steps',
      title: 'The mixed-fleet contract',
      items: [
        '**Additive-only contract changes within a major.** The shell may add services; it may not change or remove one until every remote has moved. Deprecate for at least one release cycle, and measure usage before removing.',
        '**Remotes declare the contract version they were built against.** The shell reads it at mount and refuses to mount an incompatible major, rendering a fallback rather than crashing.',
        '**Keep N-1 and N-2 remote builds live.** Never delete old hashed chunks. A user on old HTML must still be able to fetch the chunks that HTML references.',
        '**Contract fixtures in every remote\'s CI.** Each remote tests against a published mock of shell contract N-1 and N, so incompatibility fails a build instead of a user session.',
        '**Pin remote versions in a manifest the shell reads at runtime**, so the combination in production is a deliberate, recorded choice you can revert.',
        '**A staleness signal for long-lived tabs.** When the shell detects its build is more than a release behind, prompt a reload at a safe moment rather than letting the tab drift for days.'
      ]
    },

    { t: 'h', text: 'Runtime resolution and rollback' },
    {
      t: 'prose',
      md: `If remote URLs are string literals in your webpack config, a remote rollback requires a
...
can achieve the same thing by hand with the promise-based remote syntax.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'Manifest-driven remotes with a fallback and a timeout',
      code: `// Served as /mfe-manifest.json with Cache-Control: max-age=30, stale-if-error=3600
// {
...
export async function loadFeature(name) {
  ...
  try {
    ...
    },
    {
      t: 'note',
      tone: 'danger',
      title: 'Error boundaries do not catch what you think',
      md: `A React error boundary catches errors thrown *during render* of its subtree. It does not
...
your generic error bucket.`
    ...
    { t: 'h', text: 'Types, local development, and the ESM alternatives' },
    {
      t: 'prose',
      md: `A federated import is a network call, so TypeScript has nothing to check by default and
...
Federation is not the only runtime-composition mechanism, and webpack is no longer a given.`
    ...
    {
      t: 'table',
      title: 'Runtime integration options in 2026',
      cols: ['Approach', 'Mechanism', 'Dependency sharing', 'Honest assessment'],
      rows: [
    ...
    {
      t: 'prose',
      md: `Import maps deserve a fair hearing because they are a platform feature rather than a
...
fleet mid-migration it is a wall.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Two Reacts survive negotiation', blast: 'Invalid hook call, or a context consumer silently getting the default value across a whole feature.', fix: '`singleton: true` plus `strictVersion: true` on `react` and `react-dom`; assert one instance at boot and log the resolved version.' },
        { mode: '`remoteEntry.js` cached immutably', blast: 'Remote deploys stop reaching users; they load a stale manifest pointing at deleted chunks.', fix: '`max-age=30, must-revalidate` on `remoteEntry.js` only; everything else content-hashed and `immutable`.' },
        { mode: 'Old chunks deleted on deploy', blast: '`ChunkLoadError` for every user holding previous HTML -- often thousands of sessions.', fix: 'Keep N-1 and N-2 builds live under versioned paths; retain for at least 30 days; alert on `ChunkLoadError` rate.' },
        { mode: 'Shell makes a breaking contract change', blast: 'Every remote built against the old contract fails at once, defeating the isolation premise.', fix: 'Additive-only within a major, contract fixtures in each remote\'s CI, and a mount-time version check that renders a fallback.' },
        { mode: 'Remote host is slow, not down', blast: 'The shell hangs with no error, so no boundary fires and no alert triggers.', fix: 'Explicit timeout on every remote load, usually 5-8 s, then fall back and emit a `remote_timeout` metric per remote.' },
        { mode: 'Implicit `requiredVersion` drifts', blast: 'A routine `npm update` in one remote changes negotiation and a different React wins in production.', fix: 'Declare `requiredVersion` explicitly, and snapshot-test the resolved shared versions in CI.' },
        { mode: 'Manifest fetch fails at boot', blast: 'No remotes resolve; the whole app is a shell with empty regions.', fix: 'Inline a last-known-good manifest into the host build, serve the live one with `stale-if-error`, and alert on manifest fetch failures.' }
      ]
    },

    {
      t: 'staff',
      md: `Senior candidates describe the config. Staff candidates describe the *operational* model
...
least once, and you design so the next one is loud, attributable, and revertible without a build.`
...

    {
      ...
      items: [
        {
          q: 'Host declares `react: { requiredVersion: "^18.2.0" }` and carries 18.2.0. A remote declares the same range but carries 18.3.1. Both are `singleton: true`. Which React runs?',
          options: [
            'The host\'s 18.2.0 — hosts take priority.',
            '18.3.1, for both host and remote, because the highest satisfying version wins.',
            'Both load; `singleton` only warns.',
            'Resolution fails because the versions differ.'
          ],
          answer: 1,
          why: 'Shared-scope resolution selects the highest version among candidates satisfying the requesting range, regardless of which side provided it. So your host silently upgrades to the remote\'s React. This is a real deploy risk: a remote team bumping a patch version changes the framework your shell executes on. Log the resolved version at boot and snapshot-test it in CI.'
        },
        {
          q: 'You build with `output.publicPath: "/"` in a remote. Its exposed component renders, then its lazy sub-chunks 404. Why?',
          options: [
            '`remoteEntry.js` was cached too long.',
            'Chunk URLs resolve against the host\'s origin and path instead of the remote\'s.',
            'The shared scope did not include the chunk.',
            'The host needs `eager: true` for that dependency.'
          ],
          answer: 1,
          why: 'A hardcoded `publicPath` makes the remote\'s runtime request `/chunk.7c1e.js` from wherever the page is served -- the host\'s origin -- where that file does not exist. `publicPath: "auto"` makes webpack derive the base URL from the location of the executing `remoteEntry.js`, so chunks resolve back to the remote\'s CDN path. It is the single most common first-day federation bug.'
        },
        {
          q: 'Which caching policy is correct for a federated remote?',
          options: [
            'Everything `immutable, max-age=31536000`, including `remoteEntry.js`.',
            '`remoteEntry.js` short-TTL and revalidated; all hashed chunks immutable and retained across releases.',
            'Everything `no-store` so users always get the newest code.',
            '`remoteEntry.js` immutable, chunks `no-store`.'
          ],
          answer: 1,
          why: '`remoteEntry.js` is the pointer that makes a deploy visible, so caching it immutably means deploys never reach users. Hashed chunks are content-addressed and safe to cache forever -- and must be *retained*, because a user holding the previous `remoteEntry` still references them. Deleting them turns a normal deploy into a wave of `ChunkLoadError`.'
        },
        {
          q: 'A remote built against shell contract v3 is loaded by a shell still running v2. What is the best-designed outcome?',
          options: [
            'It throws a TypeError deep inside the remote when it calls a missing service.',
            'The shell reads the remote\'s declared contract version at mount, refuses, and renders a scoped fallback with a telemetry event.',
            'The shared scope rejects the remote automatically.',
            'TypeScript catches it at build time.'
          ],
          answer: 1,
          why: 'The shared scope only negotiates package versions -- it knows nothing about your shell API. Types are checked against whatever the remote published at *your* build time, not what is deployed. So the only reliable control is an explicit runtime handshake: the remote declares the contract major it was built for, the shell compares, and on mismatch you degrade one region and emit an attributable event instead of crashing a page.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Whether to do any of this belongs to **Microfrontends** -- federation is the mechanism,
...
remote name and version so a regression is attributable in one query is **Frontend Observability**.`
  ...
  flashcards: [
    { q: 'What does `remoteEntry.js` contain?', a: 'A small manifest-plus-loader: what the remote exposes, what it can contribute to the shared scope, and an `init` function the host calls with the scope. It is the deploy pointer, so it must be short-TTL and revalidated, never immutable.' },
    { q: 'How does shared-scope version resolution choose a copy?', a: 'Among all registered candidates satisfying the requesting side\'s `requiredVersion` range, the highest semver version wins, and an already-instantiated copy is reused. The host has no inherent priority.' },
    { q: 'Difference between `singleton` and `strictVersion`?', a: '`singleton: true` guarantees one instance in the realm and only warns on a version mismatch. `strictVersion: true` makes an unsatisfiable range throw at resolution time instead of silently falling back.' },
    { q: 'Why does `eager: true` exist, and what does it cost?', a: 'It inlines a shared copy into the initial chunk so synchronous entry code can use it, avoiding the "not available for eager consumption" error. The cost is losing that package\'s negotiability -- prefer an async `bootstrap` import instead.' },
    { q: 'Why must remote URLs come from a runtime manifest?', a: 'Hardcoded URLs in webpack config mean rolling back a remote requires rebuilding and redeploying the host, destroying deploy independence. A short-TTL manifest makes rollback an atomic pointer flip.' },
    { q: 'What does a React error boundary fail to catch here?', a: 'Failed `remoteEntry.js` requests, chunk-load errors, rejected dynamic imports, and errors thrown while a module body evaluates. You need a `try/catch` with a timeout around the import as well as the boundary.' },
    { q: 'What is the N-1/N/N+1 problem?', a: 'Production runs a combination of independently deployed versions, including browsers holding old shell HTML. The tested set is a list; the live set is a product. Survive it with additive-only contract changes, retained old chunks, and a mount-time contract check.' },
    { q: 'How do Import Maps dedupe dependencies?', a: 'By URL identity -- every module importing the bare specifier `react` resolves to the same URL, so the browser evaluates it once. Simple and standards-based, but there is one entry per specifier per document, so no semver negotiation.' },
    { q: 'Why is `publicPath: "auto"` required in a remote?', a: 'Without it, the remote\'s lazy chunk URLs resolve against the host\'s origin and path and 404. `auto` derives the base URL from the location of the running `remoteEntry.js`.' }
  ],

  drills: [
    {
      prompt: 'Your shell hosts four federated remotes. After the billing team deploys, about 15% of sessions see a blank page and your error tracker fills with "Invalid hook call". The billing team insists their app works standalone. You are on call. Walk me through the next 30 minutes and then the fix.',
      probes: [
    ...
    {
      prompt: 'You are adding federation to a five-year-old webpack 4 SPA with 90 engineers, and leadership wants the first remote in production within six weeks. Design the rollout so the first remote cannot take down the app.',
      probes: [
