export default {
  blocks: [
    {
      t: 'prose',
      md: `Collaborative editing is what happens when two people change the same document at the
...
to that one problem.`
    ...
    { t: 'h', text: 'Why last-write-wins loses edits' },
    {
      t: 'prose',
      md: `Start with the design most teams ship first: the editor sends the full document text on a
...
and there are two families of technique for arranging that.`
  ...
  OPS --> CONV["Both converge to<br/>Hello there!"]`,
      caption: 'Sending state forces a choice between losing an edit and interrupting a user. Sending operations makes both avoidable.'
    },

    { t: 'h', text: 'Operational Transformation' },
    {
      t: 'prose',
      md: `Operational Transformation, which is what Google Docs and Etherpad are built on, keeps a
...
Here is the transform concretely. Both clients start from \`Hello world\` at revision 4.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Two operations, transformed',
      code: `Base (rev 4):  "Hello world"
...
two clients that both inserted at index 6 diverge permanently.`
    ...
    {
      t: 'prose',
      md: `That last paragraph is why OT has the reputation it has. The transform matrix grows
...
correctness rather than just for persistence.`
    ...
    {
      t: 'prose',
      md: `A Conflict-free Replicated Data Type takes the opposite approach: instead of rewriting
...
removing it outright would invalidate other operations that reference it.`
    ...
    {
      t: 'table',
      title: 'The CRDT families you should be able to name',
      cols: ['Family', 'Mechanism', 'Converges to', 'Used for'],
      rows: [
    ...
    {
      t: 'prose',
      md: `The practical properties matter more than the taxonomy. Sequence CRDT metadata is
...
history and no compaction strategy will surprise you.`
    ...
    {
      t: 'table',
      title: 'OT versus CRDT, without the tribalism',
      cols: ['Dimension', 'OT', 'CRDT'],
      rows: [
    ...
    { t: 'h', text: 'What CRDTs genuinely do not solve' },
    {
      t: 'prose',
      md: `This is the part that separates a real answer from a repeated marketing claim. CRDTs
...
CRDT for free.`
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Convergence is a floor, not a ceiling',
      md: `"CRDTs solve collaborative editing" is true only for the narrow meaning of *solve* --
...
and none of it is in the algorithm.`
    ...
    { t: 'h', text: 'Architecture: server-authoritative or peer-to-peer' },
    {
      t: 'prose',
      md: `CRDTs make peer-to-peer *possible*, which leads people to conclude it is preferable. For
...
never touch your persistence layer.`
  ...
  R-->>B: only the missing updates`,
      caption: 'The state vector on reconnect is the CRDT equivalent of a resume token -- it is why a client offline for a day does not need a full document transfer.'
    },

    { t: 'h', text: 'Awareness, persistence and snapshots' },
    {
      t: 'prose',
      md: `**Awareness** is who is here, where their cursor is, and what they have selected. It is
...
serialised snapshots with timestamps and authors as a first-class product feature.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'What using Yjs actually looks like',
...
const snapshot = Y.encodeStateAsUpdate(doc);`
    },
    {
      t: 'note',
      tone: 'good',
      title: 'The recommendation, stated plainly',
      md: `Use Yjs or Automerge. Do not write your own.
...
prevented by product design rather than merged by mathematics.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Full-document last-write-wins sync', blast: 'Concurrent edits silently overwrite each other; users lose paragraphs with no error.', fix: 'Send operations, not states. OT with a central transform, or a CRDT.' },
        { mode: 'No deterministic tie-break on concurrent inserts at the same position', blast: 'Permanent divergence -- clients show different text and never reconcile.', fix: 'Total order by client id plus counter; property-test random interleavings and assert byte-identical convergence.' },
        { mode: 'No snapshotting of the update log', blast: 'Document load time grows linearly with lifetime edits; a two-year document takes 30 s to open.', fix: 'Snapshot every N updates or T minutes, truncate the log, and load snapshot plus tail.' },
        { mode: 'Unbounded tombstone growth', blast: 'A heavily-edited document\'s state dwarfs its text and memory use climbs on every client.', fix: 'Snapshot-based horizon: clients more than one snapshot behind reload rather than merge, enabling compaction.' },
        { mode: 'Awareness persisted with the document', blast: 'Ghost cursors from users who left, and write amplification of 60 messages per second per user.', fix: 'Separate ephemeral channel, throttle to one update per frame, lease-based expiry after ~30 s.' },
        { mode: 'Undo not scoped by origin', blast: 'Ctrl-Z reverts a collaborator\'s sentence -- reported as data loss, and correctly so.', fix: 'Track operation origin and restrict undo to the local client\'s own operations.' },
        { mode: 'No authorisation on the relay', blast: 'Any client with the document id can write, since the merge function validates nothing.', fix: 'Server-authoritative relay that authenticates the connection and authorises writes before persisting and rebroadcasting.' },
        { mode: 'Client reconnects with a state vector older than the retained history', blast: 'Merge fails or silently drops updates; that client\'s offline work is lost.', fix: 'Detect a pre-horizon state vector, refuse the merge, and reload from snapshot -- with an explicit warning if unsynced local work exists.' }
      ]
    },

    {
      t: 'staff',
      md: `The trap in this question is enthusiasm. Candidates who have read about CRDTs describe
...
shipped one.`
...

    {
      ...
      items: [
        {
          q: 'Alice rewrites a paragraph while Bob deletes the same paragraph. Both are offline, then both sync. Using Yjs, what is the outcome?',
          options: [
            'The deletion wins, since deletions take precedence.',
            'The replicas converge to the same state -- Bob\'s deletions plus Alice\'s surviving insertions -- which is coherent but is not what either person intended.',
            'A conflict is raised for the user to resolve.',
            'The replicas diverge permanently.'
          ],
          answer: 1,
        ...
        {
          q: 'Why does OT require a central server while a CRDT does not?',
          options: [
            'OT operations are larger and need server compression.',
            'OT transforms an operation against the specific sequence of operations committed since its base revision, so correctness depends on one authoritative order.',
            'CRDTs cannot be persisted on a server.',
            'OT cannot work offline at all.'
          ],
          answer: 1,
          why: 'The transform function is defined relative to a concrete history, so every participant must agree on that history -- which is what the server provides. CRDT merge is commutative, associative and idempotent, so any arrival order in any grouping yields the same state and no authority is needed for convergence. Note this says nothing about authorisation or durability, which is why production CRDT systems still run a server.'
        },
        {
          q: 'A collaborative document has been edited for two years. Load time has crept to 28 seconds. Most likely cause and fix?',
          options: [
            'Too many awareness updates; throttle cursors.',
            'The full update log is replayed on load; snapshot periodically, truncate behind it, and load snapshot plus tail.',
            'The CRDT algorithm is incorrect; switch to OT.',
            'IndexedDB is too slow; move to OPFS.'
          ],
          answer: 1,
          why: 'CRDT persistence is append-only, so a naive implementation replays every historical update at load and cost grows with document lifetime rather than document size. Snapshotting merges accumulated updates into one encoded state you can load in one step. It also gives you the horizon needed to compact tombstones, since you can declare that clients more than one snapshot behind must reload rather than merge.'
        },
        {
          q: 'Which of these does a CRDT give you for free?',
          options: [
            'Enforcement that only editors, not viewers, can change the document.',
            'Convergence of all replicas to the same state regardless of message order or duplication.',
            'Per-user undo.',
            'Bounded memory growth over the document\'s lifetime.'
          ],
          answer: 1,
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `This builds directly on the socket lifecycle, resume and backpressure work in
...
**Deployment, Rollout & Migration** problem.`
  ...
  flashcards: [
    { q: 'Why does full-document last-write-wins lose edits?', a: 'Each client sends a complete state built from what it last saw, so the later write asserts values for text the other user changed. One person\'s edit is overwritten with no error. Sending operations rather than states is what makes both preservable.' },
    { q: 'What does Operational Transformation do?', a: 'A central server rewrites an incoming operation against every operation committed since its base revision -- adjusting indices so intent survives -- then applies it. Clients mirror the transform for concurrent remote operations.' },
    { q: 'Why is OT risky to implement yourself?', a: 'You need a transform function for every ordered pair of operation types, so the matrix grows quadratically, each entry must satisfy transformation property 1, and a single wrong entry diverges only under specific concurrent timing. Several published algorithms shipped incorrect.' },
    { q: 'How does a sequence CRDT avoid index-based positions?', a: 'Every element gets a globally unique immutable id -- client id plus counter -- and inserts are expressed relative to a neighbour\'s id. Deletes leave tombstones, because removing an element would invalidate operations referencing it.' },
    { q: 'Name four CRDT families and their use.', a: 'LWW-Register for scalar fields where one value must win; PN-Counter for increments that must all count; OR-Set for tags and collaborator lists; sequence CRDTs such as RGA or YATA for text and lists.' },
    { q: 'What do CRDTs not solve?', a: 'Intent -- convergence is not meaning; authorisation -- the merge function rejects nothing; garbage collection -- tombstones grow monotonically; rich-text semantics across concurrent structural edits; and per-user undo.' },
    { q: 'Why server-authoritative rather than peer-to-peer for a CRDT document?', a: 'You need one enforceable authorisation point, durable storage when nobody is online, an audit trail, and debuggability. Peer-to-peer additionally brings NAT traversal, TURN relays and N² mesh growth. Use peer-to-peer for awareness only.' },
    { q: 'What problem do snapshots solve?', a: 'Two. Load time, since append-only update logs otherwise replay the document\'s entire history. And garbage collection, since a snapshot defines a horizon past which clients must reload rather than merge, which makes tombstone compaction safe.' },
    { q: 'Why must undo be scoped by origin?', a: 'Users expect ctrl-Z to revert their own last change, not the document\'s. Without origin tracking you undo whatever a collaborator just typed, which users correctly report as data loss.' }
  ],

  drills: [
    {
      prompt: 'You are adding realtime collaboration to an existing rich-text document editor. Documents average 4,000 words, up to 12 people edit concurrently, and some users work offline on flights for six hours. Documents also have per-section permissions: legal can edit the terms section, sales cannot. Design it.',
      probes: [
