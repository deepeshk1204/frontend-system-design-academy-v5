export default {
  blocks: [
    {
      t: 'prose',
      md: `A microfrontend architecture splits one website into several independently built and
...
you pay for that.`
    ...
    {
      t: 'prose',
      md: `Take a single frontend repository with 120 engineers committing to it. The symptoms are
...
decision belongs to whoever owns the org chart, not whoever owns the webpack config.`
    },
    {
      t: 'note',
      tone: 'info',
      title: 'Conway\'s Law, stated usefully',
      md: `Conway observed that a system's structure mirrors the communication structure of the
...
it is expensive and invisible.`
    ...
    { t: 'h', text: 'The integration taxonomy' },
    {
      t: 'prose',
      md: `"Microfrontend" is not one technique. There are six meaningfully different ways to
...
whole page.`
  ...
  WC --> PAGE
  IF --> PAGE`,
      caption: 'Move down the diagram and you gain deploy independence while losing shared-runtime efficiency.'
    },
    {
      t: 'table',
      title: 'Choosing an integration style',
      cols: ['Style', 'How it composes', 'Deploy independence', 'Bundle cost', 'Isolation', 'Best fit'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      md: `Notice that iframes -- the option every architecture document dismisses in a sentence --
...
correct one.`
    ...
    { t: 'h', text: 'The shell / feature contract' },
    {
      t: 'prose',
      md: `Everything that makes a microfrontend system survivable lives in one artefact: the
...
A usable contract answers these questions explicitly.`
    },
    {
      ...
      cols: 2,
      items: [
        { b: 'Mount and unmount', md: 'The shell calls `mount(el, props)` and expects a `unmount()` back. Leaked timers, listeners and subscriptions on unmount are the single most common MFE memory bug.' },
        { b: 'What the shell provides', md: 'Auth token accessor, current user, locale, feature-flag client, telemetry client, router, theme tokens. Features must never construct these themselves.' },
        { b: 'Routing ownership', md: 'Shell owns the top-level path segment; the feature owns everything under it. The shell must not know that `/billing/invoices/:id` exists.' },
        { b: 'Error and loading behaviour', md: 'Every remote is wrapped in an error boundary with a named fallback. A feature failing to load degrades to a message in its own region, never a blank page.' },
        { b: 'Shared dependency policy', md: 'Which packages are singletons, which version ranges are acceptable, and who is allowed to bump them. Usually React, the router and the design system.' },
        { b: 'Telemetry identity', md: 'Every error, metric and trace carries the owning team and the feature version, so a page-level regression is attributable in one query.' }
      ]
    },
    {
      t: 'code',
      lang: 'ts',
      title: 'A contract worth publishing as a versioned package',
      code: `// @acme/mfe-contract — the only thing both sides depend on.
...
export interface FeatureModule {
  ...
  unmount(el: HTMLElement): void;   // must remove every listener it added
    ...
    { t: 'h', text: 'The duplicate-React problem' },
    {
      t: 'prose',
      md: `This is the failure that turns a microfrontend demo into a support ticket, so it is worth
...
lock-step upgrades across ten teams.`
    ...
    {
      t: 'numbers',
      title: 'What duplication actually costs, minified + brotli',
      items: [
        { v: '~45 kB', k: 'react + react-dom 18', note: 'Per duplicated copy' },
        { v: '~11 kB', k: 'react-router v6', note: 'Breaks routing if duplicated, not just size' },
        { v: '~14 kB', k: 'A typical emotion/styled runtime', note: 'Duplicate class-name registries' },
        { v: '~180 kB', k: 'Realistic overhead, 4 remotes duplicating a shared set', note: '≈400 ms extra parse+exec on a mid-tier Android device' }
      ]
    },
    {
      t: 'note',
      tone: 'danger',
      md: `The performance argument for microfrontends is usually stated backwards. People say "each
...
and then defend performance actively -- do not expect performance as a side effect.`
    ...
    { t: 'h', text: 'Routing, deep links and cross-feature communication' },
    {
      t: 'prose',
      md: `Routing is where the org chart leaks into the URL. The stable arrangement is that the
...
For communication between features, the ranking is not close.`
    ...
    {
      t: 'table',
      title: 'Cross-microfrontend communication, best to worst',
      cols: ['Mechanism', 'Coupling', 'When it is right', 'The trap'],
      rows: [
    ...
    { t: 'h', text: 'Design-token drift' },
    {
      t: 'prose',
      md: `Independently deployed features will look different, and the divergence is usually invisible
...
unavoidable -- but colour, spacing and typography stay coherent, which is what people actually see.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Duplicate React in the shared scope', blast: 'Invalid hook call, or silent context fallback across a whole feature.', fix: 'Declare `react`/`react-dom`/router as `singleton: true`; assert at boot that the realm holds one copy and log loudly if not.' },
        { mode: 'Remote `remoteEntry.js` 404s or times out', blast: 'Shell hangs or white-screens if the import is on the critical path.', fix: 'Error boundary plus timeout per remote, a named fallback UI, and a pinned last-known-good manifest.' },
        { mode: 'Shell ships a breaking contract change', blast: 'Every remote built against the old contract breaks at once -- the opposite of the promised isolation.', fix: 'Semver the contract, support N-1 for at least one deploy cycle, and run remotes against contract fixtures in CI.' },
        { mode: 'Remote leaks listeners on unmount', blast: 'Memory growth and duplicated network calls as users navigate between features.', fix: 'Contract-test `unmount`: mount and unmount 50 times and assert listener and timer counts return to baseline.' },
        { mode: 'Design-token version skew', blast: 'One page with three spacing scales and two brand blues.', fix: 'Tokens as CSS custom properties from the shell; a CI check that fails a remote pinning a major behind.' },
        { mode: 'CSS global leakage between features', blast: 'One team\'s `.button` restyles another team\'s page.', fix: 'Hashed class names or scoped-by-default styling; ban unscoped element selectors; forbid global resets outside the shell.' },
        { mode: 'Two remotes each fetch `/api/me`', blast: 'Duplicated requests and inconsistent user state on the same screen.', fix: 'Shell owns session and passes it through the contract; a shared query client keyed by a documented convention.' }
      ]
    },

    {
      t: 'staff',
      md: `The interviewer is usually testing whether you will reach for microfrontends because they
...
between a contract enforced by a compiler and a contract enforced by hope.`
...

    {
      ...
      items: [
        {
          q: 'A 12-engineer team wants microfrontends because their bundle is 2.1 MB and LCP is 4 s. What is the best response?',
          options: [
            'Split by route so each team ships less code.',
            'Microfrontends will likely make it worse; fix it with route-level code splitting and a bundle budget.',
            'Adopt Module Federation with everything marked as shared.',
            'Move to iframes for isolation.'
          ],
          answer: 1,
          why: 'Bundle size is a code-splitting problem, and route-level lazy chunks solve it inside one deployable with no runtime contract. Microfrontends add a shell boot cost plus duplicated shared dependencies, so the critical path usually grows. At 12 engineers there is also no queueing problem to solve -- one team does not block itself.'
        },
        {
          q: 'Two remotes each bundle their own React 18 copy. A shared ThemeProvider in the shell appears to work, but one remote always renders the light theme. Why?',
          options: [
            'The remote loaded before the provider mounted.',
            'Context identity is per React copy, so the consumer resolves a different context object and gets its default value.',
            'CSS custom properties do not cross remote boundaries.',
            'The theme provider needs `singleton: true` on itself.'
          ],
          answer: 1,
          why: 'A React context is an object identity created by a specific React instance. Consumer and provider from different copies never match, so the consumer falls back to the `createContext` default -- silently, with no error. This is why React, the router and any context-based library must be singletons in the shared scope, and why a boot-time assertion that only one React exists in the realm is worth the twenty lines.'
        },
        {
          q: 'Which routing arrangement preserves deploy independence?',
          options: [
            'The shell enumerates every route and maps each to a remote component.',
            'The shell owns path prefixes from a static manifest and delegates the rest to each remote\'s router.',
            'Each remote registers its routes with the shell at runtime after loading.',
            'A shared routes package imported by shell and remotes.'
          ],
          answer: 1,
          why: 'Prefix delegation means adding a page inside a remote needs no shell change. Enumerating routes centrally or sharing a routes package reintroduces the coordinated deploy. Runtime registration sounds elegant but breaks cold deep links: the shell must know which remote owns `/billing/invoices/9f2` *before* fetching any remote code, which is exactly what a static prefix manifest gives you.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The mechanics of runtime composition -- shared scope, version negotiation, manifests and
...
needs a feature-and-version dimension on every event.`
  ...
  flashcards: [
    { q: 'What do microfrontends actually buy you?', a: 'Independent deployment. A team ships without waiting on another team\'s build, tests or release train. Not performance, not smaller bundles, not simplicity -- those are costs you pay for the autonomy.' },
    { q: 'Why does duplicate React break context silently?', a: 'A context is an object identity created by one React instance. A provider from copy A and a consumer from copy B never match, so the consumer gets the `createContext` default value with no error thrown.' },
    { q: 'Which packages must be singletons in a shared scope?', a: 'Anything holding module-level mutable state or relying on identity: `react`, `react-dom`, the router, the styling runtime, and the query client. Pure leaf utilities can duplicate cheaply.' },
    { q: 'Best-to-worst cross-microfrontend communication?', a: 'URL/query state, then the server plus cache invalidation, then a shell-provided service, then namespaced custom DOM events. Direct imports between features are effectively never right.' },
    { q: 'How do you stop design-token drift across remotes?', a: 'Ship tokens as CSS custom properties from the shell so components read `var(--ds-space-3)` instead of importing a number. Token changes then propagate on shell deploy regardless of bundled component versions.' },
    { q: 'What is the honest default instead of microfrontends?', a: 'A modular monolith: one deployable, enforced module boundaries, code owners per directory, affected-only CI. You get ownership clarity while keeping a compile-time contract.' },
    { q: 'What conditions justify a real runtime split?', a: 'Roughly eight or more teams shipping to one URL, a heterogeneous stack being strangled incrementally, or a compliance/trust boundary that forbids sharing a runtime.' },
    { q: 'Why are iframes sometimes the right microfrontend?', a: 'They are the only option with a separate JS realm and origin, so a crash or a hostile script is contained. When you host untrusted or third-party code, that isolation beats bundle efficiency.' }
  ],

  drills: [
    {
      prompt: 'You join a company with 140 frontend engineers across 11 teams shipping one logged-in SaaS app. CI takes 28 minutes, trunk is red about twice a day, and releases go out weekly on Thursdays. The CTO has already announced microfrontends. Walk me through what you would do in your first quarter.',
      probes: [
    ...
    {
      prompt: 'A federated remote owned by another team starts throwing during module evaluation after their deploy. Your shell white-screens for every user, including on routes that remote does not own. Post-incident, what do you change?',
      probes: [
