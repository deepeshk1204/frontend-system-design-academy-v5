export default {
  blocks: [
    {
      t: 'prose',
      md: `Fine-tuning means continuing to train a model on your own examples so that its weights
...
domain vocabulary it kept getting subtly wrong.`
    ...
    {
      t: 'prose',
      md: `Prompting has a ceiling, and you hit it in a specific way. The prompt that reliably
...
is the main thing people get wrong.`
  ...
  PREF -->|yes| DPO["Preference optimisation on top"]`,
      caption: 'Work down the tree. Most quality gaps resolve before the fine-tuning branch, and the most common win is the distillation branch.'
    },
    {
      t: 'prose',
      md: `Two branches of that tree deserve emphasis because they are where the real decisions
happen.
...
an uncertain outcome, and you should size it accordingly.`
    ...
    {
      t: 'table',
      title: 'What each adaptation method actually changes',
      cols: ['Method', 'Changes', 'Needs', 'Good for', 'Weak at'],
      rows: [
    ...
    { t: 'h', text: 'What supervised fine-tuning actually does' },
    {
      t: 'prose',
      md: `Mechanically, SFT is the same next-token prediction objective used in pre-training,
...
is not tidiness -- it is the specification.`
    ...
    { t: 'h', text: 'LoRA and QLoRA' },
    {
      t: 'prose',
      md: `Full fine-tuning updates every weight, so you need optimiser state for all of them --
...
The parameter arithmetic is the reason it changed everything:`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'LoRA parameter count, worked',
      code: `One attention projection in a 70B-class model: d = k = 8192
...
Rank selection, empirically:
  ...
  r = 64-128 larger behavioural shifts; diminishing returns and overfitting risk
    ...
    {
      t: 'prose',
      md: `**QLoRA** goes further: quantise the frozen base weights to 4-bit and train the LoRA
...
trained against, or re-evaluate after the change rather than assuming it transfers.`
    ...
    { t: 'h', text: 'Preference optimisation' },
    {
      t: 'prose',
      md: `SFT teaches from demonstrations: "given this input, produce this output." Some things
...
sent, or the response they regenerated versus the one they kept.`
    ...
    { t: 'h', text: 'Distillation: the most common real win' },
    {
      t: 'prose',
      md: `Distillation is fine-tuning a small model on the outputs of a large one. You take the
...
and a bad one for an open-ended assistant.`
    ...
    { t: 'h', text: 'The dataset is the project' },
    {
      t: 'prose',
      md: `Expect roughly 90% of the effort to be data work. Teams consistently budget for GPU
...
because the only meaningful question is before-versus-after on identical cases.`
    ...
    {
      t: 'numbers',
      title: 'Figures to plan against',
      items: [
        { v: '~500-1,000', k: 'Minimum useful SFT examples for style or format', note: 'Consistency matters more than count' },
        { v: '2k-10k', k: 'Typical range for task specialisation', note: 'Returns flatten; quality dominates' },
        { v: '~0.05-0.5%', k: 'Trainable parameters with LoRA', note: 'Depends on rank and targeted modules' },
        { v: '16-32', k: 'LoRA rank that covers most cases', note: '4-8 for style, 64+ rarely worth it' },
        { v: '1-3', k: 'Epochs before overfitting risk rises', note: 'Watch validation loss, not training loss' },
        { v: '~90%', k: 'Share of project effort that is data work', note: 'Not GPU time. Budget accordingly' },
        { v: 'order of 10x', k: 'Cost reduction from a good distillation', note: 'Plus a large latency improvement' }
      ]
    },
    {
      t: 'code',
      lang: 'python',
      title: 'A LoRA SFT run, with the settings that matter',
...
# is how you catch capability regression.`
    ...
    { t: 'h', text: 'Catastrophic forgetting and capability regression' },
    {
      t: 'prose',
      md: `Training on a narrow distribution degrades performance outside it. Fine-tune
...
actively worse.`
    ...
    { t: 'h', text: 'Serving many adapters on one base model' },
    {
      t: 'prose',
      md: `The operational superpower of LoRA is that adapters are small and the base is shared.
...
base, re-run your evals rather than assuming the merged artefact is equivalent.`
    ...
    { t: 'h', text: 'The maintenance cost nobody quotes' },
    {
      t: 'prose',
      md: `A fine-tune is not a deliverable, it is a dependency with an expiry date, and this is
...
is the better engineering decision.`
    ...
    {
      t: 'tradeoffs',
      title: 'Fine-tuning versus prompting plus retrieval',
      gains: [
    ...
    {
      t: 'failures',
      title: 'How fine-tuning projects fail',
      items: [
        { mode: 'Fine-tuning to inject knowledge', blast: 'Model is confidently wrong in a more plausible way, cannot cite sources, and facts cannot be updated without retraining.', fix: 'Retrieval for facts, fine-tuning for form. If the requirement includes citations or freshness, it is a RAG requirement.' },
        { mode: 'Inconsistent dataset formatting', blast: 'Unpredictable output format in production; the model learned that several formats are acceptable.', fix: 'Enforce a single schema and a single closing convention with a linter over the dataset; treat formatting as the specification.' },
        { mode: 'Capability regression outside the target slice', blast: 'Target task improves 15% while tool calling or instruction following quietly breaks.', fix: 'Run the full eval suite plus a dedicated general-capability regression set; lower LR, fewer epochs, load the best checkpoint not the last.' },
        { mode: 'Train/test contamination', blast: 'Eval score looks excellent, production does not improve, and nobody can explain the gap.', fix: 'Split before training, de-duplicate by near-match rather than exact string, and keep a held-out set that is never used for iteration.' },
        { mode: 'Distilling unfiltered teacher output', blast: 'Student faithfully reproduces the teacher\u2019s errors, including malformed outputs.', fix: 'Apply your deterministic eval checks to teacher outputs and drop failures before training.' },
        { mode: 'Base model deprecated', blast: 'Adapter is unusable; a re-train and full re-qualification lands as unplanned work.', fix: 'Keep the dataset and training pipeline reproducible from a script and version-controlled; treat the base model as a dependency with a lifecycle.' },
        { mode: 'Over-training', blast: 'Model memorises the training set, becomes brittle on paraphrases, and refuses to generalise.', fix: 'Validation-based early stopping, 1-3 epochs, and evaluate on paraphrased variants of training inputs to detect memorisation.' },
        { mode: 'No baseline before starting', blast: 'Cannot demonstrate the fine-tune helped, and a cheaper prompt fix might have closed the gap.', fix: 'Measure the base model on the identical harness per slice first, and try prompt and few-shot fixes before committing to weights.' },
        { mode: 'Merging an adapter into a quantised base unchecked', blast: 'Quality shifts silently because higher-precision updates were folded into lower-precision weights.', fix: 'Re-run evals on the merged artefact; serve against the same quantisation you trained against.' }
      ]
    },

    {
      t: 'staff',
      md: `Fine-tuning is the topic where enthusiasm is most expensive, so interviewers listen for
...
maintenance cost rather than being asked for it.`
...

    {
      ...
      items: [
        {
          q: 'Your assistant does not know your internal product terminology or your API details, and makes up endpoint names. What should you do?',
          options: [
            'Fine-tune on the API documentation so the model learns the endpoints.',
            'Retrieval over the API docs for the facts, and optionally a small fine-tune for vocabulary and response format.',
            'Full SFT on all internal documentation.',
            'Use DPO to prefer answers that mention real endpoints.'
          ],
          answer: 1,
        ...
        {
          q: 'Which project has the highest probability of success?',
          options: [
            'Fine-tuning a 7B model to beat a frontier model at general reasoning.',
            'Distilling a frontier model into a small model for a single high-volume classification task, using filtered teacher outputs on real production inputs.',
            'Fine-tuning to make the model know your company\u2019s current org chart.',
            'DPO on 200 preference pairs where annotators disagreed about half the time.'
          ],
          answer: 1,
          why: 'Distillation is the highest-success-rate adaptation project because the hard part -- the dataset -- is solved by having an oracle: you can generate as much labelled data as you can afford, on your real input distribution, with a measurable quality target. Beating a frontier model at general reasoning with a small model is not a realistic goal. An org chart is mutable facts and belongs in retrieval. And 200 noisy preference pairs is far too little signal, especially where annotators disagree -- that disagreement means there is no consistent preference to learn.'
        },
        {
          q: 'Why does LoRA reduce training memory by roughly two orders of magnitude?',
          options: [
            'It quantises the base model to 4-bit.',
            'Optimiser state scales with *trainable* parameters, and only the small low-rank matrices are trainable.',
            'It trains on fewer examples.',
            'It uses a smaller base model.'
          ],
          answer: 1,
          why: 'Optimiser state -- momentum and variance for something like Adam -- plus gradients are allocated per trainable parameter, and they dominate training memory. Freezing the base and training only rank-r matrices cuts trainable parameters to a fraction of a percent, so that dominant term nearly disappears. Four-bit quantisation of the frozen base is QLoRA, a separate and complementary trick that shrinks the *inference* copy of the weights; the two stack, which is why a 70B LoRA fits on one 80 GB GPU.'
        },
        {
          q: 'After a successful fine-tune that improves your target task 18%, what is the most important thing to check before shipping?',
          options: [
            'Whether the adapter file size is reasonable.',
            'Performance on capabilities the fine-tune was not meant to affect -- instruction following, tool-call formatting, refusal behaviour.',
            'That training loss reached a new minimum.',
            'Inference latency of the merged model.'
          ],
          answer: 1,
          why: 'Narrow training degrades behaviour outside the trained distribution, and the damage is invisible if you only measure the slice you were targeting. A fine-tune that gains 18% on support replies and breaks tool-call formatting is a net loss for the product. Keep a standing general-capability regression set and run the full suite. Training loss is actively misleading here -- a new minimum can mean memorisation, which is why you watch validation loss and load the best checkpoint rather than the last.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The facts-versus-form boundary is the dividing line with **RAG: The Reference
...
Preference data is best harvested from the implicit signals in **AI Observability & Tracing**.`
  ...
  flashcards: [
    { q: 'The one-line rule for fine-tuning versus retrieval?', a: 'Fine-tuning teaches form and behaviour; retrieval supplies facts. If the requirement involves citations, freshness or per-document permissions, it is a retrieval requirement and weights are the wrong place for it.' },
    { q: 'What does SFT mechanically change?', a: 'Next-token prediction on your examples, with loss usually masked to the response tokens. It shifts the probability distribution over continuations so outputs look like your responses -- excellent for structure, tone and schema, poor for facts.' },
    { q: 'How does LoRA work, and what does the parameter count look like?', a: 'Freeze W, learn A (r x k) and B (d x r), effective weight W + (alpha/r)·BA. For an 8192x8192 projection at r=16 that is about 260k trainable parameters instead of 67M -- roughly 0.4% for that matrix, and 0.05-0.5% across a whole model.' },
    { q: 'What do rank and alpha control?', a: 'Rank r is adapter capacity: 4-8 for style, 16-32 as the common default for task specialisation, 64+ rarely worth the overfitting risk. Alpha scales the adapter contribution; the alpha/r ratio is what matters, not either value alone.' },
    { q: 'Why does QLoRA make large-model fine-tuning affordable?', a: 'The base weights are quantised to 4-bit and frozen, and only the higher-precision adapters are trained. Since the base is never updated, quantising it costs much less than quantising a model you intend to train fully -- a 70B LoRA then fits on one 80 GB GPU.' },
    { q: 'DPO versus RLHF, and when to use either?', a: 'RLHF trains a reward model then optimises with RL -- powerful, complex, prone to reward hacking. DPO gets a similar objective in closed form from preferred/rejected pairs with a KL anchor, and is now the default. Use either *after* SFT, and only for taste you cannot demonstrate.' },
    { q: 'Why is distillation the most common production win?', a: 'You have an oracle. The hard part of fine-tuning is the dataset, and a teacher generates unlimited labelled data on your real input distribution with a measurable quality target -- typically yielding ~10x lower cost and much lower latency on a narrow task.' },
    { q: 'How do you detect capability regression after fine-tuning?', a: 'Run the full eval suite plus a standing regression set of capabilities the fine-tune was never meant to touch -- instruction following, tool-call formatting, refusal behaviour, unrelated task types. Watch validation loss, use 1-3 epochs, load the best checkpoint not the last.' },
    { q: 'What is the maintenance cost of a fine-tune?', a: 'Adapters are specific to a base architecture and checkpoint, so a base deprecation forces re-training, re-evaluation and re-qualification. Meanwhile a general model two generations newer with a good prompt may match your fine-tune for free.' }
  ],
