export default {
  blocks: [
    {
      t: 'prose',
      md: `An eval is a test suite for a system that does not give the same answer twice. You
...
verify or refute.`
    ...
    {
      t: 'prose',
      md: `The playground lies to you, and it lies in a specific way: you type inputs you already
...
alarm you get is the one you built.`
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'The test you are actually replacing',
      md: `In deterministic software the compiler catches type errors, unit tests catch logic
...
on top of your tests -- for the non-deterministic part of your system they *are* the tests.`
    ...
    { t: 'h', text: 'The golden dataset' },
    {
      t: 'prose',
      md: `Everything rests on the dataset, and the dataset is where the actual work is. A good
...
quietly deleted eight failing cases are a real and common form of self-deception.`
    ...
    {
      t: 'numbers',
      title: 'Dataset sizing that works in practice',
      items: [
        { v: '30-50', k: 'Enough to start and find real bugs', note: 'Day one; a bad prompt fails this badly' },
        { v: '100-300', k: 'Typical steady-state CI set', note: 'Runs in minutes, catches most regressions' },
        { v: '~1,000+', k: 'When you need to detect small deltas', note: 'Below this, a 2% change is inside the noise' },
        { v: '70/30', k: 'Development versus held-out split', note: 'Held-out is run pre-release only' },
        { v: '5-8', k: 'Slices worth reporting separately', note: 'Aggregate scores hide slice regressions' },
        { v: '~2 weeks', k: 'Cadence for refreshing from production', note: 'Traffic shifts; a stale set stops representing users' }
      ]
    },
    {
      t: 'note',
      tone: 'good',
      title: 'Start today, not when it is perfect',
      md: `Thirty examples in a JSON file with a script that prints a pass rate is worth more
...
how the set grows into something valuable, and it means every bug is permanently regression-tested.`
    ...
    { t: 'h', text: 'The metric hierarchy: use the cheapest sufficient check' },
    {
      t: 'prose',
      md: `New teams reach for LLM-as-judge immediately, which is the expensive, noisy,
hard-to-trust option. Work down this list and stop at the first level that answers your question.`
    },
    {
      t: 'table',
      title: 'Four levels of evaluation, cheapest first',
      cols: ['Level', 'What it checks', 'Cost and latency', 'Trust', 'Use for'],
      rows: [
    ...
    {
      t: 'prose',
      md: `The under-appreciated move is pushing work *up* the list. "Did the answer cite a real
...
the same time.`
    },
    {
      t: 'code',
      lang: 'javascript',
      title: 'Deterministic assertions catch more than people expect',
      code: `// Level 1 -- runs in microseconds, never flaky, no model call.
...
export function mrr(retrievedIds, relevantIds) {
  ...
  return rank === -1 ? 0 : 1 / (rank + 1);
    ...
    { t: 'h', text: 'Evaluate retrieval separately from generation' },
    {
      t: 'prose',
      md: `This is the most common structural mistake in RAG evaluation, and it makes debugging
...
rather not write them yourself.`
  ...
  D -->|"recall fine, low groundedness"| F2["Fix prompt, model, context order"]`,
      caption: 'Two stages, two metric sets, two different fixes. One blended score tells you nothing actionable.'
    },
    {
      t: 'note',
      tone: 'info',
      title: 'The diagnostic that saves days',
      md: `Run generation once with your retriever and once with the *known correct* documents
...
to spend a sprint on the wrong half of the system.`
    ...
    { t: 'h', text: 'LLM-as-judge, done properly' },
    {
      t: 'prose',
      md: `Sometimes there is no reference answer -- "write a release note from these commits"
...
regressed.`
    },
    {
      t: 'code',
      lang: 'python',
      title: 'Pairwise judge with position-swap control',
      code: `JUDGE_PROMPT = """You are comparing two answers to the same question.
...
def win_rate(examples, judge_model):
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'The honest caveat you should say out loud',
      md: `An LLM judge is a noisy proxy with correlated errors, not a measurement instrument.
...
by reading ten outputs yourself.`
    ...
    { t: 'h', text: 'Evals in CI, and the non-determinism problem' },
    {
      t: 'prose',
      md: `Evals earn their keep when they block a merge. The trigger set is anything that can
...
release.`
    ...
    {
      t: 'table',
      title: 'What to gate on, and how hard',
      cols: ['Signal', 'Gate', 'Rationale'],
      rows: [
    ...
    },
    {
      t: 'code',
      lang: 'yaml',
      title: 'CI wiring -- what triggers what',
      code: `# .github/workflows/evals.yml
on:
  pull_request:
    paths:
...
jobs:
  ...
  nightly:
    ...
    { t: 'h', text: 'Online evaluation: the only score that is real' },
    {
      t: 'prose',
      md: `Offline evals tell you whether you broke something. They cannot tell you whether users
...
between an eval suite that reflects your users and one that reflects the sprint you wrote it in.`
    ...
    { t: 'h', text: 'Eval-driven development' },
    {
      t: 'prose',
      md: `Put together, the workflow inverts. You do not build the feature and then measure it.
...
away from an unnoticed regression.`
    },
    {
      ...
      title: 'The loop',
      items: [
        'Write 30-50 eval cases from real traffic, including known-hard and should-refuse cases, before building.',
        'Build the simplest version. Record the baseline score per slice and the noise floor from two identical runs.',
        'Read the twenty worst failures by hand. Categorise them -- missing facts, wrong form, wrong reasoning, bad retrieval.',
        'Fix the largest category with the cheapest mechanism that addresses it, and re-score on the same frozen set.',
        'Promote every check you can from judge to deterministic assertion as the failure modes become clear.',
        'Gate CI on the metrics that stabilised; run the held-out set only before release.',
        'Ship behind a flag, watch implicit signals, and feed production failures back as new eval cases.'
      ]
    },

    {
      t: 'tradeoffs',
      title: 'Investing in an eval suite',
      gains: [
    ...
    {
      t: 'failures',
      title: 'How eval programmes go wrong',
      items: [
        { mode: 'Overfitting to the eval set', blast: 'Score climbs for a quarter while user complaints climb too; the number has stopped tracking reality.', fix: 'Strict 70/30 split with a held-out set run only before release; refresh from production every couple of weeks; treat a suspiciously high score as a bug.' },
        { mode: 'Single blended end-to-end score', blast: 'Quality drops and nobody can tell whether retrieval or generation caused it; a sprint is spent on the wrong half.', fix: 'Separate retrieval metrics from generation metrics; run the known-correct-context diagnostic to attribute the loss.' },
        { mode: 'Uncalibrated LLM judge', blast: 'A change ships on a 6% judge improvement that humans cannot perceive, or a real regression passes unseen.', fix: 'Human-label 100-200 examples, report judge-human agreement, and require it to beat inter-human agreement before trusting the judge.' },
        { mode: 'Flaky gate from single-run scoring', blast: 'CI fails randomly, engineers learn to re-run until green, the gate is now decorative.', fix: 'N repeats with distributions; establish the noise floor from two identical runs; gate on a lower bound, not a point estimate.' },
        { mode: 'Aggregate score hides a slice', blast: 'Overall metric up 1%, the hard 5% of traffic that drives support volume down 20%.', fix: 'Stratified dataset with mandatory per-slice reporting; gate per slice, not only on the mean.' },
        { mode: 'Dataset drifts away from traffic', blast: 'Suite passes everything while users hit a whole intent class that has no test cases.', fix: 'Scheduled resampling from production traces; auto-create candidate cases from thumbs-down, escalations and regenerate events.' },
        { mode: 'Judge or model changed underneath you', blast: 'Historical scores become incomparable and a trend line is now fiction.', fix: 'Pin dated snapshots for both system and judge; record model, prompt and dataset versions with every result; re-baseline explicitly on change.' },
        { mode: 'No cost or latency in the eval', blast: 'A 4% quality win ships alongside a 3x cost increase that finance discovers next month.', fix: 'Report tokens, cost and p95 latency per example beside quality; make the trade an explicit decision in the pull request.' }
      ]
    },

    {
      t: 'staff',
      md: `Evals are where interviews separate people who have shipped AI from people who have
...
metrics are proxies with named biases, and you are willing to say which numbers you do not trust.`
...

    {
      ...
      items: [
        {
          q: 'Your RAG feature scores 0.72 on an end-to-end quality metric and you need it higher. What do you measure first?',
          options: [
            'Try three larger models and keep whichever scores best.',
            'Retrieval recall@k against labelled relevant documents, and generation quality with correct context injected by hand.',
            'Increase the judge\u2019s rubric detail to get a more precise score.',
            'Collect more user thumbs-down to find the pattern.'
          ],
          answer: 1,
          why: 'A blended score cannot be debugged. Splitting the stages tells you where the loss is: if recall@5 is 0.6 your generation ceiling is roughly 0.6, and no model swap fixes that -- the work is chunking, hybrid search and reranking. If recall is high but groundedness is low, the context is arriving and being ignored, which is prompt, context ordering or model work. The hand-injected-context run resolves the question in an afternoon.'
        },
        {
          q: 'A prompt change improves your LLM-judge score from 7.1 to 7.4 on 80 examples, single run each. Ship it?',
          options: [
            'Yes -- a 0.3 improvement on 80 examples is a solid result.',
            'No -- absolute scores compress, one run gives no variance estimate, and 80 examples cannot resolve a small delta.',
            'Yes, if the judge is a larger model than the system model.',
            'No, because LLM judges can never be used in a decision.'
          ],
          answer: 1,
          why: 'Three problems stack. Absolute scoring bunches everything into 7-8 so the scale carries little information. A single run per example gives no variance, and the run-to-run noise floor on a set this size is plausibly larger than 0.3 -- you would find that out by running the unchanged system twice. And 80 examples is thin for a small effect. The fix is pairwise against the frozen production prompt with position swapping, N repeats, and a lower-bound gate. Judges are usable; uncalibrated single-run absolute judges are not.'
        },
        {
          q: 'Which is the strongest production quality signal for an AI writing assistant?',
          options: [
            'Thumbs-up rate on generated drafts.',
            'The fraction of generated text surviving unedited into the sent document, plus the regenerate rate.',
            'Average session length.',
            'Token count per response.'
              ],
          answer: 1,
          why: 'Implicit behavioural signals are dense, unbiased by who chooses to click, and tied to the job the user came to do. Edit-survival directly measures usefulness; regenerate directly measures dissatisfaction. Thumbs are collected from single-digit percentages of interactions, skewed to the delighted and the furious, so the ratio tells you about your sampling. Session length is genuinely ambiguous -- engagement or struggle -- and token count is not a quality metric at all. Use thumbs as a funnel for new eval cases rather than as a score.'
        },
        {
          q: 'You are switching embedding models to improve retrieval. Which eval gate matters most before merge?',
          options: [
            'Pairwise judge win rate on final answers.',
            'recall@k and MRR on the labelled retrieval set, re-indexed with the new model, reported per slice.',
            'Schema conformance of the generated output.',
            'p95 end-to-end latency.'
          ],
          answer: 1,
          why: 'An embedding change alters the retrieval stage and nothing else directly, so measure that stage with its own metrics -- and it demands a full re-index, making it the most expensive change to get wrong. Per-slice reporting is essential here because embedding models differ sharply by domain and language: aggregate recall can improve while your non-English or code-heavy slice collapses. The downstream judge score is worth reporting but it is a laggy, noisy view of a change you can measure directly.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Eval datasets are sourced from the traces described in **AI Observability &
...
**Model Routing, Caching & Cost Control**.`
  ...
  flashcards: [
    { q: 'Why is the playground not a test?', a: 'Sample size of about four, inputs you already had in mind, no held-out data, and the most biased possible evaluator -- you, reading charitably because you know what you meant. It cannot detect a silent regression in a behaviour you were not looking at.' },
    { q: 'What is the metric hierarchy, and what is the rule?', a: 'Deterministic assertions, then code-based metrics, then LLM-as-judge, then human review. Use the cheapest sufficient one, and keep promoting checks upward -- "citations are real" is a set-membership test, not a judgement call.' },
    { q: 'Why score retrieval separately from generation?', a: 'One blended number cannot distinguish "never retrieved the right doc" from "retrieved it and ignored it", and those have different fixes. recall@k caps every generation metric: if recall@5 is 0.6, no model choice gets you much above 0.6.' },
    { q: 'Three biases in LLM-as-judge and their controls?', a: 'Position bias -- run both orders and require the preference to survive the swap. Verbosity bias -- normalise or instruct against length, and check the win rate is not just tracking token count. Self-preference -- use a different, frozen, dated judge model than the system model.' },
    { q: 'Why is pairwise better than absolute scoring?', a: 'Absolute 1-10 judges compress everything into 7-8, so movements are small and uninterpretable. Pairwise against a frozen production baseline asks a question models are reliable at and yields a win rate you can reason about statistically.' },
    { q: 'How do you stop an eval gate from being flaky?', a: 'Run each example N times (3-5) and use distributions. Establish the noise floor by running the *unchanged* system twice -- any delta smaller than that is meaningless -- then gate on a lower bound rather than a point estimate.' },
    { q: 'Why keep a held-out set?', a: 'Iterating against every example overfits your prompt to your test set, and the score keeps climbing while real quality does not. Split roughly 70/30 and run the held-out portion only before release.' },
    { q: 'Why are thumbs-up/down nearly worthless as a quality metric?', a: 'Single-digit response rates and severe selection bias toward the delighted and the furious, so the ratio measures your sampling. Use implicit signals instead -- edit survival, regenerates, rephrase-and-ask-again, abandonment, escalation -- and use thumbs to collect eval cases.' },
    { q: 'What does eval-driven development actually look like?', a: 'Write cases before building, get a baseline from the simplest version, read the twenty worst failures and categorise them, then fix the largest category with the cheapest mechanism and re-score on the same frozen set. Failures tell you what to build.' }
  ],

  drills: [
    {
      prompt: 'You inherit a customer-support RAG assistant with no evals. It answers roughly 40,000 questions a week over a 90,000-document knowledge base, support leadership says quality "feels like it got worse after the last index rebuild", and nobody can confirm it. You have three weeks. Build the eval programme.',
      probes: [
    ...
    {
      prompt: 'Your provider announces the model you use is deprecated in eight weeks. The replacement is cheaper and scores better on public benchmarks. You have 14 prompts in production across four product surfaces, and one of them drives a compliance-sensitive workflow. Plan the migration.',
      probes: [
