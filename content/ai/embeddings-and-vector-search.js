/* AI track — Embeddings & Vector Search. See CONTENT-SCHEMA.md. */

export default {
  blocks: [
    {
      t: 'prose',
      md: `An embedding is a list of numbers -- typically a few hundred to a few thousand
...
about those three facts.`
    ...
    {
      t: 'prose',
      md: `Keyword search matches strings. A user asking "why is my card declining" against a
...
subject of *Advanced Retrieval*.`
    ...
    { t: 'h', text: 'What the numbers actually mean' },
    {
      t: 'prose',
      md: `No individual dimension is interpretable. There is no "formality axis" at index 47.
...
and is a strong match for none of them. Chunking quality is upstream of retrieval quality.`
    ...
    { t: 'h', text: 'Distance metrics and when normalisation matters' },
    {
      t: 'table',
      cols: ['Metric', 'Formula intuition', 'Sensitive to magnitude?', 'Use when'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'The metric mismatch bug',
      md: `If you normalise your vectors to unit length at insert time but configure the index
...
mismatch never throws an error.`
    ...
    { t: 'h', text: 'Why exact search stops working, and the curse of dimensionality' },
    {
      t: 'prose',
      md: `Exact k-nearest-neighbour search is a scan: compute the distance from the query to
...
single number you must be able to state about your index, and most teams cannot.`
    ...
    {
      t: 'numbers',
      title: 'Where the regimes change (rough, CPU, 768-dim)',
      items: [
        { v: '< ~100K', k: 'Exact scan is fine', note: 'Simpler, exact, no index to rebuild' },
        { v: '~100K-10M', k: 'HNSW in memory', note: 'Millisecond queries, high recall' },
        { v: '~10M-1B', k: 'IVF-PQ or DiskANN', note: 'Compression or SSD becomes mandatory' },
        { v: '95-99%', k: 'Recall@10 you should target', note: 'State it, do not assume it' }
      ]
    },

    { t: 'h', text: 'HNSW, explained properly' },
    {
      t: 'prose',
      md: `Hierarchical Navigable Small World graphs (Malkov & Yashunin, 2016) are the default
...
work. Query cost ends up roughly logarithmic in N.`
    ...
    {
      t: 'diagram',
      caption: 'Coarse hops at the top, fine refinement at the bottom. efSearch controls how wide the layer-0 beam is.',
      code: `flowchart TB
  ...
  F --> R["top-k results<br/>recall set by efSearch"]`
    ...
    {
      t: 'table',
      title: 'The three HNSW parameters that decide everything',
      cols: ['Parameter', 'What it controls', 'Raising it', 'Typical range'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'good',
      title: 'The one that saves you',
      md: `\`efSearch\` is a per-query parameter. That means you can run cheap, low-recall search
...
lowers it under pressure rather than dropping requests.`
    ...
    {
      t: 'table',
      title: 'Illustrative recall vs latency curve — measure your own, these are shapes not promises',
      cols: ['`efSearch`', 'Recall@10', 'Relative query latency', 'Reasonable use'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'info',
      title: 'How to actually measure recall',
      md: `Sample 1,000 queries from production. Run each against an exact scan -- that is your
...
shift, because recall on today's corpus tells you nothing about recall after you triple it.`
    ...
    { t: 'h', text: 'IVF-PQ and DiskANN: when memory is the constraint' },
    {
      t: 'prose',
      md: `HNSW is fast because the whole graph lives in RAM. Past a few tens of millions of
...
is usually worth it above ~50M vectors.`
    ...
    { t: 'h', text: 'Memory maths: do this before you choose anything' },
    {
      t: 'code',
      lang: 'js',
      title: 'Worked example — 20M chunks, 1536-dim embeddings',
      code: `const N = 20_000_000;       // chunks
...
// ~10 GB of RAM instead of ~128 GB, at the cost of one extra pass.`
    ...
    {
      t: 'table',
      title: 'Quantisation: storage saved vs recall paid',
      cols: ['Technique', 'Size vs float32', 'Typical recall impact', 'Notes'],
      rows: [
    ...
    { t: 'h', text: 'Filtering: the recall trap nobody warns you about' },
    {
      t: 'prose',
      md: `Real queries are never "nearest neighbours in the whole corpus". They are "nearest
...
that fallback is where your p99 latency lives.`
    },
    {
      t: 'note',
      tone: 'danger',
      title: 'Post-filtering plus permissions is a correctness bug, not a perf issue',
      md: `If your ACL check is applied after retrieval, a user in a small workspace will get
...
filter during traversal. This is developed at length in *Advanced Retrieval*.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'pgvector: HNSW index, filtered search, and the parameters that matter',
...
LIMIT 20;`
    ...
    { t: 'h', text: 'pgvector or a dedicated vector database?' },
    {
      t: 'table',
      cols: ['Dimension', 'pgvector (Postgres)', 'Dedicated (Qdrant, Weaviate, Milvus, managed services)'],
      rows: [
    ...
    { t: 'h', text: 'The operational facts that bite later' },
    {
      t: 'prose',
      md: `**Changing the embedding model means reindexing everything.** Vectors from two models
...
tombstone ratio; it is a real operational metric with a real rebuild trigger.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Post-filtering by tenant or ACL', blast: 'Users in small workspaces get few or zero results; looks like "search is broken for some customers".', fix: 'Denormalise the filter field into the index and filter during traversal (`iterative_scan` in pgvector, payload filters in Qdrant). Alert on the rate of queries returning fewer than k rows.' },
        { mode: 'Embedding model upgraded, index partially re-embedded', blast: 'Rankings become incoherent across the corpus with no error anywhere.', fix: 'Blue/green index with a version tag on every vector; refuse queries against a mixed-version index; flip by alias only after a recall comparison.' },
        { mode: 'Recall never measured', blast: 'Retrieval silently degrades as the corpus grows; downstream LLM answers get worse and the vector store looks healthy.', fix: 'Keep a golden query set with exact-scan ground truth; compute recall@10 in CI and on a schedule; alert on drift.' },
        { mode: 'Vectors not normalised, index uses dot product', blast: 'Long documents systematically outrank short relevant ones. No error, just steadily worse results.', fix: 'Normalise at write time; use the metric the model card specifies; record the metric in index metadata and assert it at startup.' },
        { mode: 'High-churn corpus, never compacted', blast: 'Tombstoned nodes accumulate; memory grows and latency creeps up over months.', fix: 'Track deleted-node ratio; schedule rebuild or compaction at a threshold; size capacity for the rebuild, not the steady state.' },
        { mode: 'Matryoshka truncation applied to a non-MRL model', blast: 'Storage drops 3x and quality collapses in a way that looks like a chunking problem.', fix: 'Only truncate models explicitly trained with Matryoshka representation learning; validate recall before and after on the golden set.' },
        { mode: 'Index build exceeds the maintenance window', blast: 'Reindex cannot complete; team ships with a stale or partial index.', fix: 'Measure build time at target N early; use concurrent builds and raise `maintenance_work_mem`; shard so builds parallelise.' }
      ]
    },

    {
      t: 'staff',
      md: `The tell for depth here is whether someone treats the index as a tuned system with a
...
and they treat filtering as a correctness problem rather than a performance one.`
...

    {
      ...
      items: [
        {
          q: 'Enterprise customers with small private workspaces report that search "returns almost nothing", while large customers are fine. Most likely cause?',
          options: [
            '`efSearch` is too high, over-filtering candidates.',
            'The ACL predicate is applied after vector retrieval, so the top-k global neighbours mostly belong to other tenants and get discarded.',
            'Their documents were embedded with a different model.',
            'Cosine distance is the wrong metric for small corpora.'
          ],
          answer: 1,
          why: 'This is the classic post-filter trap, and it scales inversely with tenant size — which is exactly why it looks like a customer-specific bug rather than an architectural one. Fetching the global top 20 and then dropping anything outside the workspace leaves close to nothing when that workspace is 0.1% of the corpus. Push the predicate into traversal: denormalise `workspace_id` and the ACL into the index and filter during search. Alert on the fraction of queries returning fewer than k rows.'
        },
        {
          q: 'You need to cut vector RAM by ~10x on a 50M-vector index while keeping recall@10 above 0.95. Best first approach?',
          options: [
            'Halve `efSearch` and `M`.',
            'Binary or product quantisation for the in-memory traversal, then re-rank the top ~200 candidates against full-precision vectors held on NVMe.',
            'Truncate every vector from 1536 to 256 dimensions.',
            'Switch cosine to dot product.'
          ],
          answer: 1,
          why: 'Quantise-then-rerank is the standard way to buy memory back without giving up ranking quality: the compressed representation only has to get the right candidates into the top 200, and the full-precision pass fixes the ordering. Lowering `M` degrades graph connectivity permanently and saves far less. Blind truncation only works on models trained with Matryoshka representation learning, and the metric choice has no effect on memory at all.'
        },
        {
          q: 'A new embedding model scores meaningfully better on your eval. What does adopting it actually require?',
          options: [
            'Point new writes at the new model; old vectors remain usable.',
            'Re-embed and rebuild the entire index, because vectors from different models share no geometry and a mixed index degrades silently.',
            'Re-normalise the existing vectors to the new dimensionality.',
            'Nothing, as long as dimensionality matches.'
          ],
          answer: 1,
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Vector search is one half of retrieval; the lexical half, reranking and
...
shedding is a lever in **Model Routing, Caching & Cost Control**.`
  ...
  flashcards: [
    { q: 'What is recall@k for an ANN index and how do you measure it?', a: 'The fraction of the true k nearest neighbours that the approximate index returned. Measure it by running a sample of production queries against an exact scan for ground truth, then comparing top-k overlap at each `efSearch`. If you cannot state this number, you do not know what your index is serving.' },
    { q: 'Which HNSW parameter can you change per query, and which is permanent?', a: '`efSearch` is per-query and trades recall for latency freely. `M` is fixed at build time and determines memory and graph connectivity — changing it requires a full rebuild. `efConstruction` affects build quality and build time only.' },
    { q: 'Why does post-filtering break multi-tenant search?', a: 'It retrieves the global top-k then drops rows failing the predicate. With a highly selective filter — a small workspace, a restrictive ACL — almost nothing survives, so users see empty results. It is a correctness bug, not a latency one; push the predicate into graph traversal.' },
    { q: 'Why does high dimensionality defeat kd-trees?', a: 'As dimensions grow, the ratio between nearest and farthest distances approaches 1, so no branch can be safely pruned. The tree ends up examining most of the space, which is a linear scan with worse constants. Hence approximate graph and clustering methods instead.' },
    { q: 'Rough memory for 10M vectors at 768 dimensions in float32 HNSW?', a: 'Vectors are 10M × 768 × 4 bytes ≈ 30.7 GB, plus graph links of roughly N × M × 2 × 8 bytes (≈2.8 GB at M=16), so about 33-35 GB resident. Always do this arithmetic before choosing an index type.' },
    { q: 'What does product quantisation trade?', a: 'It splits each vector into sub-vectors and replaces each with a codebook centroid ID, giving 8-64x compression at a real recall cost. The cost is usually recovered by re-ranking the top few hundred candidates against full-precision vectors.' },
    { q: 'Why can you not incrementally migrate to a new embedding model?', a: 'Different models produce unrelated geometries even at the same dimensionality, so distances between old and new vectors are meaningless. A mixed index ranks incoherently and never errors. You need a full re-embed, blue/green indexes with version tags, and an alias flip.' },
    { q: 'When is pgvector the right choice over a dedicated vector DB?', a: 'Below roughly 10-50M vectors, and especially when chunks, metadata and ACLs must update transactionally with your application data — that removes dual-write bugs around deletes and permission changes. Move only when you have a measured reason: build time, RAM cost, or unachievable recall at your latency target.' },
    { q: 'Why do HNSW deletes degrade an index over time?', a: 'Deletes are typically tombstones: the node stays in the graph and is filtered from results, so a high-churn corpus accumulates dead nodes that cost memory and lengthen traversal. Track the tombstone ratio and rebuild or compact at a threshold.' }
  ],

  drills: [
    {
      prompt: 'You are designing retrieval for a B2B document product: 30M chunks at 1536 dimensions, 200 queries per second at peak, p95 retrieval budget of 150 ms, strict per-document ACLs, and roughly 2% of documents changing daily. Choose an architecture and defend the numbers.',
      probes: [
    ...
    {
      prompt: 'Retrieval quality complaints have risen steadily over two quarters. Nothing was deployed to the retrieval service in that time. The corpus grew from 4M to 22M chunks. Diagnose.',
      probes: [
