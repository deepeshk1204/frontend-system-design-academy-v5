/* AI track — Prompting & Structured Output. See CONTENT-SCHEMA.md. */

export default {
  blocks: [
    {
      t: 'prose',
      md: `A prompt is the program. It is the only thing standing between a general-purpose
...
modes, and treating them that way is most of the difference between a demo and a system.`
    ...
    {
      t: 'prose',
      md: `The model has no application-specific priors. It does not know your tone, your legal
...
answer" precisely because parsing an unconstrained generative model is unbounded work.`
    ...
    { t: 'h', text: 'The system prompt is code' },
    {
      t: 'prose',
      md: `Everything you already do for code applies, and skipping any of it will hurt in a
recognisable way:
...
"what changed", which is the only question that matters during a regression.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'A prompt as a versioned, testable module',
      code: `// prompts/triage.v4.js
export const meta = {
  id: 'support.triage',
  version: 4,
...
// attributable to an exact prompt revision.`
    ...
    { t: 'h', text: 'Prompt structure that actually works' },
    {
      t: 'prose',
      md: `There is no magic incantation, but there is a shape that consistently outperforms
...
full rate.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Before: the prompt everyone writes first',
      code: `Analyse this customer support ticket and tell me what it's about, how urgent
...
Ticket: {{ticket}}`
    ...
    {
      t: 'prose',
      md: `That prompt has four defects, and they are the same four every time. The category
...
internal honesty dial for it to turn.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'After: same task, specified',
      code: `You are the triage step of a support pipeline. You classify exactly one ticket.
...
<ticket>{{ticket}}</ticket>`
    },
    {
      t: 'note',
      tone: 'good',
      title: 'The delimiter is doing real work',
      md: `Wrapping untrusted input in named tags and stating that the content inside is *data*
...
customer pastes an email that happens to contain "ignore previous instructions".`
    ...
    { t: 'h', text: 'Few-shot vs zero-shot: an economics question' },
    {
      t: 'prose',
      md: `Examples are the most reliable way to fix format and edge-case behaviour, and they
...
dynamically per request.`
    ...
    {
      t: 'table',
      title: 'When to spend tokens on examples',
      cols: ['Situation', 'Approach', 'Why'],
      rows: [
    ...
    { t: 'h', text: 'Chain of thought is a latency and cost decision' },
    {
      t: 'prose',
      md: `Asking a model to reason step by step before answering improves accuracy on
...
temperature is the standard recipe for a beautifully argued wrong answer.`
    ...
    { t: 'h', text: 'Three ways to get structured output' },
    {
      t: 'diagram',
      caption: 'The same request under three enforcement regimes. Only the third makes invalid output impossible.',
      code: `flowchart TB
  ...
  R --> V`
    ...
    {
      t: 'table',
      title: 'Structured output: three enforcement levels',
      cols: ['Approach', 'Mechanism', 'Guarantees', 'Costs / limits'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'info',
      title: 'What constrained decoding does and does not fix',
      md: `Masking logits guarantees *shape*, not *truth*. A grammar that requires
...
wrong label, which is strictly worse because it is now machine-readable.`
    ...
    { t: 'h', text: 'Designing a JSON Schema for a model to fill in' },
    {
      t: 'prose',
      md: `A schema written for a model is not the same artefact as a schema written for a
...
- **Bound your arrays.** \`maxItems\` prevents the runaway list that eats your output budget.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'Schema + validate → repair → fail, with a bounded loop',
...
export async function triage(ticket, trace) {
  ...
  throw new SchemaRepairFailed({ route: 'human_queue' });
    ...
    },
    {
      t: 'note',
      tone: 'danger',
      title: 'Never parse model output with a regex',
      md: `The reflex to write \`/\\{[\\s\\S]*\\}/\` and call \`JSON.parse\` on the match is how you get
...
not passed the validator.**`
    ...
    {
      t: 'numbers',
      title: 'What a validate-repair loop typically buys (measure yours)',
      items: [
        { v: '1 retry', k: 'Recovers the large majority of schema failures', note: 'A second retry rarely converges — fall back instead' },
        { v: '2x', k: 'Worst-case latency on a repaired request', note: 'Budget for it in your p99, not your p50' },
        { v: '~0%', k: 'Syntactic failures under constrained decoding', note: 'Semantic errors are unaffected — still validate' },
        { v: 'Every attempt', k: 'Should emit a trace span with the validator issues', note: 'Repair rate is a leading quality indicator' }
      ]
    },

    { t: 'h', text: 'Prompts in production: versioning, rollout, evaluation' },
    {
      t: 'steps',
      ordered: true,
      title: 'Shipping a prompt change safely',
      items: [
        'Write the change against a **golden set** -- 100-300 real inputs with agreed-correct outputs, drawn from production traffic including the cases you got wrong.',
        'Run the eval offline. Report per-category accuracy, schema-repair rate, refusal rate, and p95 output tokens. A prompt that is 2% more accurate and 40% more verbose may be a net loss.',
        'Ship behind a flag with a percentage rollout, holding the model version fixed so you are testing one variable.',
        'Run both versions on a shadow slice of live traffic and compare on **production** inputs, which always contain shapes your golden set does not.',
        'Watch online signals that do not need labels: repair rate, tool-error rate, user edit rate, regeneration rate, abandonment.',
        'Keep the previous version loadable for one-step revert, and stamp `promptVersion` on every trace so historical quality stays attributable.'
      ]
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'A prompt A/B test is a two-variable experiment if you let it be',
      md: `Changing the prompt and the model version in the same rollout makes the result
...
change before your users do.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Enum has no `other` member', blast: 'Model is forced to pick a wrong label; downstream routing sends tickets to the wrong team with high confidence.', fix: 'Always include an escape member; alert when its rate moves, since a rising `other` rate is your taxonomy telling you it is stale.' },
        { mode: 'Required field the input does not contain', blast: 'Invented order IDs and customer names enter your system as well-typed data.', fix: 'Make it nullable, instruct "copy verbatim or null", validate the format with a regex *in the validator*, and verify existence against the system of record.' },
        { mode: 'Regex-extracted JSON from a fenced response', blast: 'Silent wrong-object parse; symptoms appear far from the cause.', fix: 'Constrained decoding plus a schema validator. Nothing reaches business logic unvalidated.' },
        { mode: 'Unbounded repair retries', blast: 'A malformed-input class produces a latency and cost spike that looks like an outage.', fix: 'Cap at one repair; deterministic fallback path; emit a metric per attempt.' },
        { mode: 'Prompt edited in a vendor console', blast: 'Quality regression with no diff, no author and no revert path.', fix: 'Prompt lives in the repo, is code-reviewed, and its version is stamped on every trace.' },
        { mode: 'Untrusted text concatenated into the instruction region', blast: 'Injected instructions change classification or leak the system prompt.', fix: 'Delimit and label input as data; keep authority out of the model — authorise the *action*, not the text (see *AI Security & Guardrails*).' },
        { mode: '`rationale` placed after `decision` in the schema', blast: 'Chain of thought becomes decoration; accuracy gain silently disappears.', fix: 'Order reasoning fields before the fields they justify — JSON is generated left to right.' }
      ]
    },

    {
      t: 'staff',
      md: `Anyone can say "we use JSON mode". The Staff-level version is treating the prompt as a
...
this rather than integrated it once.`
...

    {
      ...
      items: [
        {
          q: 'You enable strict JSON-schema constrained decoding. Which class of bug does it *not* eliminate?',
          options: [
            'Responses wrapped in a markdown code fence.',
            'Enum values outside the declared set.',
            'A syntactically valid but factually invented `order_id`.',
            'Missing required keys.'
          ],
          answer: 2,
          why: 'Constrained decoding masks logits so only tokens that keep the output schema-valid can be sampled — that handles fences, missing keys and illegal enum members by construction. It says nothing about truth. A `string` matching your pattern can still be an order ID that does not exist, so you need existence checks against the system of record, and nullable fields so the model has a legal way to say "absent".'
        },
        {
          q: 'Your extraction step has a 6% schema-repair rate. The team proposes raising max retries from 1 to 4. What is the better first move?',
          options: [
            'Raise retries — repairs are cheap relative to a failed request.',
            'Look at the validator issues on the failing 6%: they usually cluster on one field, which is a schema or prompt defect, not a retry-count problem.',
            'Raise temperature so the model explores different formats.',
            'Switch to prompt-and-pray with a tolerant parser.'
          ],
          answer: 1,
          why: 'Repair failures are rarely uniform. Grouping validator issues by field path almost always shows one culprit — a required field that should be nullable, an enum with no escape member, or an ambiguous description. Fixing that removes the failures instead of paying twice for them. Extra retries add cost and p99 latency while hiding the signal, and the repair rate is one of your best unlabelled quality indicators.'
        },
        {
          q: 'Which prompt ordering is best for both accuracy and cost?',
          options: [
            'Current timestamp, then user question, then system rules, then examples.',
            'System rules and examples first (byte-stable), then retrieved context, then the user turn last.',
            'User question first so the model knows the goal immediately.',
            'Interleave rules between each retrieved document for emphasis.'
          ],
          answer: 1,
          why: 'Two effects align here. Prefix caching requires a byte-identical prefix, so stable instructions and examples must come first and must not contain an interpolated timestamp — that discount is often an order of magnitude on those tokens plus a TTFT saving. And position effects mean the most recent, most specific content benefits from being last, adjacent to the generation point.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Structured output is the same machinery as **Tool Calling & Typed Actions** -- a
...
boundary is the opening move of **AI Security & Guardrails**.`
  ...
  flashcards: [
    { q: 'What does JSON mode guarantee, and what does it not?', a: 'It guarantees the output parses as JSON. It does not guarantee the declared keys are present, the types are right, enum values are legal, or the content is true. Always follow it with a real schema validator.' },
    { q: 'How does constrained decoding work?', a: 'A grammar or finite-state machine tracks the partial output and masks the logits at each step so only tokens that keep the output schema-valid can be sampled. Invalid output becomes impossible rather than unlikely — see Outlines, XGrammar, llguidance in vLLM, or `response_format: json_schema` on hosted APIs.' },
    { q: 'Why should a `rationale` field come before a `decision` field?', a: 'JSON is generated token by token in key order. Reasoning generated before the decision conditions the decision on it. Reasoning generated after is a post-hoc rationalisation of a label already chosen, so you pay for chain of thought and get no accuracy.' },
    { q: 'Why must every enum include an `other` member?', a: 'A closed enum with no escape forces the model to pick a wrong label when nothing fits, converting honest uncertainty into confident, machine-readable error. A rising `other` rate is also a useful signal that your taxonomy has drifted from reality.' },
    { q: 'What belongs in a schema `description` field?', a: 'The instruction for that specific field — "copy verbatim or null, never infer", "one sentence, max 300 chars". Descriptions are part of the prompt and sit adjacent to the field they govern, which survives refactoring far better than a rules paragraph elsewhere.' },
    { q: 'Why cap the repair loop at one retry?', a: 'A single repair with the validator\'s own error text recovers most schema failures; a second rarely converges. Unbounded retries turn a bad-input class into a latency and cost spike, and mask the repair rate that should be telling you the schema has a defect.' },
    { q: 'Why is regex extraction of JSON dangerous?', a: 'A regex has no model of nesting. It will match an inner object inside a string field, or the first of two objects, and hand you something that parses cleanly and means the wrong thing — a silent failure whose symptoms show up far from the cause.' },
    { q: 'What is the cost of chain-of-thought reasoning?', a: 'Reasoning tokens are output tokens — the expensive kind — generated serially in the decode phase, so tripling reasoning roughly triples time to the visible answer. Worth it for multi-step problems, wasteful for classification and extraction.' },
    { q: 'Why pin the model version inside the prompt module?', a: 'So a prompt A/B test has one variable. Provider aliases roll to new builds without a deploy of yours; if the model moves during a prompt experiment the result is uninterpretable, and a silent provider change looks like a prompt regression.' }
  ],

  drills: [
    {
      prompt: 'You are adding an LLM step that reads an inbound invoice email and emits `{vendor_id, amount_cents, currency, due_date, confidence}` for an automated payments pipeline that pays anything above 0.9 confidence without human review. Design the prompt, the schema and the safety envelope.',
      probes: [
    ...
    {
      prompt: 'Support quality dropped noticeably over the last three weeks. Three engineers have edited the triage prompt in the provider console during that time, and the model is configured as `gpt-5.1` with no date suffix. Walk through how you would find the cause and what you would change structurally.',
      probes: [
