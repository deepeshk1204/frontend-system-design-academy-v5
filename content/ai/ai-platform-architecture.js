export default {
  blocks: [
    {
      t: 'prose',
      md: `An AI platform is the shared layer that sits between your product teams and the models
...
platform itself fails.`
    ...
    {
      t: 'prose',
      md: `Watch how this happens, because it is the same story everywhere.
...
a gateway that 60% of traffic bypasses provides 0% of the governance.`
  ...
  EV --> RG`,
      caption: 'One ingress for policy, then specialised services behind it. The eval plane feeds the registry, which is what makes staged rollout possible.'
    },
    {
      t: 'prose',
      md: `Everything a product team sends goes through one ingress, which is the only way to
...
they have genuinely different scaling and ownership characteristics. Let us take them in order.`
    ...
    { t: 'h', text: 'The gateway, and why it is non-negotiable' },
    {
      t: 'prose',
      md: `The gateway is the only component that is not optional. It is where every control that
...
telemetry outage cannot take down inference.`
    },
    {
      t: 'code',
      lang: 'yaml',
      title: 'Gateway policy -- declarative, versioned, reviewed',
      code: `# Policy is data in git, not code in the gateway. Reviewable by security,
...
features:
  ...
  code_assistant:
    ...
    { t: 'h', text: 'Router, provider abstraction and caching tiers' },
    {
      t: 'prose',
      md: `The **router** turns a logical request -- "the support assistant needs a reply" -- into a
...
will implement a global semantic cache and create a cross-tenant leak.`
    ...
    {
      t: 'table',
      title: 'Build versus buy, component by component',
      cols: ['Component', 'Default', 'Reasoning'],
      rows: [
    ...
    { t: 'h', text: 'Shared retrieval with tenant isolation' },
    {
      t: 'prose',
      md: `Retrieval is the component teams most often want to build themselves and most often get
...
change is the most expensive change in the whole platform and deserves the heaviest gate.`
    ...
    { t: 'h', text: 'Tool runtime and the prompt registry' },
    {
      t: 'prose',
      md: `The **tool runtime** is where the security model from **AI Security & Guardrails**
...
correctness-critical artefact. Draft in a UI if you like; ship through a pull request.`
    ...
    { t: 'h', text: 'Cost attribution and chargeback' },
    {
      t: 'prose',
      md: `Because the gateway sees every call, it can compute cost per request at write time and
...
attacker-controllable in a way an ordinary endpoint's is not.`
    ...
    {
      t: 'numbers',
      title: 'Platform figures worth planning against',
      items: [
        { v: '5-15 ms', k: 'Acceptable gateway overhead budget', note: 'Trivial next to a model call; keep policy reads cached' },
        { v: '99.95%+', k: 'Gateway availability target', note: 'It is in the path of every AI feature you own' },
        { v: '2-4', k: 'Engineers to run a platform like this', note: 'Beyond ~5 consuming teams it pays for itself' },
        { v: '3-6 months', k: 'Realistic time to stage the migration', note: 'Gateway first, retrieval last' },
        { v: '100%', k: 'Traffic that must go through the gateway', note: 'A bypassed gateway provides zero governance' },
        { v: '1-5%', k: 'Production sampling into the eval plane', note: 'Plus all errors and negative-feedback traces' },
        { v: '<1 day', k: 'Target for a model-deprecation config change', note: 'The test of whether routing is really configuration' }
      ]
    },

    { t: 'h', text: 'Governance that is not a bottleneck' },
    {
      t: 'prose',
      md: `Every organisation past a certain size grows an AI review process, and most of them
...
can evidence and one you hope is true.`
    ...
    { t: 'h', text: 'The migration: three teams to a platform' },
    {
      t: 'prose',
      md: `You do not get to build this greenfield. You get three teams already in production with
...
they want.`
    },
    {
      ...
      items: [
        '**Stage 0 -- inventory (1-2 weeks).** Find every place a model is called: repos, notebooks, cron jobs, that one Lambda. Record model, provider, key, data class, traffic and owner. You will find at least one thing nobody remembered. Produce the list and get agreement that it is complete.',
        '**Stage 1 -- transparent gateway (3-4 weeks).** Ship an OpenAI-compatible proxy that does auth, tenant tagging, telemetry, cost attribution and retries, and nothing else opinionated. Migration is a base URL and a key. Sell it on what teams get for free: a cost dashboard, traces, correct retry behaviour, and no API keys in their code. Aim for 100% of traffic before adding a single restriction.',
        '**Stage 2 -- quotas, policy and redaction (3-4 weeks).** Now that you see everything, add per-tenant budgets with staged degradation, provider and region policy per data class, and PII redaction before egress. This is where the compliance question gets a real answer. Roll out in report-only mode first, so you find out what you would have blocked before you block it.',
        '**Stage 3 -- prompt and config registry (2-3 weeks).** Move prompts and model identifiers out of application code into versioned registry entries with staged rollout. The immediate payoff that makes teams want it: a prompt change no longer needs a deploy, and it can be canaried and reverted in minutes.',
        '**Stage 4 -- eval plane (4-6 weeks).** Sample production traffic into datasets, wire the eval runner into CI, and gate registry promotion on passing suites. Start with the one team that has the most quality pain; make their life visibly better and the others will ask to join. Do not mandate this early -- eval suites built under duress are worthless.',
        '**Stage 5 -- router and caching (3-4 weeks).** With evals in place you can safely introduce cascades, prefix-cache-friendly prompt assembly and exact-match caching, because you can now *prove* a routing change did not regress quality. Doing this before stage 4 is how you ship a silent quality regression.',
        '**Stage 6 -- shared retrieval and tool runtime (ongoing).** Last, because it is the most invasive and the most tenant-sensitive. Migrate one team\u2019s corpus at a time, dual-run the old and new retrieval paths, and compare recall@k on a labelled set before switching. Bring tools under the runtime as each team needs a new one, rather than rewriting all of them at once.'
      ]
    },
    {
      t: 'note',
      tone: 'good',
      title: 'Why this order and not the obvious one',
      md: `The instinct is to build the impressive parts first -- the router, the shared retrieval
...
you start saying no.`
    ...
    { t: 'h', text: 'Who owns what' },
    {
      t: 'prose',
      md: `The org question determines whether the platform survives, and the failure mode is
...
meet. Without one, they will build a shadow path and you will not know about it.`
    ...
    {
      t: 'tradeoffs',
      title: 'Centralising AI infrastructure',
      gains: [
    ...
    {
      t: 'failures',
      title: 'How the platform itself fails',
      items: [
        { mode: 'Gateway as single point of failure', blast: 'Every AI feature in the company is down simultaneously.', fix: 'Stateless multi-region deployment, policy evaluated from cached config with a stale-if-error path, asynchronous telemetry that fails open, and a documented direct-provider break-glass with audit.' },
        { mode: 'Teams bypassing the gateway', blast: 'Governance coverage silently drops; the compliance claim becomes false while the dashboard still looks complete.', fix: 'Provider keys held only by the platform and rotated, egress restricted at the network layer to platform paths, and -- most importantly -- make the platform the easiest path.' },
        { mode: 'Noisy neighbour', blast: 'One tenant\u2019s batch job consumes the shared rate-limit pool and every other tenant sees latency and 429s.', fix: 'Per-tenant quotas and concurrency limits, a separate queue class for batch work, fair-share scheduling, and per-tenant capacity reservation for enterprise tiers.' },
        { mode: 'Version skew', blast: 'A gateway change breaks one team\u2019s integration mid-incident; nobody can say which version any consumer is on.', fix: 'Versioned platform API with a deprecation window, contract tests run against consumers in CI, and consumer version recorded on every span.' },
        { mode: 'Platform team as bottleneck', blast: 'AI delivery across the company slows to the platform team\u2019s throughput; teams start building shadow paths.', fix: 'Self-serve for low-risk tiers, product teams own prompts and eval sets, automated governance checks instead of human review, and a documented escape hatch.' },
        { mode: 'Config change rolled out globally', blast: 'A routing or prompt change hits 100% of every feature at once and degrades several products.', fix: 'Treat config as code: canary percentage, eval gate before promotion, instant revert, and a config change log visible in the same dashboard as deploys.' },
        { mode: 'Cost attribution wrong or disputed', blast: 'Chargeback is contested, teams stop trusting the platform, and the numbers get ignored.', fix: 'Compute cost at write time on the span with the price-table version recorded; reconcile monthly against provider invoices; start with showback before chargeback.' },
        { mode: 'Shared semantic cache without tenant scoping', blast: 'Cross-tenant answer leakage -- a security incident originating in a cost optimisation.', fix: 'Tenant and permission scope mandatory in every cache key, semantic caching opt-in per feature and restricted to whitelisted intents, every hit logged for audit.' },
        { mode: 'Retrieval ACL filter omitted on one code path', blast: 'Cross-tenant document exposure through a single missing predicate.', fix: 'A single shared query-builder applications cannot bypass, filters applied inside the query, and a continuous job verifying retrieved doc IDs against caller entitlements.' },
        { mode: 'Eval plane treated as optional', blast: 'Routing, prompt and model changes ship unmeasured; quality drifts with nobody accountable.', fix: 'Eval gate wired into registry promotion, so the only way to ship a prompt or model change is through a passing suite.' }
      ]
    },

    {
      t: 'staff',
      md: `This is the question that decides a Staff or Senior Staff loop, because it is
...
as SPOF, noisy neighbour, version skew, platform-as-bottleneck -- volunteered rather than extracted.`
...

    {
      ...
      items: [
        {
          q: 'Three teams call model providers directly. You have been asked to build an AI platform. What is your first deliverable?',
          options: [
            'A shared retrieval service, since all three teams need RAG.',
            'A transparent OpenAI-compatible gateway doing auth, tenant tagging, telemetry, cost attribution and retries -- migration is a base URL change.',
            'A model router with cascades to cut cost immediately.',
            'A governance review process for all AI features.'
          ],
          answer: 1,
          why: 'You cannot control or optimise what you cannot see, and you cannot prove any later change helped without a baseline. A transparent proxy is also the only version teams will adopt voluntarily, because it costs them a base URL and gives them dashboards, traces and correct retry behaviour for free. Get to 100% coverage before adding restrictions -- a gateway that 60% of traffic bypasses provides none of the governance you built it for. Retrieval is the most invasive component and belongs last; routing before evals ships silent quality regressions; and governance without visibility is unenforceable.'
        },
        {
          q: 'Which components should you build rather than buy?',
          options: [
            'Provider adapters and the tracing backend, since they are core to the platform.',
            'The gateway policy layer, the prompt/config registry, cost attribution, and your eval datasets.',
            'Everything -- vendor dependencies are a risk in AI infrastructure.',
            'Nothing -- assemble entirely from off-the-shelf components.'
          ],
          answer: 1,
          why: 'Build what encodes knowledge only you have: your tenancy and residency policy, your rollout and approval process, your pricing and chargeback rules, and your evaluation datasets -- which are the actual asset and are unavoidably yours. Buy what is translation or commodity: provider adapters (LiteLLM), tracing (OpenTelemetry plus a backend), eval runners, indexes, sandboxes, and above all the serving engine. Writing your own provider adapters is maintenance with zero differentiation; writing your own inference engine is a category error.'
        },
        {
          q: 'Your gateway now handles all AI traffic for twelve teams. What is the most important reliability property to design for?',
          options: [
            'Horizontal scalability to handle peak token throughput.',
            'That the gateway is stateless and multi-region, policy is evaluated from cached config, and telemetry export fails open -- plus a documented break-glass path.',
            'Detailed logging of every request payload for debugging.',
            'A sophisticated caching layer to reduce provider calls.'
          ],
          answer: 1,
          why: 'The gateway is now a single point of failure in front of every AI feature in the company, so the design must ensure nothing non-essential can take it down. Cached policy with a stale-if-error path means a config-store outage does not stop inference; asynchronous fail-open telemetry means an observability outage does not either; stateless multi-region means you can lose a region. The break-glass direct-provider path -- documented, audited, rarely used -- is what stops a gateway incident becoming a company-wide AI outage. Full payload logging is actively harmful here: latency on the hot path plus a PII liability.'
        },
        {
          q: 'A product team complains that shipping any AI feature now requires a platform team ticket and takes three weeks. What does this indicate?',
          options: [
            'The team needs better training on the platform.',
            'The platform has become a bottleneck; low-risk features must self-serve and product teams must own their own prompts and eval sets.',
            'This is the expected cost of governance.',
            'The platform needs more engineers.'
          ],
          answer: 1,
          why: 'A platform that gates every feature has traded one problem for a worse one, and the predictable outcome is shadow paths you cannot see -- losing the governance that justified the platform. The fix is structural, not staffing: risk-tier the process so internal, read-only, human-in-the-loop features self-serve against automated CI checks, and move prompt and eval-dataset ownership to the teams with the domain knowledge. The health test is whether a team can ship a low-risk AI feature without talking to the platform team at all. Adding engineers to a bottleneck designed as a queue just makes a faster queue.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `This capstone assembles the whole track. The gateway's quota and degradation logic is
...
and multi-adapter serving is **Fine-Tuning, LoRA & Distillation**.`
  ...
  flashcards: [
    { q: 'Why is a gateway non-negotiable at org scale?', a: 'Auth, tenant resolution, quota, residency policy, PII redaction, retries, telemetry and audit are cross-cutting and correctness-critical. A control implemented per application is one the next team will forget -- and a gateway 60% of traffic bypasses provides 0% of the governance.' },
    { q: 'What is the first deliverable of an AI platform, and why?', a: 'A transparent OpenAI-compatible proxy doing auth, tenant tagging, telemetry, cost attribution and retries -- nothing opinionated. Migration costs a base URL and a key, and you get 100% visibility before adding any restriction. Visibility, then control, then optimisation.' },
    { q: 'Build versus buy, in one rule?', a: 'Build what encodes your tenancy, policy, rollout and pricing model -- gateway policy layer, registry, cost attribution, eval datasets. Buy translation and commodity infrastructure -- provider adapters, tracing backends, eval runners, indexes, sandboxes. Never write a serving engine.' },
    { q: 'Why must routing be configuration rather than code?', a: 'So a model deprecation is a one-day config change with a staged rollout and eval gate, not an eight-team coordination project. The gateway should refuse raw provider model names and accept only logical names the registry resolves.' },
    { q: 'What three properties does a prompt registry need?', a: 'Immutable versioning, so a trace recording v17 can be replayed exactly; staged rollout with a canary percentage, because a prompt change is a production change; and eval gating on promotion, which wires the eval plane into the deployment path.' },
    { q: 'How do you isolate tenants in shared retrieval?', a: 'Tiered: namespaces for the long tail with ACL filters applied inside the query through one shared query-builder applications cannot bypass, dedicated indexes for large enterprise tenants, separate infrastructure where contracts require it -- plus a continuous job verifying retrieved doc IDs against caller entitlements.' },
    { q: 'Who owns prompts and eval datasets?', a: 'Product teams. They require domain knowledge the platform team does not have, and centralising authorship creates a queue and produces worse prompts. Platform owns the mechanism and defaults; security and legal own the model-and-data-class catalogue, not per-feature review.' },
    { q: 'How do you keep AI governance from becoming a bottleneck?', a: 'Risk-tier it. Internal, read-only, human-in-the-loop, no-PII features self-serve against automated CI checks. Reserve human review for customer-facing autonomous action, regulated workflows, new data classes and new providers. Approval is a catalogue entry, not a meeting.' },
    { q: 'Name four ways the platform itself fails, and would you start with showback or chargeback?', a: 'Gateway as SPOF (stateless multi-region, cached policy with stale-if-error, fail-open telemetry, audited break-glass); noisy neighbour (per-tenant quotas, separate batch queue class); version skew (versioned API, contract tests, consumer version on every span); platform-as-bottleneck (self-serve tiers plus a documented escape hatch). Start with showback -- most of the behavioural benefit, far less friction, and time to reconcile attribution against provider invoices before anyone\u2019s budget depends on your numbers.' }
  ],
