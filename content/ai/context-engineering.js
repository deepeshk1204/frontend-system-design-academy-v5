/* AI track — Context Engineering. See CONTENT-SCHEMA.md. */

export default {
  blocks: [
    {
      t: 'prose',
      md: `Context engineering is the discipline of deciding what goes into the model's context
...
surprised that quality degraded gradually rather than failing loudly.`
    ...
    {
      t: 'prose',
      md: `Two things happened at once. Context windows grew from a few thousand tokens to
...
makes this call correct".`
    ...
    { t: 'h', text: 'The budget table' },
    {
      t: 'prose',
      md: `Write this table down for your own application. The act of writing it is most of the
value, because it forces every consumer to justify its allocation against the others.`
    },
    {
      t: 'table',
      title: 'Example allocation for an agentic assistant on a 200K window — the budget is what you ENFORCE, not what fits',
      cols: ['Consumer', 'Typical', 'Hard cap', 'Stable prefix?', 'Eviction policy'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'Reserve the output budget before you fill the input',
      md: `Input and output come out of the same window. A request that fits perfectly and then
...
count with the real tokeniser at the boundary, not a 4-chars-per-token estimate.`
    ...
    { t: 'h', text: 'Position effects and context rot' },
    {
      t: 'prose',
      md: `The "lost in the middle" finding (Liu et al., 2023) showed that a model's ability to
...
turn three will be used at turn thirty.`
    ...
    {
      t: 'prose',
      md: `Compaction is replacing a span of history with a shorter representation that preserves
...
the decisions already made.`
    ...
    {
      t: 'prose',
      md: `The strongest pattern in practice is **structured state plus externalisation**. Rather
...
in production.`
    },
    {
      t: 'code',
      lang: 'js',
      title: 'Threshold-triggered compaction with a pinned head and structured state',
...
export async function prepareContext(session) {
  ...
  return [
    ...
    { t: 'h', text: 'Memory architecture' },
    {
      t: 'prose',
      md: `"Memory" gets used for four different things that have different stores, lifetimes and
...
but forgets what it did two steps ago.`
    },
    {
      ...
      items: [
        { b: 'Working memory', md: 'The current context window: this turn\'s task, recent exchanges, active tool results. Lifetime is the session. It *is* the window — everything else is about what gets promoted into it.' },
        { b: 'Episodic memory', md: 'What happened in past sessions. "Last Tuesday you helped me migrate the billing schema." Stored as summarised session records with timestamps, retrieved by similarity and recency. Users notice its absence immediately.' },
        { b: 'Semantic memory', md: 'Durable facts about the user or organisation: role, timezone, preferred language, product tier, team ownership. Small, high-value, cheap to always include — and it needs an edit and forget path for privacy.' },
        { b: 'Procedural memory', md: 'How to do things here: learned workflows, house style, corrections the user has made repeatedly. Belongs in the system prompt or a skill file, not in a retrieval store, because it must be reliably present rather than probabilistically retrieved.' }
      ]
    },
    {
      t: 'prose',
      md: `Two rules keep this honest. **Retrieve memory, do not stuff it.** A user with two
...
because it persists, is trusted, and nobody knows where it came from.`
    ...
    { t: 'h', text: 'Prompt-prefix caching: the largest cost lever you control' },
    {
      t: 'prose',
      md: `Providers cache the processed state of a prompt prefix. If your next request begins
...
ordering from an aesthetic choice into a cost decision.`
    ...
    {
      t: 'diagram',
      caption: 'Order by volatility. Everything after the first changed byte is re-prefilled and re-billed.',
      code: `flowchart TB
  ...
  F -.-> MISS`
    },
    {
      t: 'note',
      tone: 'danger',
      title: 'The one-line change that costs six figures',
      md: `Putting \`Current time: 2026-09-13T14:22:07Z\` at the top of your system prompt
...
cache hit rate you do not have.`
    ...
    {
      t: 'numbers',
      title: 'What prefix ordering is worth (verify against your provider\'s current pricing)',
      items: [
        { v: '~10x', k: 'Typical discount on cached input tokens', note: 'Applies only to the byte-identical prefix' },
        { v: '>70%', k: 'Achievable cache hit ratio for a chat product', note: 'Stable system prompt and tool definitions' },
        { v: 'Minutes', k: 'Typical cache TTL', note: 'Bursty traffic and long user pauses lose the entry' },
        { v: '0%', k: 'Hit rate with a timestamp at the top', note: 'One interpolated value defeats the whole mechanism' }
      ]
    },

    { t: 'h', text: 'Tool results and retrieval on demand' },
    {
      t: 'prose',
      md: `Tool results are the most common way a context window fills unexpectedly, because
...
round trip when the information is needed and saves the entire budget when it is not.`
    ...
    { t: 'h', text: 'Sub-agent context isolation' },
    {
      t: 'prose',
      md: `When a task has a bulky, self-contained sub-problem -- read forty files and find where
...
keep anything requiring negotiation with the main thread inline.`
    ...
    { t: 'h', text: 'Observability: what was actually in the window?' },
    {
      t: 'prose',
      md: `When a model produces a bad answer, the first question is always "what did it
...
have is the input and the output.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Compaction drops the original constraint', blast: 'The agent silently violates a requirement stated at turn 2 — "only in the staging environment" — thirty turns later.', fix: 'Pin the first user turn; extract constraints into structured state that is never summarised away; log compaction events with what was removed and diff state before and after.' },
        { mode: 'Timestamp or request ID at the top of the system prompt', blast: 'Prefix cache hit rate is zero; cost and TTFT are several times higher than necessary and nobody notices because nothing errors.', fix: 'Volatile values go last; round timestamps to the day; monitor `cached_tokens / prompt_tokens` as a dashboard metric with an alert on a drop.' },
        { mode: 'Unbounded tool result', blast: 'One `SELECT *` blows the window, the request fails or truncates mid-thought, and the agent loses the task.', fix: 'Per-tool output caps with actionable truncation notices; return references to large artefacts; prefer tools that return answers rather than data.' },
        { mode: 'No output reservation', blast: 'Requests fit on input and truncate mid-JSON, failing after prefill has been paid for.', fix: 'Compute `window - max_output - safety` before allocating input; count with the real tokeniser; keep ~10% headroom.' },
        { mode: 'Stale fact from early in the session', blast: 'The model acts on superseded state — an old plan, an old file version, a value the user already corrected.', fix: 'Maintain current state explicitly rather than relying on the newest mention winning; remove superseded turns during compaction; put current state near the end.' },
        { mode: 'Memory store written to indiscriminately', blast: 'Wrong or private facts persist across sessions, are trusted, and have no visible provenance.', fix: 'Explicit write policy with provenance; user-visible memory with edit and delete; scope memory to the principal and never share it across a workspace boundary without an ACL.' },
        { mode: 'Retrieved chunks appended in document order', blast: 'The best passage lands in the middle of a long context and is attended to least.', fix: 'Order best-first by reranker score, cap at 4-8, and place them adjacent to the question.' },
        { mode: 'Assembled prompt never logged', blast: 'Quality incidents are undiagnosable; every investigation ends in speculation.', fix: 'Per-section token accounting, retrieved chunk IDs and scores, compaction generation, prompt version and a content hash on every span; retain full prompts for negatively-rated requests.' }
      ]
    },

    {
      t: 'staff',
      md: `The signal here is treating the window as a resource with an allocation policy and an
...
and insisting the assembled context is logged.`
...

    {
      ...
      items: [
        {
          q: 'Your chat product\'s system prompt begins with `Current time: {{iso8601}}`. The team is surprised that input costs are ~4x the estimate. Explain and fix.',
          options: [
            'Timestamps tokenise poorly; shorten the format.',
            'The interpolated timestamp changes every request, so the prefix cache never matches and the entire prompt is re-prefilled and billed at full rate every time.',
            'Time strings force the provider to disable batching.',
            'The model spends extra reasoning tokens parsing the timestamp.'
          ],
          answer: 1,
          why: 'Prefix caching matches on exact bytes from the start of the prompt, so one volatile value at the top invalidates everything after it. The tokens themselves are trivial; the cache miss is not — cached input is commonly around a tenth the price, and skipping prefill also improves TTFT. Move the timestamp to the end or into the user turn, round it to the day if only the date matters, and monitor `cached_tokens / prompt_tokens`.'
        },
        {
          q: 'A coding agent works well for 15 turns then begins ignoring a constraint the user gave at turn 2. Most likely cause?',
          options: [
            'The model\'s context window was exceeded and requests are failing.',
            'A sliding-window or summarisation step removed or diluted the early turn where the constraint was stated.',
            'Temperature drift over a long session.',
            'The tool schema changed mid-session.'
          ],
          answer: 1,
          why: 'Constraints are almost always stated early and referenced implicitly thereafter, which is exactly what naive history management discards — a sliding window drops it outright, and a prose summary tends to compress it into something vague. Pin the first user turn, and better, extract constraints into structured state that is never summarised away. Log what each compaction removed so this is diagnosable rather than mysterious.'
        },
        {
          q: 'An agent must analyse 40 source files to answer one architectural question. Best context strategy?',
          options: [
            'Load all 40 files into the main context — modern windows are large enough.',
            'Delegate to a sub-agent with its own window that reads the files and returns a concise findings summary to the parent.',
            'Summarise each file with a cheap model and inline all 40 summaries.',
            'Retrieve the 5 most similar files by embedding and ignore the rest.'
          ],
          answer: 1,
          why: 'Isolation is the right tool for a read-heavy, well-specified, independently verifiable sub-task: the exploration cost stays inside the sub-agent, the parent grows by a few hundred tokens, and the parent\'s stable prefix is undisturbed so its cache stays warm. Loading everything inline both costs the full amount and degrades reasoning through distraction. Option 4 fails because architectural questions are structural, not similarity-shaped — the important file may share no vocabulary with the question.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The prefill cost that makes context size a latency problem comes from
...
Architecture**, and the per-section token spans with **AI Observability & Tracing**.`
  ...
  flashcards: [
    { q: 'Why does a larger context window not remove the need for context engineering?', a: 'Three costs still scale with what you put in it: input tokens are re-sent every turn so cost grows roughly quadratically over a session, prefill is compute-bound so context size is directly your TTFT, and quality degrades with distractors and position effects. The question is the smallest set of tokens that makes the call correct.' },
    { q: 'What is "lost in the middle"?', a: 'Liu et al. (2023) showed retrieval accuracy within a context depends on position — highest at the start and end, lowest in the middle. Softened in newer models, not gone. Consequences: order retrieved chunks best-first and put the most important material adjacent to the question.' },
    { q: 'Why is needle-in-a-haystack an easy benchmark?', a: 'There is exactly one plausible answer and no distractors. Real workloads contain many passages that look relevant, so effective performance degrades far faster than those benchmarks imply. Treat a large advertised window as a budget, not a guarantee.' },
    { q: 'How does prompt-prefix caching work and what breaks it?', a: 'The provider caches processed state for a byte-identical prefix, skipping prefill and billing those tokens at a steep discount — often around 10x cheaper. Any change invalidates everything after it: an interpolated timestamp, a request ID, an unsorted tool list, or non-deterministic JSON key order.' },
    { q: 'What order should you assemble context in, and why?', a: 'Most stable first, most volatile last: system prompt, tool definitions, durable memory, history, retrieved documents, current question. Stability-first maximises prefix cache hits; volatility-last puts the most important material in the strongest attention position. Both effects point the same way.' },
    { q: 'Why compact at a threshold rather than at the limit?', a: 'Compacting at ~85% of budget is a planned operation you can test and monitor. Compacting after the provider rejects the request means doing surgery on a failed request mid-conversation, in production, with a user waiting.' },
    { q: 'Name the four kinds of memory and where each lives.', a: 'Working memory is the context window itself. Episodic is past-session records, retrieved by similarity and recency. Semantic is durable facts about the user or org — small, high-value, usually always included. Procedural is how-to-work-here knowledge, which belongs in the system prompt because it must be reliably present rather than probabilistically retrieved.' },
    { q: 'Why is structured state better than a rolling prose summary?', a: 'A typed object holding goal, constraints, decisions and open questions is diffable, validatable and inspectable, and constraints in it are never summarised away. Recursive prose summarisation accumulates drift silently, and an error introduced in an early tier is unrecoverable.' },
    { q: 'When is a sub-agent with an isolated context the right call?', a: 'For read-heavy, well-specified, independently verifiable sub-tasks. The sub-agent spends 100K tokens exploring and returns 500, so the parent stays small and its prefix cache stays warm. The cost: it cannot see the parent\'s context, so an incomplete brief yields a confidently irrelevant answer.' }
  ],

  drills: [
    {
      prompt: 'A coding agent handles sessions of 40-80 turns. Average cost per session is $3.20 against a $0.80 target, p95 TTFT is 5.8 s, and quality degrades noticeably after turn 25. Diagnose and design the context strategy.',
      probes: [
    ...
    {
      prompt: 'Product wants the assistant to "remember users across sessions". Design the memory architecture, including what you would refuse to build in v1.',
      probes: [
