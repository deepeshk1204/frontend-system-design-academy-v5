export default {
  blocks: [
    {
      t: 'prose',
      md: `Between a user pressing a button and your handler function running, the request passes
...
sending work to the one machine that was already busy.`
    ...
    {
      t: 'prose',
      md: `A single process on a single machine needs none of this. You bind a port, accept
...
queue, and queues are where latency is born.`
    ...
    {
      t: 'numbers',
      title: 'Costs along the path, order-of-magnitude',
      items: [
        { v: '~100 ns', k: 'Main-memory read', note: 'Free, for our purposes' },
        { v: '~0.5 ms', k: 'Same-AZ network round trip', note: 'Your service-to-service floor' },
        { v: '~1-2 ms', k: 'Cross-AZ round trip', note: 'Charged per hop, and per retry' },
        { v: '~1 ms', k: 'SSD random read', note: 'A cold page in Postgres' },
        { v: '2-3 RTT', k: 'TCP + TLS 1.2 handshake', note: '1 RTT with TLS 1.3, 0 on resume' },
        { v: '~30 ms', k: 'Cross-continent-US RTT', note: '~200 ms intercontinental' }
      ]
    },
  ...
  L-->>C: 200 plus access log`,
      caption: 'Three queues on one request: the LB backend queue, the app accept queue, and the DB pool. Any of them can own your p99.'
    },

    { t: 'h', text: 'L4 versus L7 load balancing' },
    {
      t: 'prose',
      md: `An **L4** balancer forwards packets or TCP connections. It reads the IP header and the
...
reason a "load-balanced" service has one node at 90% CPU and the rest at 20%.`
    },
    {
      ...
      cols: ['Dimension', 'L4 (NLB, IPVS)', 'L7 (Envoy, ALB, NGINX)'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'The gRPC balancing trap',
      md: `gRPC runs over a single long-lived HTTP/2 connection per channel. Behind an L4 balancer,
...
\`keepalive.ServerParameters{MaxConnectionAge: 30 * time.Minute}\` exists for exactly this.`
    ...
    { t: 'h', text: 'Balancing algorithms, and why round robin is a trap' },
    {
      t: 'table',
      cols: ['Algorithm', 'How it picks', 'Where it shines', 'Where it hurts'],
      rows: [
    ...
    {
      t: 'prose',
      md: `The reason least-connections beats round robin is worth working out, because it is a
...
(\`consecutive_5xx\`) for a base ejection time, then let it back gradually.`
    ...
    { t: 'h', text: 'TLS termination' },
    {
      t: 'prose',
      md: `TLS 1.3 needs one round trip to establish, plus an asymmetric key operation on the
...
handshake once per connection rather than once per request.`
    ...
    { t: 'h', text: 'Keep-alive and pool sizing maths' },
    {
      t: 'prose',
      md: `Without keep-alive, every HTTP request pays a TCP handshake plus a TLS handshake before
...
number of connections you need to keep busy is throughput times average hold time.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Sizing a Postgres pool with Little\'s Law',
      code: `Target throughput            λ = 1,200 rps for this service
...
and every instance restart is a connection storm.`
    ...
    {
      t: 'prose',
      md: `That last line is the whole argument for a pooler. Postgres forks a backend process per
...
your application where you own the timeout, can shed load, and can emit a metric.`
    },
    {
      t: 'note',
      tone: 'info',
      title: 'The one pool metric to alert on',
      md: `Not pool utilisation -- **connection acquisition wait time**. If threads are waiting
...
database timer.`
    ...
    { t: 'h', text: 'Concurrency models' },
    {
      t: 'prose',
      md: `Your server has to hold thousands of half-finished requests, most of which are waiting
...
and whether one slow dependency can freeze everything.`
    ...
    {
      t: 'table',
      title: 'Four ways to hold 10,000 concurrent in-flight requests',
      cols: ['Model', 'Examples', 'Cost per in-flight request', 'Blocking a request blocks...', 'Overload behaviour'],
      rows: [
    ...
    {
      t: 'prose',
      md: `A concrete comparison. Ten thousand concurrent requests, each spending 50 ms waiting on
...
requests accumulate and the JVM dies of memory rather than shedding load.`
    ...
    { t: 'h', text: 'Where p99 latency actually comes from' },
    {
      t: 'prose',
      md: `Almost never from the code in your handler. The recurring sources, roughly in order of
...
- **Retries.** A 3x retry on a 1-second timeout is a 3-second request. Your p99.9 is often somebody
    ...
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Latency budget worked example: GET /checkout/summary, 250 ms p99 target',
      code: `Client -> edge (TLS resume, 1 RTT)                 15 ms
...
Decisions this forces:
  ...
  * If p99 target were 120 ms, the region hop must go -- read replica
    ...
    {
      t: 'tradeoffs',
      title: 'Adding an L7 proxy and pooling layer',
      gains: [
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'L4 balancer plus long-lived HTTP/2 or gRPC connections', blast: 'One instance at 90% CPU while the fleet idles; scaling out changes nothing.', fix: 'L7 proxy balancing per stream, client-side LB, or server-side `MaxConnectionAge` to force redistribution.' },
        { mode: 'Fail-fast instance attracts least-connections traffic', blast: 'The broken node takes the largest share of requests -- a black hole.', fix: 'Outlier detection on consecutive 5xx with ejection, plus health checks that test the dependency, not the port.' },
        { mode: 'Connection pool smaller than concurrency, no acquire timeout', blast: 'Requests block indefinitely on acquire; the service hangs rather than erroring.', fix: 'Bounded acquire timeout under the request deadline, and alert on acquire wait p99.' },
        { mode: 'Fleet restart against a bare database', blast: 'Hundreds of simultaneous connection attempts exhaust `max_connections`; healthy traffic gets "too many clients".', fix: 'pgBouncer in transaction mode, staggered rollouts, and jittered pool warm-up.' },
        { mode: 'CPU-bound handler on an event loop', blast: 'Latency rises on every unrelated route on that instance; no single slow endpoint to blame.', fix: 'Move the work to a worker thread or separate service; alert on event-loop lag, not just CPU.' },
        { mode: 'Keep-alive timeout on the LB shorter than on the backend', blast: 'Sporadic connection resets surfacing as 502s at a low, constant rate.', fix: 'Make the backend idle timeout strictly longer than the proxy\'s, so the proxy always closes first.' },
        { mode: 'Health check hits `/health` returning static 200', blast: 'An instance with a dead DB pool stays in rotation and serves errors indefinitely.', fix: 'Separate liveness from readiness; readiness checks the pool with a cheap `SELECT 1` and a short timeout.' }
      ]
    },

    {
      t: 'staff',
      md: `Most candidates draw a box labelled "load balancer" and move on. The signal is in the
...
these sentences.`
...

    {
      ...
      items: [
        {
          q: 'A gRPC service behind an AWS NLB has one pod at 95% CPU and nine at 15%. Scaling to 20 pods does not help. Why?',
          options: [
            'NLB does not support gRPC, so requests are being retried.',
            'Balancing happens once per TCP connection, and gRPC keeps one long-lived HTTP/2 connection per channel.',
            'The pods have different CPU limits.',
            'HTTP/2 flow control is throttling the other pods.'
          ],
          answer: 1,
          why: 'An L4 balancer picks a backend at connection establishment and then forwards every byte of that connection there. gRPC opens one HTTP/2 connection per channel and multiplexes thousands of RPCs onto it, so a client is pinned for the life of the channel and new pods never receive traffic from existing clients. Fix it at L7 (Envoy balances per stream), with client-side load balancing, or by setting `MaxConnectionAge` so channels periodically reconnect and rebalance.'
        },
        {
          q: 'You raise a service\'s Postgres pool from 20 to 200 connections per instance. Throughput is flat and p99 doubles. What happened?',
          options: [
            'The pool is now too large to fit in memory.',
            'Queueing moved from your application into the database, where it is unbounded and unprioritised.',
            'Postgres serialises all queries on a single connection.',
            'Connection acquisition became slower because the pool must be scanned.'
          ],
          answer: 1,
          why: 'The database can only execute a few dozen queries concurrently before it is CPU- or IO-bound. A bigger pool does not create capacity; it just lets more work queue somewhere you do not control, behind locks and buffer contention, with no timeout and no way to shed. Keeping the pool near the database\'s real concurrency pushes the wait into your app where you own the deadline and can emit acquire-wait metrics.'
        },
        {
          q: 'A request fans out to 20 services in parallel and waits for all of them. Each has a 1% chance of exceeding 100 ms. Roughly what fraction of requests exceed 100 ms?',
          options: ['About 1%', 'About 5%', 'About 18%', 'About 50%'],
          answer: 2,
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The queueing behaviour sketched here is derived properly in **Scalability & Capacity
...
data-placement strategy in **Sharding & Partitioning**.`
  ...
  flashcards: [
    { q: 'Why does an L4 load balancer create imbalance with HTTP/2 or gRPC?', a: 'It balances once per TCP connection. HTTP/2 multiplexes many requests onto one long-lived connection, so a client is pinned to one backend for the connection\'s lifetime and new instances never receive its traffic.' },
    { q: 'Why does least-connections beat round robin under variable request cost?', a: 'In-flight count is a free proxy for how busy a backend is. Round robin sends a cheap request to a node already stuck behind slow ones, creating queueing delay at the balancer. Least-connections routes around it.' },
    { q: 'What is the failure mode of least-connections?', a: 'A backend failing fast has near-zero in-flight requests, so it looks idlest and attracts the most traffic. Pair it with outlier detection that ejects hosts on consecutive 5xx.' },
    { q: 'State Little\'s Law and apply it to a connection pool.', a: '`L = λ × W`. Busy connections equal query throughput times average query duration. At 2,400 queries/s and 4 ms each you need about 10 busy connections; size for p95 hold time, not p50.' },
    { q: 'Why does Postgres need pgBouncer while MySQL tolerates more connections?', a: 'Postgres forks an OS process per connection, each with its own memory and a slot in snapshot computation, so throughput degrades past a few hundred. A transaction-mode pooler multiplexes hundreds of clients onto tens of server connections.' },
    { q: 'Why can a bigger connection pool make p99 worse?', a: 'It moves queueing from your application into the database, where the wait is unbounded, unprioritised, invisible to your timeouts and not exposed as a metric.' },
    { q: 'What breaks when a CPU-bound handler runs on an event loop?', a: 'It blocks every in-flight request on that instance, so latency rises uniformly across unrelated routes. Watch event-loop lag, not CPU, and move the work off the loop.' },
    { q: 'Why must virtual threads be paired with an explicit concurrency limit?', a: 'The old thread pool was also the admission control. Virtual threads are effectively unbounded, so a slow dependency lets requests accumulate until the process dies of memory instead of shedding load. Reintroduce the bound with a semaphore or bulkhead.' },
    { q: 'Which single metric most often explains p99 that queries cannot?', a: 'Connection acquisition wait time. Database instrumentation starts the clock when the query is sent, so time spent waiting for a pooled connection is invisible to it.' }
  ],

  drills: [
    {
      prompt: 'A checkout API is at 1,800 rps across 30 pods behind an ALB. p50 is 40 ms, p99 is 1.4 s. CPU averages 45%, the slowest query in `pg_stat_statements` is 18 ms, and the p99 spikes do not correlate with traffic. You have one hour before a launch. Where do you look, in order?',
      probes: [
