export default {
  blocks: [
    { t: 'prose',
      md: `Monitoring tells you whether the things you predicted would break are broken. Observability is whether you can
...
the compute bill for the service being observed.` },
  ...
  T -.->|exemplar links<br/>metric to trace| M`,
      caption: 'The value is in the arrows. A stack where you cannot jump from a metric spike to an exemplar trace is four disconnected tools.' },
    { t: 'table',
      cols: ['Signal', 'Cost per unit', 'Cardinality tolerance', 'Retention you can afford', 'Answers'],
      rows: [
    ...
    { t: 'prose',
      md: `Continuous profiling is the fourth pillar and it is the one most teams are missing. eBPF-based profilers (Parca,
...
field nobody reads.` },
    ...
    { t: 'h', text: 'Metric types, and the histogram argument' },
    { t: 'table',
      cols: ['Type', 'Semantics', 'Use for', 'Trap'],
      rows: [
    ...
    { t: 'prose',
      md: `A gauge for latency is close to useless and it is a common mistake. If you record "current request duration" as a
...
# exponential ones: ~1% relative error at a fraction of the series count.` },
    { t: 'note',
      ...
      md: `\`histogram_quantile\` interpolates linearly within a bucket, so its accuracy is entirely determined by boundary
...
values, and email addresses. Every one is effectively infinite cardinality.` },
    { t: 'numbers',
      ...
      items: [
        { v: '2,400', k: 'method x status x route', note: 'Fine' },
        { v: '480k', k: '+ pod (200 pods)', note: 'Survivable; use `sum without (pod)` in queries' },
        { v: '2.4B', k: '+ customer_id (5,000)', note: 'Fatal -- the multiplication, not the addition' },
        { v: '~2-10M', k: 'Practical Prometheus ceiling', note: 'Active series on one well-resourced instance' },
        { v: '~2-4 KB', k: 'RAM per active series', note: 'So 5M series is roughly 10-20 GB resident' }
      ] },
    { t: 'prose',
      md: `The discipline is to keep high-cardinality identity in **traces and logs**, where it belongs and is cheap, and keep
...
known-bad labels at ingest, so a single bad deploy cannot double your bill before anyone notices.` },
    ...
    { t: 'h', text: 'OpenTelemetry and context propagation' },
    { t: 'prose',
...
child span, and passes it on. The W3C format is compact and worth recognising on sight.` },
    { t: 'code',
      lang: 'yaml',
      title: 'W3C trace context, and a Collector with tail sampling',
      code: `# traceparent: version-traceId(16 bytes)-spanId(8 bytes)-flags
...
field deny-list enforced in the logging library and again in the Collector is the control that actually works.` },
    { t: 'code',
      ...
      code: `{ "ts": "2026-03-04T11:02:07.412Z", "level": "error",
  ...
  "error_code": "card_declined", "retry_count": 2 }` },
    ...
    { t: 'h', text: 'RED and USE: what to measure without thinking' },
    { t: 'prose',
      md: `Two checklists that cover most of what you need, and they apply to different things. **RED** is for
...
suffering, USE tells you why.** Alert on RED, diagnose with USE.` },
    ...
    { t: 'prose',
      md: `Precision here matters because the three terms get used interchangeably and they are not the same kind of thing.
...
dashboard.` },
    { t: 'numbers',
      title: 'Error budget for 99.9% over 28 days',
      items: [
        { v: '40 min', k: 'Total budget of bad time', note: '0.1% of 28 days' },
        { v: '43,200', k: 'Bad requests allowed', note: 'At 500 rps: 0.001 x 43.2M valid requests' },
        { v: '14.4x', k: 'Burn rate consuming it in 2 days', note: 'The classic fast-burn page threshold' },
        { v: '6x', k: 'Burn rate consuming it in ~5 days', note: 'One-hour-window fast burn' },
        { v: '1x', k: 'Exactly on budget', note: 'Sustainable; no action needed' }
      ] },
    { t: 'prose',
      md: `**Burn rate** is how fast you are spending, expressed as a multiple of the sustainable rate. A burn rate of 1
...
provides sensitivity, the short one prevents the alert from staying lit for hours after the incident resolved.` },
    { t: 'table',
      title: 'Multi-window multi-burn-rate policy for a 99.9% / 28-day SLO',
      cols: ['Burn rate', 'Long window', 'Short window', 'Budget consumed before firing', 'Action'],
      rows: [
    ...
    { t: 'code',
      lang: 'yaml',
      title: 'The fast-burn rule, written out',
      code: `- alert: CheckoutSLOFastBurn
  ...
  annotations:
    ...
    { t: 'h', text: 'Alerting on symptoms, and on-call you can sustain' },
    { t: 'prose',
      md: `Alert on what the user experiences, not on what you happen to be able to measure. CPU at 90% is not an incident --
...
compensating time off after a genuinely disrupted night.` },
    ...
    { t: 'failures',
      items: [
        { mode: 'Averaging per-instance p99s', blast: 'One thrashing pod at p99 = 5 s among nine healthy ones reads as a moderate global slowdown; you chase a fleet-wide theory for an hour.', fix: 'Export histogram buckets and aggregate with `histogram_quantile(0.99, sum by (le) (rate(..._bucket[5m])))` -- never average quantiles.' },
        { mode: 'Unbounded label such as `user_id` or raw URL', blast: 'Series count explodes, Prometheus OOMs, and you lose all monitoring during the incident that caused it.', fix: 'Normalise routes to `/users/:id`, enforce a label deny-list in `metric_relabel_configs`, and alert on series growth rate.' },
        { mode: 'Head sampling at 1%', blast: 'Almost no error traces exist, so the one trace you need for a 0.1% failure was discarded before anything was known about it.', fix: 'Tail sampling in the Collector: 100% of errors and slow requests, 1% baseline.' },
        { mode: 'Cause-based alerts instead of symptom-based', blast: 'Twenty pages for one root cause, plus total silence for the failure mode nobody predicted.', fix: 'Page on SLO burn rate for user-facing symptoms; demote resource alerts to tickets used for diagnosis.' },
        { mode: 'No trace-id propagation across the async boundary', blast: 'A trace ends at the Kafka publish; debugging the event-driven half is correlating timestamps by hand.', fix: 'Inject `traceparent` into message headers, extract on consume, and include `trace_id` in every log line.' },
        { mode: 'PII in logs', blast: 'Emails and tokens replicated to a vendor with 30-day retention and broad read access -- a reportable breach with no attacker involved.', fix: 'Field deny-list in the logging library, a second scrub in the Collector, and a CI check on new log statements.' },
        { mode: 'SLO with no error-budget policy, or buckets far from the SLO threshold', blast: 'The SLO is breached monthly and nothing changes, so it stops being believed; and with buckets at 0.1 and 0.5 a 300 ms target is measured by interpolated guesswork anyway.', fix: 'Write the consequence down -- budget exhausted means feature freeze -- agreed with the product owner in advance; and place dense bucket boundaries around the threshold, or adopt native histograms.' }
      ] },

    { t: 'staff',
      md: `Observability is where interviewers most often find out whether someone has actually carried a pager. The
...
are things you only learn by owning the bill and the pager.` },
    ...
    { t: 'quiz',
      items: [
        {
          q: 'Nine pods report p99 latency of 20 ms; one reports 5,000 ms. Your dashboard shows `avg(p99) = 518 ms`. What is wrong?',
          options: ['Nothing -- 518 ms is the correct fleet p99.', 'Percentiles cannot be averaged; you must sum histogram buckets across pods and compute the quantile once.', 'The scrape interval is too long.', 'You should use `max()` instead of `avg()`.'],
          answer: 1,
          why: 'A percentile is an order statistic over a population, and averaging two of them yields a number with no interpretation -- it is neither the fleet p99 nor the p99 of anything else. Here it hides the real story (one broken pod) behind a plausible-looking general slowdown. `max()` is better as a diagnostic but it is still not the fleet p99. The correct approach is to export bucket counters, sum them across pods, and call `histogram_quantile` once over the combined distribution, because bucket counts are additive while quantiles are not.' },
        {
          q: 'You add a `customer_id` label to `http_requests_total{method, status, route}` with 5 methods, 12 statuses, 40 routes and 5,000 customers. Impact?',
          options: ['About 5,000 new series.', 'About 7,000 series total -- labels add.', 'Up to 12 million series, because cardinality is the product of label values.', 'No impact; Prometheus compresses repeated labels.'],
          answer: 2,
          why: 'Each unique combination of label values is its own time series, so the count multiplies: 5 x 12 x 40 x 5,000 = 12,000,000. That alone exceeds what a single well-resourced Prometheus instance handles, and at roughly 2-4 KB of RAM per active series it is tens of gigabytes of memory. The consequence is worse than a bill -- your metrics backend OOMs during the incident its own label caused. Put tenant identity in traces and logs, and if you need per-tenant metrics, emit them for your top N tenants and bucket the rest.' },
        {
          q: 'Your SLO is 99.9% over 28 days and you are currently burning budget at 14.4x. What does that mean and what should happen?',
          options: ['You have breached the SLO already; declare an outage.', 'At this rate the entire 28-day budget is consumed in about 2 days, so it should page.', 'You are within budget, so no action is needed.', 'The error rate is 14.4%.'],
          answer: 1,
          why: 'Burn rate is a multiple of the sustainable consumption rate: 1x exactly exhausts the budget at the end of the window, so 14.4x exhausts it in 28/14.4 ≈ 2 days. That is the canonical fast-burn page threshold, and pairing the 1-hour window with a 5-minute short window means the alert is both sensitive and prompt to resolve. Note the error rate is 14.4 x 0.1% = 1.44%, not 14.4%, and waiting for the 28-day SLO to actually breach would mean alerting long after the damage was irreversible.' },
        {
          q: 'Which alert best fits the symptom-based principle for a checkout service?',
          options: ['CPU on any checkout pod above 85% for 5 minutes.', 'Checkout error-budget burn rate above 14.4x over 1 hour, confirmed by a 5-minute window.', 'Database connection pool utilisation above 90%.', 'Any pod restart in the checkout deployment.'],
          answer: 1,
          why: 'The burn-rate alert fires when users are actually being harmed, regardless of cause -- including causes nobody anticipated, which is the property that makes it valuable. The other three are cause-based: high CPU may be a well-utilised machine, a pool at 90% utilisation with an empty wait queue is healthy, and a pod restart during a rolling deploy is normal. They are excellent *diagnostic* signals and belong on the dashboard you open after the page, not on the pager.'
        }
      ] },
    ...
    { t: 'details',
      ...
      blocks: [{ t: 'prose',
        md: `Consumer lag alerting and per-partition metrics are covered in **Queues & Event Streaming**. Saturation as the
...
**Containers, Kubernetes & Safe Deploys**.`
  ...
  flashcards: [
    { q: 'Why can you not average percentiles?', a: 'A percentile is an order statistic over a population, so the mean of two percentiles is not a percentile of anything. Nine pods at 20 ms plus one at 5 s averages to 518 ms, hiding one dead pod behind an apparent general slowdown. Sum histogram buckets across instances, then compute the quantile once.' },
    { q: 'Why is a histogram better than a gauge for latency?', a: 'A gauge samples one request every scrape interval, missing the tail almost entirely. A histogram counts every request into buckets, so any quantile can be computed over any range afterwards -- and bucket counts are additive across instances, which quantiles are not.' },
    { q: 'How does metric cardinality grow with labels?', a: 'Multiplicatively -- series count is the product of distinct values across all labels. 5 methods x 12 statuses x 40 routes x 5,000 customers is 12 million series. Keep identity in traces and logs; metrics get bounded dimensions only.' },
    { q: 'Head sampling vs tail sampling?', a: 'Head sampling decides at the root span before anything is known, so a 1% rate discards nearly all errors. Tail sampling buffers until the trace completes, letting you keep 100% of errors and slow traces plus a 1% baseline -- at the cost of stateful Collectors and trace-id-aware load balancing.' },
    { q: 'Define SLI, SLO, SLA precisely.', a: 'SLI is a measured ratio of good events to valid events. SLO is a target on that SLI over a stated window (99.9% / 28 rolling days). SLA is a customer contract with a financial penalty, and your SLO should be a nine tighter so breaching it is an internal signal, not a refund.' },
    { q: 'What is an error budget and what makes it useful?', a: '`1 - SLO`, expressed as allowed bad events or bad time -- 99.9% over 28 days is about 40 minutes. It is useful only with a policy attached: spare budget buys faster deploys and riskier experiments, exhausted budget means feature work stops.' },
    { q: 'What is a 14.4x burn rate, and why alert on two windows?', a: 'You are consuming error budget 14.4x faster than sustainable, so a 28-day budget is gone in about 2 days -- the canonical fast-burn page threshold. Pair a long window (sensitive enough to catch real erosion) with a short confirmation window (so the alert clears after recovery); requiring both avoids paging on every blip and avoids noticing only once the SLO has already broken.' },
    { q: 'RED vs USE?', a: 'RED (Rate, Errors, Duration) measures request-driven services and tells you the user is suffering. USE (Utilisation, Saturation, Errors) measures resources and tells you why. Saturation -- the queue of waiting work -- is the most predictive and the most often omitted.' },
    { q: 'What makes on-call sustainable?', a: 'Every page is actionable or it becomes a ticket; under roughly two pages per shift, with excess treated as a reliability bug getting engineering time; a runbook linked from every alert; blameless postmortems with owned action items; and rotations no heavier than one week in four.' }
  ],

  drills: [
    {
      prompt: 'You inherit a 14-service platform with 400 Grafana dashboards, 220 alert rules, and an on-call rotation averaging 11 pages per night. MTTR is around 90 minutes. The observability bill is $180k/month against $600k of compute. Give a 90-day plan.',
      probes: [
    ...
    {
      prompt: 'Checkout p99 has risen from 180 ms to 1.4 s over four hours. There has been no deploy. Aggregate error rate is 0.3%, roughly normal. You have Prometheus, OpenTelemetry traces at 1% head sampling, and structured logs. Walk through your investigation and say what you would change afterwards.',
      probes: [
