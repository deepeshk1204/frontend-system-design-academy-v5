export default {
  blocks: [
    {
      t: 'prose',
      md: `An index is a second copy of some of your data, sorted, so the database can find rows
...
problems you actually have.`
    ...
    {
      t: 'prose',
      md: `A table is a heap of 8 KB pages on disk. Answering \`WHERE email = 'x@y.com'\` without
...
for several more columns.`
    ...
    { t: 'h', text: 'B+trees, and why depth is the whole story' },
    {
      t: 'prose',
      md: `Relational databases use a B+tree: a shallow, wide, balanced tree where internal nodes
...
of those, which is why B+trees are the default and hash indexes are a niche.`
  ...
  L2 --> H[("Heap pages<br/>the actual rows")]`,
      caption: 'Leaves are doubly linked, which is what makes range scans and ORDER BY free. The hop from leaf to heap is the cost a covering index removes.'
    },
    {
      t: 'numbers',
      title: 'Why depth beats size',
      items: [
        { v: '~300', k: 'Entries per 8 KB internal page', note: 'For a bigint or short text key' },
        { v: '4 levels', k: 'Addresses ~8 billion entries', note: 'Depth grows logarithmically' },
        { v: '1-2', k: 'Real disk reads per point lookup', note: 'Upper levels stay cached' },
        { v: '~0.1 ms', k: 'Warm indexed point read', note: 'Versus ~100 s for a 50 GB seq scan' }
      ]
    },

    { t: 'h', text: 'Clustered versus secondary indexes' },
    {
      t: 'prose',
      md: `In **InnoDB** (MySQL) the primary key *is* the table: rows are stored inside the primary
...
Postgres has a visibility map, index-only scans and the \`VACUUM\` machinery.`
    ...
    {
      t: 'table',
      cols: ['Property', 'InnoDB (MySQL)', 'Postgres heap'],
      rows: [
    ...
    { t: 'h', text: 'Composite indexes and the leftmost-prefix rule' },
    {
      t: 'prose',
      md: `A composite index on \`(a, b, c)\` is sorted by \`a\`, then by \`b\` within equal \`a\`,
...
from reading 40 entries to reading 400,000 and filtering.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'Column order is not cosmetic',
      code: `-- Query we must serve:
SELECT * FROM events
...
CREATE INDEX good_active ON events (tenant_id, type, created_at DESC)
  WHERE archived_at IS NULL;`
    ...
    { t: 'h', text: 'Covering indexes and index-only scans' },
    {
      t: 'prose',
      md: `The expensive part of an index scan is often not the descent -- it is the random heap
...
row" with no query change at all -- a classic mystery regression.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'Turning three plans into one',
      code: `SELECT order_id, status, total_cents
  FROM orders
...
-- --> Index Only Scan. Heap Fetches: 0  (when the visibility map is current)`
    ...
    { t: 'h', text: 'Selectivity, cardinality, and why the planner ignores your index' },
    {
      t: 'prose',
      md: `**Cardinality** is how many distinct values a column has; **selectivity** is what
...
\`default_statistics_target\` for columns with long tails.`
    ...
    { t: 'h', text: 'Reading an EXPLAIN ANALYZE, line by line' },
    {
      t: 'code',
      lang: 'sql',
      title: 'The query and the plan',
...
Execution Time: 1894.006 ms`
    ...
    {
      t: 'prose',
      md: `Read it inside out, and read only three things at first: where the time actually
...
routinely hide full aggregations underneath.`
    ...
    {
      t: 'table',
      title: 'Plan nodes and what they are telling you',
      cols: ['Node', 'Mechanism', 'Healthy when', 'Red flag'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'info',
      title: 'The estimate-versus-actual habit',
      md: `Scan the plan for the largest ratio between \`rows=\` (estimate) and \`rows=\` (actual).
...
error -- you fix it with \`ANALYZE\`, extended statistics, or by restructuring the predicate.`
    ...
    { t: 'h', text: 'The cost of writes: amplification and over-indexing' },
    {
      t: 'prose',
      md: `Every index is a tax on every write. An \`INSERT\` into a table with eight indexes is
...
insert and consuming buffer cache that useful indexes want.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'Finding the indexes you are paying for and not using',
      code: `SELECT relname            AS table,
...
CREATE INDEX CONCURRENTLY idx_sessions_user ON sessions (user_id);`
    ...
    { t: 'h', text: 'LSM trees versus B-trees' },
    {
      t: 'prose',
      md: `A B+tree updates data **in place**: find the page, modify it, write it back. That is one
...
reclaims them.`
    ...
    {
      t: 'table',
      title: 'Amplification trade-offs',
      cols: ['Property', 'B+tree (Postgres, InnoDB)', 'LSM (RocksDB, Cassandra)'],
      rows: [
    ...
    { t: 'h', text: 'Connection pooling and the N+1 pattern' },
    {
      t: 'prose',
      md: `Two application-side problems that look like database problems.
...
trace, and it is worth making that count a tracked metric with an assertion in tests.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'N+1, and the two ways out',
      code: `-- What the ORM does: 1 + 100 queries, ~0.8 ms each.
...
-- when the relationship is 1:1 and you need the data in one pass.`
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Range column placed before equality columns in a composite index', blast: 'Query reads a million entries and filters them; looks indexed, performs like a scan.', fix: 'Equality columns first, range column last; verify with `EXPLAIN` that no `Filter` line follows the index scan.' },
        { mode: 'Predicate wrapped in a function (`WHERE lower(email) = ...`)', blast: 'Index on `email` is unusable; silent full scan.', fix: 'Expression index `CREATE INDEX ON users (lower(email))`, or store a normalised column.' },
        { mode: 'Stale statistics after a bulk load', blast: 'Planner estimates 1 row, chooses a nested loop, executes 200,000 probes; query goes from 20 ms to 4 minutes.', fix: '`ANALYZE` in the load job; extended statistics for correlated columns; alert on estimate/actual skew.' },
        { mode: 'Autovacuum falling behind on a hot table', blast: 'Index-only scans regress to heap fetches, table bloats, plans change with no deploy.', fix: 'Per-table autovacuum thresholds, monitor `n_dead_tup` and transaction-id age, and remove long-running transactions holding snapshots.' },
        { mode: 'Twelve indexes on a write-heavy table', blast: 'Insert throughput a fraction of expected; WAL volume saturates replication.', fix: 'Drop indexes with `idx_scan` near zero; consolidate overlapping prefixes; set `fillfactor` for HOT updates.' },
        { mode: 'N+1 from ORM lazy loading', blast: 'Latency grows linearly with page size; no individual query is slow so nothing alerts.', fix: 'Batch loading or explicit joins; track queries-per-request in traces and assert on it in tests.' },
        { mode: 'Thousands of direct Postgres connections', blast: 'Throughput drops as connections rise; restarts cause connection storms and "too many clients".', fix: 'pgBouncer transaction mode, small per-instance pools, and `SET LOCAL` instead of session state.' },
        { mode: 'UUIDv4 as an InnoDB primary key', blast: 'Random inserts split pages across the whole tree; every secondary index widened by 16 bytes.', fix: 'Auto-increment bigint, or UUIDv7/ULID so the key is time-ordered.' }
      ]
    },

    {
      t: 'staff',
      md: `The tell of a strong candidate here is that they ask for the plan before proposing a
...
than queried one.`
...

    {
      ...
      items: [
        {
          q: 'You have `CREATE INDEX ON events (tenant_id, created_at, type)` and run `WHERE tenant_id = 5 AND type = \'click\' AND created_at > now() - interval \'1 day\'`. What does the plan do?',
          options: [
            'Seeks on all three columns -- the index is fully used.',
            'Seeks `tenant_id`, range-scans a day of all types, then filters `type` row by row.',
            'Ignores the index entirely because `type` is third.',
            'Uses a bitmap index scan to combine all three predicates.'
          ],
          answer: 1,
        ...
        {
          q: 'A query on a 2 million row table uses a `Seq Scan` and removes 1.4 million rows by filter. Estimated rows match actual almost exactly. What is the right conclusion?',
          options: [
            'Statistics are stale; run `ANALYZE`.',
            'The planner is correct -- at ~30% selectivity a scan beats random heap access; change the query or add a covering index.',
            'Add an index on the filtered column and the plan will improve.',
            'The table needs partitioning.'
          ],
          answer: 1,
          why: 'Matching estimates mean the planner had good information and made a defensible choice: sequential I/O is roughly an order of magnitude cheaper per page than random I/O, so retaining 30% of a table is faster to scan than to look up. The productive moves are to make the predicate more selective, or to add an index that covers all the referenced columns so an index-only scan avoids the heap altogether.'
        },
        {
          q: 'Why can raising a Postgres connection pool from 50 to 500 reduce throughput?',
          options: [
            'Each connection consumes a file descriptor on the client.',
            'Postgres forks a process per connection, and snapshot computation plus memory overhead scale with backend count.',
            'The planner caches fewer plans.',
            'TCP slow start applies per connection.'
          ],
          answer: 1,
          why: 'Per-connection processes each carry memory and appear in the work every transaction must do to build a snapshot, so past a few hundred connections you add contention rather than capacity. The deeper point is that the database can only execute a few dozen queries concurrently anyway -- a larger pool moves queueing inside the database where it is unbounded and unobservable. A transaction-mode pooler in front is the standard answer.'
        },
        {
          q: 'A write-heavy time-series ingest workload is saturating a Postgres instance on random write I/O. What property of an LSM store would help, and what would you give up?',
          options: [
            'Lower read amplification; you give up range scans.',
            'Sequential writes and higher ingest throughput; you give up predictable read latency and pay compaction I/O and write amplification.',
            'Stronger transactional guarantees; you give up indexes.',
            'Smaller storage footprint; you give up durability.'
          ],
          answer: 1,
          why: 'An LSM turns every write into an append to a memtable plus a sequential log, so ingest is bounded by sequential rather than random I/O. The bill arrives later: background compaction rewrites the same data many times over its lifetime, competing for I/O and producing tail-latency spikes, and reads may consult several SSTables. Delete-heavy workloads suffer extra because tombstones must be read through until compaction removes them.'
        }
      ]
    },

    {
      ...
      blocks: [{
        t: 'prose',
        md: `MVCC, dead tuples and why \`VACUUM\` is load-bearing for index-only scans are covered
...
read replicas change which queries you can serve consistently.`
  ...
  flashcards: [
    { q: 'Why is a B+tree lookup fast even on a 100 million row table?', a: 'Fan-out of ~300 per 8 KB page means four levels address billions of entries, and the upper levels stay cached. A point lookup is one or two real disk reads regardless of table size.' },
    { q: 'State the leftmost-prefix rule.', a: 'An index on `(a, b, c)` can seek on `a`, `(a, b)`, or `(a, b, c)` -- any leftmost prefix. It cannot seek on `b` alone, because `b` values are scattered across every value of `a`.' },
    { q: 'Where does a range predicate belong in a composite index, and why?', a: 'Last. A range consumes the rest of the index because columns after it are not globally ordered within the range, so they degrade to row-by-row filters.' },
    { q: 'What is an index-only scan, and what can silently break it in Postgres?', a: 'The query is answered from index leaves with no heap access. It requires pages marked all-visible in the visibility map, so when autovacuum falls behind, `Heap Fetches` becomes non-zero and the plan quietly gets slower.' },
    { q: 'When is the planner right to choose a sequential scan?', a: 'When selectivity is poor -- roughly above 5-10% of the table. Sequential I/O is about an order of magnitude cheaper per page than random heap fetches, so scanning beats looking up most of a table.' },
    { q: 'What is a bitmap heap scan and when does Postgres use it?', a: 'It collects matching row pointers from an index, sorts them by physical page, then reads each page once. It is chosen at medium selectivity -- too many rows for per-row random fetches, too few for a full scan.' },
    { q: 'Why is a UUIDv4 primary key bad in InnoDB?', a: 'Rows live inside the clustered PK tree, so random keys cause page splits and fragmentation on insert, and every secondary index stores the 16-byte PK as its pointer. A monotonic bigint or UUIDv7 avoids the insert problem.' },
    { q: 'Give the write cost of an index in Postgres specifically.', a: 'A non-HOT update writes a new tuple version and must add an entry to every index on the table, even for unindexed columns. That multiplies WAL sent to disk, replicas and backups. `fillfactor` enables HOT updates when no indexed column changed.' },
    { q: 'Why is an N+1 query pattern hard to spot in database metrics?', a: 'Each individual query is fast and well indexed, so nothing appears in slow-query logs. It shows up only as queries-per-request in a trace, and its cost scales linearly with page size.' }
  ],

  drills: [
    {
      prompt: 'An endpoint that lists a tenant\'s recent orders has gone from 30 ms to 2.5 s over three months. The table is 80 million rows, there is an index on `(tenant_id)`, and the query is `WHERE tenant_id = ? AND status = \'open\' ORDER BY created_at DESC LIMIT 50`. One large tenant has 4 million orders. Diagnose and fix it.',
      probes: [
    ...
    {
      prompt: 'Your Postgres primary is at 85% I/O utilisation. Writes are 4,000 inserts/s into one table that has 11 indexes, WAL generation is 90 MB/s, and a streaming replica is 40 seconds behind. Reads are fine. What do you investigate, and what are your options ranked by risk?',
      probes: [
