export default {
  blocks: [
    {
      t: 'prose',
      md: `Data modelling is the act of deciding what questions your system will be able to answer
...
DynamoDB and a requirement to filter on five optional fields.`
    ...
    {
      t: 'prose',
      md: `Relational modelling gave us one dominant answer for thirty years: normalise to third
...
have traded read cost for write cost and for the risk of those copies disagreeing.`
    ...
    { t: 'h', text: 'Derive the store from access patterns' },
    {
      t: 'prose',
      md: `The artefact worth producing before any schema is a table of access patterns. For a
mid-size social product it looks like this, and note that the *ratios* drive every subsequent
decision.`
    },
    {
      t: 'table',
      title: 'Access pattern inventory, the input to every modelling decision',
      cols: ['Pattern', 'Rate', 'Latency need', 'Consistency need', 'Implication'],
      rows: [
    ...
    {
      t: 'prose',
      md: `Read that table and the architecture has largely written itself. Profile reads at 40,000
...
being arithmetic.`
    ...
    { t: 'h', text: 'A worked single-table DynamoDB design' },
    {
      t: 'prose',
      md: `DynamoDB gives you exactly one efficient operation: fetch items by partition key,
...
due date, and list a user's projects.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Key design: one table, five item types, two indexes',
      code: `Table: app        PK = partition key,  SK = sort key
...
5. Projects a user belongs to
    ...
    {
      t: 'prose',
      md: `Three things in that design are the actual content. First, **status is encoded in the
...
That is the honest cost: DynamoDB converts query flexibility into operational predictability.`
    },
    {
      t: 'note',
      tone: 'warn',
      title: 'The partition limit that catches people',
      md: `A single DynamoDB partition sustains about 3,000 read units and 1,000 write units per
...
want, and you should choose it before launch rather than during an incident.`
    ...
    {
      t: 'numbers',
      title: 'Physical limits that constrain key design',
      items: [
        { v: '3,000 RCU', k: 'Per DynamoDB partition', note: '1,000 WCU -- a per-key ceiling, not a table one' },
        { v: '400 KB', k: 'Max DynamoDB item size', note: 'Large blobs go to S3 with a pointer' },
        { v: '5-20x', k: 'Columnar compression ratio', note: 'Per-column uniformity is why' },
        { v: '~1 eng-quarter/yr', k: 'True cost of one extra datastore', note: 'Backups, upgrades, on-call, runbooks' }
      ]
    },

    { t: 'h', text: 'Normalisation economics' },
    {
      t: 'prose',
      md: `Normalise and each fact lives once, so updates are atomic and cheap and nothing can
...
carry a price column forever.`
    },
    {
      ...
      cols: ['Signal', 'Denormalise', 'Keep normalised'],
      rows: [
    ...
    },
    {
      t: 'note',
      tone: 'info',
      md: `The rule that keeps denormalisation survivable: **there must be exactly one source of
...
denormalised copy *is* the only copy, a bug is permanent data loss.`
    ...
    { t: 'h', text: 'JSONB: when it is right, and when it is a schema you will not admit to' },
    {
      t: 'prose',
      md: `Postgres \`jsonb\` is a genuinely good tool for three shapes. Sparse, caller-defined
...
cost will be paid by whoever debugs a query plan on it in eighteen months.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'The hybrid that is usually right',
...
SELECT * FROM contacts
 WHERE custom->>'industry' = 'fintech' AND (custom->>'employees')::int > 500;`
    ...
    { t: 'h', text: 'Time-series, columnar and graph' },
    {
      t: 'prose',
      md: `Three specialised shapes that are worth reaching for when the pattern matches, and worth
resisting otherwise.
...
traversal is a core product feature, not because your data "is a graph". Most data is a graph.`
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'The recursive CTE that removes most graph-database requirements',
      code: `-- "All descendants of org unit 12, with depth, cycle-safe, depth-capped."
...
CREATE INDEX ON org_units (parent_id);`
    ...
    { t: 'h', text: 'The real cost of polyglot persistence' },
    {
      t: 'prose',
      md: `"Use the right tool for each job" is true and incomplete, because each additional store
...
project.`
  ...
  REC --> CH
  PG --> REC`,
      caption: 'One source of truth, everything else derived from its log, plus a reconciler because CDC pipelines do lose events. Dual writes from application code have no equivalent of that last box.'
    },

    { t: 'h', text: 'Schema evolution without downtime' },
    {
      t: 'prose',
      md: `Every schema change on a live system is the expand-migrate-contract dance, because at
...
seconds so a failed attempt fails fast, and retry.`
    ...
    {
      t: 'table',
      title: 'Postgres DDL: what is safe on a live table',
      cols: ['Operation', 'Lock and duration', 'Safe approach'],
      rows: [
    ...
    },
    {
      t: 'code',
      lang: 'sql',
      title: 'Backfilling 200 million rows without a lock or a replication storm',
      code: `-- Bad: one statement, one giant transaction, hours of bloat and lag.
...
ALTER TABLE contacts VALIDATE CONSTRAINT tier_present;   -- takes a weaker lock`
    ...
    {
      t: 'tradeoffs',
      title: 'Denormalised, access-pattern-driven modelling',
      gains: [
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Hot partition from a skewed key', blast: 'One tenant or celebrity exceeds the per-partition limit; throttling appears as errors for everyone on that key.', fix: 'Write-shard the key with a suffix and scatter-gather on read; detect skew before launch with a per-key rate metric.' },
        { mode: 'Dual writes to two stores from application code', blast: 'A crash between them leaves the search index or cache permanently wrong, with no error anywhere.', fix: 'Write once transactionally plus an outbox or CDC feed, and a reconciliation job that diffs and repairs.' },
        { mode: 'JSONB used for a value that queries filter on', blast: 'No type safety, poor selectivity estimates, a GIN index per access pattern, and plan regressions nobody can explain.', fix: 'Promote it to a column; keep `jsonb` for genuinely sparse, never-cross-queried attributes.' },
        { mode: '`count(*)` on a large table in a hot read path', blast: 'Full index or table scan at read rate; latency scales with table size and cannot be cached away.', fix: 'Maintained counter column or a rollup table; `reltuples` for approximate answers.' },
        { mode: 'Analytics queries on the OLTP primary', blast: 'A single report scan evicts the buffer cache and destroys p99 for transactional traffic.', fix: 'Replica for reporting, or a columnar store fed by CDC; enforce with separate credentials and `statement_timeout`.' },
        { mode: 'Unbounded fan-out on write', blast: 'A user with 30 million followers turns one post into 30 million writes, saturating the cluster.', fix: 'Hybrid fan-out: push for normal accounts, pull-at-read for large ones, merged at query time.' },
        { mode: '`ALTER TABLE` blocked behind a slow query', blast: 'The DDL waits for `ACCESS EXCLUSIVE` and every subsequent query queues behind it -- a full table outage.', fix: '`SET lock_timeout = \'2s\'` and retry; run DDL in low-traffic windows; never during a long report.' },
        { mode: 'Access patterns never written down', blast: 'Schema serves the first three screens and every later feature needs a migration or a full scan.', fix: 'Access-pattern inventory with rates and latency targets as a required design artefact.' }
      ]
    },

    {
      t: 'staff',
      md: `The signal here is direction of reasoning. Weak answers start from a technology; strong
...
job that diffs and repairs" has maintained a derived store in production.`
...

    {
      ...
      items: [
        {
          q: 'A DynamoDB table stores tasks with `PK = PROJ#<id>`, `SK = TASK#<ulid>`, and a `status` attribute. "List open tasks" uses a `FilterExpression` on `status`. What is wrong?',
          options: [
            'Nothing -- filters are evaluated inside the storage engine.',
            'The filter runs after items are read, so you pay read capacity for every task in the project and latency scales with project size.',
            '`FilterExpression` cannot be used with `Query`.',
            'You need a scan instead.'
          ],
          answer: 1,
          why: 'DynamoDB applies `FilterExpression` after retrieving items, so both cost and latency are proportional to the items read rather than the items returned -- a project with 50,000 tasks bills for 50,000 to return 40. Putting status into the sort key (`TASK#OPEN#<ulid>`) turns it into a `begins_with` range query that reads only matching items. The cost you accept is that a status change becomes a delete plus an insert, because key attributes are immutable.'
        },
        {
          q: 'Your service writes to Postgres and then indexes the same record in Elasticsearch from application code. What failure does this design have no defence against?',
          options: [
            'Elasticsearch mapping conflicts.',
            'A crash or error between the two writes, leaving the index permanently and silently inconsistent.',
            'Elasticsearch rejecting the document size.',
            'Postgres replication lag.'
          ],
          answer: 1,
          why: 'There is no transaction spanning the two systems, so any interruption after the commit and before the index call leaves a document missing with no error recorded and no process that will ever notice. Writing an outbox row inside the Postgres transaction and shipping it with a CDC consumer makes the second write retryable and at-least-once; a periodic reconciler that diffs and repairs covers the residual gaps, because CDC pipelines do drop events during failover.'
        },
        {
          q: 'A product manager asks for a monthly report joining orders, users and events over 18 months. Where should it run?',
          options: [
            'The OLTP primary, with a covering index.',
            'A columnar store or warehouse fed by CDC, because scanning few columns over many rows is its native shape.',
            'A read replica of the OLTP primary, which removes all risk.',
            'A graph database, since it involves three entities.'
          ],
          answer: 1,
          why: 'Columnar storage reads only the columns referenced, so a query touching 3 of 60 columns reads a small fraction of the bytes, and per-column uniformity yields compression of 5-20x -- typically 10-100x faster for analytical scans. On the primary the same query evicts the buffer cache and wrecks transactional p99. A replica does protect the primary from CPU and cache effects, which makes it a reasonable interim step, but it does not fix the fundamental row-store mismatch for wide scans.'
        },
        {
          q: 'Which of these is a legitimate use of `jsonb` in a Postgres OLTP table?',
          options: [
            'A `status` field, to avoid a migration when new statuses are added.',
            'Tenant-defined custom attributes that are displayed but never filtered on across tenants.',
            'A `price` field, because currencies vary.',
            'Foreign key references, stored as an array of ids.'
          ],
          answer: 1,
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `Partition-key skew and write sharding are developed properly in **Sharding &
...
when reading a derived copy is the same question as **Replication & Consistency Models**.`
  ...
  flashcards: [
    { q: 'What is the first artefact of a data-modelling exercise?', a: 'An access-pattern inventory: each read and write with its rate, latency target and consistency requirement. The ratios in that table determine whether you can afford fan-out-on-write, denormalised counters, or a separate search store.' },
    { q: 'Why does DynamoDB push you toward single-table design?', a: 'It offers one efficient primitive -- fetch items by partition key, narrowed and sorted by sort key. With no joins and no efficient non-key filters, the only way to serve a pattern in one round trip is to co-locate heterogeneous item types under a shared partition key.' },
    { q: 'Why encode status in a DynamoDB sort key rather than as an attribute?', a: '`FilterExpression` is applied after items are read, so you pay for every item in the partition. `begins_with(SK, \'TASK#OPEN#\')` reads only matching items. The cost is that changing status means delete-and-insert, since key attributes are immutable.' },
    { q: 'What is a sparse GSI and why is it useful?', a: 'Only items carrying the index key attributes are projected into it. Adding `GSI1PK` to task items alone gives you an index containing tasks and nothing else, so you pay storage and write cost only for those items.' },
    { q: 'Distinguish denormalisation from temporal snapshotting.', a: 'Denormalisation copies a live shared fact (display name on posts) and needs a fan-out job plus a reconciler. A temporal snapshot records a fact as of an instant (price at purchase) and must *never* be updated -- it is not a duplicate at all.' },
    { q: 'What single rule keeps denormalisation survivable?', a: 'Exactly one source of truth, and every derived copy must be rebuildable from it by a batch job. Then a fan-out bug is an inconvenience rather than permanent data loss.' },
    { q: 'When is `jsonb` the wrong choice?', a: 'As soon as a query filters on a value inside it or code branches on it. You lose type checking, `NOT NULL`, foreign keys and good selectivity estimates, and you need an expression or GIN index per access path.' },
    { q: 'Why are columnar stores 10-100x faster for analytics?', a: 'Each column is stored contiguously, so a query referencing 3 of 60 columns reads only those bytes, and per-column uniformity gives 5-20x compression. The trade is that reconstructing a single whole row requires many separate column reads.' },
    { q: 'When does a graph database beat a recursive CTE?', a: 'When traversal depth is variable and unknown at query time -- shortest path, unbounded permission inheritance -- and traversal is a core product feature. Depth-2 questions are two joins, and bounded hierarchies are a depth-capped, cycle-safe recursive CTE.' }
  ],
  ...
  drills: [
    ...
    {
      prompt: 'A 300-million-row Postgres table needs a column renamed from `amount_cents` to `amount_minor` and its type widened from `integer` to `bigint`. The service handles 1,200 writes/s, must not have downtime, and any deploy must be rollback-safe at every step. Write the plan.',
      probes: [
