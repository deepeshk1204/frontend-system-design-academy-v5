export default {
  blocks: [
    { t: 'prose',
      md: `Backend security is three separable questions that get collapsed into one. **Who is this?** is authentication.
...
read any other customer's data. The token was never the control.` },
    ...
    { t: 'prose',
      md: `Three shifts made backend security structurally harder than it was.
...
disclosure obligation, and a contract problem.` },
    ...
    { t: 'h', text: 'OAuth 2.1 and OIDC: which flow, and when' },
    { t: 'prose',
      md: `OAuth 2.0 is a *delegated authorisation* framework: it lets an application obtain a token to call an API on a user's
...
discouraged is a useful signal.` },
    { t: 'table',
      cols: ['Flow', 'Use for', 'Mechanism', 'Why not the others'],
      rows: [
  ...
  R->>R: validate token, then check authorisation`,
      caption: 'Two independent checks at the end. Token validity answers who; the authorisation check answers whether.' },
    { t: 'note',
      tone: 'danger',
      title: 'What an ID token is NOT for',
      md: `An ID token is a statement *to your application* that a user authenticated, with an \`aud\` claim naming your
...
sides.` },
    ...
    { t: 'h', text: 'JWTs: what they are good at, and what they are not' },
    { t: 'prose',
      md: `A JWT is a signed, base64url-encoded JSON object. Its one real property is that a verifier holding the public key
...
requires one fast cache lookup -- typically under a millisecond from Redis.` },
    { t: 'table',
      title: 'JWT vs opaque session token, honestly',
      cols: ['', 'JWT (self-contained)', 'Opaque token + server store'],
      rows: [
    ...
    { t: 'prose',
      md: `The defensible default for a web product: **opaque session cookie for the browser, short-lived JWT access tokens for
...
\`localStorage\` is readable by any script on the page.` },
    ...
    { t: 'h', text: 'Refresh token rotation and reuse detection' },
    { t: 'prose',
      md: `A refresh token is long-lived and therefore your most valuable credential. **Rotation** means every use returns a
...
family** and force re-authentication. One theft costs the user one login prompt, which is a very good trade.` },
    { t: 'code',
      lang: 'sql',
      title: 'Rotation with family-wide revocation',
...
-- Then: audit-log it, and alert if one user trips this repeatedly.` },
    ...
    { t: 'h', text: 'Service-to-service identity: mTLS and SPIFFE' },
    { t: 'prose',
      md: `Inside your platform, the question is not "which user" but "which workload". Shared static API keys are the common
...
operate, and a genuinely harder debugging experience when a handshake fails.` },
    ...
    { t: 'h', text: 'Authorisation models' },
    { t: 'table',
      cols: ['Model', 'Decision based on', 'Expresses well', 'Fails at', 'Real systems'],
      rows: [
    ...
    { t: 'prose',
      md: `The progression is predictable. You start with RBAC because it is simple and it is right for a while. Then a
...
that pin a check to a point in time.` },
    { t: 'note',
      tone: 'warn',
      title: 'The multi-tenant enforcement point problem',
      md: `Authorisation is only as good as its weakest enforcement point, and scattering \`if (user.tenantId !==
...
incrementally. The principle: make the *insecure* thing the one that requires extra effort.` },
    { t: 'code',
      lang: 'sql',
      title: 'Row-level security as a backstop your code cannot bypass',
...
-- transaction, not SET, or the value leaks to the next request on that connection.` },
    ...
    { t: 'h', text: 'Input validation and injection' },
    { t: 'prose',
      md: `**SQL injection** is solved, and it is solved by parameterisation, not by escaping. A prepared statement sends the
...
schema, and \`yaml.safe_load\`.` },
    { t: 'code',
      lang: 'python',
      title: 'The four that recur in real code review',
      code: `# 1. SQL: parameterise. Escaping is not a defence.
...
hmac.compare_digest(provided_signature, expected_signature)` },
    ...
    { t: 'h', text: 'Secrets, encryption and audit' },
    { t: 'prose',
      md: `**Secrets** should be short-lived and issued to a verified workload identity, not long-lived strings in environment
...
denials is one of the highest-signal indicators of an attack in progress, and most teams only log successes.` },
    ...
    { t: 'failures',
      items: [
        { mode: 'Tenant id taken from the request body instead of the verified token', blast: 'Any authenticated customer reads any other customer\'s data by editing one field. Full cross-tenant breach with a disclosure obligation.', fix: 'Derive tenant from the token only, and enforce with row-level security so a forgotten check cannot leak rows.' },
        { mode: 'Algorithm confusion -- verifier trusts the `alg` header', blast: 'Attacker forges tokens using your public key as an HMAC secret, or sends `alg: none`. Complete authentication bypass.', fix: 'Pin the expected algorithm and key in the verifier configuration; reject any token whose header disagrees.' },
        { mode: 'JWT used as a session with 7-day expiry in `localStorage`', blast: 'One XSS yields a week of full account access, unrevocable, and logout does nothing.', fix: 'Opaque session id in an `HttpOnly; Secure; SameSite` cookie, with 5-15 minute access tokens for APIs.' },
        { mode: 'SSRF reaching IMDSv1', blast: 'Temporary IAM credentials exfiltrated via a webhook-validation feature; attacker now has your role\'s permissions. This is the Capital One breach.', fix: 'Enforce IMDSv2 with a hop limit, resolve and validate the IP before connecting, block private ranges, disallow redirects, and egress through a restricted proxy.' },
        { mode: 'Refresh token with no rotation', blast: 'A single stolen token grants indefinite access with no detection and no signal that anything happened.', fix: 'Rotate on every use, detect reuse, and revoke the entire token family on reuse -- costing the user one login.' },
        { mode: 'Static database password in an environment variable, never rotated', blast: 'One leaked container spec or CI log is permanent database access; rotating requires a coordinated deploy nobody has rehearsed.', fix: 'Dynamic credentials from a secrets manager against a workload identity, with a one-hour TTL and automatic rotation.' },
        { mode: '`ORDER BY` column interpolated from a query parameter', blast: 'SQL injection in a codebase that "uses an ORM" and believes it is safe.', fix: 'Identifiers cannot be parameterised -- allowlist the permitted columns explicitly.' },
        { mode: 'Authorisation denials not logged', blast: 'An attacker enumerating resources is invisible; you learn about the breach from the successful request only.', fix: 'Audit-log denials with actor, resource and source IP, and alert on denial-rate spikes per principal.' }
      ] },

    { t: 'staff',
      md: `Security answers separate candidates fast, because the vocabulary is easy to acquire and the mechanisms are not.
...
forgetting harmless. "We validate the JWT" is not a security design.` },
    ...
    { t: 'quiz',
      items: [
        {
          q: 'Your API validates the JWT signature, expiry and issuer correctly, then reads `tenant_id` from the JSON request body to scope the query. What is the risk?',
          options: ['None -- the token was verified, so the caller is authenticated.', 'Any authenticated user can read any tenant\'s data by changing one field; authentication was correct and authorisation is absent.', 'Only a risk if the token is stolen.', 'The body should be signed as well.'],
          answer: 1,
          why: 'Authentication and authorisation are separate checks, and only the first was performed. A valid token proves who the caller is; it says nothing about which tenant\'s rows they may touch, so a client-supplied tenant id is simply an unauthenticated parameter. Tenant must be derived from the verified token claims, and it should be enforced at a layer that cannot be forgotten -- row-level security with `SET LOCAL app.tenant_id` inside the transaction means a handler that omits the filter returns zero rows rather than everyone\'s.' },
        {
          q: 'Why is a 7-day JWT stored in `localStorage` a poor session mechanism?',
          options: ['JWTs are too large for `localStorage`.', 'It cannot be revoked, permission changes do not take effect, and any XSS exfiltrates a week of full access.', 'JWT signatures expire independently of the token.', '`localStorage` is not available in private browsing.'],
          answer: 1,
          why: 'A self-contained token is validated offline, which is exactly why you cannot invalidate it -- logout becomes a client-side gesture and a role downgrade does not apply until the token expires. `localStorage` is readable by any script on the page, so one XSS yields seven days of unrevocable access. An opaque session id in an `HttpOnly; Secure; SameSite` cookie is unreadable by JavaScript, revocable by deleting one record, and costs a sub-millisecond cache lookup. Keep JWTs for short-lived access tokens, 5-15 minutes.' },
        {
          q: 'A rotated refresh token is presented a second time. What is the correct response?',
          options: ['Issue a new access token -- it may be a retry after a network failure.', 'Reject this request but keep the family valid.', 'Invalidate the entire token family and force re-authentication.', 'Rate-limit the client and allow the next attempt.'],
          answer: 2,
          why: 'Reuse of a rotated token means either an attacker stole it and the real client already rotated, or the attacker rotated first and the real client is now presenting the stale one. You cannot distinguish these, and in both cases a copy exists outside the legitimate client. Revoking the whole family is the only response that closes the attacker\'s access, and the cost of being wrong is one login prompt -- cheap relative to an undetected session takeover. Log it and alert if one user trips it repeatedly.' },
        {
          q: 'Your service fetches user-supplied URLs to validate webhooks. Which control most directly prevents credential theft from cloud metadata?',
          options: ['A regex blocking `169.254.169.254` in the submitted URL.', 'Enforce IMDSv2 with a hop limit, plus resolve the hostname and reject private/link-local IPs before connecting to the resolved address.', 'A 5-second request timeout.', 'Running the fetcher as a non-root user.'],
          answer: 1,
          why: 'A regex on the URL string is defeated by DNS (a hostname resolving to the link-local address), by decimal or hex IP encodings, and by a redirect. IMDSv2 is the structural fix: it requires a PUT to obtain a token and sets a hop limit so a request proxied through your application cannot reach the metadata service at all. Pair it with resolving the hostname yourself, validating the resulting IP against private and link-local ranges, and connecting to *that IP* -- otherwise DNS rebinding lets the name resolve publicly during validation and privately during the fetch.'
        }
      ] },
    ...
    { t: 'details',
      ...
      blocks: [{ t: 'prose',
        md: `Per-tenant quotas and noisy-neighbour isolation sit alongside tenant authorisation in **Rate Limiting &
...
Kubernetes & Safe Deploys**.`
  ...
  flashcards: [
    { q: 'Authentication vs authorisation, and why conflating them breaks things?', a: 'Authentication establishes who the caller is; authorisation decides whether they may perform this action on this resource. A service that validates a token perfectly and then trusts a client-supplied `tenant_id` has authenticated and authorised nothing, which is a cross-tenant breach.' },
    { q: 'Which OAuth flow for a SPA, and why?', a: 'Authorization code with PKCE. The client sends a SHA-256 challenge and redeems the code with the original verifier, so a stolen authorization code is useless. Implicit was removed in OAuth 2.1 because tokens in the URL fragment leak through history, referrers and logs.' },
    { q: 'What is an ID token not for?', a: 'Calling APIs. It is an assertion to *your application* that a user authenticated, with `aud` naming your client. Use it to establish a session, then use the access token -- which carries `scope` and an `aud` for the API -- for API calls. Check `aud` on both sides.' },
    { q: 'What is JWT algorithm confusion?', a: 'The `alg` header is attacker-controlled, so a naive verifier can be tricked into accepting `alg: none`, or into verifying an `HS256` token using your RSA public key as the HMAC secret -- which is public. Pin the expected algorithm and key in the verifier and reject mismatches.' },
    { q: 'When is an opaque session token better than a JWT?', a: 'Whenever you need immediate logout or immediate permission changes -- so, for browser sessions. It costs a sub-millisecond store lookup and buys revocability. Reserve JWTs for short-lived access tokens of 5-15 minutes, where offline validation is the point.' },
    { q: 'How does refresh token rotation with reuse detection work?', a: 'Each use returns a new token and invalidates the old. Presenting an already-rotated token means a copy exists outside the legitimate client, so you revoke the whole token family and force re-authentication. The cost of a false positive is one login prompt.' },
    { q: 'What does SPIFFE give you over static service API keys?', a: 'A cryptographic workload identity (`spiffe://trust-domain/ns/team/sa/service`) backed by short-lived, automatically rotated certificates. No long-lived secret exists to leak, and authorisation policy becomes a reviewable statement enforced at the sidecar before reaching your code.' },
    { q: 'RBAC vs ABAC vs ReBAC?', a: 'RBAC decides from roles and is simple until per-resource permissions cause a role explosion. ABAC evaluates attributes and expresses conditions well but answers "who can see this" poorly. ReBAC stores relationship tuples and handles inheritance -- "edit on the folder implies edit on the file" -- which is the Zanzibar model used by Drive and GitHub.' },
    { q: 'Why use Postgres row-level security in a multi-tenant app?', a: 'Because scattered per-handler tenant checks guarantee one will be missing. RLS with `SET LOCAL app.tenant_id` inside the transaction makes the database reject cross-tenant rows regardless of application code, turning a forgotten filter into zero rows instead of a breach.' },
    { q: 'What actually prevents SSRF reaching cloud metadata?', a: 'IMDSv2 with a hop limit, so the metadata service requires a PUT-obtained token and cannot be reached through a proxied request. Plus resolving the hostname yourself, rejecting private/loopback/link-local IPs, connecting to the resolved IP to defeat DNS rebinding, and disallowing redirects.' }
  ],

  drills: [
    {
      prompt: 'You are designing authentication and authorisation for a multi-tenant B2B analytics product: a React SPA, a mobile app, a public REST API for customer integrations, and background workers. Customers demand SSO, per-project permissions, and an audit trail. Design it.',
      probes: [
