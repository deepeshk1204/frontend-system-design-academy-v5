export default {
  blocks: [
    {
      t: 'prose',
      md: `Capacity planning is the practice of knowing, before it happens, roughly what your system
...
It behaves like a system that is about to fall over.`
    ...
    {
      t: 'prose',
      md: `Two failure modes motivate all of it. Teams build systems that cannot possibly work,
...
whether someone can reason quantitatively about a system they have just heard of.`
    ...
    { t: 'h', text: 'The method' },
    {
      t: 'steps',
      ordered: true,
      title: 'Six steps, in this order, always',
      items: [
        'Start from users: total accounts, then daily actives, then actions per active per day.',
        'Convert to a rate: divide by 86,400 for average rps, then multiply by a peak factor. Diurnal web traffic is typically **2-4x** average at peak; a launch or a scheduled event can be 10-50x.',
        'Split reads from writes. The ratio decides almost everything downstream -- caching, replicas, fan-out direction.',
        'Size one item, then multiply. Bytes per row or per object times objects per day times retention gives storage. Include indexes at roughly 20-50% of table size, and replication factor.',
        'Bandwidth: response size times rps. This is the step most often skipped, and it is the one that produces surprise egress bills.',
        'Convert to machines using a measured or assumed per-instance capacity, then divide by your target utilisation -- not by 100%.'
      ]
    },
    {
      t: 'numbers',
      ...
      items: [
        { v: '0.5 ns', k: 'L1 cache reference', note: 'The unit everything else is measured in' },
        { v: '~25 ns', k: 'L3 / last-level cache', note: '' },
        { v: '~100 ns', k: 'Main memory reference', note: '~200x slower than L1' },
        { v: '~1.5 μs', k: 'Compress 1 KB with snappy', note: 'Cheap enough to always consider' },
        { v: '~10 μs', k: 'Read 1 MB sequentially from memory', note: '' },
        { v: '~16 μs', k: 'NVMe SSD random read', note: 'Was 150 μs on SATA SSD, 10 ms on spinning disk' },
        { v: '~0.5 ms', k: 'Same-AZ network round trip', note: 'Your service-to-service floor' },
        { v: '~200 μs', k: 'Read 1 MB sequentially from NVMe', note: 'About 5 GB/s' },
        { v: '~1 ms', k: 'Read 1 MB over 10 Gbit network', note: '' },
        { v: '~30 ms', k: 'US coast-to-coast round trip', note: '~200 ms intercontinental' },
        { v: '~1-5 ms', k: 'Indexed Postgres point read, warm', note: '~0.2 ms for a Redis GET' },
        { v: '~10-100 ms', k: 'A visible GC pause', note: 'Multi-second on a badly tuned large heap' }
      ]
    },

    { t: 'h', text: 'A fully worked example' },
    {
      t: 'prose',
      md: `The requirement: a photo-sharing feed. 60 million registered users, 12 million daily
...
Everything below follows mechanically.`
    },
    {
      t: 'code',
      lang: 'text',
      title: 'Step by step, with the assumptions stated',
      code: `--- 1. Rates ---------------------------------------------------------
...
At a 90% hit rate the metadata DB sees 250 rps. Comfortable.`
    },
    {
      t: 'note',
      tone: 'good',
      title: 'What the estimate bought you',
      md: `Four design decisions, each falling out of one number rather than out of taste. Images
...
required precision -- being wrong by a factor of two changes none of the four conclusions.`
    ...
    { t: 'h', text: "Little's Law, and what it is actually for" },
    {
      t: 'prose',
      md: `\`L = λ × W\`: the average number of items in a system equals the arrival rate times the
...
rearrangements.`
    },
    {
      t: 'code',
      lang: 'text',
      title: "Little's Law, three ways",
      code: `# 1. Sizing a connection pool.
...
# 4. The reverse check that catches bad designs.
  ...
  before you deploy it -- either W or L must change by 8x.`
    ...
    { t: 'h', text: 'Why utilisation above 70% makes latency explode' },
    {
      t: 'prose',
      md: `For a single queue with random arrivals, the average time in the system is
...
traffic costs you another 3x.`
    ...
    {
      t: 'table',
      title: 'The response-time curve: W = S / (1 - ρ), with S = 10 ms',
      cols: ['Utilisation ρ', 'Multiplier 1/(1-ρ)', 'Total time in system', 'Of which waiting'],
      rows: [
        ['10%', '1.11x', '11 ms', '1 ms'],
        ['50%', '2x', '20 ms', '10 ms'],
        ['70%', '3.3x', '33 ms', '23 ms'],
  ...
  H["Incidents live here"] --> E`,
      caption: 'The knee is around 0.7-0.8. Plan capacity to the left of it, because the cost of the next 10% of traffic is not linear -- and autoscaling takes minutes you will not have.'
    },
    {
      t: 'prose',
      md: `Two refinements make this realistic. Multiple parallel servers behind one queue absorb
...
failures and the time autoscaling takes to react.`
    ...
    { t: 'h', text: 'Read scaling, write scaling, and coordination cost' },
    {
      t: 'prose',
      md: `Reads scale almost embarrassingly well. Add replicas, add cache layers, add a CDN --
...
the honest answer to "can we scale this by adding machines" is sometimes no.`
    ...
    {
      t: 'prose',
      md: `**Vertical versus horizontal**, honestly. Vertical scaling is dramatically
...
long as you can get away with, because that is where all the complexity lives.`
    ...
    { t: 'h', text: 'Autoscaling on the right signal' },
    {
      t: 'prose',
      md: `CPU utilisation is the default autoscaling metric and it is usually the wrong one. For an
...
onto the utilisation curve above.`
    },
    {
      t: 'code',
      lang: 'yaml',
      title: 'Scaling on concurrency, with the anti-oscillation settings',
...
spec:
  ...
  behavior:
    ...
    {
      t: 'prose',
      md: `The asymmetry in that config is the whole point. Scaling up is cheap and the cost of
...
five-minute warm-up.`
    ...
    {
      t: 'prose',
      md: `**Cold start** deserves separate treatment because it interacts badly with everything
...
probe reflect genuine readiness -- pool established, caches primed, one real query completed.`
    ...
    {
      t: 'tradeoffs',
      title: 'Planning to 65% utilisation with concurrency-based autoscaling',
      gains: [
    ...
    },
    {
      t: 'failures',
      items: [
        { mode: 'Planning to 90% utilisation because "CPU has headroom"', blast: 'Latency is already 10x service time; the next 10% of traffic triples it again. Looks fine on dashboards until it does not.', fix: 'Target 60-70%; alert on utilisation, not just saturation, and treat headroom as a purchased good.' },
        { mode: 'Autoscaling on CPU for an I/O-bound service', blast: 'CPU sits at 30% while requests wait 4 s downstream; no scaling occurs during the incident.', fix: 'Scale on in-flight concurrency or queue wait time; keep CPU as a secondary metric.' },
        { mode: 'Symmetric scale-up and scale-down thresholds', blast: 'Fleet oscillates on a period equal to boot time, dropping connections on each scale-down.', fix: 'Immediate scale-up, 10-minute stabilisation window on scale-down, small percentage steps.' },
        { mode: 'Readiness probe passing before the instance is warm', blast: 'Every scale-up event raises p99 as cold instances receive full traffic and time out, potentially triggering more scaling.', fix: 'Slow-start traffic ramp, readiness gated on a real dependency check, and pre-warmed pools for spiky workloads.' },
        { mode: '`minReplicas` sized for the traffic trough', blast: 'An AZ failure at 03:00 leaves one instance serving everything while new ones take five minutes to warm.', fix: 'Size `minReplicas` to survive the loss of one zone, not to minimise the night-time bill.' },
        { mode: 'Ignoring bandwidth in the estimate', blast: 'Design is sound on compute and storage, then egress costs more than the entire infrastructure budget.', fix: 'Compute egress explicitly per response; assume a CDN hit rate and state it.' },
        { mode: 'Adding nodes to a coordination-bound system', blast: 'Throughput peaks then declines -- 41 nodes slower than 40, per USL\'s quadratic coherency term.', fix: 'Remove the shared serial point (global lock, single counter, cross-shard transaction) before adding capacity.' },
        { mode: 'Peak factor taken as 1x average', blast: 'Capacity is a third of what peak needs; every day has a predictable brown-out at the diurnal maximum.', fix: 'Use measured peak-to-mean ratio (typically 2-4x for consumer traffic) and plan to peak, not to mean.' }
      ]
    },

    {
      t: 'staff',
      md: `The estimation itself is table stakes. The Staff signal is which numbers you choose to
...
valuable.`
...

    {
      ...
      items: [
        {
          q: 'A service has a service time of 10 ms and runs at 90% utilisation. Roughly what is the average total time a request spends in the system?',
          options: ['11 ms', '20 ms', '100 ms', '900 ms'],
          answer: 2,
          why: '`W = S/(1-ρ) = 10/(1-0.9) = 100 ms`, so 90 ms of the 100 is pure queueing delay that did not exist at low load. Nothing about the code changed -- at high utilisation there is no longer idle time between bursts to absorb their backlog. This is why 90% utilisation is not "10% of headroom": the next slice of traffic takes you to 95% and 200 ms.'
        },
        {
          q: 'A Kafka consumer group has 400,000 messages of lag and processes 5,000 messages per second. What should you tell the product team?',
          options: [
            'There are 400,000 unprocessed messages.',
            'Events are currently being processed about 80 seconds after they are produced.',
            'The consumer needs 400,000 more partitions.',
            'Lag cannot be converted to time.'
          ],
          answer: 1,
          why: 'Little\'s Law rearranged gives `W = L/λ = 400,000/5,000 = 80 s`, which is the delay a newly produced event will experience. That is the number a product owner can reason about and the number an SLO can be written against, whereas a raw count means nothing without throughput. It also tells you what to do: to halve the delay you must roughly double consumer throughput or halve arrivals.'
        },
        {
          q: 'Your I/O-bound API scales on CPU at a 70% target. During an incident requests take 4 s waiting on a downstream service, but no scaling happens. Why, and what would you scale on?',
          options: [
            'The HPA was misconfigured; raise the CPU target.',
            'Waiting on I/O consumes no CPU, so the autoscaler sees a healthy fleet. Scale on in-flight concurrency or queue wait instead.',
            'Kubernetes cannot scale during high latency.',
            'The metric server was down.'
          ],
          answer: 1,
          why: 'CPU measures resource consumption, not work waiting to be done, so a fleet of threads blocked on a slow dependency looks idle. In-flight concurrency captures exactly that state, leads CPU as a signal, and maps directly onto the queueing curve. Worth noting that scaling out may not even be the right response to a slow dependency -- adding instances can increase pressure on it -- which is why concurrency limits and load shedding belong alongside autoscaling.'
        },
        {
          q: 'You add a 41st node to a system and total throughput drops slightly. Which model explains this?',
          options: [
            'Amdahl\'s law -- the serial fraction caps speedup at 1/s.',
            'The Universal Scalability Law -- a coherency cost growing with the square of node count, so throughput peaks then declines.',
            'Little\'s Law.',
            'The CAP theorem.'
          ],
          answer: 1,
...

    {
      ...
      blocks: [{
        t: 'prose',
        md: `The queueing curve here is what makes load shedding and concurrency limits necessary
...
question in **Observability, SLOs & Error Budgets**.`
  ...
  flashcards: [
    { q: 'State the six steps of a back-of-envelope estimate.', a: 'Users to daily actives to actions per day; convert to average rps and apply a peak factor of 2-4x; split reads from writes; size one item and multiply for storage including indexes and replication; compute bandwidth; convert to machines at a target utilisation, not at 100%.' },
    { q: 'What is `W = S / (1 - ρ)` and why does it matter?', a: 'Average time in a single-server queue given service time `S` and utilisation `ρ`. It is a hyperbola, so at 70% utilisation you are at 3.3x service time, at 90% 10x, at 99% 100x. It is why planning to 90% CPU is planning an incident.' },
    { q: 'Why is arrival variance the underlying cause of the utilisation curve?', a: 'Requests arrive in clumps. At low utilisation there is idle time between clumps to drain the backlog; near saturation there is not, so waits compound. High variance in service time makes it worse than the formula predicts.' },
    { q: 'Give three uses of Little\'s Law.', a: 'Pool sizing (`L = λW`: 2,400 q/s at 4 ms needs ~10 connections); concurrency ceiling (20 threads at 45 ms caps you at 444 rps); and converting queue depth to delay (400k lag at 5k/s is 80 seconds).' },
    { q: 'Why is CPU the wrong autoscaling signal for an I/O-bound service?', a: 'Threads blocked on a slow dependency consume no CPU, so a badly degraded fleet looks idle. CPU also lags -- by the time it rises, queues are deep. Scale on in-flight concurrency or derived queue wait time.' },
    { q: 'What causes autoscaling oscillation and how do you stop it?', a: 'Symmetric up and down thresholds: scaling up drops per-instance load below the scale-down threshold, and so on, flapping at roughly the boot interval. Fix with immediate scale-up, a long stabilisation window on scale-down, and small percentage steps.' },
    { q: 'Why can a scale-up event temporarily raise p99?', a: 'Cold instances pass readiness before JIT warm-up, connection pools and caches are ready, then receive a full traffic share and respond slowly. Use slow-start ramping, a readiness probe that reflects real readiness, and warm pools for spiky load.' },
    { q: 'Difference between Amdahl\'s law and the Universal Scalability Law?', a: 'Amdahl says a serial fraction `s` caps speedup at `1/s` -- throughput plateaus. USL adds a coherency cost growing with the square of node count, so throughput peaks and then *declines*: adding a node can make the system slower.' },
    { q: 'Honest case for vertical scaling?', a: 'A single machine reaches ~192 cores and 2 TB of RAM, and resizing is one restart with no partitions, rebalancing or cross-shard queries. Scale the stateless tier horizontally from day one and the stateful tier vertically for as long as you can; sharding imports the whole distributed-systems syllabus.' }
  ],
  ...
  drills: [
    ...
    {
      prompt: 'An API runs 40 pods, autoscaled on 70% CPU, min 4 and max 120. Every weekday at 09:00 there is a 4-minute period of elevated p99 and some 503s, after which it stabilises. Average CPU during the event peaks at 62%. Fix it, and say how you would verify the fix before next Monday.',
      probes: [
